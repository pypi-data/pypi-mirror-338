...
source activate trl && trl {train_type} \
    ...
    --output_dir {output_dir}/{namespace}/{name}/jobs/$NEBU_CONTAINER_ID \
    --torch_dtype {dtype} \
...
LATEST="$(ls -1d {output_dir}/{namespace}/{name}/jobs/$NEBU_CONTAINER_ID/checkpoint-* | sort -V | tail -n 1)"
rclone sync "$LATEST" "s3://{bucket}/{bucket_base_key}/{namespace}/{name}/latest"
        ...
        train_volumes = [
            V1VolumePath(
                source=f"{output_dir}/{namespace}/{name}/jobs/$NEBU_CONTAINER_ID",
