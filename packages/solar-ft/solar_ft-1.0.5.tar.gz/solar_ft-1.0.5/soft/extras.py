...
def get_roi(img: numpy.ndarray, size: int = 20, plot: bool = False) -> tuple[numpy.ndarray, tuple[int, int]]:
    """
    ...
    plot (bool): Whether to plot the ROI on the image.
    ...
    """
    roi: numpy.ndarray = numpy.zeros((size, size))
    older_roi: numpy.ndarray = numpy.ones((size, size)) * 1e9
    bbox: tuple[int, int] = (0, 0)

    # Iterate over the image to find the ROI with the most values near zero
    ...
    # Plot the ROI if requested
    if plot:
        fig, ax = matplotlib.pyplot.subplots(1, 1, figsize=(10, 10))
        extent = [0, img.shape[1], 0, img.shape[0]]
        im = ax.imshow(img, cmap='gray', vmin=-100, vmax=100, origin='lower', extent=extent)
        matplotlib.pyplot.colorbar(im)
        x1, x2, y1, y2 = bbox[1], bbox[1] + size, bbox[0], bbox[0] + size
        axins = ax.inset_axes([0.35, 0.1, 0.4, 0.4], xlim=(x1, x2), ylim=(y1, y2), xticklabels=[], yticklabels=[])
        axins.imshow(older_roi, cmap='gray', vmin=-100, vmax=100, origin='lower', extent=[x1, x2, y1, y2])
