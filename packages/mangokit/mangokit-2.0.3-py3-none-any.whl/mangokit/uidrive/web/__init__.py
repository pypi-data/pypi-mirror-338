...
from mangokit.enums.ui_enum import ElementExpEnum, ElementOperationEnum
from mangokit.exceptions import MangoKitError, ERROR_MSG_0012, ERROR_MSG_0032, ERROR_MSG_0010, ERROR_MSG_0011, \
    ERROR_MSG_0031, ERROR_MSG_0019, ERROR_MSG_0017, ERROR_MSG_0048, ERROR_MSG_0052, ERROR_MSG_0041, \
    ERROR_MSG_0023, ERROR_MSG_0029, ERROR_MSG_0022, ERROR_MSG_0060, ERROR_MSG_0020, ERROR_MSG_0005, \
    ERROR_MSG_0025
...
from mangokit.uidrive.web.browser import PlaywrightBrowser
from mangokit.uidrive.web.customization import PlaywrightCustomization
from mangokit.uidrive.web.element import PlaywrightElement
from mangokit.uidrive.web.input_device import PlaywrightDeviceInput
from mangokit.uidrive.web.page import PlaywrightPage
...
class WebDevice(PlaywrightBrowser,
    ...
    async def web_action_element(self) -> dict:
        self.base_data.log.debug(f'元素操作：{self.element_model.model_dump()}')
        try:
            ...
            raise MangoKitError(*ERROR_MSG_0011, value=(self.element_model.name,))
        except TargetClosedError:
            await self.base_data.setup(self.base_data.test_data)
    ...
    async def web_assertion_element(self) -> str:
        ...
        try:
            ...
            if is_method:
                ...
                self.base_data.log.debug(f'开始断言，方法：{self.element_model.ope_key}，断言值：{ope_value}')
            ...
            self.base_data.log.error(f'WEB自动化失败，类型：{type(error)}，失败详情：{error}')
            raise MangoKitError(*ERROR_MSG_0052, value=(self.element_model.name,), )
    ...
    async def web_find_ele(self) -> tuple[int, Locator] | tuple[int, list[Locator]]:
        ...
        if self.element_model.is_iframe == StatusEnum.SUCCESS.value:
            ...
            if not ele_list and self.element_model.type == ElementOperationEnum.OPE.value:
                raise MangoKitError(*ERROR_MSG_0029, value=(self.element_model.name, locator_str))
            try:
