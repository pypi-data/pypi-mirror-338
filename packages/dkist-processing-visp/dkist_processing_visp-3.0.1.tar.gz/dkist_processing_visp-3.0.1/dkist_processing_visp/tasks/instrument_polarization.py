...
class InstrumentPolarizationCalibration(
    ...
    def run(self) -> None:
        ...
        for beam in range(1, self.constants.num_beams + 1):
            with self.apm_processing_step("Generate polcal DARK frame"):
                ...
                self.generate_polcal_dark_calibration(exp_times=polcal_exposure_times, beam=beam)

            with self.apm_processing_step("Generate polcal GAIN frame"):
                logger.info("Generating polcal gain frame")
                self.generate_polcal_gain_calibration(exp_times=polcal_exposure_times, beam=beam)
    ...
    def generate_polcal_dark_calibration(
        self, exp_times: list[float] | tuple[float], beam: int
    ) -> None:
        ...
        for exp_time in exp_times:
            ...
            self.write(
                data=avg_array,
                tags=[
                    VispTag.intermediate_frame_polcal_dark(beam=beam, readout_exp_time=exp_time),
                    VispTag.exposure_time(exp_time),
                ],
                encoder=fits_array_encoder,
            )
    ...
    def generate_polcal_gain_calibration(
        ...
        for exp_time in exp_times:
            ...
            self.write(
                data=avg_array,
                tags=[
                    VispTag.intermediate_frame_polcal_gain(beam=beam, readout_exp_time=exp_time),
                    VispTag.exposure_time(exposure_time_s=exp_time),
                ],
                encoder=fits_array_encoder,
            )
