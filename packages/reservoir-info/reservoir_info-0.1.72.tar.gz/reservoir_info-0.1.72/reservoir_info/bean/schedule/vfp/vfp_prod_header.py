...
from reservoir_info.enums.gfr_type import GfrType
from reservoir_info.enums.unit_system import UnitSystem
from reservoir_info.enums.flo_type import FloType
from reservoir_info.enums.wfr_type import WfrType


@dataclass
class VfpProdHeader(BaseData):
    """
    生产井VFP(垂直流动性能)表格的表头
    """
    ...
    bhp_deep: float = field(default=None, metadata={'description': 'BHP参考深度，默认等于首个井节点的深度'})
    flo_type: FloType = field(default=FloType.GAS, metadata={'description': 'FLO(流体流动的速率)类型'})
    wfr_type: WfrType = field(default=WfrType.WCT, metadata={'description': 'WFR(水流速或水流量)类型'})
    gfr_type: GfrType = field(default=GfrType.GOR, metadata={'description': 'GFR(气体流速或气体流量)类型'})
    thp_type: str = field(default='THP', metadata={'description': 'THP(油管头压力) 的类型'})
    alg_type: str = field(default=None, metadata={'description': 'ALG(举升工况) 的类型'})
    ...
    def from_text(cls, text: str):
        ...
        return cls(
            ...
            wfr_type=WfrType.of_code(items[3]) or WfrType.WCT,
            gfr_type=GfrType.of_code(items[4]) or GfrType.GOR,
            thp_type=items[5] or 'THP',
            alg_type=items[6] if items[6] != '*' and items != 'NA' else None,
            unit_system=UnitSystem.of_code(items[7]),
            vfp_type=items[8] or 'BHP',
        )
...
if __name__ == '__main__':
    str_ = '1 NA NA WCT GOR THP * FIELD BHP /'
    header_ = VfpProdHeader.from_text(str_)
