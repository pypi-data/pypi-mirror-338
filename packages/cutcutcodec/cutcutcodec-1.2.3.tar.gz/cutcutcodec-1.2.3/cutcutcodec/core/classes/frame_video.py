...
class FrameVideo(Frame):
    ...
    def to_numpy_uint8(self, contiguous=True) -> np.ndarray[np.uint8]:
        ...
        before if you want to control it.
        ...
        frame = self.convert(3)  # to rgb (no copy)
        frame = frame.clamp(0.0, 1.0)  # copy and saturation
        frame *= 255  # inplace safe because flip make copy
        frame += 0.5  # to transform floor in round for positive values
        frame = frame.to(torch.uint8)
        frame_np = frame.numpy(force=True)
        if contiguous:
