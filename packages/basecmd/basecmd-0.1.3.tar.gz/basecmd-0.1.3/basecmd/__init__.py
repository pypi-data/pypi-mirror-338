...
class BaseCmd:
    ...
    def init_logging(self) -> None:
        ...
        if self.LOG_FORMAT is None and (
            ...
            or (
                type(rootHandler) is logging.StreamHandler
                and rootHandler.stream.isatty()
            )
        ):
            self.LOG_FORMAT = "%(asctime).19s  %(message)s"

        if type(rootHandler) is logging.StreamHandler and rootHandler.stream.isatty():
            rootHandler.setFormatter(self.ColorFormatter(self.LOG_FORMAT))
        else:
            rootHandler.setFormatter(logging.Formatter(self.LOG_FORMAT))
