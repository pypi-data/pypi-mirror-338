...
def rectangular_spiral(
    *,
    ...
    align_ports: _typ.Literal["", "x", "y"] = None,
    ...
    if full_length > 0:
        ...
        t1 = 3
        c1, _, _ = _rectangular_spiral_geometry(t0, *args)
        l1 = _pf.route_length(c1)
        while l1 <= full_length:
            t0 = t1
            l0 = l1
            t1 *= 2
            c1, _, _ = _rectangular_spiral_geometry(t1, *args)
            l1 = _pf.route_length(c1)
        ...
        for reference in c0.references:
            for _, port_list in reference.get_ports().items():
                y = port_list[0].center[1]
                ymin = min(ymin, y)
                ymax = max(ymax, y)
