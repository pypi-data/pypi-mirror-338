...
_ImplicitMap = Union[Sequence[str], Sequence[int]]
...
def _tidy3d_from_bytes(obj_bytes: bytes) -> tidy3d.components.base.Tidy3dBaseModel:
    ...
    obj = None
    try:
        tmp_file.write_bytes(obj_bytes)
        obj = tidy3d.components.base.Tidy3dBaseModel.dict_from_hdf5(tmp_name)
    finally:
        tmp_file.unlink()
...
def _upload(queue, **kwargs):
    ...
    try:
        result = web.upload(**kwargs, verbose=False)
...
def _simulation_runner(
    *, simulation: _sim_types, task_name: str, remote_path: str, data_path: str, verbose: bool
    ...
    if use_local_mode_solver and simulation.type == "ModeSolver":
...
class Tidy3DModel(Model):
    ...
    def _process_port_symmetries(
        ...
        for port_symmetry in self.port_symmetries:
            # _ImplicitMap
            if len(port_symmetry) == len(port_names) and isinstance(port_symmetry[-1], (str, int)):
                equiv = [
            ...
            elif len(port_symmetry) == 3 and isinstance(port_symmetry[-1], dict):
                # _ExplictMap
    ...
    def get_simulations(
        ...
        if classification == "electrical":
            ...
            pml_gap = min(pml_gap, 3 * grid_scale)
    ...
    def as_bytes(self) -> bytes:
        ...
        if self.run_time is not None:
            obj["run_time"] = (
                _convert_arrays(self.run_time.dict())
                if isinstance(self.run_time, tidy3d.components.base.Tidy3dBaseModel)
                else self.run_time
            )
        if self.medium is not None:
            obj["medium"] = _convert_arrays(self.medium.dict())
        if self.boundary_spec is not None:
            obj["boundary_spec"] = _convert_arrays(self.boundary_spec.dict())
        if len(self.monitors) > 0:
            obj["monitors"] = [_convert_arrays(m.dict()) for m in self.monitors]
        if len(self.structures) > 0:
            obj["structures"] = [_convert_arrays(s.dict()) for s in self.structures]
        if self.grid_spec is not None:
            obj["grid_spec"] = (
                _convert_arrays(self.grid_spec.dict())
                if isinstance(self.grid_spec, tidy3d.GridSpec)
                else self.grid_spec
            )
        if len(self.port_symmetries) > 0:
