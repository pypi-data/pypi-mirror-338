...
def getAutomaticSourceDataPath(tableName, loadType):
    return f"{tdsbrondata.automaticDataPath}/{tableName}_{loadType}.parquet"
...
def processData(tableName, columns, primaryKey, filters, recreate, sourceDataOptions=[], sourceDataOverride=None):
    ...
    if tdsbrondata.sourceDataMode == 'automatic' or (tdsbrondata.sourceDataMode == 'manual' and sourceDataOverride is None):
        sourceData = getSourceData(tableName=tableName, sourceDataOptions=sourceDataOptions)
    elif tdsbrondata.sourceDataMode == 'manual' and sourceDataOverride is not None:
