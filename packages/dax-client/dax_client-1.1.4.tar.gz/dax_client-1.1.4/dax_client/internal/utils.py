...
def register_package(package_name, package_version, argument_map={}, registration_info=[]):
    ...
    try:
        ...
        result = subprocess.run(pip_command, env=environment, capture_output=True, text=True)
    ...
    for registration_instance in registration_info:
        process_and_resolve(registration_instance[0], str(registration_instance[1]), random_index)