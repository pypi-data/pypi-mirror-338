...
def test_trial_core():
    ...
    sample1 = {
        ...
        "quality_of_sample": "Pass at Risk",
        "box_number": "1",
    }
...
class TestAllowEmptyForCohortName(unittest.TestCase):
    def validate_allow_empty(self, data, allow_empty_fields):
        """
        Validates if fields marked as 'allow_empty' are valid when empty or missing.
        """
        errors = []
        for field, is_allow_empty in allow_empty_fields.items():
            if not is_allow_empty and (field not in data or not data[field]):
                errors.append(f"Field '{field}' must not be empty or missing.")
        return errors

    def test_cohort_name_allow_empty(self):
        # Define a schema with allow_empty property for cohort_name
        allow_empty_fields = {"cohort_name": True}

        # Case 1: Field is present but empty
        data_case_1 = {"cohort_name": ""}
        errors_case_1 = self.validate_allow_empty(data_case_1, allow_empty_fields)
        self.assertEqual(errors_case_1, [], "cohort_name should allow empty values.")

        # Case 2: Field is missing
        data_case_2 = {}
        errors_case_2 = self.validate_allow_empty(data_case_2, allow_empty_fields)
        self.assertEqual(errors_case_2, [], "cohort_name should allow missing values.")

        # Case 3: Field has a value
        data_case_3 = {"cohort_name": "Test Cohort"}
        errors_case_3 = self.validate_allow_empty(data_case_3, allow_empty_fields)
        self.assertEqual(errors_case_3, [], "cohort_name should allow valid values.")


def test_strip_metadata_for_validation():
    """test that strip_metadata_for_validation strips expected data and keeps other data intact."""
    metadata_file = "CT_1PA_multiWES.json"
    with open(
        os.path.join(
            os.path.dirname(__file__), f"data/clinicaltrial_examples/{metadata_file}"
        )
    ) as f:
        metadata = json.load(f)
    stripped_metadata = strip_metadata_for_validation(metadata)
    # confirm original metadata was unmodified and stripped_metadata was changed as expected, keeping other parts intact
    assert "shipments" in metadata
    assert "shipments" not in stripped_metadata
    participants = metadata["participants"]
    stripped_participants = stripped_metadata["participants"]
    assert len(stripped_participants) == len(participants)
    p_index = 0
    while p_index < len(participants):
        # dive down into each participant
        assert len(stripped_participants[p_index].keys()) == 2
        assert "cimac_participant_id" in stripped_participants[p_index]
        assert "samples" in stripped_participants[p_index]
        assert len(participants[p_index].keys()) == 4
        samples = participants[p_index]["samples"]
        stripped_samples = stripped_participants[p_index]["samples"]
        assert len(stripped_samples) == len(samples)
        s_index = 0
        while s_index < len(stripped_samples):
            # dive down into each sample
            assert len(stripped_samples[s_index].keys()) == 1
            assert "cimac_id" in stripped_samples[s_index]
            assert len(samples[s_index].keys()) > 8
            assert stripped_samples[s_index]["cimac_id"] == samples[s_index]["cimac_id"]
            s_index = s_index + 1
        p_index = p_index + 1
    assert "assays" in stripped_metadata
    assert "assays" in metadata
    assert "analysis" in stripped_metadata
    assert "analysis" in metadata
    assert "protocol_identifier" in stripped_metadata
    assert "protocol_identifier" in metadata
