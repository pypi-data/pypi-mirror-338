...
class _pkgconfig_from_source(dep.Provider):
  ...
  def __init__(self,name): ############################################
    ...
    self.source_dir = self.extract_dir/("pkg-config-pkg-config-%s" % VER)
    self.build_dir = self.source_dir/".build"
    self.url = "https://gitlab.freedesktop.org/pkg-config/pkg-config/-/archive/pkg-config-%s/pkg-config-pkg-config-%s.tar.gz" % (VER,VER)
    self.VERSION = VER
