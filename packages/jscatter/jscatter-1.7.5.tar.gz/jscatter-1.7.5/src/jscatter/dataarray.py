...
class dataListBase(list):
    ...
    def prune(self, *args, **kwargs):
        ...
        *args,**kwargs :
        ...
        kind : 'log', 'lin', 'unique', array, default 'lin'
    ...
    def modelValues(self, **kwargs):
        ...
        **kwargs : parname=value
        ...
        # loop over dataList elements to fill result
        for i in range(len(self)):
            # xxArgs will have X,Z,eX and eZ result and appended Y values
            xxArgs = []
            ...
            for key, item in mappedArgs.items():
                ...
                elif isinstance(item, numbers.Number):  # single numbers in parameters
                    singleArgs[key] = item
                ...
                elif isinstance(item, (list, np.ndarray, attributelist)):  # lists parameters
                    if key in self._link:
                        ureverse = self._link[key][2]
                        singleArgs[key] = item[ureverse[i]]
                    else:
                        # just in case. All list parameters should be in _link. This was befor _link
            ...
            if isinstance(fX, numbers.Integral) and fX < 0:
                ...
                result[-1].setColumnIndex(**columnIndex)
    ...
    def fit(self, model, freepar={}, fixpar={}, mapNames={}, method='lm', xslice=slice(None), condition=None,
        ...
        freepar : dictionary
            Fit parameter names with start values.
             ...
             - ``{'D':[1,2.3,4.5,...],..}`` list of float, individual parameters for each dataArray
                                            for independent fit.
             - ``{'D':[1,...,'name']}`` list of floats + one string:
                *link* parameter to attribute 'name'.
                Usefull to combine e.g. several measurements.

                For each unique value in attribute 'name' one free parameter instead for each dataArray.

                If all 'name' are different it is the same as without link.
        ...
        * For :math:`\mathbf{\chi^2 minimization}` ('normal fitting') the unbiased estimate
          ...
          Differences are weighted by measurement errors :math:`\sigma_i^2 = .eY^2`.
