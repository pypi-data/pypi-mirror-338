...
import { B as Block, S as Static, P as BlockTitle, H as Clear, w as Image, M as Music, aA as Video, y as File$1, aB as Paperclip, aC as Microphone, aD as Send, ac as Square } from './2-Dd-Yw8ID.js';
import { U as Upload } from './ModifyUpload-CvEYc3pf.js';
import { I as InteractiveAudio$1 } from './InteractiveAudio-DQqUolsG.js';
export { default as BaseExample } from './Example18-CKl0frTn.js';
...
const MultimodalTextbox = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  do {
    ...
    $$rendered = `<div class="${["full-container svelte-1q3gv4b", dragging ? "dragging" : ""].join(" ").trim()}" role="group" aria-label="Multimedia input field"${add_attribute("this", full_container, 0)}>${validate_component(BlockTitle, "BlockTitle").$$render($$result, { root, show_label, info }, {}, {
      default: () => {
        return `${escape(label)}`;
      }
    })} ${value.files.length > 0 || uploading ? `<div class="thumbnails scroll-hide svelte-1q3gv4b" aria-label="Uploaded files" data-testid="container_el" style="${"display: " + escape(value.files.length > 0 || uploading ? "flex" : "none", true) + ";"}">${each(value.files, (file, index) => {
      return `<span role="listitem" aria-label="File thumbnail"><button class="thumbnail-item thumbnail-small svelte-1q3gv4b"><button class="${["delete-button svelte-1q3gv4b", disabled ? "disabled" : ""].join(" ").trim()}">${validate_component(Clear, "Clear").$$render($$result, {}, {}, {})}</button> ${file.mime_type && file.mime_type.includes("image") ? `${validate_component(Image, "Image").$$render(
        $$result,
        {
          src: file.url,
          title: null,
    ...
    })} ${uploading ? `<div class="loader svelte-1q3gv4b" role="status" aria-label="Uploading"></div>` : ``}</div>` : ``} ${sources && sources.includes("microphone") && active_source === "microphone" ? `${validate_component(InteractiveAudio$1, "InteractiveAudio").$$render(
      $$result,
      {
        sources: ["microphone"],
        class_name: "compact-audio",
      ...
      {},
      {}
    )}` : ``} <div class="input-container svelte-1q3gv4b">${sources && sources.includes("upload") && !(file_count === "single" && value.files.length > 0) ? `${validate_component(Upload, "Upload").$$render(
      $$result,
      {
        file_count,
        filetype: file_types,
      ...
      {}
    )} <button data-testid="upload-button" class="upload-button svelte-1q3gv4b" ${disabled ? "disabled" : ""}>${validate_component(Paperclip, "Paperclip").$$render($$result, {}, {}, {})}</button>` : ``} ${sources && sources.includes("microphone") ? `<button data-testid="microphone-button" class="${["microphone-button svelte-1q3gv4b", ""].join(" ").trim()}" ${disabled ? "disabled" : ""}>${validate_component(Microphone, "Microphone").$$render($$result, {}, {}, {})}</button>` : ``}  <textarea data-testid="textbox" class="${["scroll-hide svelte-1q3gv4b", !show_label ? "no-label" : ""].join(" ").trim()}"${add_attribute("dir", rtl ? "rtl" : "ltr", 0)}${add_attribute("placeholder", placeholder, 0)}${add_attribute("rows", lines, 0)} ${disabled ? "disabled" : ""} ${autofocus ? "autofocus" : ""}${add_attribute("style", text_align ? "text-align: " + text_align : "", 0)}${add_attribute("this", el, 0)}>${escape(value.text || "")}</textarea> ${submit_btn ? `<button class="${["submit-button svelte-1q3gv4b", submit_btn !== true ? "padded-button" : ""].join(" ").trim()}" ${disabled ? "disabled" : ""}>${submit_btn === true ? `${validate_component(Send, "Send").$$render($$result, {}, {}, {})}` : `${escape(submit_btn)}`}</button>` : ``} ${stop_btn ? `<button class="${["stop-button svelte-1q3gv4b", stop_btn !== true ? "padded-button" : ""].join(" ").trim()}">${stop_btn === true ? `${validate_component(Square, "Square").$$render($$result, { fill: "none", stroke_width: 2.5 }, {}, {})}` : `${escape(stop_btn)}`}</button>` : ``}</div> </div>`;
  } while (!$$settled);
  return $$rendered;
});
...
//# sourceMappingURL=Index40-Dro_1J0D.js.map
