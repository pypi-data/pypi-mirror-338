...
def get_stock_market_str(symbol='', string=False):
    """判断股票ID对应的证券市场匹配规则
    """
    return str(get_stock_market(symbol, string=True))
...
def get_frequency(frequency) -> int:
    #print(f"para={frequency}, FREQUENCY列表当前 ={FREQUENCY}, 应该 = ['5m', '15m', '30m', '1h', 'day', 'week', 'mon', '1m', '1m', 'day', '3mon', 'year']")
    
    try:
        if isinstance(frequency, str):
            frequency = frequency.replace("1min", "ex_1m").replace('min', 'm').replace("month", "mon").replace("60min", "1h").replace("season", "3mon")

            #print(f"frequency is str={frequency}, index=", FREQUENCY.index(frequency))
            frequency = FREQUENCY.index(frequency)
        if isinstance(frequency, int):
            #print(f"frequency is int={frequency}")
            frequency = frequency
    except ValueError:
        print(f"[lyymoot][utils] error!!! frequency is not in FREQUENCY list={frequency}")
        frequency = 0
