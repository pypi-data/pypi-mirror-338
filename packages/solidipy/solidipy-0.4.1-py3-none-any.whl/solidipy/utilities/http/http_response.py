...
if FLASK_AVAILABLE := find_spec("flask") is not None:
    ...
    if PYDANTIC_AVAILABLE := find_spec("pydantic") is not None:
        ...
        from solidipy.constants.response_constants import http_success_mapping, OK
        ...
        class HttpResponse(BaseModel):
            ...
            def set_defaults_based_on_status_code(self) -> "HttpResponse":
                """
                Validator to set message based on status_code if not provided.

                :return: The updated HttpResponse instance with defaults applied.
                """

                current_response: dict = http_success_mapping.get(
                    self.status_code, OK
                )

                if self.message is None:
                    self.message = current_response["message"]
