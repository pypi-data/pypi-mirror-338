use cpython::{
    PyBytes, PyClone, PyErr, PyIterator, PyObject, PyResult, PyString, PyTuple, Python,
    PythonObject, ToPyObject, TypeError, ValueError,
};
use log::{debug, error};
use pyo3_ffi as ffi;
...
fn wsgi_iterable(obj: PyObject, py: Python) -> PyResult<PyObject> {
    unsafe {
...
impl<C: Connection> WSGIResponse<C> {
    ...
    pub fn set_pyobject(&mut self, pyobject: PyObject, py: Python) {
        ...
        if let Ok(fw) = pyobject.cast_as::<FileWrapper>(py) {
            if fw.sendfileinfo().borrow().fd != -1 {
    ...
    fn check_content_length_exceeds_data(
        ...
        if let Some(cl) = start_response.content_length() {
            if start_response.content_bytes_written() < cl {
                ...
                if let Some(method) = start_response.environ(py).get_item(py, REQUEST_METHOD) {
                    match method.extract::<PyString>(py) {
                        // HTTP method names are always upper case, see
                        // https://datatracker.ietf.org/doc/html/rfc7231#section-4.1
                        Ok(methodstr) if methodstr.to_string_lossy(py) == "HEAD" => return,
                        Err(_) => {
                            PyErr::fetch(py);
                        }
                        _ => (),
    ...
    pub fn render_next_chunk(&mut self, py: Python) -> PyResult<()> {
        match self.start_response.as_mut() {
            Some(pyob) => {
                let mut start_response = pyob.clone_ref(py).cast_into::<StartResponse>(py)?;
                            ...
                            Some(Ok(any)) => match any.cast_as::<PyBytes>(py) {
                                ...
                                Err(_) => {
                                    return Err(PyErr::new::<TypeError, _>(
                                        py,
                                        format!("Expected bytestring, got {any:?}"),
                                    ));
                                }
                            },
                        },
    ...
    fn write_chunk(&mut self, py: Python) -> io::Result<bool> {
        ...
        if self.sendfileinfo {
            if let Some(ob) = self.pyobject.as_mut() {
                self.last_chunk_or_file_sent = match ob.clone_ref(py).cast_into::<FileWrapper>(py) {
                    Ok(mut fw) => {
                        debug!("self.content_length: {:?}", self.content_length);
                        let mut c_l = 0;
                        if let Some(cl) = self.content_length {
                            c_l = cl;
                            fw.update_content_length(cl);
                        }
                        let (done, offset) = fw.send_file(&mut self.connection);
                        if done && (offset < c_l) {
                            self.connection.expire();
                            // The headers have already been sent,
                            // so rendering a 500 error is impossible.
                    ...
                    Ok(sr) => {
                        ...
                        let result = {
                            args.with_borrowed_ptr(py, |args| unsafe {
                                let res = ffi::PyObject_CallObject(application.as_ptr(), args);
                                // XXX why???
...
mod tests {
    ...
    use cpython::{PyClone, PyDict, PyObject, PyTuple, Python, PythonObject, TypeError};
    use log::debug;
        ...
        match py.run(r#"li = ["Hello", 42, None]"#, None, Some(&locals)) {
            Ok(_) => {
                ...
                match resp.render_next_chunk(py) {
                    ...
                    Err(e) => assert!(py.get_type::<TypeError>() == e.get_type(py)),
                }
            }
            _ => assert!(false),
        }
...
f = FL()"#,
            ...
            Ok(_) => {
                ...
                let fwany = fwtype
                    .call(py, PyTuple::new(py, &[filelike]), None)
                    .unwrap();
                let environ = PyDict::new(py);
                let headers = vec![(
                    "200 OK".to_string(),
                ...
                let mut resp = WSGIResponse::new(dummy_connection(), false);
                resp.start_response = Some(sr.as_object().clone_ref(py));
                resp.set_pyobject(fwany.as_object().clone_ref(py), py);
                match resp.render_next_chunk(py) {
                    Ok(_) => {
                        let expected = b"HTTP/1.1 200 OK\r\nContent-type: text/plain\r\nVia: pyruvate\r\nConnection: keep-alive\r\n\r\nHello world!\n";
    ...
    fn test_content_length_gt_data_len() {
        ...
        let py = gil.python();
        let locals = PyDict::new(py);
        match py.run(r#"li = [b"Hello world!\n"]"#, None, Some(&locals)) {
            Ok(_) => {
                let pylist = locals.get_item(py, "li").unwrap().as_object().clone_ref(py);
                let environ = PyDict::new(py);
...
f = FL()"#,
    ...
    fn test_content_length_gt_data_len_sendfile() {
        ...
        let sr = StartResponse::new(environ, headers, py).unwrap();
        let mut r = WSGIResponse::new(dummy_persistent_connection(connection), false);
        r.start_response = Some(sr.as_object().clone_ref(py));
        r.set_pyobject(fw.as_object().clone_ref(py), py);
    ...
    fn test_content_length_gt_data_len_head() {
        ...
        match py.run(r#"li = [b"Hello world!\n"]"#, None, Some(&locals)) {
            Ok(_) => {
                let pylist = locals.get_item(py, "li").unwrap().as_object().clone_ref(py);
                let environ = PyDict::new(py);
                environ.set_item(py, "REQUEST_METHOD", "HEAD").unwrap();
                let headers = vec![(
                    "200 OK".to_string(),
                    vec![
                        ("Content-type".to_string(), "text/plain".to_string()),
    ...
    fn test_content_length_gt_data_len_method_error() {
        ...
        match py.run(r#"li = [b"Hello world!\n"]"#, None, Some(&locals)) {
            Ok(_) => {
                ...
                let environ = PyDict::new(py);
                environ.set_item(py, "REQUEST_METHOD", py.None()).unwrap();
                let headers = vec![(
                    "200 OK".to_string(),
                    vec![
                        ("Content-type".to_string(), "text/plain".to_string()),
