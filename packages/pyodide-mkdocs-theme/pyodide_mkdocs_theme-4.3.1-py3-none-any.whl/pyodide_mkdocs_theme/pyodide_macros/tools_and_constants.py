...
class MacroShowConfig(MyEnum):
    ...
    SKIPPED_FIELDS: ClassVar[Tuple[str]] = ('python', 'all')

    python: 'MacroShowConfig' = AutoDescriptor()
    """ Show python code sections (ONLY for ide_manager related macros) """

    all: 'MacroShowConfig' = AutoDescriptor()
    """ Show args and python code sections (ONLY for ide_manager related macros) """
...
@dataclass_with_values
class PmtTests(MyEnum):
    """
    StrEnum-like object, referencing the possible options for the testing plugin's sub-config.
    """
    null:  'PmtTests' = AutoDescriptor()
    serve: 'PmtTests' = AutoDescriptor()
    site:  'PmtTests' = AutoDescriptor()
