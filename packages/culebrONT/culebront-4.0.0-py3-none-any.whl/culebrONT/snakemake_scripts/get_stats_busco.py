...
def main():
    ...
    dataframe_busco.reset_index(inplace=True)
    try:
        dataframe_busco.rename({"level_0": 'Assembler', "level_1": 'Steps'}, axis='columns', inplace=True, errors="raise")
    except KeyError as e:
        print(dataframe_busco)
    with open(out_stats, "w") as busco_file:
        with pd.option_context('display.float_format', '{:0.2f}'.format):
        #print(f"dataframe_busco:\n{dataframe_busco}\n")
            dataframe_busco.to_csv(busco_file, index=False, header=True)
