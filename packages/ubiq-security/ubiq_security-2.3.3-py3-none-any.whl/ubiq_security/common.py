...
def fetchDecryptKey(creds: credentials, datakey, client_id, alg):
    ...
    if (not config.get_key_caching_unstructured or 
        ...
        if creds.is_idp():
            # Check the cert is valid before it is used
            creds.renew_idp_cert()
            query['payload_cert'] = creds.idp_cert_base64
        ...
        if creds.is_idp():
            ...
            content['encrypted_private_key'] = creds.encrypted_private_key
