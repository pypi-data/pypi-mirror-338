...
from .drivers import diff_dicts
...
class Git:
    ...
    async def get_dagdiff(
        self, prev: Dict[str, Any], curr: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Compute the diff between two notebooks with Link pipelines.

        Args:
            prev: Notebook previous content in dict
            curr: Notebook current content in dict
        Returns:
            json strings which contains diff'd pipelines
        """
        return diff_dicts(prev, curr)
    ...
    def _get_script_path() -> str:
        script_path: pathlib.Path = pathlib.Path(__file__).resolve().parent / "scripts" / "mrxmerge.py"
    ...
    async def get_commit_ids_from_branch(self, branch, path):
        """
        Execute git log branch --format='%H' command & return the commit IDs.
        """
        ...
        result = {"code": code, "commit_ids": commit_ids}
        return result
    ...
    async def get_current_upstream_branch(self, path):
        """
        Execute 'git rev-parse --abbrev-ref @{u}' to get current local branch.
        If there are no no upstream configured for branch, then return None.
        """
        ...
        if code != 0:
            if error.startswith("fatal: no upstream configured for branch"):
                return {"code": 0, "current_branch": None}

            return {"code": code, "command": " ".join(cmd), "message": error}

        current_upstream_branch = output.strip("\n")
        return {"code": 0, "current_upstream_branch": current_upstream_branch}
