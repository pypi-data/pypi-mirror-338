...
class OpenRouterEntryPoint(AbstractEntryPoint):
    def __init__(
        self, model_id: str, api_key: str, base_url="https://openrouter.ai/api/v1", emb_dim: int = 1024,
          providers: list[str] = [], warmup: bool = False) -> None:
    ...
    def get_instruct_response(self, sentence: str, **kwargs) -> str:
        with contextlib.suppress(Exception):
            return (
                self._model.completions.create(
                    model=self.model_id,
                    prompt=sentence
                    **kwargs
                )
    ...
    def get_image_response(self, bytesimage: bytes, sentences: str, mimetype: str = 'image/jpeg') -> str:
        with contextlib.suppress(Exception):
            ...
            return (
                    self._model.chat.completions.create(
                        model=self.model_id,
                        messages=self.create_image_payload(system_prompt="", user_prompt=sentences,
                                                           image_encoded=encoded_image, mimetype=mimetype),
                        extra_body=self.extra_body,
                    )
                    .choices[0]
                    .message.content
