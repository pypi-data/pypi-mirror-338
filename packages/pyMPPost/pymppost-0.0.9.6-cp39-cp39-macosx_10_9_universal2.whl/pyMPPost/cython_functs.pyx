...
cpdef calc_pulses(long[:] history, long[:] cell, double[:] time, long[:] detector_mat, double[:] pgt_arr, double[:] dt_arr, double tol):
    ...
    Args:
        history (long[:] )          : Event history identifiers.
        cell (long[:] )             : Detector cell IDs for each event.
        time (double[:] )           : Time of each event.
        detector_mat (long[:] )     : Material IDs associated with each detector.
    ...
    cdef long curr_pulse = 0  # Current pulse ID
    cdef long curr_detect = -1  # Current detector ID
    cdef long curr_hist = -1  # Current event history ID

    cdef Py_ssize_t k
    pulses = np.zeros(N, dtype=long)  # Array to store pulse IDs for each event
    cdef long[::1] pulses_view = pulses  # Cython view for efficient memory access
...
cpdef shift_register_counting(long[:] history, double[:] time, double window_width, double tol):
    ...
    cdef long curr_hist = -1  # Current event history ID
    cdef long curr_pulse = -1  # Current pulse ID (not used in this function)
    cdef long curr_window = -1  # Current window ID

    cdef Py_ssize_t k
    windows = np.zeros(N, dtype=long)  # Array to store window IDs for each event
    cdef long[::1] window_view = windows  # Cython view for efficient memory access
