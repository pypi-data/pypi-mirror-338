...
def plot_confidence_intervals(
    ...
    show_underlying: bool = True,  # whether to show the underlying lines
) -> go.Figure:
    ...
    if poly_degree > 0:
        ...
        # Fit polynomials
        mean_coeffs = np.polyfit(x, mean, poly_degree)
        std_coeffs = np.polyfit(x, std, poly_degree)

        # Generate polynomial values
        mean = pd.Series(np.polyval(mean_coeffs, x), index=mean.index)
        std = pd.Series(np.polyval(std_coeffs, x), index=std.index)
