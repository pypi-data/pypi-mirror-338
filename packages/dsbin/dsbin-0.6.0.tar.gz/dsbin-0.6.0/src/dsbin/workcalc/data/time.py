...
class TimeAnalyzer:
    ...
    def format_time_span(span: TimeSpan, item_name: str = "item") -> list[str]:
        ...
        return [
            ...
            f"Time between first and last: {TextParse.plural('day', span.span_days, with_count=True)}, "
            f"{TextParse.plural('hour', span.span_hours, with_count=True)}",
        ]
    ...
    def format_distribution(dist: TimeDistribution, item_name: str = "item") -> list[str]:
        ...
        for day, (items, percentage) in dist.by_weekday.items():
            messages.append(
                f"{day.name.capitalize()}: {TextParse.plural(item_name, items, with_count=True)} ({percentage:.1f}%)"
            )
        ...
        for hour, items, percentage in dist.most_active_hours:
            messages.append(
                f"  {TimeAnalyzer.format_hour(hour)}: {TextParse.plural(item_name, items, with_count=True)} ({percentage:.1f}%)"
            )
