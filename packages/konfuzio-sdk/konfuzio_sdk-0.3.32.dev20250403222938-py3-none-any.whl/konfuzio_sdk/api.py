...
def _get_auth_token(username, password, host=KONFUZIO_HOST) -> str:
    ...
    if r.status_code == 200:
        ...
        raise PermissionError('[ERROR] Your credentials are not correct! Please run init again and provide the correct credentials.')
    else:
        raise ConnectionError(f'HTTP Status {r.status_code}: {r.text}')
    return token


def init_env(user: str, password: str, host: str = KONFUZIO_HOST, working_directory=os.getcwd(), file_ending: str = '.env'):
    """
...
def create_new_project(project_name, session=None):
    ...
    if r.status_code == 201:
        ...
        raise PermissionError(
            f'HTTP Status {r.status_code}: The project {project_name} was not created, please check' f' your permissions. Error {r.json()}'
        )
...
def post_document_annotation(
    ...
    **kwargs,
    ...
    custom_offset_string = kwargs.get('custom_offset_string', None)
    ...
    if custom_offset_string is not None:
        data['custom_offset_string'] = custom_offset_string
...
def get_category(category_id: int, session=None):
    """
    Get data about a Category and related Labels/Label Sets.

    :param category_id: ID of a Category to be requested.
    :param session: Konfuzio session with Retry and Timeout policy
    """
    if session is None:
        session = konfuzio_session()
    if hasattr(session, 'host'):
        ...
        host = None
    url = get_category_url(category_id=category_id, host=host)
    result = consume_paginated_api(url, session)
    return result
