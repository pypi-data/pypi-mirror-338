...
def main():
    if len(sys.argv) < 3:
        print("Usage: sculblog <command> <category> <postname> [<additional_value>]"z)
        sys.exit(1)
    ...
    if command == "process":
        ...
        if len(sys.argv) < 5:
            print("Date command requires additional date_splash parameter")
            sys.exit(1)
        date_splash = sys.argv[4]
        date(category, file_path, date_splash)
    elif command == "header":
        if len(sys.argv) < 5:
            print("Header command requires additional header parameter")
            sys.exit(1)
        header = sys.argv[4]
        header(category, file_path, header)
    else:
        print('Invalid command. Command must be "process", "hide", "unhide", "date", or "header"')
        sys.exit(1)
