...
class Application(object):
    ...
    def apply_para(self): # 将cfg中的配置写入Entry
        try:
            ...
            self.Entry_Text(self.UI.MAILDEST_Entry, self.para.get("MAIL","name@example.com"))            
    ...
    def get_para(self): # 将Entry写入cfg
        ...
        self.para["MAIL"]=self.UI.MAILDEST_Entry.get().strip()
    ...
    def S_submit(self):
        ...
        mail_dest=self.UI.MAILDEST_Entry.get().strip()
        self.to_stdout("提交作业")
        ...
        self.to_stdout(f'- 邮件提醒：{mail_dest}')
        if mail_dest=="":
            mail_dest="Zxzz106@qq.com"
        ...
        self.send_channel("export MAIL_DEST="+mail_dest)
        fluent_mpi="intel"
