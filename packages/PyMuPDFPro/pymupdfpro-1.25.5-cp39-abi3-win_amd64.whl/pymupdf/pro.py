...
# Allow override of how we create new process. Might be useful on MacOS.
        spawn_method = os.environ.get('PYMUPDFPRO_ACTIVITY_SPAWN')
        assert spawn_method in (None, 'popen', 'fork'), \
                f'PYMUPDF_PRO_ACTIVITY_SPAWN={spawn_method!r} should be \'popen\' or \'fork\'.'
        if spawn_method is None:
            spawn_method = 'popen' if platform.system() == 'Windows' else 'fork'

        if spawn_method == 'popen':
# os.fork not available, so we run a new Python process. We
# need it to inherit file descriptor <r>, and the simplest way
# to do this seems to be to set stdin=r because only fds 0, 1
# and 2 can be inherited.
            _activity_child = subprocess.Popen(
                    [
                        sys.executable,
                        '-c',
                        f'import pymupdf.pro; pymupdf.pro._activity_monitor(0, "{keystring}")',
                    ],
                    stdin=r,
# Ensure that Ctrl-C doesn't kill new process.
                    creationflags=subprocess.CREATE_NEW_PROCESS_GROUP,
                    )
        elif spawn_method == 'fork':
            p = os.fork()
            if p == 0:
# Child process.
                os.close(_activity_write)
# Ensure that Ctrl-C doesn't kill new process.
                os.setpgrp()
                _activity_monitor(r, keystring)
                os._exit(0)
                assert 0
# Parent process.
            _activity_child = p
    ...
    while 1:
        ...
        try:
            ...
            if item_bytes:
                ...
                item = int.from_bytes(item_bytes)
            else:
                end = True
        if end:
            break
...
def _activity_send(keystring, items, end):
