...
Usage:
  bfan.py run  [--image=<path or name>] [--execution=(local | remote | dryRun)]  [--view=(compact | summary | jsonSummary)] [--env=<env1=val1,env2=val2>] [--verbose --clearPassed --stdout --genXml --outputName=<outputName> --arch=<binArch> --tags=<equation> (--prefix <paths>)] [<path>...]
  bfan.py clean  [<path>...]
  bfan.py diff   [<path>...]
  bfan.py update [<path>...]
...
ALLOWED_ACTION={'run':['diff','exitCode','timeout','run','background','skip','kill'], 'diff':['diff'],'update':['diff']}
...
def backgroundMonitor(q,proc,t1,t2, cmd):
    ...
    while(True):
        try:
            ...
            if DBGMSG:
                print("[dbg]: backgroundMonitor got message:  {}  ".format(data), flush=True)
        except:
            data = None
        ...
        try:
            ...
            if DBGMSG:
                print("[dbg]: backgroundMonitor process  {} has finished ".format(proc.pid), flush=True)
            if DBGMSG:
                print("[dbg]: wait for streamreader 1 to be closed", flush=True)
            t1.join()
            if DBGMSG:
                print("[dbg]: wait for streamreader 2 to be closed", flush=True)
            t2.join()
            if DBGMSG:
                print("[dbg]: all stream reader jointed", flush=True)
            break
        ...
        if data:        
            try:
                if DBGMSG:
                    print("[dbg]: backgroundMonitor wait again for process {} to shutdown by himself ".format(proc.pid), flush=True)
                proc.wait(timeout=0.3)
                if DBGMSG:
    ...
    if "prefix" in config:
        for item in config["prefix"]:
            if "${version}" in item:
                ...
                regPattern =  Template( base+itemsplit[0]+"(\d+(\.\d+)*)"+itemsplit[1]).render(arch=config["arch"])
...
class CMDExecutor():
    ...
    def shellCmd(self,command, transcriptFunction,suite, config, background=False,timeout=1200,env=None,backgroundId=None):
...
def loadGlobalConfig(config, configPath, suite):
        
    config= z = {**config, 
                      **LoadSuitConfig(configPath + os.sep +"suites"+os.sep + suite + os.sep + CONFIG_FILE_NAME)}
    globalConfig = LoadSuitConfig(configPath + os.sep + os.sep + CONFIG_FILE_NAME)
    if globalConfig:
        if 'env' in config and 'env' in globalConfig:
            if 'variables' in config['env'] and 'variables' in globalConfig['env']:
                joined_map = {**config['env']['variables'], **globalConfig['env']['variables']}
                config['env']['variables'] = joined_map
    return config

class DryRunTestExecutor():
    def __init__(self,path,tel,ydef,config):
        self.ydef = ydef
        ...
        self.config = loadGlobalConfig(config,self.configPath, self.suite)
...
class TestRunner():
    def __init__(self,path,tel,ydef,config,executorName):
        self.ydef = ydef
        self.testName = os.path.basename(os.getcwd())
        self.suite=pathlib.Path(os.getcwd()).suffix[1:]
        self.tel = tel
        ...
        if executorName == "local":
            self.testExecutor = TestExecutor(path,tel,ydef,config)

        elif executorName == "dryRun":    
            self.testExecutor = DryRunTestExecutor(path,tel,ydef,config)
        else:
...
#        self.localConfigPaths=[]
    ...
    def checkexecutor(self):
        thisStatus = True
        msg = ""
        if not self.testExecutor:
            msg = "Unsupported type of executor; {}".format(self.executorName)
            self.tel.error(msg)
            thisStatus = False
...
def prepareResultInCurrentDir(tel):
    ...
    if os.path.isdir(TEST_SRC_DIR):
        tel.log("coping data...")
        shutil.copytree(TEST_SRC_DIR, TEST_RESULT_DIR,symlinks = False)
    else:
...
#         self.mode = mode;
#     def CreateTestRunner(self,item,tel,ydef,config):
#         if self.mode == "dryRun":
#             return TestRunner(item,tel,ydef,config)
...
def main():
    ...
    arguments = docopt(__doc__, version='bfan 1.0.0')
    ...
    executorName = arguments["--execution"]
    if executorName == None:
        executorName = "local"
...
#        if len(testPathToRun) > 1 :
        ...
        for item in testPathToRun:           
            ...
            testRunner = TestRunner(item,tel,item['ydef'],config,executorName)

            status,msg =  testRunner.checkexecutor()
