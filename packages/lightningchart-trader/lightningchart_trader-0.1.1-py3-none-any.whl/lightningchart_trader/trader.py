...
from lightningchart_trader.heatmap import Heatmap
from lightningchart_trader.instance import Instance
from lightningchart_trader.utils.utils import (
    normalize_data,
...
class TAChart:
    def __init__(self, license_key: str):
        ...
        self.instance.send(self.id, 'createTradingChart', {'licenseKey': license_key})
        self.point_count = 0
        self.settings = {}

    def open(
        self, method: str = None, width: int | str = '100%', height: int | str = 600
    ):
    ...
    def add_data_array(self, data: Union[dict, list, 'DataFrame'], scroll=False):
        ...
        valid_data = [
            ...
            if all(key in item for key in ['open', 'high', 'low', 'close', 'dateTime'])
        ]

        self.point_count += len(valid_data)
        self.instance.send(
            self.id, 'addDataArray', {'data': valid_data, 'scroll': scroll}
        )
        return self
    ...
    def add_chaikin_oscillator(
        self, fast_period_count: int = 3, slow_period_count: int = 10
    ):
        """
    ...
    def add_ellipse(
        ...
        """
        return Ellipse(
            self, startX=startX, startY=startY, radiusX=radiusX, radiusY=radiusY
        )
    ...
    def add_fibonacci_retracements(
        ...
        return FibonacciRetracements(
            self, startX=startX, startY=startY, endX=endX, endY=endY
        )
    ...
    def add_text_box(
        self, positionX: int | float, positionY: int | float, text: str = 'Text'
    ):
    ...
    def set_price_chart_type(self, chart_type: str):
        ...
        if chart_type in valid_chart_types:
            ...
            raise ValueError(
                f'Invalid chart type: {chart_type}. Valid types are: {", ".join(valid_chart_types)}'
            )
        return self
    ...
    def add_drawing_tool(
        ...
        Args:
            tool_name (str): The name of the drawing tool to add (e.g., 'TrendLine').
            start_x (int): The X-position (index) of the starting control point.
            start_y (int): The Y-position of the starting control point.
            end_x (int, optional): The X-position (index) of the ending control point (if applicable). Defaults to None.
            end_y (int, optional): The Y-position of the ending control point (if applicable). Defaults to None.
        ...
        self.instance.send(self.id, 'addDrawingTool', args)
        return self
    ...
    def fetch_data(
        ...
        """Fetch trading data from the provider based on the given Symbol, and add it to the chart.

        Args:
            symbol (str): The symbol whose data is to be fetched.
            name (str): The name of the stock, will be shown on top of the chart.
            start_time (datetime, optional): The earliest data value's datetime. Defaults to None.
            end_time (datetime, optional): The latest data value's datetime. Defaults to None.
        ...
        if start_time:
            args['startTime'] = start_time.strftime('%Y-%m-%dT%H:%M:%S')
        if end_time:
            args['endTime'] = end_time.strftime('%Y-%m-%dT%H:%M:%S')

        self.instance.send(self.id, 'fetchData', args)
        return self
    ...
    def add_symbol_search_results(self, response_array):
        ...
        """
        self.instance.send(
            self.id, 'addSymbolSearchResults', {'responseArray': response_array}
        )
        return self
    ...
    def set_open_interest_data(self, open_interest_data: list):
        ...
        """
        self.instance.send(
            self.id, 'setOpenInterestData', {'openInterestData': open_interest_data}
        )
        return self
    ...
    def set_data_packing(self, packing_enabled: bool):
        ...
        """
        self.instance.send(
            self.id, 'setDataPacking', {'packingEnabled': packing_enabled}
        )
        return self

    def set_line_color(self, new_color: str):
        """Sets the color for Line and Mountain chart types.

        Args:
            new_color (str): The new color as a string in HEX format, e.g., '#FFFFFF'.

        """
        self.instance.send(self.id, 'setLineColor', {'newColor': new_color})
        return self
    ...
    def set_ohlc_cursor_tracking(self, tracking_type: int):
        ...
        """
        self.instance.send(
            self.id, 'setOHLCCursorTracking', {'trackingType': tracking_type}
        )
        return self
    ...
    def set_percent_scale(self, use_percentage: bool):
        ...
        """
        self.instance.send(
            self.id, 'setPercentScale', {'usePercentage': use_percentage}
        )
        return self
    ...
    def set_watermark_text(self, watermark_text: str):
        ...
        """
        self.instance.send(
            self.id, 'setWatermarkText', {'watermarkText': watermark_text}
        )
        return self
    ...
    def show_theme_image(self, show_theme_image: bool):
        ...
        """
        self.instance.send(
            self.id, 'showThemeImage', {'showThemeImage': show_theme_image}
        )
        return self
    ...
    def show_zoom_band_chart(self, show_zoom_band_chart: bool):
        ...
        """
        self.instance.send(
            self.id, 'showZoomBandChart', {'showZoomBandChart': show_zoom_band_chart}
        )
        return self
    ...
    def show_splitter_lines(self, show_splitter_lines: bool):
        ...
        """
        self.instance.send(
            self.id, 'showSplitterLines', {'showSplitterLines': show_splitter_lines}
        )
        return self
    ...
    def set_panning_button(self, panningButton: int):
        ...
        """
        self.instance.send(
            self.id, 'setPanningButton', {'panningButton': panningButton}
        )
        return self
    ...
    def set_vertical_zooming(self, verticalZooming: bool):
        ...
        """
        self.instance.send(
            self.id, 'setVerticalZooming', {'verticalZooming': verticalZooming}
        )
        return self
    ...
    def set_vertical_panning(self, verticalPanning: bool):
        ...
        """
        self.instance.send(
            self.id, 'setVerticalPanning', {'verticalPanning': verticalPanning}
        )
        return self
    ...
    def show_file_selection(self, showFileSelection: bool):
        ...
        """
        self.instance.send(
            self.id, 'showFileSelection', {'showFileSelection': showFileSelection}
        )
        return self
