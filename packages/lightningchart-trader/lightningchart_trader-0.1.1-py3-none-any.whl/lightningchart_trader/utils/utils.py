...
def date_to_iso(data: Union[datetime, str, float, int, List, dict]):
    ...
    if isinstance(data, list):
        return [
            item for item in (date_to_iso(item) for item in data) if item is not None
        ]
...
def normalize_data(data: Union[dict, list, 'DataFrame']) -> list:
    ...
    regex_patterns = {
        ...
        'openInterest': re.compile(
            r'^(open[_\- ]*interest|OpenInterest|OPENINTEREST)$', re.IGNORECASE
        ),
    }
    ...
    if pd and isinstance(data, pd.DataFrame):
        ...
        raise ImportError(
            'Pandas is not installed. Please install pandas to use DataFrame as input.'
        )
        ...
        for item in data:
            if isinstance(item, dict):
                ...
                try:
                    ...
                    raise ValueError(
                        f"Invalid list format: {item}. Expected at least 5 values for 'open', 'high', 'low', 'close', and 'dateTime'."
                    )
...
def preprocess_data_point(strict=True, *args, **kwargs):
    ...
    if args:
        if len(args) == 1 and isinstance(args[0], (list, tuple)):
            data_point = dict(
                zip(keys, list(args[0]) + [None] * (len(keys) - len(args[0])))
            )
        else:
    ...
    regex_patterns = {
        ...
        'close': re.compile(r'^(close|Close|CLOSE)$', re.IGNORECASE),
        'dateTime': re.compile(
            r'^(date[_\- ]*time|time|date|Date|DATE)$', re.IGNORECASE
        ),
        'volume': re.compile(r'^(volume|Volume|VOLUME)$', re.IGNORECASE),
        'openInterest': re.compile(
            r'^(open[_\- ]*interest|OpenInterest|OPENINTEREST)$', re.IGNORECASE
    ...
    if isinstance(data_point, (list, tuple)):
        ...
        if len(data_point) < 5:
            raise ValueError(
                'Data point tuple/list must include at least 5 values: open, high, low, close, and dateTime.'
            )
        data_point = dict(
            zip(keys, list(data_point) + [None] * (len(keys) - len(data_point)))
        )
    ...
    if strict:
        ...
        if missing_fields:
            raise ValueError(
                f'Data point must include {", ".join(mandatory_fields)}. Missing: {", ".join(missing_fields)}'
            )
