...
class Masterclass(type):
    ...
    # Basic usage is as simple as this:
        ...
        class MyService(metaclass=Masterclass):
            def __init__(self):
                pass
    ...
    def __call__(cls: type[T], *args: Any, **kwargs: Any) -> T:
        ...
        if cls not in Masterclass.__locks:
            Masterclass.__locks[cls] = Lock()

        with Masterclass.__locks[cls]:
            if cls not in Masterclass.__instances:
                instance = super().__call__(*args, **kwargs)  # type: ignore[misc]
                Masterclass.__instances[cls] = instance
            return Masterclass.__instances[cls]
