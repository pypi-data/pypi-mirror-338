...
def verify_jobs_change(job_config_dict: dict):
    ...
    if src.jobs_qtd != qtd_schedule_jobs:
        mensagem = f"Quantidade de Jobs do módulo {src.jobs_qtd}, quantidade de jobs no Schedule{qtd_schedule_jobs} - Versão {src.version}"
        send_log('EXECUCAO', src.client_data.get('enviar_logs'), True, mensagem, 'X', 'main',
