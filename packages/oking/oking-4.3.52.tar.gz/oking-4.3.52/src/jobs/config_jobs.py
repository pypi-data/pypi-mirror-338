...
from src.jobs.product_jobs import query_products_erp, query_list_products_tax, query_product_launch, \
    query_associated_product, query_showcase_product
...
def realizar_operacao(job_config_dict: dict, job_nome: str):
    try:
        ...
        send_log(job_nome, src.client_data.get('enviar_logs'), True, f'Validando {job_nome}'
                 , 'info', job_nome, f'valida {job_nome}')
        return lines.__len__()

    except Exception as e:
        logger.error(f'Erro não tratado capturado: {str(e)}')
        send_log('realizar_operacao', src.client_data.get('enviar_logs'), True, f'Erro ao Executar: {e}'
                 , 'error', 'realizar_operacao', '')
        raise e
