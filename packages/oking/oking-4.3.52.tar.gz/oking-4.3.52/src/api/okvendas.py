...
def post_send_distribution_center(body, job_config_dict):
    try:
        ...
        if response.ok:
            ...
            send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
                     f'Erro ao realizar POST na api okvendas {src.client_data.get("url_api_principal")}/catalogo/unidadeDistribuicao '
                     + str(response.json()), 'error', 'envia_centro_distribuicao_job', 'ENVIA_CENTRO_DISTRIBUICAO')
            return False

    except Exception as e:
        send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True
                 , f'Erro {str(e)}', 'error', 'envia_centro_distribuicao_job', 'ENVIA_CENTRO_DISTRIBUICAO')
        return False
    ...
    try:
        ...
        if response.ok:
            ...
            send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
                     ...
                     + str(response.json()), 'error', 'envia_filial_job', 'ENVIA_FILIAL')
            return False

    except Exception as e:
        send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True
                 , f'Erro {str(e)}', 'error', 'envia_filial_job', 'ENVIA_FILIAL')
        return False
