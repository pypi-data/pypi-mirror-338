    ...
    def resource(
        ...
        **kwargs,
        ...
        if isinstance(name, (dict, ResourceTypeClass, ResourceTypeDescriptor)):
            ...
            if isinstance(name, ResourceTypeClass):
                attrs = {'resource_type': name.standard['resource_type']}
            else:
                attrs = name
            name = '_anonymous'
        ...
        if BindingNotFound.check(v):
            if name == '_anonymous':
                raise LookupError(f'Unknown method to calculate resource for attrs: {attrs}')
            raise LookupError(f'Unknown method to calculate "{name}" with attrs: {attrs}')
        if isinstance(v, ModelBinding):
            combined_defaults = v.defaults.copy()
            if 'params' in attrs:
