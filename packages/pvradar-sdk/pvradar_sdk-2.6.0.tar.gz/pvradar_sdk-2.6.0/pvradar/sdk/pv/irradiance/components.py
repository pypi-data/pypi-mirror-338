...
@standard_resource_type(R.ground_reflected_irradiance_on_front, override_unit=True)
def ground_reflected_irradiance_on_front(
    surface_tilt: Annotated[pd.Series, R.surface_tilt_angle],
    global_horizontal_irradiance: Annotated[pd.Series, R.global_horizontal_irradiance],
    array: ArrayDesign,
) -> pd.Series:
    ground_reflected_irradiance_on_front = pvlib.irradiance.get_ground_diffuse(
        surface_tilt=surface_tilt, ghi=global_horizontal_irradiance, albedo=array.albedo_value
    )
    return ground_reflected_irradiance_on_front


@standard_resource_type(R.sky_diffuse_irradiance_on_front, override_unit=True)
def sky_diffuse_irradiance_on_front_pvlib_perez_driesse(
    surface_tilt: Annotated[pd.Series, R.surface_tilt_angle],
    ...
    """
    sky_diffuse_irradiance_on_front = pvlib.irradiance.perez_driesse(
    ...
    return sky_diffuse_irradiance_on_front  # type: ignore


@standard_resource_type(R.sky_diffuse_irradiance_on_front, override_unit=True)
def sky_diffuse_irradiance_on_front_pvlib_haydavies(
    surface_tilt: Annotated[pd.Series, R.surface_tilt_angle],
    surface_azimuth: Annotated[pd.Series, R.surface_azimuth_angle],
    diffuse_horizontal_irradiance: Annotated[pd.Series, R.diffuse_horizontal_irradiance],
    direct_normal_irradiance: Annotated[pd.Series, R.direct_normal_irradiance],
    ...
    Sum of isotropic, horizon, and circumsolar
    """
    sky_diffuse_irradiance_on_front = pvlib.irradiance.haydavies(
        surface_tilt=surface_tilt,
        surface_azimuth=surface_azimuth,
        dhi=diffuse_horizontal_irradiance,
        dni=direct_normal_irradiance,
...
@standard_resource_type(R.direct_irradiance_on_front, override_unit=True)
def direct_irradiance_on_front(
    surface_tilt: Annotated[pd.Series, R.surface_tilt_angle],
    surface_azimuth: Annotated[pd.Series, R.surface_azimuth_angle],
    solar_zenith_angle: Annotated[pd.Series, R.solar_zenith_angle],
    solar_azimuth_angle: Annotated[pd.Series, R.solar_azimuth_angle],
    ...
    direct_irradiance_on_front = pvlib.irradiance.beam_component(
        surface_tilt=surface_tilt,
        surface_azimuth=surface_azimuth,
        solar_zenith=solar_zenith_angle,
        solar_azimuth=solar_azimuth_angle,
    ...
    return direct_irradiance_on_front


# TODO: fully implement this function based on pvfactors
@standard_resource_type(R.ground_reflected_irradiance_on_rear, override_unit=True)
def ground_reflected_irradiance_on_rear(interval: pd.Interval) -> pd.Series:
    timestamps = interval_to_index(interval)
    ground_reflected_irradiance_on_rear = pd.Series(index=timestamps, data=[0] * len(timestamps))
    return ground_reflected_irradiance_on_rear
