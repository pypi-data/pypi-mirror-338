...
class PvLibraryHandler(BaseLibraryHandler):
    ...
    def get_binders(self):
        return [
            PvBinder(),
            FirstMatchingTypeBinder(self.get_models(), name='irradiance'),
        ]

    @override
    def enrich_context(self, context: BaseModelContext) -> None:
        context.register_model(irradiance.sky_diffuse_irradiance_on_front_pvlib_perez_driesse, for_resource_type=True)
