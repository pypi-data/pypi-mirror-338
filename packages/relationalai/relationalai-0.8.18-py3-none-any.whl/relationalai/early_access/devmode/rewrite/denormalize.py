...
class Denormalize(c.Pass):
    ...
    class Context():
        ...
        denormalized: dict[ir.Relation, ir.Relation]
    ...
    def handle_model(self, model: ir.Model, parent: None, ctx:Optional[Any]=None):
        ...
        ctx = Denormalize.Context(denormalized)
        ...
        return ir.Model(
            ...
            self.walk(model.root, model, ctx),
        )
    ...
    def handle_update(self, node: ir.Update, parent: ir.Node, ctx:Optional[Any]=None):
        ...
        ctx = cast(Denormalize.Context, ctx)
        if node.relation not in ctx.denormalized:
            return super().handle_update(node, parent, ctx)

        denormalized_relation = ctx.denormalized[node.relation]
    ...
    def handle_lookup(self, node: ir.Lookup, parent: ir.Node, ctx:Optional[Any]=None):
        ...
        ctx = cast(Denormalize.Context, ctx)
        if node.relation not in ctx.denormalized:
            return super().handle_lookup(node, parent, ctx)
    ...
    def handle_logical(self, node: ir.Logical, parent: ir.Node, ctx:Optional[Any]=None):
        ...
        body = self.walk_list(node.body, node, ctx)
