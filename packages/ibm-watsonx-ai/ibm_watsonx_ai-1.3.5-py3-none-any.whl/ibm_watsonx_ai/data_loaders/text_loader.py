...
class TextLoader:
    ...
    @staticmethod
    def _json_to_string(binary_data: bytes) -> str:
        loaded_json = json.loads(binary_data)

        def get_all_str(data: Any) -> list[str]:
            match data:
                case dict():
                    return [
                        value
                        for key in data
                        for value in [key] + get_all_str(data[key])
                    ]
                case list():
                    return [value for el in data for value in get_all_str(el)]
                case str():
                    return [data]
                case _:
                    return []  # ignore non string leaves

        return "\n\n".join(get_all_str(loaded_json))
