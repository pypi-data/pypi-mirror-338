...
VAD_TYPE = 3  # 0:文件,文件内多行,每行都是i:j .. 1:文件, 格式如下:[[i,j],[i,j],[i,j]..] .. 2: str, 格式如下:[[i,j],[i,j],[i,j]..] ... 3:vad str, 格式如下:i:j i:j i:j ...
TTS_FINAL_TYPE=0  # 0:简短list型.. 1:含txt的scp型
def get_args():
...
def post_process(output_dir,tts_final_path,shard_output_dir, text_dir):
    ...
    txt_file_list = glob.glob(os.path.join(text_dir, '*.txt'))
    temp_txt_file_dict = {}
    ...
    utils_file.write_dict_to_scp(text_scp_dict, os.path.join(output_dir, 'text'))
    utils_file.logging_print(f'text_scp得到完成,text_scp_dict长度为：{len(text_scp_dict)}')

    utils_file.logging_print('删除text中的标点')
    text_with_punctuation_dict = utils_file.load_dict_from_scp(os.path.join(output_dir, 'text'))
    utils_file.write_dict_to_scp(text_with_punctuation_dict, os.path.join(output_dir, 'text_with_punctuation'))
    text_no_punctuation_dict = {}
    for key, value in tqdm.tqdm(text_with_punctuation_dict.items(), total=len(text_with_punctuation_dict)):
    ...
    utils_file.logging_print('删除text中的标点完成')

    utils_file.logging_print('开始生成shard')
    ...
    utils_file.logging_print('生成shard完成')
...
if __name__ == '__main__':
    output_dir = "/home/node36_data/xlgeng/asr_data_from_pachong/gxl_output/liukai_chuantongguoxue"
    utils_file.makedir_for_file_or_dir(output_dir)
    old2new_dir = "/home/work_nfs10/wjtian/data_handle/tlzuo/init_lists"
    vad_info_dir = "/home/work_nfs10/wjtian/data_handle/tlzuo/vad/cut_points.txt"
    final_tts_scp_path = "/home/work_nfs10/wjtian/data_handle/tlzuo/final_all.scp"

    # solution(final_tts_scp_path, output_dir, old2new_dir, vad_info_dir)
    text_dir = "/home/work_nfs10/wjtian/data_handle/tlzuo/asr"
    shard_output_dir = "/home/work_nfs14/xlgeng/asr_data_shard/pachong_data/8_liukai_chuantongguoxue/"
    # post_process(output_dir, final_tts_scp_path,shard_output_dir, text_dir)
