...
import os
# 获取当前工作目录
current_directory = "/home/work_nfs8/xlgeng/new_workspace/wenet_gxl_aishell/examples/aishell/do_goutu"
# 添加指定目录到PYTHONPATH
python_path = os.path.join(current_directory, "tools", "k2", "icefall")
python_path_2 = os.path.join(current_directory, "tools", "k2", "icefall", "egs", "aishell", "ASR", "local")
# 检查PYTHONPATH是否已设置，如果没有则设置为空字符串
if "PYTHONPATH" not in os.environ:
    os.environ["PYTHONPATH"] = ""
# 将指定目录添加到PYTHONPATH
os.environ["PYTHONPATH"] += ":" + python_path
os.environ["PYTHONPATH"] += ":" + python_path_2
...
def generate_lexicon(token_sym_table: Dict[str, int],
    ...
    """
    lexicon = []
    for word in tqdm.tqdm(words, desc="Generating lexicon", total=len(words)):
...
def generate_words(text_file: str) -> Dict[str, int]:
    ...
    """
    words = []
    lines = utils_file.load_list_file_clean(text_file)
    for line in tqdm.tqdm(lines, desc="Generating words", total=len(lines)):
        word = line.replace('\n', '').split()[0]
        # assert word not in words
        words.append(word)
    words = list(set(words))
    words.sort()
    utils_file.logging_print('耿雪龙: 生成词典list len: {}'.format(len(words)))
...
def prepare_char(input_words_path, input_tokens_path, output_dir):
    utils_file.makedir(output_dir)
    timer = utils_file.GxlTimer()
    timer.start()
    utils_file.logging_print('耿雪龙: 开始生成字典')
    token2id = generate_tokens(input_tokens_path)
    utils_file.logging_print('耿雪龙: 生成字典完成, len: {}'.format(len(token2id)))
    timer.stop_halfway()
    utils_file.logging_print('耿雪龙: 开始生成word2id')
    word2id = generate_words(input_words_path)
    utils_file.logging_print('耿雪龙: 生成word2id完成, len: {}'.format(len(word2id)))
    timer.stop_halfway()
    ...
    utils_file.logging_print('耿雪龙: 开始生成词典')
    lexicon = generate_lexicon(token2id, words)
    utils_file.logging_print('Lexicon size: {}'.format(len(lexicon)))
    timer.stop_halfway()
...
if __name__ == "__main__":
    ...
    utils_file.logging_print('哈哈哈哈')
    prepare_char(input_tokens_path=token_file, input_words_path=word_file, output_dir=output_dir)
