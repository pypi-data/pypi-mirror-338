...
def make_shards_common(jsonl_file, shards_dir, num_utts_per_shard=1000, prefix='shards', resample=16000,
                       num_threads=32):
    """"""
...
input_dir = "/mnt/sfs/asr/update_data/speech2text_token_DB-ASR-106_new_add_by_2025-3_10/tmp/"
key = "03295_03296_03295_061626_061828"
file_list = glob.glob(os.path.join(input_dir, f"{key}.*"))
print(file_list)
res_dict = {}
import copy
...
for file in file_list:
    ...
    if key == 'wav':
        ...
        value = utils_file.load_first_row_clean(file)
    res_dict[key] = value
print(res_dict)
output_data_list_path = "/mnt/sfs/asr/code/wenet_undersdand_and_speech_xlgeng/examples/wenetspeech/whisper/data/train_data.list"
res_list = []
for i in range(1000):
    new_dict = copy.deepcopy(res_dict)
    new_dict['key'] = f"gxl_{i}"

    res_list.append(new_dict)
utils_file.write_dict_list_to_jsonl(res_list, output_data_list_path)
# make_shards_common(output_data_list_path, '/mnt/sfs/asr/code/wenet_undersdand_and_speech_xlgeng/examples/wenetspeech/whisper/data/shards_train',num_threads=1)
input_file = "/mnt/sfs/asr/code/wenet_undersdand_and_speech_xlgeng/examples/wenetspeech/whisper/data/shards_train/shards_list.txt"
lines = utils_file.load_list_file_clean(input_file)
lines = lines * 1000
utils_file.write_list_to_file(lines, input_file)

# a   = utils_file.do_get_now_time()
# b = utils_file.do_get_elapsed_time(a)