...
class AsyncBithuman(Bithuman):
    ...
    def __init__(
        ...
        *,
        ...
        logger.debug(f"Initializing AsyncBithuman with token={token is not None}, api_secret={api_secret is not None}")
        super().__init__(
    ...
    async def set_model(self, model_path: str = None) -> "AsyncBithuman":
        ...
        if not model_path:
            ...
            raise ValueError("Model path must be provided either during initialization or when calling set_model")
        ...
        if self.__model_hash:
            ...
            if Path(self._model_path).is_file():
                try:
                    ...
                    loop = self._loop or asyncio.get_running_loop()
                    model_hash = await loop.run_in_executor(None, calculate_file_hash, self._model_path)
    ...
    def _frame_producer(
        ...
        try:
            ...
            frame_iterator = None
            try:
                frame_iterator = super().run(out_buffer_empty)
            except Exception as e:
