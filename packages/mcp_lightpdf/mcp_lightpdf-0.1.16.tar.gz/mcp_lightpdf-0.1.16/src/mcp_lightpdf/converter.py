...
class Converter:
    ...
    async def convert_file(self, file_path: str, format: str) -> ConversionResult:
        """转换单个文件
        ...
        await self.logger.log("info", f"正在将 {input_format.value.upper()} 转换为 {output_format.value.upper()} 格式...")

        async with httpx.AsyncClient(timeout=120.0) as client:
            try:
                # 创建转换任务
                task_id = await self._create_task(client, file_path, format, is_url)
    ...
    async def _create_task(self, client: httpx.AsyncClient, file_path: str, format: str, is_url: bool) -> str:
        """创建转换任务"""
