...
from .const import DEFAULT_PROFILER_FOLDER
...
async def profiler_middleware_dispatch(
    ...
    """
    profile_type = g.config.get("PROFILER_TYPE", "html")
    renderer_name = g.config.get("PROFILER_RENDERER", "HTMLRenderer")
    ...
    folder_name = (
        g.config.get("PROFILER_FOLDER", DEFAULT_PROFILER_FOLDER) + request.url.path
    )
    os.makedirs(
        folder_name,
        exist_ok=True,
...
class ProfilerMiddleware(BaseHTTPMiddleware):
    def __init__(self, app: ASGIApp) -> None:
        os.makedirs(
            g.config.get("PROFILER_FOLDER", DEFAULT_PROFILER_FOLDER), exist_ok=True
        )
