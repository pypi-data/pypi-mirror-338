...
from ..const import DEFAULT_API_MAX_PAGE_SIZE, EXCLUDE_ROUTES, logger
...
class ModelRestApi(BaseApi):
    ...
    def __init__(self):
        ...
        self.max_page_size = self.max_page_size or g.config.get(
            "API_MAX_PAGE_SIZE",
    ...
    async def _generate_info_schema(
        ...
        schema.add_schema = (
            ...
            or self._generate_jsonforms_schema(self.add_obj_schema, schema.filters)
        )
        schema.add_uischema = self.add_jsonforms_uischema
        schema.edit_schema = (
            self.edit_jsonforms_schema
            or self._generate_jsonforms_schema(self.edit_obj_schema, schema.filters)
        )
    ...
    def _generate_jsonforms_schema(
        self, schema: type[BaseModel], filters: dict[str, typing.Any]
    ):
        """
        ...
        Args:
            ...
            filters (dict[str, typing.Any]): The filters to get the values from for the schema.
        ...
        for key, value in jsonforms_schema["properties"].items():
            ...
            if self.datamodel.is_boolean(key):
                ...
                value["oneOf"] = [
                    ...
                    for item in filters[key]["schema"].values
                ]
                if not value["oneOf"]:
                    value["oneOf"].append(
                        {
