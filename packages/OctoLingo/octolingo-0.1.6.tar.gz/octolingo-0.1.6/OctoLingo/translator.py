...
class OctoLingo:
    def __init__(self, glossary=None):
        ...
        self.html_regex = re.compile(r'<[^>]+>')  # For handling HTML content
    ...
    def detect_language(self, text):
        ...
        try:
            ...
            clean_text = self._extract_text_from_html(text) if self._is_html(text) else text
            detection = self.translator.detect(clean_text)
    ...
    @cache_translation
    def translate(self, text, dest_language, src_language='auto', max_retries=3, is_html=False):
        """
        ...
        :param is_html: Whether the text contains HTML markup.
        :return: Translated text and confidence score.
        """
        self.validate_language(dest_language)
        
        if is_html:
            return self._translate_html(text, dest_language, src_language, max_retries)
        
        # Apply glossary before translation
        text = self.glossary.apply_glossary(text)
    ...
    def _translate_html(self, html_content, dest_language, src_language='auto', max_retries=3):
        """Special handling for HTML content to preserve tags."""
        from bs4 import BeautifulSoup
        soup = BeautifulSoup(html_content, 'html.parser')
        
        # Extract all text nodes that need translation
        text_elements = soup.find_all(text=True)
        untranslatable_tags = ['script', 'style', 'code', 'pre']
        
        for element in text_elements:
            if element.parent.name not in untranslatable_tags and element.strip():
                try:
                    ...
                    # Skip translation for this element if error occurs
                    continue
    ...
    def _is_html(self, text):
        ...
        return bool(self.html_regex.search(text))

    def _extract_text_from_html(self, html_content):
        """Extract text content from HTML for language detection."""
        from bs4 import BeautifulSoup
        soup = BeautifulSoup(html_content, 'html.parser')
        return soup.get_text()

    async def translate_async(self, text, dest_language, src_language='auto', max_retries=3, is_html=False):
        """Asynchronously translate text to the target language."""
        return await asyncio.get_event_loop().run_in_executor(
            None, self.translate, text, dest_language, src_language, max_retries, is_html
        )

    def translate_batch(self, texts, dest_language, src_language='auto', max_retries=3, is_html=False):
        """
        ...
        :param max_retries: Maximum number of retries for failed translations.
        :param is_html: Whether the texts contain HTML markup.
        :return: List of translated texts and confidence scores.
        """
        return [self.translate(text, dest_language, src_language, max_retries, is_html) for text in texts]