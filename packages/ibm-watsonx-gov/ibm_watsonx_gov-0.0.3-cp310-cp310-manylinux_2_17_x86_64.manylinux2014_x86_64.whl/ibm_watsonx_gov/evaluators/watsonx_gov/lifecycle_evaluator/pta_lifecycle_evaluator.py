...
class PTALifecycleEvaluator:
    """
    Class responsible to trigger the e2e lifecycle of pta with evaluation support
    """
    ...
    def __send_request(self, method: str, **kwargs) -> requests.Response:
        """
        Helper method to wrap requests.request method. This will raise exception if the response status code is not 2xx.
        """
        self.logger.info(
    ...
    def __validate_dict_schema(
        ...
        """
        Helper method to validate dicts against a schema. This will validate the following:
    ...
    def __validate_setup_stages(self, stages: list[EvaluationStage]) -> None:
        self.logger.info(f"Validating setup stages: {stages}")

        # Always require the development stage as it has the initial prompt template setup
        if EvaluationStage.DEVELOPMENT not in stages:
            message = f"{EvaluationStage.DEVELOPMENT} must be present in setup_stages."
            self.logger.error(message)
            raise Exception(message)

        self.setup_stages = stages
    ...
    def evaluate_df(self, input_df: pd.DataFrame, scoring_url: str) -> list[dict[str, any]]:
        """
        Method to evaluate the prompt. This will take the scoring url and will process input_df.
        ...
        try:
            prompt_template_variables = list(
                self.__stage_configurations[EvaluationStage.DEVELOPMENT].prompt_template.get(
                    "prompt_variables", {}).keys()
            )
    ...
    def create_prompt_template_asset(self, stage_configurations: ProjectConfigurations) -> str:
        """
        Method to create a prompt template asset
        """
        self.logger.info("Creating prompt template")

        display_message_with_frame("Creating Prompt Template Asset...")

        params = {"project_id": stage_configurations.project_id}
        payload = stage_configurations.prompt_template

        try:
            response = self.__send_request(
                method="post",
                url=f"{self.__platform_url}/wx/v1/prompts/",
                json=payload,
                params=params,
            )
        except Exception as e:
        ...
        try:
            project_prompt_template_id = response.json()["id"]
        except Exception as e:
            message = f"Failed to get the prompt template id from the response. {e}"
            self.logger.error(message)

        self.logger.info(
            f"Prompt template created successfully. Prompt template id: {project_prompt_template_id}"
        )

        display_message_with_frame(
            message=f"prompt template created successfully. Prompt template details:\n\n{json.dumps(response.json(), indent=2)}",
            frame_char='-'
        )

        return project_prompt_template_id
    ...
    def trigger_prompt_setup(self, stage: EvaluationStage) -> None:
        """
        Method to trigger prompt set up for prompt template in a given evaluation stage. This will poll until the
        prompt template is set up successfully.
        """
    ...
    def risk_evaluation_for_pta_subscription_in_space(
        ...
        """
        Function to trigger risk evaluation for PTA subscription in space. This assumes that
        payload logging and feedback data sets are populated.
    ...
    def get_measurement_ids(
        ...
        """
        Retrieve measurement IDs for a given subscription ID and monitor definition ID.
    ...
    def display_evaluation_url(
        ...
        """
        Helper function to build the evaluation url based in the given stage. This will raise an exception if the stage is not part of the setup.
        ...
        # determine the base url based on the environment (IBM Cloud or CPD)
        # CPD: https://cpd-cpd-instance.apps.wxg511ocs416gpu52-1.cp.fyre.ibm.com
        # IBM Cloud: https://us-south.ml.cloud.ibm.com
        base_url = self.__dataplatform_url if self.use_cpd else self.__wml_url
        ...
        if stage == EvaluationStage.DEVELOPMENT:
            ...
            base_url = self.__dataplatform_url if self.use_cpd else self.__wml_url
            evaluation_url = f"{base_url}/wx/prompt-details/{self.__prompt_template_ids[stage]}/evaluate?context=wx&project_id={self.__stage_configurations[stage].project_id}"
            ...
            base_url = self.__dataplatform_url if self.use_cpd else self.__wml_url
            evaluation_url = f"{base_url}/ml-runtime/deployments/{self.__deployment_ids[stage]}/evaluations?space_id={self.__stage_configurations[stage].space_id}&context=wx&flush=true"
            evaluation_url_message = (
    ...
    def display_factsheet_url(
        ...
        """
        Helper function to build the factsheet url based in the given stage. This will raise an exception if the stage is not part of the setup.
        ...
        if stage == EvaluationStage.DEVELOPMENT:
            ...
            base_url = self.__dataplatform_url if self.use_cpd else self.__wml_url
            factsheet_url = f"{base_url}/wx/prompt-details/{self.__prompt_template_ids[stage]}/factsheet?context=wx&project_id={self.__stage_configurations[stage].project_id}"
            ...
            base_url = self.__dataplatform_url if self.use_cpd else self.__wml_url
            factsheet_url = f"{base_url}/ml-runtime/deployments/{self.__deployment_ids[stage]}/details?space_id={self.__stage_configurations[stage].space_id}&context=wx&flush=true"
            factsheet_url_message = (
    ...
    def get_monitors_with_measurements_info(
        ...
        """
        This function will retrieve the data from the backend and will return
        a dictionary of the monitor data that was retrieved. Optionally, the function
        would display the response as a table. This will also update the object state
        to store the recent value for the monitors info.
        ...
        # Check if the stage exist in setup_stages
        if stage not in self.setup_stages:
            message = f"the stage {stage} must be part of the setup stages"
            self.logger.error(message)
            raise Exception(message)
    ...
    def get_metrics_from_monitor_list(
        ...
        """
        Retrieves metrics from a list of monitors based on the provided monitor name.
    ...
    def get_monitor_data_set_records(
        ...
        """
        Retrieves monitor data set records for a given data set type and evaluation stage.
    ...
    def space_deployment_risk_evaluation_data_set_setup(
        ...
        self.logger.info(message)
        display_message_with_frame(message=message, frame_char="-")
    ...
    def __generate_feedback_data(
        ...
        """
        Generates feedback data for a given input file path, prediction data, and prompt setup.
        ...
        try:
            fields = list(self.__stage_configurations[EvaluationStage.DEVELOPMENT].prompt_template["prompt_variables"].keys(
            )) + [prompt_setup["label_column"]]
        except Exception as e:
    ...
    def __instance_mapping_for_cpd(
        ...
        """
        Function to check if the given stage has a cpd instance mapping. If not it will be set
        to the default service instance id would be "00000000-0000-0000-0000-000000000000".
    ...
    def setup(
        ...
        """
        Function to create do the set up based on the configuration provided.
        ...
        self.__validate_setup_stages(setup_stages)
        ...
        for stage in [EvaluationStage.DEVELOPMENT, EvaluationStage.PRE_PRODUCTION, EvaluationStage.PRODUCTION]:
            ...
            if stage == EvaluationStage.DEVELOPMENT:
                ...
                self.__prompt_template_ids[stage] = self.create_prompt_template_asset(
                    stage_configurations=self.__stage_configurations[stage],
                )
            ...
            # If we are doing the set up for production or pre_production, use the setup flow for a space
                ...
                self.__prompt_template_ids[stage] = self.promote_prompt_to_space(
                    ...
                    project_prompt_template_id=self.__prompt_template_ids[EvaluationStage.DEVELOPMENT],
                    space_id=self.__stage_configurations[stage].space_id,
                )
    ...
    def __parse_configuration(self, config: dict) -> None:
        """
        Function to parse the configuration. This assumes that the configuration
        ...
        if EvaluationStage.DEVELOPMENT in self.setup_stages:
            self.__stage_configurations[EvaluationStage.DEVELOPMENT].prompt_template = self.config["development"][
                "prompt_template_payload"
            ]
            self.__stage_configurations[EvaluationStage.DEVELOPMENT].prompt_setup = self.config["development"]["prompt_setup"]
            self.__stage_configurations[EvaluationStage.DEVELOPMENT].project_id = self.config["development"]["project_id"]
