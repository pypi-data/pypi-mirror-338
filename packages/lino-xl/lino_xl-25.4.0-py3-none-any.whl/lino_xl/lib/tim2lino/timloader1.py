...
class TimLoader(TimLoader):
        ...
        if hasattr(row, 'periode'):
            ...
            if period is None:
                ...
                period = self.get_or_save(
                    StoredPeriod, ref=ref, start_date=sd, end_date=ed)
        else:
            period = None
    ...
    def load_gen2account(self, row, **kw):
        ...
        if len(idgen) == self.LEN_IDGEN:
            ...
            # kw.update(name=names[0])
            # names2kw(kw,row.libell1,row.libell2,row.libell3,row.libell4)
            yield self.get_or_save(Account, **kw)
            # obj = Account(**kw)
            # if idgen == "612410":
    ...
    def load_vnl(self, row, **kw):
        ...
        if isinstance(doc, trading.VatProductInvoice):
            ...
            if row.code == 'G':
                kw.update(account=self.get_or_save(Account, ref=idart))
    ...
    def load_par(self, row):
            ...
            if isocode:
                try:
                    ...
                    country = self.get_or_save(
                        Country, isocode=isocode, name=isocode)
                    # country.save()
        ...
        if compte1:
            ...
            if kw['iban']:
                try:
                    yield self.get_or_save(sepa.Account, **kw)
                except ValidationError:
    ...
    def load_art(self, row, **kw):
        ...
        return self.get_or_save(products.Product, **kw)

    def create_users(self):
        self.ROOT = self.get_or_save(
            users.User, username='tim', first_name='tim', id=1,
