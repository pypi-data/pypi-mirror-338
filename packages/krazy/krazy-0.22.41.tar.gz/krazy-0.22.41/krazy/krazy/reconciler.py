...
def match_many_to_many(one:list[pd.DataFrame, str], two:list[pd.DataFrame, str], col_from_one:list[str,str], col_from_two:list[str,str], match_return_col:str, match_word:str = 'Matched', tolerance:list=[1,-1])->list[pd.DataFrame, pd.DataFrame]:
    '''
    Takes:
    df_one: DataFrame from where one row matches to many from other, dataframe name to use in match return column
    df_two: DataFrame from where one rows match to one from other, dataframe name to use in match return column
    ...
    df_one_grouped = df_one.loc[~df_one[match_return_col].str.contains(match_word, case=False)].groupby([col_from_one[0]])
    df_two_grouped = df_two.loc[~df_two[match_return_col].str.contains(match_word, case=False)].groupby([col_from_two[0]])

    # check if sum columns are same and change if required
    if col_from_one[1] == col_from_two[1]:
        col_from_one_revised = col_from_one[1] + '_one'
        col_from_two_revised = col_from_two[1] + '_two'
    else:
        col_from_one_revised = col_from_one[1]
        col_from_two_revised = col_from_two[1]


    # merge dataframes
    df_merged = pd.merge(
        left=df_one_grouped.agg({col_from_one_revised:'sum'}).reset_index(), 
        right=df_two_grouped.agg({col_from_two_revised:'sum'}).reset_index(), 
        left_on=col_from_one[0],
        right_on=col_from_two[0],
        how='inner',
        indicator='Source')
    
    df_merged['Diff'] = abs(df_merged[col_from_one_revised]) - abs(df_merged[col_from_two_revised])    

    # reconcile
    df_merged.loc[(df_merged['Diff']<=tolerance[0]) & (df_merged['Diff']>=tolerance[1]), match_return_col] = f'{match_word} with {two[1]} on its column {col_from_two[0]}'
    # retain only those lines which are matched
    df_merged[match_return_col].fillna('',inplace=True)
    df_merged = df_merged.loc[df_merged[match_return_col].str.contains(match_word, case=False)]

    # updated in df one
    ## get rows which are matched

    matched_key_list = df_merged[col_from_one[0]].unique().tolist()
    ## create mapping
    matched_dict = df_merged.set_index(col_from_one[0]).to_dict()[match_return_col]
    ## update df one
    df_one.loc[df_one[col_from_one[0]].isin(matched_key_list),match_return_col] = df_one.loc[df_one[col_from_one[0]].isin(matched_key_list)][col_from_one[0]].map(matched_dict)
    
    # updated in df two
    ## updated match description for df two
    df_merged[match_return_col] = f'{match_word} with {one[1]} on its column {col_from_one[0]}'
    ## get rows which are matched
    matched_key_list = df_merged[col_from_two[0]].unique().tolist()
    ## create mapping
    matched_dict = df_merged.set_index(col_from_two[0]).to_dict()[match_return_col]
    ## update df one
    df_two.loc[df_two[col_from_two[0]].isin(matched_key_list),match_return_col] = df_two.loc[df_two[col_from_two[0]].isin(matched_key_list)][col_from_two[0]].map(matched_dict)
