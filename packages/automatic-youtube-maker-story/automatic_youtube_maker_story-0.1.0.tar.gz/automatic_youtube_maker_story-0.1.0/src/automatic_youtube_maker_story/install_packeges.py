...
def cudaKIT126(path, filename=None):
    ...
    if os.path.isdir(r'C:\Program Files\NVIDIA GPU Computing Toolkit\CUDA\v12.6'):
        print(f'O diretório "{r'C:\Program Files\NVIDIA GPU Computing Toolkit\CUDA\v12.6'}" existe.')
    else:
        print(f'O diretório "{r'C:\Program Files\NVIDIA GPU Computing Toolkit\CUDA\v12.6'}" não existe. Instalando CUDA 12.6')
...
def cudaKIT121(path):
    ...
    if os.path.isdir(r'C:\Program Files\NVIDIA GPU Computing Toolkit\CUDA\v12.1'):
        ...
        print(f'O diretório "{r'C:\Program Files\NVIDIA GPU Computing Toolkit\CUDA\v12.1'}" não existe. Instalando CUDA 12.1')
...
def addComfyUI(path_root, path_base):
            ...
            try:
                ...
                try:                
                    ...
                    try:
                        ...
                        salveYamlComfy(path_comfyUI,path_base)
...
def addNationXTTs(path_root):
    ...
    name_venv = "venv_commum"

    try:
        criar_venv(path_rest, name_venv, "python311")  # Substitua "python3.10" pela versão desejada
    except:
        print(f"❌ Erro ao criar a venv. Erro Fatal.")
        return False
    ...
    try:                
        ...
        pipInstallIndex(path_rest,name_venv,pacotes,index)
    ...
    try:                
        ...
        pacotes =["story-chilren-gemma3", "story-children-video-maker", "narration-xtts2"]
        pipInstall(path_rest,name_venv,pacotes)
        return True
...
def configPaths():
    ...
    historic_history = input("Passe o arquivo de controle conteudo .json: ")
    # Exemplo de uso
    config_data = {"path_audio": add_trailing_slash(path_root), "path_base_comfy": add_trailing_slash(path_base), "historic_history": historic_history}
...
def start():
    ...
    path_base = input("diretorio output do ComfyUI: ")
    historic_history = input("Passe o arquivo de controle conteudo .json: ")
    # path_base = "W:\\Youtube\\confyui\\"
    # path_root = "C:\\Users\\will_\\OneDrive\\Documentos\\GitHub\\Automatic-Youtube-Maker\\"
    # historic_history = "C:\\Users\\will_\\OneDrive\\Documentos\\GitHub\\Automatic-Youtube-Maker\\SOURCE\\controle.json"
    # Exemplo de uso
    config_data = {"path_audio": add_trailing_slash(path_root), "path_base_comfy": add_trailing_slash(path_base), "historic_history": add_trailing_slash(historic_history)}
    salvar_json(config_data)

    print("Adicionando ComfyUI :")

    if addComfyUI(path_root, path_base):
