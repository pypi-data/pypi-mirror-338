...
class Transfer:
    ...
    def process_data(self, 
        ...
        Args:
            ...
            time_horizon (int, optional): Time horizon parameter (in days). Defaults to None.
            market_data_path (str, optional): Path to market data file (csv/parquet). Defaults to None.
            trade_data_path (str, optional): Path to trade data file (csv/parquet). Defaults to None.
            endpoint_path (str, optional): API endpoint path. Defaults to "/fix/market".
        ...
        # Initialize variables to track filenames
        trade_filename = None
        ...
        # If no files to upload, return an error
        if not files_to_upload:
            return {
                "status": "error",
                "message": "No files provided for upload. At least one of market_data_path or trade_data_path must be provided."
            }
        ...
        # Add trade data URI only if trade_data_path was provided
        if trade_filename:
            trade_data_uri = f"s3://{bucket_name}/{trade_filename}"
            processing_params["trade_data_uri"] = trade_data_uri
        
        # Add market data URI if available
        if market_filename:
            market_data_uri = f"s3://{bucket_name}/{market_filename}"
            processing_params["market_data_uri"] = market_data_uri
