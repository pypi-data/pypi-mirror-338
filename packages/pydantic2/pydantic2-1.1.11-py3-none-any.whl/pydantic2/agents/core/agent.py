...
class FormAgent:
    ...
    async def _extract_form_info(
        ...
        try:
            ...
            # Правильно сериализуем данные формы и метаданные
            user_form_data = form_data.session_info.user_form.model_dump(mode="json") if hasattr(form_data.session_info.user_form, "model_dump") else {}
            metadata_data = form_data.session_info.metadata.model_dump(mode="json") if hasattr(form_data.session_info.metadata, "model_dump") else {}
