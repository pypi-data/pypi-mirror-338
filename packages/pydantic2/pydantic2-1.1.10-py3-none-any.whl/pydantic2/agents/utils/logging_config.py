...
# Create a console for pretty JSON output
...
class LogConsole:
    ...
    def print(self, message, level="INFO"):
        """Print a regular message with colored module name"""
        # Format header as in logger
        now = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')

        # This one works properly
        print('>>>>>>', now, self.name, level, message)


def log_json(name, message, data):
    """Log JSON data with formatting, ALWAYS displayed regardless of settings"""
    ...
    rich_console.print_json(data=data, indent=2)
...
class SimpleLogger:
    ...
    def set_agents_logs_visible(cls, visible):
        ...
        if visible:
            logger.enable("pydantic2.agents")
        else:
            logger.disable("pydantic2.agents")

            # Exception for active loggers so they can see their own logs
            for name, instance in cls._instances.items():
                if name.startswith("pydantic2.agents"):
                    logger.enable(name)
    ...
    def __init__(self, name, extra=None):
        ...
        if not SimpleLogger._show_agents_logs and name.startswith("pydantic2.agents"):
            logger.enable(name)
