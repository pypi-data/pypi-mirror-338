...
class LlamaCppTokenCounter():
    ...
    def count_tokens(self, text: dict | str) -> int:
        if isinstance(text, dict):
            token_texts = self.client.tokenize(text["text"].encode(), add_bos=False, special=False)
        elif isinstance(text, str):
            token_texts = self.client.tokenize(text.encode(), add_bos=False, special=False)
...
class LlamaCppChatHistoryManager():
    def __init__(self, 
        ...
        self.model = model
        self.client = client
        self.token_counter = LlamaCppTokenCounter(self.client, tools, initial_message_tokens, additional_message_tokens)
        with open(pathlib.Path(__file__).parent.parent / "models" / "local_models.json", "r") as f:
            models_json = json.load(f)
        
        self.defined_max_tokens = defined_max_tokens
        for model_info in models_json["local_models"]:
    ...
    def _adjust_message_content(self, contents: list[dict], max_tokens: int) -> list[dict]:
        ...
        for i, content in enumerate(reversed_contents):
            if content["type"] == "text" or content["type"] == "refusal":
                encoded_message = self.client.tokenize(co)
    ...
    def _adjust_content_text(self, text: str, max_tokens: int) -> str:
        encoded_text = self.client.tokenize(text.encode(), add_bos=False, special=False)
        decoded_text = self.client.detokenize(encoded_text)[:max_tokens-4]
        return decoded_text