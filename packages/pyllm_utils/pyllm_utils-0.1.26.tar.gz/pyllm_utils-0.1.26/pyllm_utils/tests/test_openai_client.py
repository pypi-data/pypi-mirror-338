...
import logging
import inspect
from llm.openai_api.openai_client import OpenAIClient
...
def test_request_messages_with_user_image_local_path():
    client = OpenAIClient()
    # PosixPathをstr型に変換
    image_path = str(pathlib.Path(__file__).parent.parent / 'test.png')
    messages = [
        {
            'role': 'user',
            'content': [
...
def test_request_messages_with_user_image_base64():
    client = OpenAIClient()
    file_path = str(pathlib.Path(__file__).parent.parent / 'test.png')
...
def test_request_messages_audio():
    ...
    audio_transcript = response[0][0]['content'][0]['audio']['audio_transcript']
    audio_data = response[0][0]['content'][0]['audio']['content']
    audio_id = response[0][0]['content'][0]['audio']['id']  # 後続の会話で使用可能なID
    log_test_response(f"Audio ID: {audio_id}")
    
    # オーディオデータの保存（テスト用）
    if audio_data:
        import base64
        wav_bytes = base64.b64decode(audio_data)
        output_path = str(pathlib.Path(__file__).parent.resolve() / 'test_outputs' / 'output.wav')
        with open(output_path, "wb") as f:
            f.write(wav_bytes)
            f.flush()  # バッファをフラッシュして確実にデータを書き込む
    
    correct, reason = judge_authenticity("音声の内容が書かれているか、音声が識別できないことが書かれているかのどちらかがあれば正解です。", audio_transcript)
    assert correct, reason
    
def test_multiple_response_messages():
    client = OpenAIClient()
    response = client.request_messages(messages=[{'role': 'user', 'content': 'こんにちは、世界！'}], n=2)
    response = client.get_latest_response()
    log_test_response(response)
...
def test_multiple_tool_calls():
    client = OpenAIClient()
    response = client.request_messages(messages=[{'role': 'user', 'content': '2と3を足し合わせてください。また3と4を足し合わせてください。'}], tools=tools, tool_choice="auto", n=2, tool_object=Tool())
    response = client.get_latest_response()
    log_test_response(response)
    correct1, reason1 = judge_authenticity("2と3を足し合わせた結果が書かれていれば正解です。", str(response[0]))
    correct2, reason2 = judge_authenticity("3と4を足し合わせた結果が書かれていれば正解です。", str(response[1]))
    assert len(response) == 2
    assert correct1, reason1
    assert correct2, reason2
...
def test_stream_response_messages_with_tool_calls():
    client = OpenAIClient()
    response_generator = client.request_messages(messages=[{'role': 'user', 'content': '2と3を足し合わせてください。また3と4を足し合わせてください。'}], tools=tools, tool_choice="auto", stream=True, tool_object=Tool())
    for response in response_generator:
        pass
    response = client.get_latest_response()
    log_test_response(response)
