...
class OpenAIChatHistoryManager():
    def __init__(self, 
        ...
        self.token_counter = OpenAITokenCounter(model, tools, initial_message_tokens, additional_message_tokens)
        self.model = model
    ...
    def adjust_messages_length(self, messages: list[dict]) -> list[dict]:
        # If model name does not start with "gpt" or "chatgpt",
        # convert any "system" role in messages to "user"
        if not (self.model.lower().startswith("gpt") or self.model.lower().startswith("chatgpt")):
            for message in messages:
