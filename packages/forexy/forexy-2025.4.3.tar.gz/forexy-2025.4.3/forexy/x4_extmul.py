...
def get_input(args):
  ...
  if not re.compile("^[1-9A-Z]\d{4}\.\d{3}(\.[1-9A-Z])?$").search(data_id) and data_id!="all":
    print(" ** EXFOR dataset ID "+data_id+" is illegal.")
  while not re.compile("^[1-9A-Z]\d{4}\.\d{3}(\.[1-9A-Z])?$").search(data_id) and data_id!="all":
    data_id=input("EXFOR dataset ID for extraction [23756.002.3] -> ")
    if data_id!="all":
      data_id=data_id.upper()
    if data_id=="":
