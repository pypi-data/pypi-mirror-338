...
def read_system_identifier(lines,key_keep):
  ...
  n_msg=0
  for line in lines:
    if re.compile("\S").search(line[0:10]): # first line of the keyword
    ...
    if keyword=="DICTION":
      ...
      if keyword=="TRANS" or keyword=="MASTER":
        if not re.compile("^\s{7}[0-9A-Z]\d{3}").search(line[11:22]):
      ...
      subent="000"
      nsubent=-1
      if not re.compile("^\s{6}[0-9A-Z]\d{4}$").search(line[11:22]):
      ...
      if re.compile("[0-9A-Z]\d{3}").search(line[62:66]):
      ...
      sys_id="SUBENT"
      nsubent+=1
      if not re.compile("^\s{3}[0-9A-Z]\d{7}$").search(line[11:22]):
...
#     }
      ...
      if len(line)>32 and re.compile("\d{6,8}").search(line[22:33]):
        date=str2int(line[22:33].lstrip(),line)
        date=check_date(ansan,keyword,date,line)
      else:
...
def read_bib(ansan,keyword,pointer,bib_text,line):
  if re.compile("\S").search(line[0:10]): # first line of the keyword
    ...
    pointer=""
    if re.compile("\s").search(line[10:11]):
...
def read_table(ansan,keyword,line,nentry,nsubent,sys_id,t_field,n_field,data_line,flag_line,msg_last,n_msg):
  ...
  for i in range (6):
    ...
    if t_field=="heading":
      ...
      if re.compile("\S").search(line[col2-1:col2]):
    ...
    if n_field==x4_json["entries"][nentry]["subentries"][nsubent][sys_id]["N1"]: # end of one table line
      ...
      if re.compile("\S").search(line[col2:66]):
...
def anal_code(ansan,keyword,line,lines,pointer,code_str):
  ...
  if keyword=="ANG-SEC" or \
    ...
    if re.compile("^MPH=\(\d+\-[A-Z]+\-\d+\(.+?,.+?\)(\d+\-[A-Z]+\-\d+)?\)").search(code_str):
      m=re.compile("^MPH=\((\d+\-[A-Z]+\-\d+\(.+?,.+?\)(\d+\-[A-Z]+\-\d+)?)\)").search(code_str)
      reaction=m.group(1)
      codes=re.split(",|\(|\)",reaction)
      ...
      particles=re.split("\+",process)
      for particle in particles:
        particle=re.sub("^\d+", "", particle)
    ...
    if re.compile("^\(MONIT.+?\)").search(code_str):
      char=re.search("^\(.+?\)",code_str).group()
      heading=char
    ...
    if len(codes)<5 or len(codes)>8:
      ...
      if re.compile("\S").search(codes[0]):
        if re.compile("^\(.+?\)").search(codes[0]):
          char=re.search("^\(.+?\)",codes[0]).group()
          heading=char
      ...
      if re.compile("\S").search(codes[1]):
        author=codes[1]
      else:
        author=None
      for index, item in enumerate(codes):
    ...
    if re.compile("\S").search(codes[2]):
      author=codes[2]
    else:
      author=None
    ...
    if len(codes)>1:
      ...
      if ind>=0: # status code in 2nd field
        ...
        if re.compile("\S").search(codes[1]): # SAN in 2nd field
          subaccession_number=codes[1]
        else:
    ...
    for index, item in enumerate(codes):
      ...
      if keyword=="ADD-RES":
        ...
        if not re.compile("^(\d\d|\d\d\d\d)$").search(item):
        ...
        if not re.compile("^\d+\.0*$").search(item):
...
def anal_decaydata(ansan,keyword,line,lines,codes):
  ...
  if re.compile("^\(.+?\)").search(codes[0]):
    ...
    if keyword=="DECAY-DATA":
      if not re.compile("^\d+\.0*$").search(char):
  ...
  if len(codes)>2: # radiation fields exist
    for i in range(len(codes)//3):
      ...
      if len(codes)==j+1: # energy field absent
        ...
        if re.compile("\S").search(codes[k]):
      ...
      if len(codes)<l+1: # intensity field absent
        ...
        if re.compile("\S").search(codes[l]):
...
def anal_levelprop(ansan,keyword,line,codes,code_str):
  ...
  if len(codes)<2 or len(codes)>4:
    ...
    for i, item in enumerate(codes):
      if i>0:
        if re.compile("^E-LVL\d?=|^E-EXC\d?=|^LVL-NUMB=|^IAS-NUMB=").search(item):
          arr=item.split("=")
...
def decompose_unit_combination(code_str):
  ...
  for char in arr:
    ...
    if mparen==0: # outside a unit
      if re.compile("\w").search(char): # first character of the unit
        mparen=1
        unit=char
        unit_combination+="%"
      else:
    ...
    if len(codes)<4 or len(codes)>7:
      ...
      for index, item in enumerate(codes):
        if index==0:   # target(projectile
          ...
          particles=re.split("\+",process)
          for particle in particles:
            if re.compile("\d+[A-Z]").search(particle):
              particle=re.sub("^\d+", "", particle)
            for char in lines:
...
def anal_reference(ansan,keyword,line,code_str):
  ...
  for unit in units:
    ...
    if reference_type=="A" or reference_type=="B" or \
      ...
      code=re.search("^(.+?-)(\d|\()",codes[1]).group(1)
      ...
      if len(codes)!=3 and len(codes)!=4 and len(codes)!=5:
        ...
        if len(codes)==4:
          if re.search("^\(.+?\)$",codes[2]): # assume volume rather than page if it is parenthesized
            volume=codes[2]
          else:  
            page=codes[2]
        elif len(codes)==5:
      ...
      if len(codes)!=4:
        ...
        if ind>=0: # Library without hyphen
          ...
          if code!="ENDF/B-":
            if not re.compile("(\d|\.)+").search(version):
...
def check_code_dict(ansan,keyword,dict_id,code,line):
  ...
  if ind==-10:
    if dict_id=="227":
      if re.compile("\d$").search(code): # check if it is an metastable state
        if keyword=="ASSUMED" or keyword=="ANG-SEC" or\
  ...
  if ind==-10:
    if dict_id=="006":
      if re.compile("\-$").search(code): # check if it is a report defined without -
        code=re.sub("\-$","",code)
        ind=get_index(dict_id,code)
...
def check_code_dict_product(ansan,keyword,nucl,product,line):
  ...
  nucl_org=nucl
  nucl=re.sub("-L\d?$","",nucl)

  ind=get_index(dict_id,nucl)

  if ind==-10:
    if re.compile("\d$").search(nucl):
      nucl=nucl+"-G"
      ind=get_index(dict_id,nucl)
