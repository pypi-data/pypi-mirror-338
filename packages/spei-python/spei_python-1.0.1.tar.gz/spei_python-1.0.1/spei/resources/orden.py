...
from spei.resources.validators import validate_alphanumeric
from spei.types.common import DateInt, InstitutionCode
...
class Orden(BaseModel):
    ...
    op_ref_cobranza: Optional[int] = None
    ...
    op_fecha_oper_ori: Optional[DateInt] = None
    ...
    # karpay system info
    op_fecha_cap: Optional[DateInt] = None
    ...
    def build_xml(self) -> _Element:  # noqa: WPS231, C901
        ...
        for element, value in elements.items():  # noqa: WPS110
            if element in self.model_fields:
                ...
                text = self._process_text(str(value), element)
    ...
    def _process_xml_element(
        ...
        stripped = _WHITESPACE_PATTERN.sub(' ', element.text.strip())
        ...
        if tag in cls.model_fields:
            orden_data[tag] = cls._process_text(stripped, tag)

    @classmethod
    def _process_text(cls, text: str, tag: str) -> Optional[str]:
        """Process and validate the text content of an XML element."""
        try:
            return validate_alphanumeric(text)
        except ValueError as error:
            raise ValueError(f'{error} in {tag}') from error
