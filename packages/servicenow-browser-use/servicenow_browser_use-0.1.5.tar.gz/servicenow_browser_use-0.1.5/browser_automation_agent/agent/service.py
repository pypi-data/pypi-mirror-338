...
from browser_automation_agent.agent.message_manager.service import MessageManager
from browser_automation_agent.agent.prompts import AgentMessagePrompt, SystemPrompt
from browser_automation_agent.agent.views import (
	ActionResult,
	AgentError,
	AgentHistory,
	AgentHistoryList,
...
from browser_automation_agent.browser.browser import Browser
from browser_automation_agent.browser.context import BrowserContext
from browser_automation_agent.browser.views import BrowserState, BrowserStateHistory
from browser_automation_agent.controller.registry.views import ActionModel
from browser_automation_agent.controller.service import Controller
from browser_automation_agent.dom.history_tree_processor.service import (
	DOMHistoryElement,
	HistoryTreeProcessor,
)
from browser_automation_agent.telemetry.service import ProductTelemetry
from browser_automation_agent.telemetry.views import (
	AgentEndTelemetryEvent,
	AgentRunTelemetryEvent,
	AgentStepTelemetryEvent,
)
from browser_automation_agent.utils import time_execution_async
...
class Agent:
	...
	def _set_version_and_source(self) -> None:
		try:
			...
			version = pkg_resources.get_distribution('browser-automation-agent').version
			source = 'pip'
		except Exception:
			try:
				import subprocess
			...
			while retry_count < max_retries:
				try:
					...
					if retry_count == max_retries:
						raise e
					logger.warning(f"Failed to get state (attempt {retry_count}/{max_retries}), retrying...")
			...
			try:
				...
				while retry_count < max_retries:
					try:
						...
						if retry_count == max_retries:
							raise e
						logger.warning(f"Failed to execute actions (attempt {retry_count}/{max_retries}), retrying...")
						await asyncio.sleep(1)
