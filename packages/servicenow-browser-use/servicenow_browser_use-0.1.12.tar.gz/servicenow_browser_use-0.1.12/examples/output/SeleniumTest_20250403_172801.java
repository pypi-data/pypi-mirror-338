...
public class SeleniumTest_20250403_172801 {
    ...
    public void testServiceNowAutomation() {{
        try {{
            ...
            // Login to ServiceNow
            String username = config.getProperty("servicenow.username", "admin");
            String password = config.getProperty("servicenow.password", "admin");

            // Wait for and fill in username field
            By usernameLocator = By.id("user_name");
            WebElement usernameField = waitForElement(usernameLocator);
            usernameField.clear();
            usernameField.sendKeys(username);

            // Wait for and fill in password field
            By passwordLocator = By.id("user_password");
            WebElement passwordField = waitForElement(passwordLocator);
            passwordField.clear();
            passwordField.sendKeys(password);

            // Click login button
            By loginButtonLocator = By.id("sysverb_login");
            WebElement loginButton = waitForElementClickable(loginButtonLocator);
            loginButton.click();
            waitForPageLoad();

            // Click on 'New' button
            By newButtonLocator = By.xpath("//button[contains(text(), 'New')]");
            WebElement newButton = waitForElementClickable(newButtonLocator);
            newButton.click();
            waitForPageLoad();

            // Check if 'Flow' tab is present
            By flowTabLocator = By.xpath("//span[contains(text(), 'Flow')]");
            WebElement flowTab = waitForElement(flowTabLocator);
            
            // Verify the tab is visible
            if (flowTab.isDisplayed()) {{
                System.out.println("Flow tab is present and visible");
            }} else {{
                System.out.println("Flow tab is not visible");
            }}
