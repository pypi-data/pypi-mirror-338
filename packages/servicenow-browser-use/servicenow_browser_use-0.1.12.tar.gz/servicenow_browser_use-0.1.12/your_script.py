...
def run_selenium_generator():
    ...
    try:
        ...
        from servicenow_browser_use.generate_selenium_from_recording import generate_selenium_code
...
async def main(task, llm_model):
    ...
    try:
        ...
        # Initialize Azure OpenAI with proper message format
        llm = AzureChatOpenAI(
            model=llm_model,
        ...
        agent = Agent(
            task=task_message,  # Pass task as a properly formatted message
            llm=llm,
            browser=browser,
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python script.py <task> [llm_model]")
        sys.exit(1)
        
    task = sys.argv[1]
    llm_model = sys.argv[2] if len(sys.argv) > 2 else "gpt-4"
