...
from servicenow_browser_use.agent.views import ActionResult, AgentStepInfo
from servicenow_browser_use.browser.views import BrowserState
...
2. ACTIONS: You can specify multiple actions in the list to be executed in sequence. But always specify only one action name per item.
   ...
   - Custom prompt mapping:Use custom prompt with the following mapping:[
    {"prompt": "log in", "action": "Open"""+ SERVICENOW_URL + """, then wait for page to load, reload the page. login with username: a, password: b then click on login button."}
    {"prompt": "login with username 'a' and password 'b'", "action": "Open"""+ SERVICENOW_URL+ """, then wait for page to load, reload the page. login with username: a, password: b then click on login button."},
