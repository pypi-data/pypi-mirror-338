...
def update_machine_status():
    """Update DB on machine states."""
    db_model = Machine
    desc = "machine"
    _prepare_update(db_model, desc)
    broker = ResTricksWrapper()
    okay, fail = _update_status(
        desc=desc,
        new_state=broker.get_machine_status(),
    ...
    for session in sessions:
        if session.cur_properties.state == "active":
            if dry_run:
                ...
                continue
            broker.disconnect_session(session.machine.fqdn)
            record = SiteLog(
...
def send_popup_message(uid, message, title, style):
    ...
    broker = ResTricksWrapper()
    broker.send_message(machine, message, title, style)
...
def check_session_constraints(max_parallel=1):
    """Wrapper calling all session constraint checking functions."""
    check_session_max_parallel(max_parallel)


def check_session_disconnect_time():  # WIP
    """Terminate sessions exceeding the (per-user) disconnection time limit."""
    sessions = Session.objects.filter(t_end=None)
    for session in sessions:
...
def check_reservations_for_dg(dg: DeliveryGroup):  # WIP
    ...
    for session in sessions:
        ...
        psytricks.terminate_session(v_session=session)
        record = SiteLog(
