...
class BookingSystem:
    ...
    def _process_booking_updates(self):
        """Wrapper to process changes to bookings after pulling from PPMS."""
    ...
    def _update_bookings(self, runningsheet, date):
        ...
        for booking in runningsheet:
            ...
            v_booking, created = PpmsBooking.objects.get_or_create(
            ...
            if not created:
                v_booking.sync_state = "K"
                log.debug("🔄🎫 Refreshed booking (sync_state=known) in the DB.")
            else:
                log.success(f"✨🎫 Created NEW booking in the DB: {v_booking}")
            v_booking.ppms_session = booking.session
            v_booking.save()
            okay += 1
    ...
    def _update_ppms_user_details(self, username, ppms_name, ppms_group):
        ...
        try:
            ...
            if v_user.ppms_fullname != ppms_name or v_user.ppms_group != ppms_group:
                v_user.ppms_fullname = ppms_name
                v_user.ppms_group = ppms_group
                v_user.save()
            okay += 1
    ...
    def pull_bookings(self, dt=date.today()):
        ...
        date : datetime, optional
            The date for which to fetch bookings, by default `datetime.now()`.
    ...
    def pull_users(self, force_refresh=False):
        ...
        int, int
            A tuple with counts of success / failed updates.
        """
        v_users = User.objects.filter(accessgroup_member=True, enabled=True)
        log.debug(
            f"▶🔄👩‍🔬👨‍🔬 Fetching {len(v_users)} active user details from PPMS..."
        )
        ppms_users = []
        for user in v_users:
            try:
