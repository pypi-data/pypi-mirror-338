...
class BaseGeneralResults:
    ...
    def needs_status_update(action:BaseGeneralParameters.StatusUpdateAction, status:BaseGeneralModels.StatusType) -> bool:
        if action == BaseGeneralParameters.StatusUpdateAction.DELETE:
            return status != BaseGeneralModels.StatusType.DELETED
        elif action == BaseGeneralParameters.StatusUpdateAction.RESTORE:
            return status == BaseGeneralModels.StatusType.DELETED
        elif action == BaseGeneralParameters.StatusUpdateAction.DEACTIVATE:
            return status != BaseGeneralModels.StatusType.INACTIVE
        elif action == BaseGeneralParameters.StatusUpdateAction.ACTIVATE:
            return status != BaseGeneralModels.StatusType.ACTIVE
    ...
    def validate_status_update(
        ...
        base_status_update_response_mappings:BaseGeneralResults.BaseStatusUpdateResponseMappings = {
            BaseGeneralParameters.StatusUpdateAction.DELETE: BaseGeneralResults.BaseStatusUpdateResponse(
            ...
            BaseGeneralParameters.StatusUpdateAction.RESTORE: BaseGeneralResults.BaseStatusUpdateResponse(
                message=f"{name} successfully restored",
                update_description=f"{name}'s `is_deleted` status changed to `false`",
                maintain_description=f"{name} is not deleted, no data are changed"
            ),
            BaseGeneralParameters.StatusUpdateAction.DEACTIVATE: BaseGeneralResults.BaseStatusUpdateResponse(
            ...
            BaseGeneralParameters.StatusUpdateAction.ACTIVATE: BaseGeneralResults.BaseStatusUpdateResponse(
        ...
        base_status_update_response = base_status_update_response_mappings.get(action)
        is_status_update_needed = BaseGeneralResults.needs_status_update(action=action, status=status)
        return BaseGeneralResults.StatusUpdateValidation(
            is_status_update_needed=is_status_update_needed,
            response_content=BaseGeneralResults.StatusUpdateResponseContent(
                message=base_status_update_response.message,
                description=base_status_update_response.update_description if is_status_update_needed else base_status_update_response.maintain_description
            )
        )