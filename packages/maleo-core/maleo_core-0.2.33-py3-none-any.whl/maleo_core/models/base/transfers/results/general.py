...
class BaseGeneralResults:
    ...
    def validate_status_update(
        ...
        status_update_responses = {
            ...
            BaseGeneralParameters.StatusUpdateAction.ACTIVATE: ("active", "is_active", "true"),
        }
        ...
        return BaseGeneralResults.StatusUpdateValidation(
            ...
            response_content=BaseGeneralResults.StatusUpdateResponseContent(
                ...
                description=f"{name}'s `{field_name}` status changed to `{new_value}`" if is_update_needed else f"{name} is currently {action_text}, no data changed"
            )
        )