...
class BaseGeneralResults:
    ...
    def validate_status_update(
        ...
        status_update_responses = {
            BaseGeneralParameters.StatusUpdateAction.DELETE: ("deleted", "is_deleted", "true"),
            BaseGeneralParameters.StatusUpdateAction.RESTORE: ("not deleted", "is_deleted", "false"),
            BaseGeneralParameters.StatusUpdateAction.DEACTIVATE: ("inactive", "is_active", "false"),
            BaseGeneralParameters.StatusUpdateAction.ACTIVATE: ("activated", "is_active", "true"),
        }
        ...
        action_text, field_name, new_value = action_data
        ...
        return BaseGeneralResults.StatusUpdateValidation(
            ...
            response_content=BaseGeneralResults.StatusUpdateResponseContent(
                message=f"{name} successfully {action_text}",
                description=f"{name}'s `{field_name}` status changed to `{new_value}`" if is_update_needed else f"{name} is already {action_text}, no data changed"
            )
        )