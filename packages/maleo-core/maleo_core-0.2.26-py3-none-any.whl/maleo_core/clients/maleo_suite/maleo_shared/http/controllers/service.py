...
class MaleoSharedServiceHTTPController:
    @staticmethod
    async def get_services(parameters:MaleoSharedServiceClientParameters.Get) -> BaseHTTPClientControllerResults:
    ...
    @staticmethod
    async def get_service(parameters:MaleoSharedServiceGeneralParameters.GetSingle) -> BaseHTTPClientControllerResults:
        """Fetch service from maleo-shared"""
        async with MaleoSharedHTTPClientManager.get() as client:
            #* Define URL
            url = f"{MaleoSharedHTTPClientManager._base_url}/v1/services/"
            if parameters.identifier == MaleoSharedServiceGeneralParameters.Identifiers.ID:
