...
from .utils import setup_logger, plot_search_results
...
def load_data(data_path: str) -> tuple:
    ...
    if data_path.endswith('.pt'):
        ...
        if isinstance(data, tuple) and len(data) == 2:
            ...
            raise ValueError(f"Unsupported data format in {data_path}. Expected tuple (X, y) or dict with 'X' and 'y' keys.")
    
    elif data_path.endswith('.npy'):
        # Load NumPy array
        data = np.load(data_path, allow_pickle=True)
        if isinstance(data, np.ndarray) and data.dtype == np.dtype('O') and len(data) == 2:
            X = torch.tensor(data[0], dtype=torch.float32)
            y = torch.tensor(data[1], dtype=torch.long)
        else:
            raise ValueError(f"Unsupported data format in {data_path}. Expected array with X and y.")
        ...
        try:
            ...
            logger.error("Pandas is required for loading CSV files. Install with 'pip install pandas'.")
            sys.exit(1)
    else:
        raise ValueError(f"Unsupported file format: {data_path}. Supported formats: .pt, .npy, .csv")
    
    logger.info(f"Loaded data with {X.shape[0]} samples, {X.shape[1]} features, and {len(torch.unique(y))} classes")
    return X, y
...
def parse_args():
    ...
    # Model arguments
    parser.add_argument('--input-dim', type=int, help='Input dimension (inferred from data if not provided)')
    parser.add_argument('--output-dim', type=int, help='Output dimension (inferred from data if not provided)')
    ...
    parser.add_argument('--activations', type=str, nargs='+', default=['relu', 'tanh', 'leaky_relu'],
    ...
    parser.add_argument('--no-meta-learning', action='store_true',
...
def main():
    ...
    output_dim = args.output_dim if args.output_dim is not None else len(torch.unique(y))
