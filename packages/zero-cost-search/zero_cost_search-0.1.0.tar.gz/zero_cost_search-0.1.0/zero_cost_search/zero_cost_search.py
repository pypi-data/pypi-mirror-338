...
class ZeroCostMetrics:
    ...
    @staticmethod
    def compute_activation_correlation(model: nn.Module, X: torch.Tensor, device: torch.device) -> float:
        ...
        for acts in activations:
            ...
            corr_matrix = torch.mm(flat_acts_norm, flat_acts_norm.t()) / flat_acts_norm.size(1)
    ...
    @staticmethod
    def compute_grad_conflict(model: nn.Module, X: torch.Tensor, y: torch.Tensor, 
        ...
        for i in range(len(sample_grads)):
            for j in range(i+1, len(sample_grads)):
                cos_sim = F.cosine_similarity(sample_grads[i].unsqueeze(0), 
                                             sample_grads[j].unsqueeze(0))
    ...
    @staticmethod
    def compute_zico_score(model: nn.Module, X: torch.Tensor, device: torch.device) -> float:
    ...
    @staticmethod
    def compute_synflow_score(model: nn.Module, X: torch.Tensor, device: torch.device) -> float:
    ...
    @staticmethod
    def compute_grasp_score(model: nn.Module, X: torch.Tensor, y: torch.Tensor, 
                           device: torch.device, num_classes: int) -> float:
...
class DatasetFeatureExtractor:
    ...
    @staticmethod
    def extract_features(X: torch.Tensor, y: Optional[torch.Tensor] = None) -> Dict[str, float]:
        ...
        features['skewness'] = np.mean(((X_np - np.mean(X_np)) / (np.std(X_np) + 1e-8)) ** 3)
        features['kurtosis'] = np.mean(((X_np - np.mean(X_np)) / (np.std(X_np) + 1e-8)) ** 4) - 3
        
        # Correlation structure
        if X.shape[1] > 1:
            corr_matrix = np.corrcoef(X_np.T)
            features['mean_correlation'] = np.mean(np.abs(corr_matrix - np.eye(X.shape[1])))
        else:
        ...
        if X.shape[0] > 10 and X.shape[1] > 1:
            try:
                ...
                features['effective_dim'] = effective_dim / X.shape[1]  # Normalize by input dimension
        ...
        if y is not None:
            ...
            if len(y.unique()) < 20:  # Likely classification
                ...
                features['target_skewness'] = np.mean(((y_np - np.mean(y_np)) / (np.std(y_np) + 1e-8)) ** 3)
...
class ArchitecturePredictor:
    ...
    def __init__(self):
        ...
        self.rules = [
            ...
            (lambda f: f.get('task_type', 0) == 0 and f.get('num_classes', 0) > 5 and f.get('class_entropy', 0) > 0.9,
            ...
            (lambda f: f.get('task_type', 0) == 1 and f.get('target_std', 0) > 1.0,
             lambda f: {'hidden_dims': [128, 128], 'activation_fn_str': 'leaky_relu'}),
    ...
    def predict_architecture(self, dataset_features: Dict[str, float]) -> Dict[str, Any]:
...
class ZeroCostNAS:
    ...
    def __init__(
        ...
        logger.info(f"Initialized ZeroCostNAS with input_dim={input_dim}, output_dim={output_dim}")
    ...
    def _compute_ensemble_score(self, model: nn.Module, X: torch.Tensor, 
                               y: Optional[torch.Tensor] = None) -> Dict[str, float]:
        ...
        scores['correlation'] = ZeroCostMetrics.compute_activation_correlation(model, X, self.device)
        ...
        if y is not None:
            scores['grad_conflict'] = -ZeroCostMetrics.compute_grad_conflict(  # Negate because lower is better
        ...
        max_scores = {'correlation': 1.0, 'zico': 10.0, 'synflow': 1e6, 
                     'grad_conflict': 0.0, 'grasp': 1e6}
        min_scores = {'correlation': 0.0, 'zico': 0.0, 'synflow': 0.0, 
                     'grad_conflict': -2.0, 'grasp': -1e6}
        
        normalized_scores = {}
        for metric, score in scores.items():
            normalized_scores[metric] = (score - min_scores[metric]) / (max_scores[metric] - min_scores[metric])
            normalized_scores[metric] = max(0.0, min(1.0, normalized_scores[metric]))  # Clip to [0, 1]
        
        # Weighted ensemble
        weights = {'correlation': 0.3, 'zico': 0.2, 'synflow': 0.2, 
                 'grad_conflict': 0.15, 'grasp': 0.15}
    ...
    def _cache_score(self, config: Dict[str, Any], scores: Dict[str, float], model_state: Dict[str, torch.Tensor]) -> None:
    ...
    def predict_architecture(self, X: torch.Tensor, y: Optional[torch.Tensor] = None) -> Dict[str, Any]:
        ...
        architecture = self.architecture_predictor.predict_architecture(dataset_features)
    ...
    def search(self, 
            ...
            for depth in depths:
                if depth != len(hidden_dims):
                    if depth < len(hidden_dims):
                        ...
                        new_dims = hidden_dims + [hidden_dims[-1]] * (depth - len(hidden_dims))
                    all_configs.append({
            ...
            for depth in depths:
                for width in widths:
                    ...
                    if isinstance(width, int):
                        ...
                        hidden_dims_configs.append(width[:depth] if len(width) >= depth 
        ...
        if verbose:
            print(f"Testing {len(all_configs)} configurations using zero-cost proxies...")
        logger.info(f"Testing {len(all_configs)} configurations using zero-cost proxies")
        ...
        for i, config in enumerate(all_configs):
            ...
            if cached_scores is not None:
                ...
                logger.info(f"Using cached scores for configuration {i+1}/{len(all_configs)}: {config}")
            else:
                ...
                if verbose:
                    ...
                    print(f"Ensemble score: {scores['ensemble']:.4f}")
                logger.info(f"Evaluated configuration {i+1}/{len(all_configs)}: {config}")
            ...
            if scores['ensemble'] > self.best_score:
                ...
                if verbose:
                    print(f"New best configuration found! Score: {scores['ensemble']:.4f}")
                logger.info(f"New best configuration found! Score: {scores['ensemble']:.4f}")
    ...
    def get_best_model(self, include_dropout: float = 0.0, include_batchnorm: bool = False) -> MLP:
        ...
        if self.best_model_state is not None:
            ...
            filtered_state = {k: v for k, v in self.best_model_state.items() 
    ...
    def get_results_summary(self) -> Dict[str, Any]:
        ...
        sorted_results = sorted(self.all_results, key=lambda x: x['scores']['ensemble'], reverse=True)
        ...
        for result in sorted_results[:5]:  # Top 5 configurations
            top_configs.append({
                ...
                'individual_scores': {k: v for k, v in result['scores'].items() if k != 'ensemble'}
            })
        ...
        for result in self.all_results:
            ...
            width_scores[config['hidden_dims'][0]].append(score)  # Use first layer width as proxy
            activation_scores[config['activation_fn_str']].append(score)
        
        # Compute average score for each architectural choice
        depth_analysis = {depth: np.mean(scores) for depth, scores in depth_scores.items()}
        width_analysis = {width: np.mean(scores) for width, scores in width_scores.items()}
        activation_analysis = {act: np.mean(scores) for act, scores in activation_scores.items()}
