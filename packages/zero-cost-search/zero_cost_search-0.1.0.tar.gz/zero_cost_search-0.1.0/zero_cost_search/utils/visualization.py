...
def plot_search_results(
    ...
    if 'all_results' in results:
        ...
        sorted_results = sorted(results['all_results'], 
                               key=lambda x: x['scores']['ensemble'], 
                               reverse=True)
    ...
    if 'all_results' in results:
        ...
        axs[0, 1].hist(all_scores, bins=10, alpha=0.7, color='skyblue', edgecolor='black')
        axs[0, 1].axvline(results['best_score'], color='red', linestyle='dashed', 
                         linewidth=2, label=f'Best score: {results["best_score"]:.4f}')
    ...
    if 'best_scores' in results:
        ...
        metric_scores = {k: v for k, v in results['best_scores'].items() if k != 'ensemble'}
        ...
        axs[1, 0].set_thetagrids(np.degrees(angles[:-1]), metrics)
    ...
    if 'all_results' in results:
        ...
        for i, (depth, score) in enumerate(sorted(depth_avg.items())):
            axs[1, 1].bar(x[0] + (i - len(depth_avg)/2 + 0.5) * width, score, 
                         width, label=f'Depth {depth}')
        
        # Plot activations
        for i, (act, score) in enumerate(sorted(act_avg.items())):
            axs[1, 1].bar(x[1] + (i - len(act_avg)/2 + 0.5) * width, score, 
                         width, label=f'{act}')
        ...
        axs[1, 1].legend(loc='upper center', bbox_to_anchor=(0.5, -0.15), 
