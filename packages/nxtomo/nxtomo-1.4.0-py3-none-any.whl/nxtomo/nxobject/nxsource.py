...
from nxtomo.nxobject.nxobject import NXobject, ElementWithUnit
...
class NXsource(NXobject):
    ...
    def __init__(
        ...
        self._probe = probe
        self._distance = ElementWithUnit(default_unit=MetricSystem.METER)
    ...
    @property
    def distance(self) -> ElementWithUnit | None:
        return self._distance

    @distance.setter
    def distance(self, value: float | None) -> None:
        if not isinstance(value, (float, type(None))):
            raise ValueError(
                f"distance value is expected to be a float. {type(value)} given."
            )
    ...
    def to_nx_dict(
        ...
        if self.distance.value is not None and nexus_source_paths.DISTANCE is not None:
            path_source = f"{self.path}/{nexus_source_paths.DISTANCE}"
            nx_dict[path_source] = self.distance.value
            nx_dict["@".join([path_source, "units"])] = str(self.distance.unit)
    ...
    def concatenate(nx_objects: tuple, node_name="source"):
        ...
        nx_souce.distance.value = nx_objects[0].distance.value
        nx_souce.distance.unit = nx_objects[0].distance.unit
