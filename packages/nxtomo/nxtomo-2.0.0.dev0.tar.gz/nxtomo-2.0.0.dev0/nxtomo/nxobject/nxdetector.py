...
class NXdetector(NXobject):
    ...
    def x_pixel_size(self) -> pint.Quantity | None:
        """
        x pixel size as a field with a pint Unit.
        Know as 'x sample pixel size' in some application
        """
        return self._x_pixel_size

    @x_pixel_size.setter
    def x_pixel_size(self, x_pixel_size) -> None:
        if x_pixel_size is None:
            self._x_pixel_size = None
        elif isinstance(x_pixel_size, pint.Quantity):
            self._x_pixel_size = x_pixel_size
        else:
            raise TypeError(
                f"x_pixel_size is expected to be a pint.Quantity, or None. Not {type(x_pixel_size)}"
            )

    @property
    def y_pixel_size(self) -> pint.Quantity | None:
        """
        y pixel size as a field with a pint Unit.
        Know as 'y sample pixel size' in some application
        """
        return self._y_pixel_size

    @y_pixel_size.setter
    def y_pixel_size(self, y_pixel_size) -> None:
        if y_pixel_size is None:
            self._y_pixel_size = None
        elif isinstance(y_pixel_size, pint.Quantity):
            self._y_pixel_size = y_pixel_size
        else:
            raise TypeError(
                f"y_pixel_size is expected to be a pint.Quantity, or None. Not {type(y_pixel_size)}"
            )
    ...
    def distance(self, value: pint.Quantity | None) -> None:
        if value is None:
            ...
            raise TypeError(f"distance expects a pint.Quantity. Got {type(value)}")
    ...
    def count_time(self, count_time: None | pint.Quantity):
        if count_time is not None and not isinstance(count_time, pint.Quantity):
            raise TypeError("count_time must be a pint.Quantity or None")
        if (
            count_time is not None
            and count_time.dimensionality != _second.dimensionality
        ):
    ...
    def to_nx_dict(
        ...
        if self.count_time is not None:
            ...
            nx_dict["@".join([path_count_time, "units"])] = str(self.count_time.units)
        # tomo n
    ...
    def _concatenate_except_data(nx_detector, nx_objects: tuple):
        ...
        for nx_obj in nx_objects[1:]:
            if nx_detector.x_pixel_size is not None and not numpy.isclose(
                nx_detector.x_pixel_size.magnitude, nx_obj.x_pixel_size.magnitude
            ):
                _logger.warning(
                    f"found different x pixel size value. ({nx_detector.x_pixel_size.magnitude} vs {nx_obj.x_pixel_size.magnitude}). Pick the first one"
                )
            if nx_detector.y_pixel_size is not None and not numpy.isclose(
                nx_detector.y_pixel_size.magnitude, nx_obj.y_pixel_size.magnitude
            ):
                _logger.warning(
                    f"found different y pixel size value. ({nx_detector.y_pixel_size.magnitude} vs {nx_obj.y_pixel_size.magnitude}). Pick the first one"
                )
