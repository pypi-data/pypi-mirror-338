...
from nxtomo.nxobject.utils import concatenate_pint_quantities
...
class NXsample(NXobject):
    ...
    def rotation_angle(self, rotation_angle: None | pint.Quantity):
        if not isinstance(rotation_angle, (type(None), pint.Quantity)):
            raise TypeError(
                f"rotation_angle expects a pint.Quantity. Got {type(rotation_angle)}"
            )
    ...
    def x_translation(self, x_translation: None | pint.Quantity):
        if not isinstance(x_translation, (type(None), pint.Quantity)):
            raise TypeError(
                f"x_translation expects a pint.Quantity. Got {type(x_translation)}"
            )
        self._x_translation = cast_and_check_array_1D(x_translation, "x_translation")
    ...
    def y_translation(self, y_translation: None | pint.Quantity):
        if not isinstance(y_translation, (type(None), pint.Quantity)):
            raise TypeError(
                f"y_translation expects a pint.Quantity. Got {type(y_translation)}"
            )
        self._y_translation = cast_and_check_array_1D(y_translation, "y_translation")
    ...
    def z_translation(self, z_translation: None | pint.Quantity):
        if not isinstance(z_translation, (type(None), pint.Quantity)):
            raise TypeError(
                f"z_translation expects a pint.Quantity. Got {type(z_translation)}"
            )
        self._z_translation = cast_and_check_array_1D(z_translation, "z_translation")
    ...
    def concatenate(nx_objects: tuple, node_name="sample"):
        ...
        # Rocking
        rocking_list = [
            nx_obj.rocking for nx_obj in nx_objects if nx_obj.rocking is not None
        ]
        if rocking_list:
