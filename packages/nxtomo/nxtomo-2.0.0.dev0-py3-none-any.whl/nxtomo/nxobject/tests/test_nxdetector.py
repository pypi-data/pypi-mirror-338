...
def test_nx_detector():
    ...
    # check x_flipped and y_flipped
    with pytest.raises(TypeError):
        nx_detector.x_flipped = 12
    with pytest.raises(TypeError):
        nx_detector.y_flipped = 12

    nx_detector.x_flipped = True
    nx_detector.y_flipped = False
...
def test_nxtransformations_with_nxdetector(tmp_path):
    """
    test behavior of an Nxtransformations with an NXtomo
    and coherence between lr_flip / ud_flip API (provided for convenience) and providing directly the
    transformations
    """
    ...
    nx_detector_6.x_flipped = True
    nx_detector_6.y_flipped = True
...
def test_several_nxtransformations(tmp_path):
    ...
    nx_detector.image_key_control = [1] * 5
    nx_detector.image_key_control = [ImageKey.PROJECTION] * 5
    nx_detector.transformations.add_transformation(DetZFlipTransformation(flip=True))
    ...
    )  # the DetZFlipTransformation + gravity
...
def test_detector_flips(tmp_path):
    """
    Make sure the 'deprecated' API 'x_flip' and 'y_flip' are working
    """
    ...
    nx_detector.x_flipped = True
    assert (
        DetZFlipTransformation(flip=True) in nx_detector.transformations.transformations
    )
    nx_detector.x_flipped = False
    assert (
        DetZFlipTransformation(flip=True)
        not in nx_detector.transformations.transformations
    )
    ...
    assert loaded_nx_detector.x_flipped is False
    for i in range(3):
        print(f"{loaded_nx_detector.transformations.transformations[i]}\n")
    assert loaded_nx_detector.y_flipped is True
