...
class NPWorld():
    ...
    def get_val_for_rgb_cell(self, rgb_cell: np.ndarray):
        ...
        if self.raw_nptype is None:
            if len(rgb_cell.shape) == 3:
                self.raw_nptype = type(rgb_cell[0][0][0])
            elif len(rgb_cell.shape) == 2:
                self.raw_nptype = type(rgb_cell[0][0])


        for dim in range(len(rgb_cell.shape)):
        ...
        if len(rgb_cell.shape) == 3:
            ...
            assert isinstance(rgb_cell[0][0], self.raw_nptype)
        ...
        if self.raw_cell_shape is None:
            ...
            for dim in [0, 1, 2]:
                assert (self.raw_cell_shape[dim] == rgb_cell.shape[dim])
