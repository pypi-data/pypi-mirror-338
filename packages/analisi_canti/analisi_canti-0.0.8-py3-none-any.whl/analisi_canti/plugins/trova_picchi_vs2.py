...
class Main(QWidget):
    ...
    def load_wav(self, wav_file):
        ...
        data_file_path = Path(self.wav_file).parent / Path(Path(self.wav_file).parent.name).with_suffix(".json")
    ...
    def detect_calls(self):
        ...
        for ic in range(len(rms_peaks)):
            ...
            for ss in range(before):
                ...
                #    break
                x = np.mean(self.rms[c] - self.rms[c + 1 : min(c + before, len(self.rms))])
