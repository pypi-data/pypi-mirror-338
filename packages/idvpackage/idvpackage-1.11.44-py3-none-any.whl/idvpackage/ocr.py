    ...
    def extract_front_id_info(self, front_id, country):
        ...
        if country == 'SDN':
                ...
                try:
                    ...
                    st = time.time()
                    compressed_image = BytesIO()
                    processed_front_id.save(compressed_image, format="JPEG", quality=95, optimize=True)
                    compressed_image_data = compressed_image.getvalue()
    ...
    def extract_back_id_info(self, back_id, country):
        ...
        if country == 'SDN':
                ...
                try:
                    ...
                    st = time.time()
                    compressed_image = BytesIO()
                    processed_back_id.save(compressed_image, format="JPEG", quality=95, optimize=True)
                    compressed_image_data = compressed_image.getvalue()
                    ...
                    ## TODO: template matching for SDN back ID's
                    # image_data = base64.b64decode(back_id)
                    # template_result = detect_logo(self.client, image_data, country, compare_type='template', side='back')
                    # if template_result == 'consider':
                    #     back_data["error"] = "not_back_id"
    ...
    def exract_passport_info(self, passport, country):
        ...
        if country.upper() == 'SDN':
                ...
                try:
                    ...
                    # if not front_face_locations or not front_face_encodings:
                    #     print(f"----------------Face not clear in SDN passport document")
                    #     passport_data['error'] = 'face_not_clear'
                    #     return passport_data
                    ...
                    non_optional_keys = ["passport_number_mrz", "dob_mrz", "expiry_date_mrz", "gender", "full_name"]
                    empty_string_keys = [key for key, value in passport_data.items() if key in non_optional_keys and (value == '' or value == [] or value == '[]')]
