...
class AscendRequests:
    ...
    @ascend_request_handling
    @retry(tries=3, delay=2, backoff=2)
    def _get_appointments_by_id(self, id: int) -> list[dict]:
        """Gets the location information from Dentrix."""
        response = self.session.get(AscendUrls.APPOINTMENT_BY_ID(id))
        response.raise_for_status()
        return response.json()
    ...
    @ascend_request_handling
    @retry(tries=3, delay=1, backoff=2)
    def _get_bulk_insurance_payments_by_id(self, id: int) -> dict:
        """Returns a list of bulk insurance payments based on id.

        Args:
            id (str): id of the bulk insurance payments

        Returns:
            dict: json resulted
        """
        response = self.session.get(url=AscendUrls.BULK_INSURANCE_PAYMENT_BY_ID(id), timeout=300)
        response.raise_for_status()
        return response.json()
    ...
    @ascend_request_handling
    @retry(tries=3, delay=1, backoff=2)
    def _get_carrier_insurance_plan_by_id(self, id: int) -> dict:
        """Returns a carrier insurance plan by entry ID.

        Args:
            id (str): id of the carrier insurance plan

        Returns:
            dict: json resulted
        """
        response = self.session.get(url=AscendUrls.CARRIER_INSURANCE_PLAN_BY_ID(id), timeout=300)
        response.raise_for_status()
        return response.json()
    ...
    @ascend_request_handling
    @retry(tries=3, delay=1, backoff=2)
    def _get_carrier_plan_copay_exception_by_id(self, id: int) -> dict:
        """Returns a carrier plan copay exception by entry ID.

        Args:
            id (int): ID of the carrier plan copay exception.

        Returns:
            dict: JSON result.
        """
        response = self.session.get(url=AscendUrls.CARRIER_PLAN_COPAY_EXCEPTION_BY_ID(id), timeout=300)
        response.raise_for_status()
        return response.json()
    ...
    @ascend_request_handling
    @retry(tries=3, delay=1, backoff=2)
    def _get_carrier_plan_coverage_exception_by_id(self, id: int) -> dict:
        """Returns a carrier plan coverage exception by entry ID.

        Args:
            id (int): ID of the carrier plan coverage exception.

        Returns:
            dict: JSON result.
        """
        response = self.session.get(url=AscendUrls.CARRIER_PLAN_COVERAGE_EXCEPTION_BY_ID(id), timeout=300)
        response.raise_for_status()
        return response.json()
    ...
    @ascend_request_handling
    @retry(tries=3, delay=1, backoff=2)
    def _get_clinical_note_by_id(self, id: int) -> dict:
        """Returns a clinical note by entry ID.

        Args:
            id (int): ID of the clinical note.

        Returns:
            dict: JSON result.
        """
        response = self.session.get(url=AscendUrls.CLINICAL_NOTE_BY_ID(id), timeout=300)
        response.raise_for_status()
        return response.json()
    ...
    @ascend_request_handling
    @retry(tries=3, delay=1, backoff=2)
    def _get_location_by_id(self, id: int) -> dict:
        ...
        Returns:
            dict: JSON response
        """
        response = self.session.get(url=AscendUrls.LOCATION_BY_ID(id), timeout=300)
        response.raise_for_status()
        return response.json()
    ...
    @ascend_request_handling
    @retry(tries=3, delay=1, backoff=2)
    def _get_user_by_id(self, id: int) -> dict:
        ...
        Returns:
            dict: JSON response
        """
        response = self.session.get(url=AscendUrls.USER_BY_ID(id), timeout=300)
        response.raise_for_status()
        return response.json()
    ...
    @ascend_request_handling
    @retry(tries=3, delay=1, backoff=2)
    def _get_transaction_by_id(self, id: int) -> dict:
        ...
        Returns:
            dict: JSON response
        """
        response = self.session.get(url=AscendUrls.TRANSACTION_BY_ID(id), timeout=300)
        response.raise_for_status()
        return response.json()
    ...
    @ascend_request_handling
    @retry(tries=3, delay=1, backoff=2)
    def _get_visit_by_id(self, id: int) -> dict:
        ...
        Returns:
            dict: JSON response
        """
        response = self.session.get(url=AscendUrls.VISIT_BY_ID(id), timeout=300)
        response.raise_for_status()
        return response.json()
    ...
    @ascend_request_handling
    @retry(tries=3, delay=1, backoff=2)
    def _get_timeclock_by_id(self, id: int) -> dict:
        """Returns a timeclock by entry ID.

        Args:
            id (int): ID of the timeclock

        Returns:
            dict: JSON response
        """
        response = self.session.get(url=AscendUrls.TIMECLOCK_BY_ID(id), timeout=300)
        response.raise_for_status()
        return response.json()
