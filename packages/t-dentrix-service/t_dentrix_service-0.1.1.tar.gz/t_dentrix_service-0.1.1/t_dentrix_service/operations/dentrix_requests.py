...
class DentrixServiceRequests(DentrixServiceLogin):
    ...
    @dentrix_request_handling
    @retry(tries=3, delay=2, backoff=2)
    def _get_schedules(self, timestamp: int, location_id: int):
    ...
    def generate_chart_number(self, patient_information: dict) -> str:
        ...
        response = self.session.get(
            DentrixUrls.GENERATE_CHART_NUMBER_URL, headers=self._headers(), params=params, timeout=300
        )
        response.raise_for_status()
    ...
    def perform_claim_action(self, activity: dict) -> dict:
        ...
        response = self.session.post(
            DentrixUrls.ACTIVITY_URL, headers=self._headers(content_type=""), json=payload, timeout=300
        )
        response.raise_for_status()
        return response.json()
    ...
    @dentrix_request_handling
    @retry(tries=3, delay=2, backoff=2)
    def _search_patient(self, patient_name: str, params: Optional[dict] = None) -> list:
    ...
    @dentrix_request_handling
    @retry(tries=5, delay=1, backoff=2)
    def _update_patient_plan_benefits(self, patient_id, payload) -> dict:
    ...
    def query_aged_receivables(self, payload: dict) -> dict:
        ...
        """
        response = self.session.post(
            url=DentrixUrls.AGED_RECEIVABLES, headers=self._headers(), json=payload, timeout=300
        )
        response.raise_for_status()
        return response.json()
    ...
    @dentrix_request_handling
    @retry(tries=3, delay=1, backoff=2)
    def get_transaction_payload(self, transaction_id: str):
    ...
    @dentrix_request_handling
    @retry(tries=3, delay=1, backoff=2)
    def _get_transaction_charges(self, params: dict, transaction_type: str) -> list:
    ...
    def check_patient(self, patient_id: int) -> None:
        ...
        response = self.session.get(
            DentrixUrls.PATIENTS_BILLING_STATEMENT(patient_id), headers=self._headers(), json=payload
        )
        response.raise_for_status()
