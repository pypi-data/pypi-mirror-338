...
from webullpaysdkcore.auth.algorithm import sha_hmac1
from webullpaysdkcore.auth.composer import default_signature_composer
import webullpaysdkcore.headers as hd
import webullpaysdkcore.utils.common as core_common
import logging

logger = logging.getLogger(__name__)


def _build_sign_metadata(app_key_id, host, signer_spec=sha_hmac1):
    sign_params = {}
...
def calc_signature(app_key_id, app_key_secret, host, uri, pb_object, signer_spec=sha_hmac1):
    metadata, sign_params = _build_sign_metadata(app_key_id, host, signer_spec)
