...
from webullpaysdkquotescore.grpc.grpc_retry_policy import SubscribeRetryPolicyContext
        ...
        with self._lock:
            if isinstance(exception, grpc.RpcError):
                ...
                msg = "gRPC error code:%s, error msg:%s, details:%s. Host:%s SDK-Version:%s " % (
                    state.code, state.details, state.debug_error_string, self._host, webullpaysdkquotescore.__version__)
                ...
                msg = "gRPC exception Host:%s SDK-Version:%s" % (
                    self._host, webullpaysdkquotescore.__version__)
                logger.error(compat.ensure_string(msg))
