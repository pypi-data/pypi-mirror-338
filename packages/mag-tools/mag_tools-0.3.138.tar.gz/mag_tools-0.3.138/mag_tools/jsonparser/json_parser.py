...
from typing import Any, Optional, Type, TypeVar, get_origin, Union, get_args
...
class JsonParser:
    ...
    def to_bean(json_src: str, clazz: Optional[Type[T]]) -> Optional[T]:
        ...
        for attr_name, attr_type in clazz.__annotations__.items():
            # 判定是否为Option[XXX]
            if get_origin(attr_type) is Union and len(get_args(attr_type)) == 2 and NoneType in get_args(attr_type):
            ...
            if get_origin(attr_type) == tuple:
                ...
                attr_value = getattr(bean, attr_name, None)
                if attr_value is not None and hasattr(attr_type, 'of_code'):
                    enum_value = attr_type.of_code(attr_value)
                    if enum_value is None and hasattr(attr_type, 'of_name'):
                        enum_value = attr_type.of_name(attr_value)
