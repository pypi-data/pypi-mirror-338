...
def _check_kwargs(kwargs: Dict[_KwargsKeyType, Any], attributes: Sequence[_KwargsKeyType]) -> None:
    ...
    for key in kwargs:
        if key not in attributes:
            if key == "inline" and "inlined" in attributes:
                raise BadSchemaDefinition(
                    '"inline" is not a possible property name, you probably want "inlined"'
                )
            message = f"no such property {key!r} in {attributes!r}"
            if len(key) > 3:
                matches = get_close_matches(key, attributes)
                nb_matches = len(matches)
                if nb_matches == 1:
                    attribute = matches[0]
                    message += f"\nyou probably wanted property {attribute!r}"
                elif nb_matches > 1:
                    attributes = matches.join(", ")
                    message += f"\nyou probably wanted one of this properties: {attributes!r}"
...
class Definition(metaclass=autopackage):
        ...
        **kwargs,
        ...
        if size is not None:
            if minsize is not None or maxsize is not None:
                raise BadSchemaDefinition("you cannot define size and minsize or maxsize")
            if size < 0:
                raise ValueError("an attribute's size must be positive")
            if size == 0:
                raise ValueError("an attribute with always size 0 is useless")
            _add_constraint(kwargs, SizeConstraint(min=size, max=size))
        else:
            assert minsize is None or minsize >= 0
            assert maxsize is None or maxsize >= 0
            if minsize is not None and maxsize is not None:
                if minsize > maxsize:
                    raise ValueError(
                        "minsize ({minsize}) must be equal or less than maxsize ({maxsize})"
                    )
                _add_constraint(kwargs, SizeConstraint(min=minsize, max=maxsize))
            elif minsize is not None:
                _add_constraint(kwargs, SizeConstraint(min=minsize))
        ...
        super().__init__(self.entity_type, **kwargs)
        ...
        for base in bases:
            for relation_definition in getattr(base, "__relations__", ()):
                if (
                    ...
                    relations[relation_definition.name].creation_rank = (
                        relation_definition.creation_rank
                    )
