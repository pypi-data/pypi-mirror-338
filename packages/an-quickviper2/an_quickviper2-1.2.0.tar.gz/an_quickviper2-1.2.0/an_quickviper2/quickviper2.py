# -*- coding: utf-8 -*-
"""QVP2_20250403_00.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1gSYGIMFGOkjf1FC6p8rUtICSSq35tVzs
...
2025/04/01 1.1.1 extract 修正
2025/04/03 1.2.0 ToolKit 分離
```
...
class QuickViper2:
    ...
    def pack( self, archive, target, destination ):
        """
        conda-pack を使用して、仮想環境を tar.zst 形式のアーカイブに圧縮します。
        args
            archive( str ): 圧縮仮想環境ファイルのパス
            archive_zst( str ): zst形式圧縮仮想環境ファイルのパス(未整備)
            target( str ): 仮想環境のパス
        ...
        # zsrファイルに二重圧縮するプログラムを載せておくが、conda-packがzsrファイルを
        # 直接出力できなければ経過時間的に意味がないので、コメントアウトしておく。
        # cmd = [
        #     "tar",
...
class MainLoop():
    def __init__( self, venv_name = "kohya_env" ):
        ...
        [ self.bch_path,
          ...
          self.tmp_path,
          self.org_path,
          self.orz_path ] = self.even.setup( venv_name = self.venv_name )
