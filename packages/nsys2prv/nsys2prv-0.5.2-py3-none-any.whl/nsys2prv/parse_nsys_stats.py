...
def main():
    ...
    threads.reset_index()
    tasks_set = threads.groupby(["task"]).agg({'hostname': 'first',
                                            'Pid': 'first',
                                            'Tid': lambda x: set(x),
                                            'thread': 'count',
                                            'device': 'first' })
    ...
    if t_mpi:
        ...
        mpi_unknown_values_mask = pd.isna(mpi_df["event_value"])
        mpi_df["event_value"].mask(mpi_unknown_values_mask, mpi_unknown_values_mask.cumsum() + mpi_values["event_value"].max(), inplace=True)
    ...
    memops_names = kernel_names.loc[kernel_names["Name"].isin(KernelsSemantic.memops_names)]
    ...
    memops_df = kernels_df.loc[kernels_df["Name"].isin(KernelsSemantic.memops_names)]
