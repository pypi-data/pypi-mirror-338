...
class JesusGraphCore:
    ...
    def run(self, initial_state: Optional[Union[Dict[str, Any], BlessingState, ExecutionState]] = None) -> ExecutionState:
        """
        ...
        Returns:
            Estado final após a execução completa do grafo
            
        Raises:
            ValueError: Se o grafo não tiver um nó inicial definido
        """
        ...
        if not state.current_node:
            ...
            if state.current_node in self.edges and self.edges[state.current_node]:
                ...
                return state  # Não há para onde ir
        else:
