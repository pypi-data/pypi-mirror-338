...
class RsEngineStart:
    ...
    def start():
        ...
        if RsEngineStart.config.ray_address_ip is not None:
            ...
            ray.init(f"ray://{RsEngineStart.config.ray_address_ip}:{RsEngineStart.config.ray_address_port}",runtime_env={"working_dir": f"{os.getcwd()}","pip":["rayspatial"]})
            serve.start(http_options={"host": RsEngineStart.config.ray_address_ip, "port": config.config_ray["serve_port"]})
        else:
            ray.init(runtime_env={"working_dir": f"{os.getcwd()}","pip":["rayspatial"]})
