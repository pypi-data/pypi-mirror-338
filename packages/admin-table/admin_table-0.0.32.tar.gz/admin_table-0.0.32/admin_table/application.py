...
current_user: ContextVar[AuthProviderBase.AuthorizedUserInfo] = ContextVar("current_user")
...
class _HasConfig:
    ...
    async def process_possibly_callable(
        self,
        value: Callable[..., str | Awaitable[str]] | str | None,
    ) -> str:
        if value is None:
            return ""
        if callable(value):
            if asyncio.iscoroutinefunction(value):
                return await cast(Callable[[], Awaitable[str]], value)()
            else:
...
class AuthRouteMixin(_HasConfig):
    ...
    async def check_request(
        ...
        If the request is authorized, set the user in the request and return None.
        If the request is not authorized, return a RouteResponse.
        """
    ...
    def protected(
        ...
        async def wrapped(
            ...
            # the cast is safe because check_request always sets the user
            token = current_user.set(cast(AuthProviderBase.AuthorizedUserInfo, request.user))
            try:
    ...
    def check_capabilities(self, resource: CapabilitiesMixin) -> AdminTableRoute.RouteResponse | None:
        try:
            if not resource.capabilities:
                return None
            if set(current_user.get().capabilities).issuperset(resource.capabilities):
                return None
            return AdminTableRoute.RouteResponse(
                status_code=403,
...
class ListViewMixin(AuthRouteMixin, _HasConfig):
    ...
    async def resource_list_handler(self, request: AdminTableRoute.RouteRequest) -> AdminTableRoute.RouteResponse:
        ...
        description = await self.process_possibly_callable(view.description)
...
class AdminTable(ListViewMixin, _HasConfig):
    ...
    async def page_view_handler(self, request: AdminTableRoute.RouteRequest) -> AdminTableRoute.RouteResponse:
        ...
        if not page.public:
            ...
            if response is not None:
                return response
            # cast is safe because check_request sets the user
            current_user.set(cast(AuthProviderBase.AuthorizedUserInfo, request.user))
    ...
    async def resource_create_get_schema_handler(
        ...
        # supply extra filed json schema  with async functions (pydantic only supports sync functions)
        for field, json_schema_extra in (view.json_schema_extra or {}).items():
            await json_schema_extra(schema[field])
    ...
    async def resource_detail_handler(self, request: AdminTableRoute.RouteRequest) -> AdminTableRoute.RouteResponse:
        ...
        description = await self.process_possibly_callable(detail.description)
    ...
    async def get_input_form_handler(self, request: AdminTableRoute.RouteRequest) -> AdminTableRoute.RouteResponse:
        ...
        description = await self.process_possibly_callable(form.description)
