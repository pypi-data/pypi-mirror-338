...
class ViewBase(CapabilitiesMixin):
    ...
    description: Annotated[
        Callable[..., str | Awaitable[str]] | str | None,
...
class CreateView(ViewBase, Generic[CreateSchemaModel]):
    """
    The generated schema should correspond to json-schema (see https://json-schema.org for more)
    The frontend uses react-jsonschema-form which then turns it into a form.
    the output is then passed to the callback function.
    """

    schema: Annotated[type[CreateSchemaModel], Doc("Schema of the form")]
    callback: Callable[[CreateSchemaModel], GenericCallbackReturnValue | Awaitable[GenericCallbackReturnValue]]
    json_schema_extra: Annotated[
        dict[str, Callable[[dict[str, Any]], Awaitable[None]]] | None,
        Doc(
            "Dirty fix for lacking support of async functions in pydantic."
            "pass a dict of field to which `json_schema_extra` would normally be passed if it was synchronous."
        ),
    ] = None
