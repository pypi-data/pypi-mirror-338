...
class TensorMassFunction_CSC(dict):
    ...
    def __getitem__(self, hypothesis):
        return dict.__getitem__(self, TensorMassFunction_CSC._convert(hypothesis))
    ...
    def __setitem__(self, hypothesis, value):
        ...
        dict.__setitem__(self, TensorMassFunction_CSC._convert(hypothesis), value)
    
    def __delitem__(self, hypothesis):
        return dict.__delitem__(self, TensorMassFunction_CSC._convert(hypothesis))
    @staticmethod
    ...
    @staticmethod
    def SetFrame(frame): 
        TensorMassFunction_CSC.sortedPowerset = None
        TensorMassFunction_CSC.setMap = None
        TensorMassFunction_CSC.setMapChar2Int = {}
        TensorMassFunction_CSC.setMapInt2Char = {}
        TensorMassFunction_CSC.projectionMatrix = None
        mydict = {k:0.0 for k in frame}
        tmp = TensorMassFunction_CSC(mydict)
    ...
    def GetSortedPowerset(self):
        if TensorMassFunction_CSC.sortedPowerset != None:
            return TensorMassFunction_CSC.sortedPowerset
        ...
        for i in range(0, 2**n):
            ...
            result.append(combo)
            TensorMassFunction_CSC.setMapChar2Int[comboChar] = combo
            TensorMassFunction_CSC.setMapInt2Char[combo] = comboChar
        
        TensorMassFunction_CSC.sortedPowerset = result
        TensorMassFunction_CSC.setMap = {result[i]:i for i in range(len(result))    }
    ...
    def ConvertMassFuntionToVector(self):
        ...
        for k,v in self.items():
            k_int = TensorMassFunction_CSC.setMapChar2Int[k]
            index = TensorMassFunction_CSC.setMap[k_int]
            V[index,0] = v
        self.vec = V.tocsc()
        return self.vec
    ...
    def GenerateProjectionMatrix(self):
        if TensorMassFunction_CSC.projectionMatrix is not None: return TensorMassFunction_CSC.projectionMatrix
        sortedPowerset = self.GetSortedPowerset()
        ...
        TensorMassFunction_CSC.projectionMatrix = pMatrix
        return pMatrix
   

    def Combine(self,other):
        pMatrix = self.GenerateProjectionMatrix()
        other:TensorMassFunction_CSC = other
        ...
        return TensorMassFunction_CSC(m3)
    ...
    def ConvertToOriginalMassFunction(self):
        ...
        for i,j in zip(rows,cols):
            v = self.vec[i,0] 
            k = TensorMassFunction_CSC.setMapInt2Char[sortedPowerset[i]]
            self[k] = v
        return self
            
    def bel(self, hypothesis=None):
        if hypothesis is None:
            return {h:self.bel(h) for h in  TensorMassFunction_CSC.setMapChar2Int.keys()}
        else:
            k_int = TensorMassFunction_CSC.setMapChar2Int[frozenset(hypothesis)]
            index = TensorMassFunction_CSC.setMap[k_int]
            val = 0
    ...
    def pl(self, hypothesis=None):
        if hypothesis is None:
            return {h:self.pl(h) for h in  TensorMassFunction_CSC.setMapChar2Int.keys()}
        else:
            k_int = TensorMassFunction_CSC.setMapChar2Int[frozenset(hypothesis)]
            index = TensorMassFunction_CSC.setMap[k_int]
            val = 0
...
if __name__ == '__main__':
    ...
    tm1 = TensorMassFunction_CSC({'a':0.3, 'c':0.2, 'ab':0.2, 'ac':0.3}) # using a dictionary

    tm2 = TensorMassFunction_CSC({'b':0.2, 'c':0.1, 'ab':0.5, 'abc':0.2}) # using a dictionar
    elements = ['a', 'b', 'c', 'd']
