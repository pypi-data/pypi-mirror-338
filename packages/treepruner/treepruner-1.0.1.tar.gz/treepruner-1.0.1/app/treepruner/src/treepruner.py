...
import random
from .utils import (one_primitive_step, is_valid_tree, prune_by_branch_length, prune_by_root_to_tip)
...
def prune_tree_CPA(name_of_the_tree, root_to_node_ratio=0.1, min_num_of_roots=15, M_n=0, threshold=90, beta=20, radius_ratio = 0, name_of_output="cpa_tree.nwk", show_plot=False, show_pruned_tips=False, safe_tips=[]):
    if not is_valid_tree(name_of_the_tree):
        raise TypeError("The input file is not a valid phylogenetic tree. Please make sure that name_of_the_tree is a valid .nwk tree.")
