...
from ._tables_restriction_condition import TablesRestrictionCondition
from ._tables_restrictions import Restrictions
...
class Tables(
    ...
    def _get_unambiguous_keys(self, *, key: TableName | None) -> list[TableName]:
        ...
        if key is None:
            tables = graphql_client.get_tables().data_model.database.tables
            return [table.name for table in tables]

        table = graphql_client.find_table(
            table_name=key,
    ...
    def _update_delegate(
        ...
        for index, (table_name, definition) in enumerate(other.items()):
            data_types = [
                ColumnDataTypeInput(
            ...
            default_values = [
                ColumnDefaultValueInput(column_name=column_name, value=value)
                for column_name, value in definition.default_values.items()
            ]
            primary_index = (
            ...
            output_fields = Mutation.create_table(
                CreateTableInput(
            ...
            ).alias(f"createTable_{index}")
            table_fields = output_fields.table()
            table_fields.fields(table_fields.name)
            output_fields.fields(table_fields)
            fields.append(output_fields)

        graphql_client.mutation(*fields, operation_name="updateTables")
    ...
    def _delete_delegate_keys(self, keys: AbstractSet[TableName], /) -> None:
        ...
        for index, key in enumerate(keys):
            output_fields = Mutation.delete_table(
                DeleteTableInput(table_identifier=TableIdentifier(key)._graphql_input)
            ).alias(f"deleteTable_{index}")
            table_fields = output_fields.tables()
            table_fields.fields(table_fields.name)
            output_fields.fields(table_fields)
            fields.append(output_fields)
        graphql_client.mutation(*fields, operation_name="deleteTables")
    ...
    def data_transaction(
        ...
        * If an exception is raised during a data transaction, it will be rolled back and the changes made until the exception will be discarded.
        ...
        return transact_data(
            ...
            commit=lambda transaction_id: java_api.end_data_transaction(
                transaction_id,
                has_succeeded=True,
            ),
            rollback=lambda transaction_id: java_api.end_data_transaction(
                transaction_id,
                has_succeeded=False,
            ),
            session_id=self._session_id,
            start=lambda: java_api.start_data_transaction(
    ...
    def infer_data_types(self, data: _LoadArgument, /) -> dict[str, DataType]:  # pyright: ignore[reportUnknownParameterType]
        ...
        match data:
            ...
            case DataLoad():
                return java_api.infer_data_types(data)
            case _:
                assert_never(data)

    @property
    def owners(self) -> MutableSet[RoleName]:
        ...
        Example:
            ...
            >>> with tt.Session.connect(  # doctest: +ELLIPSIS
        ...
        """
        return DatabaseOwners(atoti_client=self._atoti_client)

    @property
    def readers(self) -> MutableSet[RoleName]:
        ...
        """
        return DatabaseReaders(atoti_client=self._atoti_client)

    @property
    def restrictions(self) -> MutableMapping[RoleName, TablesRestrictionCondition]:
    ...
    def schema(self) -> object:
        ...
        """
        graphql_client = require_live_extension(self._atoti_client._graphql_client)
        data_model = graphql_client.get_database_schema().data_model
