...
class ShorelineAnalyzer:
    ...
    def __init__(
        ...
        self.log = {"timestamp": int(time.time())}
...
class AnalysisResult:
    ...
    def visualize_coastal_slopes(self, plot_indices=True):
        """
        Visualize analyzed coastal slopes with multiple plots showing various metrics

        Parameters
        ----------
        self.slopes : GeoDataFrame
            A GeoDataFrame containing ray information
        self.lat_line : LineString
            The offshore line (LAT)
        self.shore_line : LineString
            The onshore line (HAT)
        self.metrics : dict
            The self.metrics calculated by self.intertidal_gradient_metrics
        plot_indices : bool
            Whether to include plot showing different gradient indices
        ...
        if plot_indices:
            ...
            axs[1].set_title("Comparison of Gradient Indices")
            axs[1].set_ylabel("Angle (degrees)")
            axs[1].set_ylim(0, max(values) * 1.2)
            axs[1].grid(axis="y", zorder=1, lw=0.5)
