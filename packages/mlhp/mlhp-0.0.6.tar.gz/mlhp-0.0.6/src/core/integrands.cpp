...
template<size_t D>
void computeL2ElementSystem( std::optional<double> mass, double rhs,
                             ...
                             double weightDetJ, size_t ifield )
{
    ...
    auto computeTangent = mass != std::nullopt;
    ...
    linalg::elementRhs( targets[computeTangent].data( ) + offset, ndofI, 0, expr2 );
}
...
template<size_t D>
void computeL2ElementSystem( std::optional<std::span<double>> massValue,
                             ...
                             double weightDetJ )
{
    ...
    for( size_t ifield = 0; ifield < nfields; ++ifield )
    {
        auto NI = shapes.noalias( ifield, 0 );
        auto ndofI = shapes.ndof( ifield );
        auto computeTangent = massValue != std::nullopt;
                                          ...
                                          size_t ifield )
{
    auto evaluate = [=]( const BasisFunctionEvaluation<D>& shapes,
                         ...
                         double weightDetJ )
    {
        computeL2ElementSystem( mass( shapes.xyz( ) ), rhs( shapes.xyz( ) ), shapes, targets, weightDetJ, ifield);
    };

    return DomainIntegrand<D>( symmetricTargets( true, true ), DiffOrders::Shapes, evaluate);
}
                                          ...
                                          const spatial::VectorFunction<D>& rhs )
{
                         ...
                         double weightDetJ )
    {
        ...
        computeL2ElementSystem( massValue, rhsValue, shapes, targets, weightDetJ );
    };
...
DomainIntegrand<D> makeInternalEnergyIntegrand( memory::vptr<const std::vector<double>> solutionDofs,
                                                ...
                                                const ConstitutiveEquation<D>& constitutive )
{
    auto evaluate = [=]( const BasisFunctionEvaluation<D>& shapes,
                         const LocationMap& locationMap,
                         AlignedDoubleVectors& targets,
                                             ...
                                             bool computeTangent )
{
                         ...
                         double weightDetJ )
    {
        ...
        auto optionalMass = std::optional<double> { std::nullopt };

        if( computeTangent )
        {
            optionalMass = massValue;
        }

        computeL2ElementSystem( optionalMass, rhs( shapes.xyz( ) ) - massValue * u, shapes, targets, weightDetJ, ifield );
    };
                                             ...
                                             bool computeTangent )
{
                         ...
                         double weightDetJ )
    {
        ...
        auto optionalMass = computeTangent ? std::optional { massValue } : std::optional<std::span<double>> { };

        computeL2ElementSystem( optionalMass, rhsValue, shapes, targets, weightDetJ );
    };
                                           ...
                                           const std::optional<spatial::ScalarFunction<D>>& rhs )
{
    ...
    return internalNormalL2Integrand<D>( lhs, std::move( rhsCallback ), rhs != std::nullopt );
}
