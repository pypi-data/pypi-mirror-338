...
from src.pygridsio.geotiffIO import write_grid_to_geotiff, read_geotiff_to_grid
from src.pygridsio.grid import Grid
from src.pygridsio.grid_operations import resample_xarray_grid_to_other_grid_resolution, assert_grid_geometry_is_equal
from src.pygridsio.grid_to_xarray import custom_grid_to_xarray, isValidDataArrayGrid, xarray_to_custom_grid
from src.pygridsio.netcdfIO import read_netcdf_to_custom_grid, write_to_netcdf_raster, read_netcdf_to_dataarray
...
def write_grid(grid: xr.DataArray, filename: Path, RDnew=True, epsg=None):
    ...
    if filename.suffix in [".asc", ".zmap"]:
        if epsg is not None:
            raise ValueError("epsg projection code was provided to write_grid, while the desired output filetype is .asc or .zmap; which cannot accept arbitrary epsg codes.")
        xarray_to_custom_grid(grid).write(str(filename))
    if filename.suffix == ".nc":
        if epsg is not None:
            raise ValueError("epsg projection code was provided to write_grid, while the desired output filetype is .nc; which at the moment cannot accept arbitrary epsg codes.")
