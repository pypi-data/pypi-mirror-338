...
class BaseParser:
    ...
    def _find_parenthesis(cls, stmt: Statement):
        """
        查找并返回第一个括号的 Token。如果未找到，返回 None。
        """
        def find_in_tokens(tokens):
            for token in tokens:
                if isinstance(token, Parenthesis):
                    return token
                # 对嵌套类型进行递归检查
                if hasattr(token, 'tokens') and token.ttype is None:
                    nested_result = find_in_tokens(token.tokens)
                    if nested_result:
                        return nested_result
            return None

        return find_in_tokens(stmt.tokens)
