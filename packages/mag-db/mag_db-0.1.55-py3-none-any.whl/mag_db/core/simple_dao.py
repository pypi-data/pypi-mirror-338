...
class SimpleDao(BaseDao):
    ...
    def query_batch_beans(self, bean_class: type, where: Where) -> DbList[T]:
        """
        按条件列表数据库记录
        """
        select = self.__get_select(where)
    ...
    def query_batch_maps(self, where: Where) -> DbList[dict[str, Any]]:
        select = self.__get_select(where)
    ...
    def query_batch_all_beans(self, bean_class: type) -> 'DbList[T]':
        """
        列表数据库的全部记录
    ...
    def query_batch_all_maps(self) -> DbList[dict[str, Any]]:
        select = self.__get_select(Where())
        query = IndexQueryExecutor(select.__str__(), self._session)
        return query.execute_maps(self._table_info.get_column_names(True))
