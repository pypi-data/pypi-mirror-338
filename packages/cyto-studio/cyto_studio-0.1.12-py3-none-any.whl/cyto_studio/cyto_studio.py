...
class CYTOSTUDIO:
    ...
    def set_image_slice_value(self):
        ...
        for chn in range(len(channel_names)):
            ...
            # Directly delete the data from the layer
#             try:
#                 data_list = self.viewer.layers[str(channel_name)].data
#                 for i in range(len(data_list)):
#                     data_list[i] = None
                    
#                 # Clear the reference to the entire data list
#                 self.viewer.layers[str(channel_name)].data = None
#             except:
#                 continue
#             gc.collect()
            ...
            if chn in self.selected_channels:
                ...
                # Example of extracting parameters you want to preserve
                layer_params = {
                    'scale': layer.scale,  # Preserves spacing or any scaling factors
                    'translate': layer.translate,  # Translation offset
                    'opacity': layer.opacity,  # Transparency setting
                ...
                # Assuming im1, im2, etc., are the new image data arrays
...
#             tifffile.imsave(file_name, volume)
    ...
    def main(self):
        ...
        icon_data = pkgutil.get_data("cyto_studio", "logo.ico")
        pixmap = QPixmap()
