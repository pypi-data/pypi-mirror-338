...
class MultiProcessHandler():
    ...
    def run_schedule(self, schedule:dict, max_workers:int = os.cpu_count(), lock:object = None) -> dict:
        ...
        self._logger.info(f"Starting multiprocessing schedule with {max_workers} workers ...")
        main_loop = True
        while main_loop:
    ...
    def run_func(self, func, args:list, max_workers:int = os.cpu_count(), lock:object = None) -> list:
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
            self._logger.info(f"Spliting function <{func}> into {max_workers} multiprocesses ...")
            proc_results = [executor.submit(self._run_multiprocess, func, curr_args, lock) for curr_args in args]
    ...
    def _run_multiprocess(self, func, args:list, lock:object) -> object:
        ...
        try:
            ...
            p._logger.exception(f"Multiprocess failed due to: {err}", add_info=True, terminate=True)
            p.status = "ERROR"
            p.error = err
