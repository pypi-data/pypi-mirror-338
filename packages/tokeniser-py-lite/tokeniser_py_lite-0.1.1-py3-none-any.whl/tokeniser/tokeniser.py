...
class Tokeniser:
  ...
  def one_hot_tokens(self, token_ids: List[int], op: str = 'np') -> Union[np.ndarray, torch.Tensor]:
    ...
    if op == 'np':
      ...
      raise ValueError("op argument value 'torch' is not supported for tokeniser-py-light. use tokeniser py for using torch directly.")

    else:
      raise ValueError("Invalid op argument. Use 'np' for numpy.")
