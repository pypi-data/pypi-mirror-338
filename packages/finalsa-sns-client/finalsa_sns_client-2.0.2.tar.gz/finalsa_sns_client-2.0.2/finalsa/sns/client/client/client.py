...
class SnsClientImpl(SnsClient):
    ...
    def publish_batch(self, topic_name: str, payloads: List[Dict | str], attrs: Dict | None = ...) -> Dict:
        topic = self.get_topic(topic_name)
        attrs = attrs or {}
        entries = self.__dump_batch_payload__(payloads, to_sns_message_attributes(attrs))
        groups = []
        counter = 0
        helper = []
    ...
    def publish(
        ...
        response = topic.publish(
            Message=payload,
            MessageAttributes=to_sns_message_attributes(attrs)
        )
