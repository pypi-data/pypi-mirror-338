from finalsa.common.models import to_sns_message_attributes
...
class SnsClientImpl(SnsClient):
    ...
    def publish_batch(self, topic_name: str, payloads: List[Dict | str], att_dict: Dict | None = ...) -> Dict:
        topic = self.get_topic(topic_name)
        entries = self.__dump_batch_payload__(payloads, to_sns_message_attributes(att_dict))
        groups = []
        counter = 0
        helper = []
    ...
    def publish(
        ...
        response = topic.publish(
            Message=payload,
            MessageAttributes=to_sns_message_attributes(att_dict)
        )
