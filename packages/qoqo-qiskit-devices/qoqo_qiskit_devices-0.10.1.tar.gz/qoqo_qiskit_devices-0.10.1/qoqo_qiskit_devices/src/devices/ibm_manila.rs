...
use roqoqo_qiskit_devices::{IBMDevice, IBMManilaDevice};

/// IBM Manila device
///
#[pyclass(name = "IBMManilaDevice", module = "ibm_devices")]
#[derive(Clone, Debug, PartialEq)]
pub struct IBMManilaDeviceWrapper {
    /// Internal storage of [roqoqo_qiskit_devices::IBMManilaDevice]
    pub internal: IBMManilaDevice,
}

#[pymethods]
impl IBMManilaDeviceWrapper {
    /// Create a new IBMManilaDevice instance.
    #[new]
    pub fn new() -> Self {
        Python::with_gil(|py| {
            py.run(c_str!("import warnings; warnings.warn(\"Device ibmq_manila has been retired. Setting noise information is not possible.\", category=DeprecationWarning, stacklevel=2)"), None, None).unwrap();
        });
        Self {
    ...
    pub fn name(&self) -> &str {
        roqoqo_qiskit_devices::IBMManilaDevice::name(&self.internal)
    }
...
impl IBMManilaDeviceWrapper {
    ...
    pub fn from_pyany(input: &Bound<PyAny>) -> PyResult<IBMManilaDevice> {
        if let Ok(try_downcast) = input.extract::<IBMManilaDeviceWrapper>() {
            Ok(try_downcast.internal)
        } else {
            let get_bytes = input.call_method0("to_bincode")?;
            let bytes = get_bytes.extract::<Vec<u8>>()?;
            deserialize(&bytes[..]).map_err(|err| {
                PyValueError::new_err(format!("Cannot treat input as IBMNairobiDevice: {}", err))
            })
        }
    }
}

impl Default for IBMManilaDeviceWrapper {
    fn default() -> Self {
        Self::new()
    }
}
