...
/// IBM Perth device
///
#[pyclass(name = "IBMPerthDevice", module = "ibm_devices")]
...
impl IBMPerthDeviceWrapper {
    ...
    pub fn from_pyany(input: Py<PyAny>) -> PyResult<IBMPerthDevice> {
        Python::with_gil(|py| -> PyResult<IBMPerthDevice> {
            let input = input.bind(py);
            if let Ok(try_downcast) = input.extract::<IBMPerthDeviceWrapper>() {
                Ok(try_downcast.internal)
            } else {
                let get_bytes = input.call_method0("to_bincode")?;
                let bytes = get_bytes.extract::<Vec<u8>>()?;
                deserialize(&bytes[..]).map_err(|err| {
                    PyValueError::new_err(format!("Cannot treat input as IBMPerthDevice: {}", err))
                })
            }
        })
    }
