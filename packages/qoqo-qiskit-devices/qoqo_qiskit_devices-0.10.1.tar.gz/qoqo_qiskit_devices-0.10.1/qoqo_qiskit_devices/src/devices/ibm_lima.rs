...
use roqoqo_qiskit_devices::{IBMDevice, IBMLimaDevice};

/// IBM Lima device
///
#[pyclass(name = "IBMLimaDevice", module = "ibm_devices")]
...
impl IBMLimaDeviceWrapper {
    ...
    pub fn new() -> Self {
        Python::with_gil(|py| {
            py.run(c_str!("import warnings; warnings.warn(\"Device ibmq_lima has been retired. Setting noise information is not possible.\", category=DeprecationWarning, stacklevel=2)"), None, None).unwrap();
        });
        Self {
    ...
    pub fn name(&self) -> &str {
        roqoqo_qiskit_devices::IBMLimaDevice::name(&self.internal)
    }
...
impl IBMLimaDeviceWrapper {
    ...
    pub fn from_pyany(input: &Bound<PyAny>) -> PyResult<IBMLimaDevice> {
        if let Ok(try_downcast) = input.extract::<IBMLimaDeviceWrapper>() {
            Ok(try_downcast.internal)
        } else {
            let get_bytes = input.call_method0("to_bincode")?;
            let bytes = get_bytes.extract::<Vec<u8>>()?;
            deserialize(&bytes[..]).map_err(|err| {
                PyValueError::new_err(format!("Cannot treat input as IBMLimaDevice: {}", err))
            })
        }
    }
}
