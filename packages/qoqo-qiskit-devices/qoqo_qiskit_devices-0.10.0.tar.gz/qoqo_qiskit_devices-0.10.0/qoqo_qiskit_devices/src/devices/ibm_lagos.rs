...
use roqoqo_qiskit_devices::{IBMDevice, IBMLagosDevice};

/// IBM Lagos device
///
#[pyclass(name = "IBMLagosDevice", module = "ibm_devices")]
...
impl IBMLagosDeviceWrapper {
    ...
    pub fn new() -> Self {
        Python::with_gil(|py| {
            py.run(c_str!("import warnings; warnings.warn(\"Device ibm_lagos has been retired. Setting noise information is not possible.\", category=DeprecationWarning, stacklevel=2)"), None, None).unwrap();
        });
        Self {
    ...
    pub fn name(&self) -> &str {
        roqoqo_qiskit_devices::IBMLagosDevice::name(&self.internal)
    }
...
impl IBMLagosDeviceWrapper {
    ...
    pub fn from_pyany(input: Py<PyAny>) -> PyResult<IBMLagosDevice> {
        Python::with_gil(|py| -> PyResult<IBMLagosDevice> {
            let input = input.bind(py);
            if let Ok(try_downcast) = input.extract::<IBMLagosDeviceWrapper>() {
                Ok(try_downcast.internal)
            } else {
                let get_bytes = input.call_method0("to_bincode")?;
                let bytes = get_bytes.extract::<Vec<u8>>()?;
                deserialize(&bytes[..]).map_err(|err| {
                    PyValueError::new_err(format!("Cannot treat input as IBMLagosDevice: {}", err))
                })
            }
        })
    }
