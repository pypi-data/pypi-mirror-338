...
use roqoqo_qiskit_devices::{IBMDevice, IBMJakartaDevice};

/// IBM Jakarta device
///
#[pyclass(name = "IBMJakartaDevice", module = "ibm_devices")]
#[derive(Clone, Debug, PartialEq)]
pub struct IBMJakartaDeviceWrapper {
    /// Internal storage of [roqoqo_qiskit_devices::IBMJakartaDevice]
    pub internal: IBMJakartaDevice,
}

#[pymethods]
impl IBMJakartaDeviceWrapper {
    /// Create a new IBMJakartaDevice instance.
    #[new]
    pub fn new() -> Self {
        Python::with_gil(|py| {
            py.run(c_str!("import warnings; warnings.warn(\"Device ibmq_jakarta has been retired. Setting noise information is not possible.\", category=DeprecationWarning, stacklevel=2)"), None, None).unwrap();
        });
        Self {
            internal: IBMJakartaDevice::new(),
        }
    }
    ...
    pub fn name(&self) -> &str {
        roqoqo_qiskit_devices::IBMJakartaDevice::name(&self.internal)
    }
...
impl IBMJakartaDeviceWrapper {
    ...
    pub fn from_pyany(input: &Bound<PyAny>) -> PyResult<IBMJakartaDevice> {
        if let Ok(try_downcast) = input.extract::<IBMJakartaDeviceWrapper>() {
            Ok(try_downcast.internal)
        } else {
            let get_bytes = input.call_method0("to_bincode")?;
            let bytes = get_bytes.extract::<Vec<u8>>()?;
            deserialize(&bytes[..]).map_err(|err| {
                PyValueError::new_err(format!("Cannot treat input as IBMJakartaDevice: {}", err))
            })
        }
    }
}

impl Default for IBMJakartaDeviceWrapper {
    fn default() -> Self {
        Self::new()
    }
}
