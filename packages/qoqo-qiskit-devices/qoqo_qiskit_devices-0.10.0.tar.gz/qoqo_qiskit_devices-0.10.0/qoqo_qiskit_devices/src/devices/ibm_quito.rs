...
use roqoqo_qiskit_devices::{IBMDevice, IBMQuitoDevice};

/// IBM Quito device
///
#[pyclass(name = "IBMQuitoDevice", module = "ibm_devices")]
...
impl IBMQuitoDeviceWrapper {
    ...
    pub fn new() -> Self {
        Python::with_gil(|py| {
            py.run(c_str!("import warnings; warnings.warn(\"Device ibmq_quito has been retired. Setting noise information is not possible.\", category=DeprecationWarning, stacklevel=2)"), None, None).unwrap();
        });
        Self {
    ...
    pub fn name(&self) -> &str {
        roqoqo_qiskit_devices::IBMQuitoDevice::name(&self.internal)
    }
...
impl IBMQuitoDeviceWrapper {
    ...
    pub fn from_pyany(input: &Bound<PyAny>) -> PyResult<IBMQuitoDevice> {
        if let Ok(try_downcast) = input.extract::<IBMQuitoDeviceWrapper>() {
            Ok(try_downcast.internal)
        } else {
            let get_bytes = input.call_method0("to_bincode")?;
            let bytes = get_bytes.extract::<Vec<u8>>()?;
            deserialize(&bytes[..]).map_err(|err| {
                PyValueError::new_err(format!("Cannot treat input as IBMQuitoDevice: {}", err))
            })
        }
    }
}
