...
use roqoqo_qiskit_devices::{IBMDevice, IBMNairobiDevice};

/// IBM Nairobi device
///
#[pyclass(name = "IBMNairobiDevice", module = "ibm_devices")]
...
impl IBMNairobiDeviceWrapper {
    ...
    pub fn new() -> Self {
        Python::with_gil(|py| {
            py.run(c_str!("import warnings; warnings.warn(\"Device ibm_nairobi has been retired. Setting noise information is not possible.\", category=DeprecationWarning, stacklevel=2)"), None, None).unwrap();
        });
        Self {
    ...
    pub fn name(&self) -> &str {
        roqoqo_qiskit_devices::IBMNairobiDevice::name(&self.internal)
    }
            ...
            if let Ok(try_downcast) = input.extract::<IBMNairobiDeviceWrapper>() {
                ...
                deserialize(&bytes[..]).map_err(|err| {
                    PyValueError::new_err(format!(
                        "Cannot treat input as IBMNairobiDevice: {}",
                        err
                    ))
                })
            }
