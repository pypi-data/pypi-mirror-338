...
impl IBMNairobiDevice {
    /// Creates a new IBMNairobiDevice.
    ///
    /// # Returns
    ///
    /// An initiated IBMNairobiDevice with single and two-qubit gates and decoherence rates set to zero.
    ///
    pub fn new() -> Self {
        let mut device = Self {
            number_qubits: 7,
...
impl Default for IBMNairobiDevice {
    fn default() -> Self {
        Self::new()
    }
}
...
impl From<&IBMNairobiDevice> for IBMDevice {
    fn from(input: &IBMNairobiDevice) -> Self {
        Self::IBMNairobiDevice(input.clone())
    }
}

impl From<IBMNairobiDevice> for IBMDevice {
    fn from(input: IBMNairobiDevice) -> Self {
        Self::IBMNairobiDevice(input)
    }
}
...
/// Implements QoqoDevice trait for IBMNairobiDevice.
///
/// The QoqoDevice trait defines standard functions available for roqoqo devices.
///
impl QoqoDevice for IBMNairobiDevice {
