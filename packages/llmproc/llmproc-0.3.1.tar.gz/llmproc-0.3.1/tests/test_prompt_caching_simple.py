...
async def test_basic_caching():
    ...
    # Print cache metrics
    print(f"First call cache metrics:")
    ...
    print(f"Second call cache metrics:")
    ...
    assert result1.cached_tokens > 0 or result2.cached_tokens > 0, "Caching should work in at least one call"
    
    # Test passes as long as one of the calls shows cache activity
    total_cache_activity = (result1.cached_tokens + result1.cache_write_tokens + 
                           result2.cached_tokens + result2.cache_write_tokens)
