...
def solveTH(silent: bool = False, iverb: int = -1, retry: bool = True) -> int:
...
VectF = List[float]
try:
    setdensity: callable[[int, VectF, VectF], int] = _dll_api.setdensity
...
try:
    ...
    def symmetric_node(sid: str) -> str:
        return ""
else:
    del _symmetric_node
    def symmetric_node(sid: str) -> str:
