...
OBJECTS_VERSION="1.2.37"
FORMAT_OPTIONS = ["dataframe", "list"]
TYPE_OPTIONS = [None, "benchmark", "audit", "dimension", "custom"]
DIMENSION_LIST = ["security","privacy","hallucination","robustness", "toxicity","stereotype","fairness","ethics","Performance"]
...
class Harnesses:
    ...
    def list(self, type: Optional[str] = None, format: str = "dataframe"):
        ...
        harness_list = pd.DataFrame.from_records([h["harness_config"] for h in response["results"]])
        
        # filter by type if specified
        if type is not None:
            harness_list = harness_list[harness_list["harness_type"].str.contains(type.upper(), na=False)]
            if type == "dimension": # further filter to small only if type is 'dimension' 
                harness_list = harness_list[harness_list["id"].str.contains("_Small", na=False)]
        ...
        for h in unique_harnesses:
            ...
            if len(versions) > 0:
                max_version_row = id_harness[id_harness["version"] == str(max(versions))].iloc[0]
            else:
                max_version_row = id_harness.iloc[0]
            unique_harness_list.append(max_version_row)

        unique_harness_df = pd.concat(unique_harness_list, axis=1).T.reset_index(drop=True)
        unique_harness_df["id"] = unique_harness_df["id"].apply(lambda x: ".".join(x.split(".")[2:])) # drop prefix 'vijil.harnesses.'
...
class Evaluations:
    ...
    def create(self, model_hub: str, harness_version: Optional[str]=OBJECTS_VERSION, model_name: Optional[str]=None, name: Optional[str]=None, api_key_name: Optional[str]=None, model_url: Optional[str]=None, model_params={}, harness_params={}, harnesses=[]):
