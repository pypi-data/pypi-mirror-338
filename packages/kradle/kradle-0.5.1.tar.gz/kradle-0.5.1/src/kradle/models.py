...
class Run:
    ...
    @classmethod
    def from_api_response(cls, data: Dict) -> "Run":
        """
        ...
        arena = RunArena(
            arena_id=arena_data.get('arenaId') if arena_data else None,
            domain=DomainEnum(arena_data.get('domain')) if arena_data and 'domain' in arena_data else None,
            public_address=arena_data.get('publicAddress') if arena_data else None
        ) if arena_data else None
        
        # Parse datetime strings to datetime objects
        creation_time = datetime.fromisoformat(data.get('creationTime').replace('Z', '+00:00')) if data.get('creationTime') else None
        update_time = datetime.fromisoformat(data.get('updateTime').replace('Z', '+00:00')) if data.get('updateTime') else None
...
class ExperimentResult():
    ...
    logs: Dict[str, Dict[str, List[Dict]]] # keyed by run id
    ...
    def display_results(self):
        ...
        for participant_id, result in self.results.items():
            ...
            if result.win_count > 0:
                print(f"    Average Time to Success: {f'{result.average_time_to_success:.2f} seconds' if result.win_count > 0 else 'N/A'}")
        
    def display_runs(self, show_logs: bool = False):
        print("\n----- Run Details -----")
        for run in self.runs:
            print(f"\n[[[ Run ID: {run.id} ]]]")
            print(f"  Status: {run.status}")
            print(f"  Duration: {run.total_time/1000:.2f} seconds")
