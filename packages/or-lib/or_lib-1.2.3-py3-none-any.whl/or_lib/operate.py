...
async def kg_query(
    ...
    logger.info("this is image ids after llm processing", image_ids)
    ...
    logger.info("this is image ids before returning ", image_ids)
...
async def _build_query_context(
    ...
    ```
    """.strip()
    logger.info("this is result in build context", result)
    logger.info("this is image chunk in build context", image_chunk)
    return result, image_chunk
...
async def _get_node_data(
    ...
    logger.info(
        "this is low level image chunks", image_chunks
    )
...
async def _find_most_related_text_unit_from_entities(
    ...
    logger.info("this is all image chunk from node to chunk", image_text_units)
...
async def _get_edge_data(
    ...
    for i, t in enumerate(use_text_units):
        text_units_section_list.append([i, t["content"], t["file_path"]])
    text_units_context = list_of_list_to_csv(text_units_section_list)
    logger.info("this is image chunk in high lebvel get edge", image_chunks)
    return entities_context, relations_context, text_units_context , image_chunks
...
async def _find_related_text_unit_from_relationships(
    ...
    logger.info("this is image chunks from edges root level ", image_text_units)
