...
def room(num: int):
    if num == 242:
        return [DEFAULT_COMMAND_PATH, DEFAULT_METHOD_DIR, SEQUENCE_DIR], [
            DATA_DIR,
            SEQUENCE_DATA_DIR_2,
            SEQUENCE_DATA_DIR,
        ]
    elif num == 254:
        return [DEFAULT_COMMAND_PATH_254, DEFAULT_METHOD_DIR_254, SEQUENCE_DIR_254], [
            DATA_DIR_254,
...
def set_up_utils(num: int, offline: bool = False, runs: bool = False) -> HPLCController:
    path_constants, data_dirs = room(num)
    if not offline:
