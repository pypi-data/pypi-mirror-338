...
from tests.constants import DEFAULT_METHOD, DEFAULT_SEQUENCE, clean_up, set_up_utils, DEFAULT_METHOD_254, \
    DEFAULT_METHOD_242
...
class TestRunsStable(unittest.TestCase):
    ...
    def test_run_method_immidiate_return(self):
        ...
        rand_method = MethodDetails(
            name=DEFAULT_METHOD,
            params=HPLCMethodParams(organic_modifier=5, flow=0.65),
            timetable=[TimeTableEntry(start_time=3.5, organic_modifer=100, flow=0.65)],
            stop_time=4,
            post_time=0.5,
        )
        self.hplc_controller.edit_method(rand_method, save=True)
        try:
            for _ in range(5):
                ...
                uv = self.hplc_controller.get_last_run_method_data(read_uv=True)
                repos = self.hplc_controller.get_last_run_method_report()
                self.assertIsNotNone(repos.vial_location)
                self.assertEqual(repos.signals[0].wavelength, 210)
                self.assertTrue(210 in uv.keys())
                self.assertTrue(len(chrom.A.x) > 0)
        except Exception as e:
        ...
        try:
            ...
            seq_table = SequenceTable(
                ...
                rows=[
                    ...
                    SequenceEntry(
                        vial_location=FiftyFourVialPlate(
                            plate=Plate.ONE, letter=Letter.F, num=Num.TWO
                        ),
                        sample_name="P1-F2",
                        method=self.other_default,
                        inj_source=InjectionSource.HIP_ALS,
            ...
            self.assertEqual(reports[0].vial_location.value(), FiftyFourVialPlate.from_str("P1-A2").value())
            self.assertEqual(reports[1].vial_location.value(), FiftyFourVialPlate.from_str("P1-F2").value())
