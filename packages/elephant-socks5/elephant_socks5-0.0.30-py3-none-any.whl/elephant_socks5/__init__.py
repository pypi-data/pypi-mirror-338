...
from elephant_socks5.protocol import bytes_to_frame, OP_CONTROL, OP_DATA, JRPCResponse, SessionRequest, Hello, Frame, TerminationRequest, SessionTrafficControl
...
_disconnected = 0               # disconnected counter
...
class Tunnel:
    ...
    _always_reconnect: bool = field(default=False)
    ...
    def start(self):
        ...
        log_print(f"Starting Tunnel#{self._count} (URL: {self.url}, always_reconnect: {self._always_reconnect}) ...", underline=True, force_print=True)
    ...
    def _handle_frame(self, ws, frame_bytes):
        ...
        if frame.op_type == OP_CONTROL:
            ...
            if method == 'echo-request':
                ...
                pass
            elif method == 'session-suspend':
                session_id = jrpc['params']['session-id']
                if session_id in self._clients:
                    ctx = self._clients[session_id]
                    ctx.channel().set_auto_read(False)
                    log_print(f"Session suspended: {session_id}: {ctx.channel()}", fg='yellow')
            elif method == 'session-resume':
                session_id = jrpc['params']['session-id']
                if session_id in self._clients:
                    ctx = self._clients[session_id]
                    ctx.channel().set_auto_read(True)
                    log_print(f"Session resumed: {session_id}: {ctx.channel()}", fg='green')
    ...
    def send_session_traffic_control(self, session_id: int, resume: bool) -> None:
        assert self._thread is not None and threading.current_thread() != self._thread
        if session_id not in self._clients:
            return
        tc = SessionTrafficControl.resume(session_id) if resume else SessionTrafficControl.suspend(session_id)
        self._send_frame(tc.to_frame())
    ...
    def _on_close(self, ws, close_status_code, close_msg):
        ...
        if close_status_code == STATUS_TRY_AGAIN_LATER or self._always_reconnect:
            ...
            with _lock:
                _disconnected += 1
            if _disconnected >= len(_tunnels):
                log_print("All tunnels disconnected, exit", fg='red', force_print=True, level=logging.ERROR)
                os._exit(1)
...
class ProxyChannelHandler(ChannelHandlerAdapter):
    ...
    def channel_writability_changed(self, ctx):
        ...
        if not writable:
            ...
            log_print(f"[channel_writability_changed({writable})] {ctx.channel()}", fg='yellow', level=logging.WARNING)
        else:
            recovery_time_seconds = time.perf_counter() - self._unwritable_seconds
            log_print(f"[channel_writability_changed({writable}/{recovery_time_seconds:.2f}s)] {ctx.channel()}", fg='green', level=logging.INFO)
        self._tunnel.send_session_traffic_control(self._session_id, writable)
...
@click.option(
    '--server', '-s', 'urls',
    help=f"Elephant tunnel server URLs (like: {URL})",
    envvar='ELEPHANT_SERVERS',
...
@click.option('--reverse-endpoint', '-re', 'reverse_endpoints', help="The endpoints(host:port) agent will help for reverse proxy", multiple=True, type=str)
...
@click.option('--proxy-endpoint', '-pe', 'proxy_endpoint', help="The endpoint(host:port) server will help for proxy", type=str)
@click.option('--proxy-ip', type=str, help="The IP server will help to proxy", hidden=True)
@click.option('--proxy-port', type=int, default=-1, help="The port server will help to proxy", hidden=True)
@click.option('--always-reconnect', is_flag=True, help="Always reconnect to server on connection lost")
...
def _cli(
    ...
    urls = cycle(urls)
    _tunnels = [Tunnel(next(urls), hello_params.copy(), always_reconnect) for _ in range(tunnel_count)]
