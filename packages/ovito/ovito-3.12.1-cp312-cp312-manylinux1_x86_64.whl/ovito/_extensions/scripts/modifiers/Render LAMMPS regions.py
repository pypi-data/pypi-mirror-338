...
class RegionStyle:
    ...
    def generate_mesh(self, mesh: SurfaceMesh):
        ...
        existing_region_id = mesh.regions['Region ID'].type_by_name(self.region_id, require=False)
...
class RenderLAMMPSRegionsModifier(ModifierInterface):
    ...
    color_by_region: bool = Bool(default_value=False, label='Color by region', ovito_group='Options')
    """
    ...
    .. versionchanged:: 3.11.0 Renamed from :py:attr:`!color_by_type`
    """

    select_atoms: bool = Bool(default_value=False, label='Select interior particles', ovito_group='Options')
    """
    ...
    vis: SurfaceMeshVis = OvitoObject(SurfaceMeshVis, title='LAMMPS Regions', smooth_shading=False, clip_at_domain_boundaries=True)
    """
    ...
    def modify(self, data: DataCollection, **kwargs):
        ...
        if 'lammps-regions' not in data.surfaces:
            ...
            open_cell = copy.deepcopy(data.cell)
            open_cell.pbc = (False, False, False)
            open_cell.vis = None

            surface_mesh = data.surfaces.create('lammps-regions', domain=open_cell, title='LAMMPS Regions', vis=self.vis)
        else:
            surface_mesh = data.surfaces['lammps-regions_']
    ...
    def cleanup_input(self, code: str):
        code = code.strip()
        lines = []
        input = iter([line for line in code.split("\n") if line])
