...
@pytest.mark.system
def test_plain_label_vs_template_data_id_compatibility():
    # In version v194 and earlier, Workbook/Worksheet Templates were pushed with Data IDs that had the label repeated
    # as a suffix. When you push a non-template workbook, the Data ID does not have this suffix. This means that you
    # loading a workbook as a template with a label is not "compatible" with loading it normally and then pushing with
    # the same label. This was an unnecessary/confusing limitation, and so now the suffix is removed. But we have some
    # code to remain compatible with workbooks that have already been pushed.

    guid = _common.new_placeholder_guid()

    # First time through: Template's Data ID has label suffix (two different workbooks pushed)
    # Second time through: Pushed template is still found via Data ID with label suffix (two different workbooks pushed)
    # Third time through: "New" code that harmonizes the Data ID is used (single workbook pushed)
    for v in [194, 195, None]:
        spy.options.compatibility = v

        if v is None:
            label = f'template.test_plain_label_vs_template_data_id_compatibility {guid} (fresh)'
        else:
            label = f'template.test_plain_label_vs_template_data_id_compatibility {guid}'

        workbooks_as_plain = _load_doc_templates(None)
        workbooks_as_templates = _load_doc_templates(label)

        template_workbook = workbooks_as_templates['Analysis Template']
        plain_workbook = workbooks_as_plain['Analysis Template']

        template_workbook_push_df = spy.workbooks.push(template_workbook)
        template_workbook_push_df['ID'] = template_workbook_push_df['Pushed Workbook ID']
        pulled_template_workbooks = spy.workbooks.pull(template_workbook_push_df)
        pulled_template_workbook = pulled_template_workbooks['Analysis Template']
        pulled_template_worksheet = pulled_template_workbook.worksheets['Trend Template']

        plain_workbook_push_df = spy.workbooks.push(plain_workbook, label=label, refresh=False)
        plain_workbook_push_df['ID'] = plain_workbook_push_df['Pushed Workbook ID']
        pulled_plain_workbooks = spy.workbooks.pull(plain_workbook_push_df)
        pulled_plain_workbook = pulled_plain_workbooks['Analysis Template']
        pulled_plain_worksheet = pulled_plain_workbook.worksheets['Trend Template']

        if v is None:
            assert pulled_template_workbook.id == pulled_plain_workbook.id
            assert pulled_template_workbook['Data ID'] == pulled_plain_workbook['Data ID']
            assert pulled_template_worksheet.id == pulled_plain_worksheet.id
            assert pulled_template_worksheet['SPy ID'] == pulled_plain_worksheet['SPy ID']
        else:
            assert pulled_template_workbook.id != pulled_plain_workbook.id
            assert pulled_template_workbook['Data ID'] != pulled_plain_workbook['Data ID']
            assert pulled_template_worksheet.id != pulled_plain_worksheet.id
