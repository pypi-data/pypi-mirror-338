...
# Default to script directory if not running under Claude
log_dir = os.path.dirname(os.path.abspath(__file__))
log_file = "discord_mcp.log"

# If running under Claude Desktop, use its logging path
# Claude Desktop sets environment variables for MCP servers
if os.environ.get("PIPX_HOME") and "discord-mcp" in os.environ.get("PIPX_PACKAGE", ""):
    # macOS path
    if sys.platform == "darwin":
        log_dir = os.path.expanduser("~/Library/Logs/Claude")
    # Windows path
    elif sys.platform == "win32":
        log_dir = os.path.join(os.environ.get("LOCALAPPDATA", ""), "Claude", "Logs")
    # Linux path
    else:
        log_dir = os.path.expanduser("~/.local/share/Claude/logs")
    
    log_file = "mcp-server-Discord.log"
