    ...
    def run(self):
        ...
        for action, files in self.files_to_db.items():
            ...
            for basename, item in files.data.items():
                ...
                # Filtering CAL CDF files
                if basename.startswith("solo_CAL"):
                    logger.debug(f'Skip {basename}')
    ...
    def item_to_file_log(item, root):
        ...
        if not is_data:
            possible_patterns = [
                r"^([^\.]+)_V([0-9]+)\.(" + re_ext + ")",
                r"^([^\.]+)_([^\.]+)_([0-9]+)\.(" + re_ext + ")",
                r"^([^\.]+)_([^\.]+)_([0-9]+)_v([0-9]+)\.(" + re_ext + ")",
                r"^([^\.]+)_v([0-9]+)\.(" + re_ext + ")",
            ]
            for current_pattern in possible_patterns:
                res = re.search(
                    current_pattern, file_dict["basename"]
                )
                if res:
                    break
        else:
    ...
    def commnt_get_meta(filename, key):
        ...
        if len(val) == 0:
            ...
            for i in range(len(val)):
                ...
                for strformat in ["@%Y-%b-%d-%H:%M:%S.%f", "@%Y-%b-%dT%H:%M:%S.%f"]:
                    try:
                        val[i] = datetime.strptime(val[i], strformat)
                    except ValueError:
                        pass
                    else:
                        break
