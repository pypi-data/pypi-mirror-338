        ...
        for f in fields:
            ...
            if time is not None:
                if isinstance(time, str):
                    for strformat in ["%Y-%m-%dT%H:%M:%S.%f",
                                      "%Y-%m-%d %H:%M:%S",
                                      "%Y-%m-%d %H:%M:%S.%f",
                                      "@%Y-%m-%dT%H:%M:%S.%f"]:
                        try:
                            time = datetime.strptime(time, strformat)
                        except ValueError:
                            pass
                        else:
                            break
                try:
                    ...
                    logger.warning(f'Time format ({time}) is not valid for {f} in {log_file_dict["basename"]}')
                    log_file_dict[f] = None
