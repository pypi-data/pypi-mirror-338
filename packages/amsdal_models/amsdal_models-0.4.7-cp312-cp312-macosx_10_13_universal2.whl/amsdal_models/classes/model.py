    ...
    def __new__(
        ...
        **kwargs: Any,
        ...
        if cls_name not in {'Model', 'LegacyModel'}:
            ...
            if cls_name == 'FrontendModelConfig':
                pass
...
class Model(TypeModel, metaclass=AmsdalModelMetaclass):
    ...
    async def amodel_dump(  # type: ignore[override]
        self,
        *,
        mode: Literal['json', 'python'] | str = 'python',
        include: IncEx = None,
    ...
    async def _async_model_dump(cls, obj: Any) -> Any:
        ...
        if isinstance(obj, dict):
            try:
                if 'ref' in obj:
                    ref_obj = Reference(**obj)
                    return await cls._async_model_dump((await ref_obj).model_dump())
            except ValueError:
                ...
