...
from pydantic import BaseModel, Field, create_model, ConfigDict, SkipValidation
...
def _create_model_from_dict(name: str, data: dict[str, Any]) -> BaseModel:
    ...
    for key, value in data.items():
        ...
        #     # For Enum types, use the actual type instead of SkipValidation
        #     fields[key] = (type(value), Field(default=value))
        # else:
        #     # Use SkipValidation for other fields to avoid type validation issues
        #     fields[key] = (SkipValidation, Field(default=value))
        fields[key] = Annotated[type(value), SkipValidation]
