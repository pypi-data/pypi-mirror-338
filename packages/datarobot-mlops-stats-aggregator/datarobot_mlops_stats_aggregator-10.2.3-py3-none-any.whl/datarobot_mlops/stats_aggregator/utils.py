...
def aggregate_numeric_stats(
    series: pd.Series, histogram_bin_count: Optional[int], decision_count: Optional[int] = 0
):
...
def aggregate_prediction_stats(predictions: pd.DataFrame, histogram_bin_count: Optional[int]):
    """Aggregate predictions into statistics"""
    prediction_stats = []
    decisions = predictions.idxmax(axis=1)
    for col_name in predictions:
        # For numeric stats of multiclass model we need to count number of times concrete class has highest probability
        decision_count = len(np.where(decisions == col_name)[0])
        prediction_stats.append(
            aggregate_numeric_stats(predictions[col_name], histogram_bin_count, decision_count)
        )
