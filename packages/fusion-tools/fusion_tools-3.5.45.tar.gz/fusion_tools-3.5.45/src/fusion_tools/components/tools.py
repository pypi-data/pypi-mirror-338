...
class GlobalPropertyPlotter(MultiTool):
    ...
    def extract_all_properties(self, session_data):
        ...
        property_names = session_properties.select_dtypes(exclude='object').columns.tolist()
        structure_col = 'Structure'
        slide_col = 'Slide Name'
        bbox_cols = ['min_x','min_y','max_x','max_y']


        return session_properties, property_names, structure_col, slide_col, bbox_cols
    ...
    def update_layout(self, session_data:dict, use_prefix:bool):
        ...
        if self.preloaded_properties is None:
            property_data, property_names, structure_col, slide_col, bbox_cols = self.extract_all_properties(session_data)
            property_tree, property_keys = self.generate_property_dict(self.preloaded_options)

        property_info = self.extract_property_info(property_data)
            ...
            dbc.Card([
                dbc.CardBody([
                    ...
                    dbc.Row([
                        html.Div(
                            dcc.Dropdown(
                                options = [] if property_names is None else property_names,
                                value = [],
                    ...
                    dbc.Row([
                        dcc.Dropdown(
                            options = [] if structure_col is None or property_data is None else property_data[structure_col].unique().tolist(),
                            value = [],
                    ...
                    dbc.Row([
                        dcc.Dropdown(
                            options = [] if property_names is None else property_data.columns.tolist(),
                            value = [],
                            multi = False,
    ...
    def update_properties_and_filters(self, property_selection, property_checked, structure_selection, label_selection, filter_prop_mod, filter_prop_remove, filter_prop_selector, property_divs,current_data, keys_info):
        ...
        if not get_pattern_matching_value(property_selection) is None:
            ...
            current_data['property_names'] = [keys_info['name_key'][i] for i in get_pattern_matching_value(property_checked) if i in keys_info['name_key']]
    ...
    def generate_plot(self, butt_click, current_data):
        ...
        include_cols = property_names+[label_names]+[self.slide_column]+self.bbox_columns
        plot_report_tabs = self.make_property_plot_tabs(filtered_properties.loc[:,include_cols], label_names, property_names, [self.slide_column]+self.bbox_columns)

        return [json.dumps(current_data)],[return_fig],[plot_report_tabs]
