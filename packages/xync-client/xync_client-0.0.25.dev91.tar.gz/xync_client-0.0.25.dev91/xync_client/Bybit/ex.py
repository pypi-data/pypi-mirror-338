...
class ExClient(BaseExClient):  # Bybit client
    ...
    async def curs(self) -> dict[types.CurEx]:
        ...
        return {
            c["currencyId"]: types.CurEx(exid=c["currencyId"], ticker=c["currencyId"]) for c in config["currencies"]
        }
    ...
    async def coins(self) -> dict[types.CoinEx]:
        ...
        for c in config["symbols"]:
            all_pms[c["tokenId"]] = types.CoinEx(exid=c["tokenId"], ticker=c["tokenId"])
        return all_pms
