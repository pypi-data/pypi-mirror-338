class DefaultPartitioner:
    ...
    def __init__(self, all_partitions: list[int]):
        self._all_partitions = all_partitions

    def __call__(self, key: bytes):
        """
        Get the partition corresponding to key
        :param key: partitioning key
        :return: one of the values from all_partitions
        """

        idx = murmur2(key)
        idx &= 0x7FFFFFFF
        idx %= len(self._all_partitions)
        return self._all_partitions[idx]


# https://github.com/apache/kafka/blob/0.8.2/clients/src/main/java/org/apache/kafka/common/utils/Utils.java#L244
def murmur2(data: bytes):
