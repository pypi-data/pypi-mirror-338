...
from DLMS_SPODES.cosem_interface_classes import cosem_interface_class as ic, Parameter
...
from .client import Client, pdu, logL, Status, Security, Data, mechanism_id, AcseServiceUser, State
...
class Sequence(Base):
    ...
    def __init__(self, *tasks: ExTask, msg: str = None):
...
class InitType(Base):
    ...
    async def exchange(self, c: Client) -> result.Simple[collection.Collection]:
        res: result.Result
        if (ldn := (res := await GetLDN().exchange(c)).value) is None:
            ...
        elif (f_id := (res := await FindFirmwareId().exchange(c)).value) is None:
            ...
        elif (f_ver := (res := await FindFirmwareVersion().exchange(c)).value) is None:
            ...
        else:
            col_id = collection.ID(
...
class UpdateFirmware(ExTask):
    ...
    async def exchange(self, c: Client) -> result.Simple[bool]:
        ...
        if semver < SemVer(0, 0, 25):
            ...
            if (firm_id_value := res.value) is None:
                c.set_error(Transmit.READ_ERROR, message=F"error firmware ID read: {res.value}")
                return res
            else:
                res.append(await ReadAttribute("00 00 80 64 00 ff", 2).exchange(c))
