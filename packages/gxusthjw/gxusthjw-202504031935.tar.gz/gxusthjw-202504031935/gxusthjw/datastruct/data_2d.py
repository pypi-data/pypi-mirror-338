...
# ----------------------------------------------------------------
# File Name:        data_2d.py
# Author:           Jiwei Huang
# Version:          0.0.1
# Created:          2012/01/01
# Description:      Main Function: 定义“表征`二维数据`的类”。
...
from ..commons import NumberArrayLike, Number
...
__doc__ = """
Defining a class that represents `two-dimensional data`.
"""
...
class Data2d(object):
    ...
    def __init__(
        self, data_y: NumberArrayLike, data_x: Optional[NumberArrayLike] = None
    ):
        """
        类`Data2d`的初始化方法。
        ...
        # ----------------------------------------------------------
        # 初始化数据y。
        self.__data_y = np.asarray(data_y)
        # 禁止修改data_y。
        self.__data_y.setflags(write=False)
        # 检查data_y是否是一维数组。
        if self.__data_y.ndim != 1:
            raise ValueError("data_y must be a one-dimensional array.")
        ...
        if data_x is None:
            ...
            self.__data_x = np.asarray(data_x)
        # 禁止修改data_x。
        self.__data_x.setflags(write=False)
        # 检查data_x是否是一维数组。
        if self.__data_x.ndim != 1:
            raise ValueError("data_x must be a one-dimensional array.")
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, Data2d):
            return False
        return np.array_equal(self.data_y, other.data_y) and np.array_equal(
    ...
    def __str__(self):
        ...
        """
        return (
            f"Data2d(data_y={self.data_y.tolist()}, " f"data_x={self.data_x.tolist()})"
        )
    ...
    def __repr__(self):
        ...
        """
        return (
            f"Data2d(data_y=np.array({self.data_y.tolist()}), "
