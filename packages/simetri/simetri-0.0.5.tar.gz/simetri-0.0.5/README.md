...
# Check if simetri.graphics can be used in an environment

- In a terminal type ```xelatex -help``` If you get an error message, this may mean that you don't have a LaTeX installation or the path to the ```xelatex``` is not in your system path. Make sure that the system path includes the correct path to ```xelatex```.
