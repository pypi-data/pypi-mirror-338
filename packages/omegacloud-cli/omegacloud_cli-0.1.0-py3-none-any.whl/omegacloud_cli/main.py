...
from omega_cli import api
from omega_cli.common.enums import (
    AgentStatusEnum,
    JobStatusEnum,
    JobTypeEnum,
    TaskStatusEnum,
)
from omega_cli.common.schemas import AgentStatusResponseModel, TaskStatusResponseModel
from omega_cli.controllers.filesync import FileSyncManager
from omega_cli.utils import load_config, load_script, save_config, save_script
...
def cli():
    """
    Omega Cloud CLI: One-click platform for AI-apps deployment
    """
    pass
...
async def run_task_flow(status: Status, fixed_agent_id: Optional[str] = None) -> str:
    ...
    if task_info.task_status == TaskStatusEnum.ON and task_info.job_status == JobStatusEnum.RUNNING:
        console.print("- Scheduled job started")
        await watch_flow(status)
