...
namespace sptlz{
  class VORONOI_IDW: public VORONOI {
    ...
    public:
      ...
      VORONOI_IDW(std::vector<sptlz::VoronoiTree*> _voronoi_forest, std::vector<std::vector<float>> _coords, std::vector<float> _values, float _exponent):VORONOI(_voronoi_forest, _coords, _values){
        exponent = _exponent;
      }
