...
def get_queries(conn, name=None, description=None, user=None, page=1, size=25, return_pd=True):
    ...
    response = requests.get(conn.server_url + "/v1/customQueries", headers={**conn._headers(), 'accept': '*/*'}, params=params)
...
def get_query(conn, query_id, return_pd=True):
    """
    ...
    """
    response = requests.get(conn.server_url + f"/v1/customQueries/{query_id}", headers=conn._headers())
    if response.status_code != 200:
        raise Exception(f"Error: {response.status_code}, {response.text}")
    item = response.json()
    data = {
...
def execute_query(conn, query_id, environment_id, input_variables=[], page=1, size=25, commit=True, return_pd=True):
    """
    ...
    response = requests.post(conn.server_url + f"/v1/customQueries/{query_id}", headers=conn._headers(), json=body)
    if response.status_code != 200:
        raise Exception(f"Error: {response.status_code}, {response.text}")
    item = response.json()
...
def query(conn, environment, name=None, id=None, input_variables={}, page=1, size=25, commit=True, return_pd=True):
    ...
    Args:
        ...
        id (str, optional): ID of the query to look up.
    ...
    if name is not None:
        ...
        id = queries.loc['id']
    query = get_query(conn=conn, query_id=id)
    ...
    if input_variables:
        ...
        input_variables = []
    return execute_query(conn=conn, query_id=id, environment_id=enviroment_id, input_variables=input_variables, page=page, size=size, commit=commit, return_pd=return_pd)