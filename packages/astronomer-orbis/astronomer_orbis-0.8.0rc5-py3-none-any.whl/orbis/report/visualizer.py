...
class Visualizer:
    def __init__(self, namespace: str, organization_name: str, release_name: str | None = None, resume: bool = False, metadata: ReportMetadata = None):
    ...
    def get_figure_if_resume(self, metric: Metric) -> Figure:
    ...
    def generate_graph(self, dataframes: list[pl.DataFrame], metric: Metric) -> Figure:
        ...
        total_stats = self._calculate_stats(total_dfs) if total_dfs else None
        df = self._prepare_dataframe(total_dfs, total_stats)
    ...
    def _calculate_statistics(self, df: pl.DataFrame, metric: Metric) -> dict:
        if df.is_empty():
            return {k: 0 for k in ["mean_value", "median_value", "max_value", "min_value", "p90_value", "last_value"]}
    ...
    def downsample_if_needed(self, df: pl.DataFrame) -> pl.DataFrame:
        rows, _ = df.shape
        if rows > 3500000:
            return adaptive_downsample(df=df, column="Value", window_size=100, threshold=df["Value"].mean() * 1.25)
        return df
