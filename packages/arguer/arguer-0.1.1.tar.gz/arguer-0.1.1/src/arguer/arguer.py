...
try:
    from devpkg import VersionChecker
...
class Arguer(argparse.ArgumentParser):
    ...
    Args:
        ...
        add_version: If True, automatically adds a version argument using devpkg.
        version_flags: List of flags to use for the version argument. Defaults to ['--version'].

    NOTE: Version handling requires the devpkg package. If it's not available, the version
          argument won't be added. If desired, you can install it using 'pip install devpkg'.
    ...
    def __init__(self, *args: Any, **kwargs: Any):
        ...
        # Add --version argument if requested and dsbase is available
        if version_available and self.add_version:
    ...
    def _add_version_argument(self) -> None:
        ...
        if not version_available:
            return  # devpkg is not available
