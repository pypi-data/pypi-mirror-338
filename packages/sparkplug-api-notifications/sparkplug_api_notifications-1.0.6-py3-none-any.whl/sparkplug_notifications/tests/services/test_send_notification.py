...
class TestSendNotification(TestCase):
    ...
    def test_send_notification_creates_notification(
        ...
        notification = Notification.objects.filter(
            recipient_id=self.user.id, actor=self.actor, message="Test message"
        ).first()
    ...
    def test_send_notification_handles_invalid_recipient(self):
        with self.assertLogs(
            "sparkplug_notifications.services.send_notification", level="ERROR"
        ) as log:
            send_notification(
        ...
        assert "User not found - sending notification" in log.output[0]
