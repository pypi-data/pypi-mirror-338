...
from sparkplug_notifications.filters.list import ListFilter
from sparkplug_notifications.queries.notifications_list import (
    notifications_list,
)
...
class TestNotificationsList(TestCase):
    ...
    def test_notifications_list_with_recipient_filter(self):
        filters = ListFilter(recipient=self.user)
    ...
    def test_notifications_list_with_has_read_filter(self):
        filters = ListFilter(recipient=self.user, has_read=True)
        queryset = notifications_list(filters)
    ...
    def test_notifications_list_with_is_starred_filter(self):
        filters = ListFilter(recipient=self.user, is_starred=True)
        queryset = notifications_list(filters)
