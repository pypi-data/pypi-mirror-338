...
class HostDiagnoser(Diagnoser):
    ...
    def diagnose_iommu(self) -> None:
        ...
        if HostModule.IOMMU.name in self._log:
            ...
            iommu_out, iommu_err, iommu_code = self._log[HostModule.IOMMU.name]

            host_memory_size = self._check_status[
                HostModule.host_memory.name
            ].host_memory_size
            gt_256GB = True if host_memory_size > 256 else False

            if not iommu_code:
                if "intel_iommu=on" in iommu_out or "amd_iommu=on" in iommu_out:
                    iommu_status.status = Status.SUCCESS
                    iommu_status.iommu_enable = True
                elif "intel_iommu=off" in iommu_out or "amd_iommu=off" in iommu_out:
                    iommu_status.iommu_enable = False
                    if not gt_256GB:
                        iommu_status.status = Status.SUCCESS
                    else:
                        iommu_status.status = Status.WARNING
                        iommu_status.solution = f"The current machine memory is {host_memory_size}, greater than 256GB, it is recommended to enable IOMMU."
            else:
                if EXECUTED_ON_HOST_FLAG:
                    iommu_status.status = Status.ATTENTION
                    iommu_status.solution = "Unable to get whether IOMMU is enabled."
                else:
...
class MTLinkDiagnoser(Diagnoser):
    ...
    def diagnose_mtlink(self):
        ...
        if MTLinkModule.MTLink.name in self._log:
            err_info, err, code = self._log[MTLinkModule.MTLink.name]
            mtlink_dict = CheckStatus(name=MTLinkModule.MTLink.name, tag=self._tag)
            pattern = r"(Fail|fail|Error|error)"
            matches = re.findall(pattern, err_info)
            if matches or code:
                mtlink_dict.status = Status.FAILED
                mtlink_dict.info = err_info
