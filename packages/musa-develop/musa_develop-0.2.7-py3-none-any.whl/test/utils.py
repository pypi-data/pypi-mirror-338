...
class TestInstall:
    ...
    def check_report_includes(self, truth_log=None, truth_log_pattern=None):
        if truth_log_pattern:
            assert re.search(
                self.remove_ansi_escape(truth_log_pattern),
                self.remove_ansi_escape(self._install_log),
            ), f"(Ignore version info) truth_log_pattern: {truth_log_pattern} should in self._install_log: {self._install_log}"
        else:
            assert (
                truth_log in self._install_log
            ), f"truth_log: {truth_log} should in self._install_log: {self._install_log}"

    def check_report_excludes(self, truth_log=None, truth_log_pattern=None):
        if truth_log_pattern:
            assert not re.search(
                self.remove_ansi_escape(truth_log_pattern),
                self.remove_ansi_escape(self._install_log),
            ), f"(Ignore version info) truth_log_pattern: {truth_log_pattern} should not in self._install_log: {self._install_log}"
        else:
            assert (
                truth_log not in self._install_log
            ), f"truth_log: {truth_log} should not in self._install_log: {self._install_log}"
