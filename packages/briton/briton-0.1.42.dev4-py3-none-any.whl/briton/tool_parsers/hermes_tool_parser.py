"""Adapted from https://github.com/vllm-project/vllm/blob/main/vllm/entrypoints/openai/tool_parsers/hermes_tool_parser.py"""
    ...
    def extract_tool_calls(
        ...
        if self.tool_call_start_token not in model_output:
            ...
            try:
                ...
                # Tool Call
                raw_function_calls = [
                    json.loads(match[0] if match[0] else match[1]) for match in function_call_tuples
                ]
                tool_calls = [
                    ChatCompletionMessageToolCall(
                        type="function",
        ...
        try:
            ...
            # case -- the current tool call is being closed.
                ...
                if diff:
                    ...
                    logger.debug(
                        "Finishing tool and found diff that had not " "been streamed yet: %s", diff
                    )
            ...
            # case -- otherwise we're just generating text
                ...
                delta = ChoiceDelta(tool_calls=[], content=text)
                return delta
            ...
            except partial_json_parser.MalformedJSON:
            ...
            if tool_call_portion is None:
                ...
                delta = ChoiceDelta(content=delta_text) if text_portion is not None else None
                return delta
            ...
            # last case -- we have an update to existing arguments.
                ...
                delta = ChoiceDelta(
                    tool_calls=[
                        ChoiceDeltaToolCall(
                            index=self.current_tool_id,
                            function=ChoiceDeltaToolCallFunction(arguments=delta_text),
                        )
                    ]
                )
