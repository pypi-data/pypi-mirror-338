...
def _refresh_cpp_metrics():
    ...
    for counter_entry in counter_entries:
        name = "briton_" + counter_entry.name
        if name not in prom_metrics:
            prom_metrics[name] = prometheus_client.Counter(
                name, "Counter from cpp"
            )

        metric = prom_metrics[name]
        metric.inc(counter_entry.value)

    for histogram_entry in histogram_entries:
        name = "briton_" + histogram_entry.name
        if name not in prom_metrics:
            prom_metrics[name] = prometheus_client.Histogram(
                name, "Histogram from cpp"
            )
    ...
    for gauge_entry in gauge_entries:
        name = "briton_" + gauge_entry.name
        if name not in prom_metrics:
            prom_metrics[name] = prometheus_client.Gauge(
                name, "Gauge from cpp"
            )
