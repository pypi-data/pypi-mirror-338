...
def _refresh_cpp_metrics():
    try:
        import briton.briton_core as briton_core
    except ImportError:
        global briton_core_import_warned
        if not briton_core_import_warned:
            logger.warning("Failed to import briton_core. Assuming an older version of Briton is running.")
            briton_core_import_warned = True
        return

    import prometheus_client

    global prom_metrics
    if 'prom_metrics' not in globals():
        prom_metrics = {}

    counter_entries = briton_core.get_counter_entries()
    histogram_entries = briton_core.get_histogram_entries()
    gauge_entries = briton_core.get_gauge_entries()

    for counter_entry in counter_entries:
        if counter_entry.name not in prom_metrics:
            prom_metrics[counter_entry.name] = prometheus_client.Counter(counter_entry.name, "Counter from cpp")

        metric = prom_metrics[counter_entry.name]
        metric.inc(counter_entry.value)

    for histogram_entry in histogram_entries:
        if histogram_entry.name not in prom_metrics:
            prom_metrics[histogram_entry.name] = prometheus_client.Histogram(histogram_entry.name, "Histogram from cpp")

        metric = prom_metrics[histogram_entry.name]
        metric.observe(histogram_entry.value)

    for gauge_entry in gauge_entries:
        if gauge_entry.name not in prom_metrics:
            prom_metrics[gauge_entry.name] = prometheus_client.Gauge(gauge_entry.name, "Gauge from cpp")

        metric = prom_metrics[gauge_entry.name]
        metric.set(gauge_entry.value)
...
def _briton_monitor(briton_process):
    while True:
        ...
        # TODO(mahmoud): move refreshing metrics to a separate thread
        _refresh_cpp_metrics()
