...
from futurehouse_client.clients.rest_client import JobFetchError, RestClient
from futurehouse_client.models.app import JobRequest, Stage
...
def test_job_response(subtests: SubTests):
    ...
    with subtests.test("Test JobResponse with queued job"):
        ...
        assert job_response.job_name == job_data.name
    ...
    with subtests.test("Test PQA job response"):
        ...
        assert job_data.name in job_response.job_name
