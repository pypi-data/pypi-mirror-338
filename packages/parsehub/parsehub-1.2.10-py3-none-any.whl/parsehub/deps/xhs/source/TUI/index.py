...
class Index(Screen):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield ScrollableContainer(
            Label(Text(f"{self.message("开源协议")}: {LICENCE}", style=MASTER)),
            Label(Text(f"{self.message("项目地址")}{REPOSITORY}", style=MASTER)),
            Label(
                Text(self.message("请输入小红书图文/视频作品链接"), style=PROMPT),
                classes="prompt",
            ),
    ...
    async def deal_button(self):
        if self.url.value:
            ...
            self.tip.write(
                Text(self.message("未输入任何小红书作品链接"), style=WARNING)
            )
