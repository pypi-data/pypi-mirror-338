...
export const initializeClient = async (
  ...
  try {
    const settings = await settingRegistry.load('jovyan-ai-extension:plugin');

    backendUrl = settings.get('backendUrl').composite as string;
    authToken = settings.get('authToken').composite as string;
  } catch (error) {
    console.error('Failed to load settings for jovyan-ai-extension:', error);
  }
...
export const getJovyanClient = () => {
  if (!jovyanClientInstance) {
    throw new Error('JovyanClient instance not initialized');
  }
  if (!isConnected) {
    throw new Error('JovyanClient instance not connected');
  }
