...
def index_file(file, every):
    if file.endswith('.bam'):
        ...
        # Indexing calculates the index of every `every`-th object.
        # For BAM files, the objects are BGZF blocks with a maximal size of 64KB.
        # For index_fastqgz_file, the objects are reads, which leads to a missmatch
        # between the indexing behavior for BAM files and fastq files.
        # As a workaround every gets multiplied here by 64000 to closer match
        # how indexing works for BAM files.
        return index_fastqgz_file(file, every*64000)
