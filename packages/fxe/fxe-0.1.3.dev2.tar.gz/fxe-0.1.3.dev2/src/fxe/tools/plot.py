...
def plot_candles(
    ...
    if overlay_indicator:
        ...
        for o in fig_overlay.data:
            fig.add_trace(
                go.Scatter(
                    x=o["x"], y=o["y"], name=o["name"], line=dict(color=next(c)), legendgroup="1"
                ),
                row=1,
                col=1,
                secondary_y=True,
    ...
    fig.update_layout(
        yaxis1=dict(
            title="volume",
            titlefont=dict(color="#777"),
            tickfont=dict(color="#777"),
            range=[df.volume.min(), df.volume.max() * 2],
        ),
        yaxis2=dict(
            title="price", titlefont=dict(color="#777"), tickfont=dict(color="#777"), showgrid=False
        ),
        hovermode="x unified",
    )
    ...
    fig.update_yaxes(titlefont=dict(color="#777"), tickfont=dict(color="#777"))

    fig.update_layout(
        title={"text": f"Candlestick Plot {stock_id}", "font": {"size": 18, "color": "gray"}}
    )
...
def create_treemap_data(start, end, item="return_ratio", clip=None):
    ...
    basic_info_df["stock_id_name"] = (
        basic_info_df["stock_id"].astype(str) + basic_info_df["公司簡稱"]
    )
...
def plot_tw_stock_treemap(
    ...
    fig = px.treemap(
        ...
        title=f"TW-Stock Market TreeMap({start}~{end})---area_ind:{area_ind}---item:{item}",
        width=1600,
        height=800,
    )
...
def plot_radar(df, mode="bar_polar", line_polar_fill=None, title=None, polar_range=10):
    ...
    fig.update_layout(
        title={"text": title, "x": 0.49, "y": 0.99, "xanchor": "center", "yanchor": "top"},
        paper_bgcolor="rgb(41, 30, 109)",
        polar=dict(radialaxis=dict(visible=True, range=[0, polar_range])),
        width=1200,
        height=600,
    )
    if mode == "line_polar":
...
def get_rank(item: str, period=None, cut_bins=10):
    ...
    df_rank = pd.cut(x=df_rank, bins=cut_bins, labels=list(range(1, cut_bins + 1)))
    return df_rank
...
def plot_tw_stock_radar(
    ...
    fig = plot_radar(
        df=df, mode=mode, line_polar_fill=line_polar_fill, title=title, polar_range=polar_range
    )
    return fig
...
def plot_tw_stock_river(stock_id="2330", start=None, end=None, mode="pe", split_range=8):
    ...
    for n, c in enumerate(col_name_set):
        ...
        fig.add_trace(
            go.Scatter(
                ...
                y=df[c],
                fill=fill_mode,
                line=dict(width=0, color=px.colors.qualitative.Prism[n]),
                name=c,
            )
        )
    customdata = [(c, p) for c, p in zip(df["close"], df[mode], strict=False)]
    hovertemplate = (
        "<br>date:%{x|%Y/%m/%d}<br>close:%{customdata[0]}" + f"<br>{mode}" + ":%{customdata[1]}"
    )
    fig.add_trace(
        go.Scatter(
            x=df.index,
            y=df["close"],
            line=dict(width=2.5, color="#2e4391"),
    ...
    fig.update_layout(
        title=f"{stock_id} {stock_name} {mode.upper()} River Chart",
        template="ggplot2",
        yaxis=dict(
            title="price",
        ),
...
def upload_data(self: None | ReportPyx = None) -> dict:
    """use report to create a upload data"""
    ...
    # creturn
    creturn = {
        "time": self.daily_creturn.index.astype(str).to_list(),
        "value": self.daily_creturn.values.tolist(),
    }

    # ndays return
    ndays_return = {d: self.get_ndays_return(self.daily_creturn, d) for d in [1, 5, 10, 20, 60]}

    d = {
        # create upload data format for report
        "asset_type": self.asset_type,
        "fee_ratio": self.fee_ratio,
        "last_trading_date": self.last_trading_date.isoformat(),
        "last_updated": self.update_date.isoformat(),
        "ndays_return": ndays_return,
        "next_trading_date": self.next_trading_date.isoformat(),
        "positions": {"position": self.position_info(), "position2": self.position_info2()},
...
class StrategySunburst:
    ...
    Args:
        Report
    Returns:
        (plotly.graph_objects.Figure): 旭日圖
    Examples:
    --------
    create a sunburst chart with a list of reports
        ```py
        from finlab.plot import StrategySunburst
        from finlab.data import data
        from strategy.基因演算法初版 import strategy as S0001
        from strategy.小資族資優生策略_NEW import strategy as S0004
        from strategy.營收股價雙渦輪 import strategy as S0003
        from strategy.現金流價值成長 import strategy as S0005
        from strategy.高殖利率烏龜 import strategy as S0002
        ...
        StrategySunburst([r1, r2, r3, r4, r5]).plot()

        # ex1: 實例化物件
        strategies = StrategySunburst([r1, r2, r3, r4, r5])
    ...
    def __init__(self, d: None | list[ReportPyx] = None):
        """繪製策略部位旭日圖

        監控多策略。
        """
        if d is not None and not all(isinstance(r, ReportPyx) for r in d):
            raise ValueError("All elements in the list must be instances of Report")

        if d is None:
            d = data.get_strategies()
        else:
            d = {r.name: upload_data(r) for r in d}
    ...
    def process_position(self, s_name, s_weight=1):
        if s_name == "現金":
            ...
            security_categories["stock_id"] = (
                security_categories["stock_id"] + " " + security_categories["name"]
            )
    ...
    def get_strategy_df(self, select_strategy=None):
        ...
        all_position = pd.concat(
            [
                ...
                for name, weight in zip(s_name, s_weight, strict=False)
            ]
        )
...
class StrategyReturnStats:
    def __init__(
        ...
        report: None | list[ReportPyx] = None,
    ):
        """繪製策略報酬率統計比較圖
        ...
        ``` py
        # 統計 2022-12-31 ~ 2023-07-31 的報酬率數據
        # 回測起始時間
        ...
        # 選定策略範圍
        if report is not None and not all(isinstance(r, ReportPyx) for r in report):
            raise ValueError("All elements in the list must be instances of Report")
        if report is None:
            self.s_data = data.get_strategies()
        else:
            self.s_data = {r.name: upload_data(r) for r in report}

        self.strategy_names = strategy_names
        self.benchmark_return = (
            data.get("benchmark_return:發行量加權股價報酬指數")["發行量加權股價報酬指數"]
    ...
    def _get_returns_set(self):
        ...
        returns_set = {}
        strategy_names = (
            self.s_data.keys() if len(self.strategy_names) == 0 else self.strategy_names
        )
    ...
    def plot_strategy_last_return(self):
        ...
        fig = go.Figure(
            go.Bar(
                ...
                marker=dict(
                    color="rgba(50, 171, 96, 0.6)", line=dict(color="rgba(50, 171, 96, 1)", width=3)
                ),
                orientation="h",
            )
        )
    ...
    def plot_strategy_creturn(self):
        ...
        fig.update_layout(
            yaxis_range=(returns_df["creturns"].min() - 0.1, returns_df["creturns"].max() + 0.1)
        )

        return fig


def plot_position_non_zero(self):
    return (self.position != 0).sum(axis=1).plot(marker="o", figsize=(10, 5))


def extend_finlab() -> None:
    ReportPyx.plot_position_non_zero = plot_position_non_zero
