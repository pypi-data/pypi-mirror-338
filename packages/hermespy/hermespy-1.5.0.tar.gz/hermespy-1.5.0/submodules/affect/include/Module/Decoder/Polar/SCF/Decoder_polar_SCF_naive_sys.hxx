...
Decoder_polar_SCF_naive_sys<B, R, F, G, H>::Decoder_polar_SCF_naive_sys(const int& K,
                                                                        const int& N,
...
Decoder_polar_SCF_naive_sys<B, R, F, G, H>::clone() const
{
...
template<typename B, typename R, tools::proto_f<R> F, tools::proto_g<B, R> G, tools::proto_h<B, R> H>
void
Decoder_polar_SCF_naive_sys<B, R, F, G, H>::_store(B* V, bool coded) const
{
    ...
    if (!coded)
    {
        ...
        for (auto i = 0; i < this->N; i++)
            if (!this->frozen_bits[i]) V[k++] = contents_root->s[i];
    }
    else
...
template<typename B, typename R, tools::proto_f<R> F, tools::proto_g<B, R> G, tools::proto_h<B, R> H>
bool
Decoder_polar_SCF_naive_sys<B, R, F, G, H>::check_crc(const size_t frame_id)
{
    std::vector<B> U_test;
    U_test.clear();

    for (auto i = 0; i < this->N; i++)
        if (!this->frozen_bits[i]) U_test.push_back(this->polar_tree.get_root()->get_c()->s[i]);
