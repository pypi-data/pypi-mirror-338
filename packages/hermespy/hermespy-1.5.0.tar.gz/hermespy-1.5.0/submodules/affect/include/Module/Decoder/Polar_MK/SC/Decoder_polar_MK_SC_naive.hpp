...
class Contents_MK_SC
{
  public:
    ...
    explicit Contents_MK_SC(int size)
      : l(size)
      , s(size)
      , is_frozen_bit(false)
    {
...
template<typename B = int, typename R = float>
class Decoder_polar_MK_SC_naive
  ...
  , public tools::Interface_get_set_frozen_bits
{
  ...
  private:
    ...
    void recursive_initialize_frozen_bits(const tools::Generic_node<Contents_MK_SC<B, R>>* node_curr,
                                          const std::vector<bool>& frozen_bits);
