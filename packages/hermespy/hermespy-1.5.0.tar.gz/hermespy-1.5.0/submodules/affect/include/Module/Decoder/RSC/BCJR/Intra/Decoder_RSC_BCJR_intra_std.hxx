...
Decoder_RSC_BCJR_intra_std<B, R, MAX>::Decoder_RSC_BCJR_intra_std(const int& K,
...
Decoder_RSC_BCJR_intra_std<B, R, MAX>::clone() const
{
...
template<typename B, typename R, tools::proto_max_i<R> MAX>
void
Decoder_RSC_BCJR_intra_std<B, R, MAX>::compute_gamma(const R* sys, const R* par)
{
...
template<typename B, typename R, tools::proto_max_i<R> MAX>
void
Decoder_RSC_BCJR_intra_std<B, R, MAX>::compute_alpha()
{
    ...
    for (auto i = 1; i < this->K + 3; i++)
    {
        ...
        if ((i - 1) % 4 == 0) r_g4.load(&this->gamma[(i - 1) * 2]);
...
template<typename B, typename R, tools::proto_max_i<R> MAX>
void
Decoder_RSC_BCJR_intra_std<B, R, MAX>::compute_beta_ext(const R* sys, R* ext)
{
    ...
    for (auto i = this->K - 8; i >= 0; i -= 8)
    {
        ...
        for (auto j = 7; j >= 0; j--)
        {
            if ((i + j + 0) % 4 == 3) r_g4.load(&this->gamma[(i + j - 3) * 2]);
