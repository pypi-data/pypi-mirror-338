...
Encoder_turbo<E1, E2>::Encoder_turbo(const std::string& prefix)
...
Encoder_turbo<E1, E2>::get_names() const
{
...
template<class E1, class E2>
std::vector<std::string>
Encoder_turbo<E1, E2>::get_short_names() const
{
...
template<class E1, class E2>
std::vector<std::string>
Encoder_turbo<E1, E2>::get_prefixes() const
{
...
template<class E1, class E2>
void
Encoder_turbo<E1, E2>::get_description(cli::Argument_map_info& args) const
{
...
template<class E1, class E2>
void
Encoder_turbo<E1, E2>::store(const cli::Argument_map_value& vals)
{
    Encoder::store(vals);
...
template<class E1, class E2>
void
Encoder_turbo<E1, E2>::get_headers(std::map<std::string, tools::header_list>& headers, const bool full) const
{
    Encoder::get_headers(headers, full);

    if (itl != nullptr) itl->get_headers(headers);

    auto p = this->get_prefix();

    if (this->tail_length) headers[p].push_back(std::make_pair("Tail length", std::to_string(this->tail_length)));

    if (!this->json_path.empty()) headers[p].push_back(std::make_pair("Path to the JSON file", this->json_path));

    sub1->get_headers(headers, full);
    if (!std::is_same<E1, E2>()) sub2->get_headers(headers, full);
}

template<class E1, class E2>
template<typename B>
module::Encoder_turbo<B>*
Encoder_turbo<E1, E2>::build(const module::Encoder<B>& enc_n,
