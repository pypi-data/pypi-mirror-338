...
template<typename T>
Full_matrix<T>
sparse_to_full(const Sparse_matrix& sparse)
{
...
template<typename T>
Sparse_matrix
full_to_sparse(const Full_matrix<T>& full)
{
    ...
    for (unsigned i = 0; i < full.get_n_rows(); i++)
        for (unsigned j = 0; j < full.get_n_cols(); j++)
            if (full[i][j]) sparse.add_connection(i, j);
...
template<typename T>
bool
all_zeros(const Full_matrix<T>& M)
{
    bool all_z = true;
