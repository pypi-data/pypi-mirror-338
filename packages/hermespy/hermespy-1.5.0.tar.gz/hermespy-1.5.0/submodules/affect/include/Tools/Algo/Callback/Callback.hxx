...
template<class... Args>
Callback<Args...>::Callback(const Callback& c)
{
...
template<class... Args>
uint32_t
Callback<Args...>::record(std::function<void(Args...)> callback)
{
    std::lock_guard<std::mutex> lg(this->mtx);
    ;
    uint32_t id = 0;
    if (this->callbacks.size()) id = this->callbacks[this->callbacks.size() - 1].second + 1;
...
template<class... Args>
bool
Callback<Args...>::unrecord(const uint32_t id)
{
    std::lock_guard<std::mutex> lg(this->mtx);
...
template<class... Args>
void
Callback<Args...>::notify(Args... args)
{
    std::lock_guard<std::mutex> lg(this->mtx);
