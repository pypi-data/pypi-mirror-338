...
{
template<typename B>
Frame_trace<B>::Frame_trace(const int n_bits, const int prec, std::ostream& stream)
  : stream(stream)
  , n_bits(n_bits)
  , prec(prec)
{
...
void
Frame_trace<B>::display_bit_vector(std::vector<D, AD> vec, std::vector<B, AB> ref)
{
...
template<typename D, class AD, class AB>
void
Frame_trace<B>::display_real_vector(std::vector<D, AD> vec, std::vector<B, AB> ref)
{
...
void
Frame_trace<B>::display_hex_vector(std::vector<D, AD> vec)
{
...
template<typename D, class AD, class AB>
void
Frame_trace<B>::display_bit_vector(std::vector<D, AD> vec, unsigned int row_width, std::vector<B, AB> ref)
{
...
template<typename D, class AD, class AB>
void
Frame_trace<B>::display_real_vector(std::vector<D, AD> vec, unsigned int row_width, std::vector<B, AB> ref)
{
...
template<typename D, class AD>
void
Frame_trace<B>::display_hex_vector(std::vector<D, AD> vec, unsigned int row_width)
{
...
template<typename D, class AD, class AB>
void
Frame_trace<B>::display_vector(std::vector<D, AD> vec,
                               ...
                               debug_version version)
{
    ...
    const auto n_bits =
      this->n_bits ? (this->n_bits <= (int)vec.size() ? this->n_bits : (int)vec.size()) : (int)vec.size();
    ...
    if (this->display_indexes)
    {
        ...
        for (unsigned i = 0; i < row_width; i++)
            stream << rang::style::bold << std::setw(prec + 2) << i << rang::style::underline << "|"
                   << rang::style::reset;

        if (n_bits < (int)vec.size()) stream << " ...";
...
template<typename D>
void
Frame_trace<B>::display_value(D value, debug_version version, B ref)
{
    if (ref != -1) stream << rang::style::bold;

    switch (version)
    {
        case debug_version::BIT:
            if (ref != -1) stream << (((value == 0) != (ref == 0)) ? rang::fg::red : rang::fg::green);
            stream << std::setw(prec + 2) << ((value == 0) ? (int)0 : (int)1);

            break;

        case debug_version::HEX:
            if (ref != -1) stream << (value != ref ? rang::fg::red : rang::fg::green);
        ...
        case debug_version::REAL:
        {
            ...
            if (ref != -1) stream << (((value >= (D)0) != (ref == 0)) ? rang::fg::red : rang::fg::green);
            stream << value_string;
    ...
    if (ref != -1) stream << rang::style::reset;
