...
mipp::Reg<T>
Bitwise_diff<T, cus>::apply(const mipp::Reg<T>& in1, const mipp::Reg<T>& in2)
{
...
template<typename T, bool cus>
mipp::Reg<T>
Bitwise_diff<T, cus>::apply(const mipp::Reg<T>& in)
{
    const mipp::Reg<T> zeros = (T)0, ones = (T)1;
...
template<typename T, bool cus>
T
Bitwise_diff<T, cus>::apply(const T& in1, const T& in2)
{
...
template<typename T, bool cus>
T
Bitwise_diff<T, cus>::apply(const T& in)
{
...
struct Bitwise_diff<T, true>
{
    ...
    static T apply(const T& in) { return in != (T)0 || is_unknown_symbol<T>(in) ? (T)1 : (T)0; }
};
