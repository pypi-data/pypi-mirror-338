...
class Histogram : public spu::tools::Interface_reset
{
  ...
  public:
    ...
    inline void cumul_borders(
      bool val); // accumulate the values over the given range [hist_min, hist_max] on those border values
    inline void n_rung(unsigned val); // the number of values plotted on the abscissa in the asked range (if 0 then dump
                                      // directly the values)
