...
class Dumper
{
  ...
  public:
    ...
    template<typename T>
    void register_data(const T* ptr,
                       const unsigned size,
                       const unsigned add_threshold = 0,
                       const std::string& file_ext = default_ext,
                       const bool binary_mode = false,
                       const unsigned n_frames = 1,
                       std::vector<unsigned> headers = std::vector<unsigned>());
    template<typename T, class A = std::allocator<T>>
    void register_data(const std::vector<T, A>& data,
                       const unsigned add_threshold = 0,
                       const std::string& file_ext = default_ext,
                       const bool binary_mode = false,
                       const unsigned n_frames = 1,
  ...
  protected:
    void write_header_text(std::ofstream& file,
                           const unsigned n_data,
                           const unsigned data_size,
                           const std::vector<unsigned>& headers);
    void write_body_text(std::ofstream& file,
                         const std::vector<std::vector<char>>& buffer,
                         const unsigned size,
                         const std::type_index type);
    void write_header_binary(std::ofstream& file,
                             const unsigned n_data,
                             const unsigned data_size,
                             const std::vector<unsigned>& headers);
