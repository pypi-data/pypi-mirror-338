    ...
    inline static Noise<T>* cast(const Noise<R>& n)
    {
        ...
        switch (n.get_type())
        {
            case Noise_type::SIGMA:
                cast_n = new Sigma<T>(dynamic_cast<const Sigma<R>&>(n));
                break;
            case Noise_type::EP:
                cast_n = new Event_probability<T>(dynamic_cast<const Event_probability<R>&>(n));
                break;
            case Noise_type::ROP:
                cast_n = new Received_optical_power<T>(dynamic_cast<const Received_optical_power<R>&>(n));
                break;
        }
...
{
    inline static Noise<R>* cast(const Noise<R>& n) { return n.clone(); }
};

template<typename T, typename R>
Noise<T>*
cast(const Noise<R>& n)
{
...
template<typename R>
mipp::Msk<mipp::N<R>()>
is_unknown_symbol(const mipp::Reg<R>& q_in)
{
...
template<typename R>
mipp::Msk<mipp::N<R>()>
is_unknown_llr(const mipp::Reg<R>& q_in)
{
