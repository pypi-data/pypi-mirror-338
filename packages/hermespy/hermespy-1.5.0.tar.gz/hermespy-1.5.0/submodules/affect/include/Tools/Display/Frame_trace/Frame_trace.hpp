...
{
  private:
    enum class debug_version : uint8_t
    {
        BIT,
        REAL,
        HEX
  ...
  public:
     ...
     * \param ref:       a reference vector to compare with the other one (do not perform the comparison if ref is
     * unset). ref at 1 means a bit at 1 and ref at 0 means a bit at 0
     */
    template<typename D, class AD = std::allocator<D>, class AB = std::allocator<B>>
     ...
     * \param ref:       a reference vector to compare with the other one (do not perform the comparison if ref is
     * unset). ref at 1 means a negative value and ref at 0 means a positive value
     */
    template<typename D, class AD = std::allocator<D>, class AB = std::allocator<B>>
