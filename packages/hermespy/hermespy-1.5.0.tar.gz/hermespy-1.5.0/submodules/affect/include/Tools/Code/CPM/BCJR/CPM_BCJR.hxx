...
void
BCJR_normalize(Q* metrics, const int& n_states)
{
    // normalization
...
CPM_BCJR<SIN, SOUT, Q, MAX>::CPM_BCJR(const CPM_parameters<SIN, SOUT>& _cpm, const int _n_symbols)
  : cpm(_cpm)
...
template<typename SIN, typename SOUT, typename Q, proto_max<Q> MAX>
void
CPM_BCJR<SIN, SOUT, Q, MAX>::decode(const std::vector<Q>& Lch_N, std::vector<Q>& Le_N)
{
...
template<typename SIN, typename SOUT, typename Q, proto_max<Q> MAX>
void
CPM_BCJR<SIN, SOUT, Q, MAX>::decode(const Q* Lch_N, Q* Le_N)
{
...
template<typename SIN, typename SOUT, typename Q, proto_max<Q> MAX>
void
CPM_BCJR<SIN, SOUT, Q, MAX>::decode(const std::vector<Q>& Lch_N, const std::vector<Q>& Ldec_N, std::vector<Q>& Le_N)
{
    if (Lch_N.size() != chn_size)
    {
        std::stringstream message;
...
template<typename SIN, typename SOUT, typename Q, proto_max<Q> MAX>
void
CPM_BCJR<SIN, SOUT, Q, MAX>::decode(const Q* Lch_N, const Q* Ldec_N, Q* Le_N)
{
...
template<typename SIN, typename SOUT, typename Q, proto_max<Q> MAX>
void
CPM_BCJR<SIN, SOUT, Q, MAX>::LLR_to_logsymb_proba(const Q* Ldec_N)
{
    std::fill(symb_apriori_prob.begin(), symb_apriori_prob.end(), (Q)0);
    ...
    for (int i = 0; i < dec_size / cpm.n_b_per_s; i++)
        for (int tr = 0; tr < cpm.m_order; tr++)
            for (int b = 0; b < cpm.n_b_per_s; b++)
            {
                ...
                symb_apriori_prob[i * cpm.m_order + tr] +=
                  (bit_state == 0) ? Ldec_N[i * cpm.n_b_per_s + b] / 2 : -Ldec_N[i * cpm.n_b_per_s + b] / 2;
            }
}

template<typename SIN, typename SOUT, typename Q, proto_max<Q> MAX>
void
CPM_BCJR<SIN, SOUT, Q, MAX>::compute_alpha_beta_gamma(const Q* Lch_N)
{
        ...
        for (auto st = 0; st < cpm.n_st; st++)
            for (auto tr = 0; tr < cpm.m_order; tr++)
                gamma[(i * cpm.max_st_id + cpm.allowed_states[st]) * cpm.m_order + tr] =
                  Lch_N[i * cpm.max_wa_id + cpm.trellis_related_wave_form[cpm.allowed_states[st] * cpm.m_order +
                                                                          tr]] // info from the channel
            ...
            for (auto tr = 0; tr < cpm.m_order; tr++)
                alpha[(i - 0) * cpm.max_st_id + cpm.allowed_states[st]] =
                  MAX(alpha[(i - 0) * cpm.max_st_id + cpm.allowed_states[st]],
                      alpha[(i - 1) * cpm.max_st_id +
                            cpm.anti_trellis_original_state[cpm.allowed_states[st] * cpm.m_order + tr]] +
                        gamma[((i - 1) * cpm.max_st_id +
                               cpm.anti_trellis_original_state[cpm.allowed_states[st] * cpm.m_order + tr]) *
                                cpm.m_order +
            ...
            for (auto tr = 0; tr < cpm.m_order; tr++)
                beta[(n_symbols - (i + 1)) * cpm.max_st_id + cpm.allowed_states[st]] =
                  MAX(beta[(n_symbols - (i + 1)) * cpm.max_st_id + cpm.allowed_states[st]],
                      beta[(n_symbols - (i + 0)) * cpm.max_st_id +
                           cpm.trellis_next_state[cpm.allowed_states[st] * cpm.m_order + tr]] +
...
template<typename SIN, typename SOUT, typename Q, proto_max<Q> MAX>
void
CPM_BCJR<SIN, SOUT, Q, MAX>::symboles_probas()
{
...
template<typename SIN, typename SOUT, typename Q, proto_max<Q> MAX>
void
CPM_BCJR<SIN, SOUT, Q, MAX>::bits_probas()
{
    ...
    for (auto i = 0; i < n_symbols; i++)
    {
        for (auto b = 0; b < cpm.n_b_per_s; b++)
            for (auto tr = 0; tr < cpm.m_order; tr++)
            {
                const auto bit_state =
                  cpm.transition_to_binary[tr * cpm.n_b_per_s +
                                           b]; // bit_state = 0 or 1 ; bit 0 is msb, bit cpm.n_b_per_s-1 is lsb

                proba_msg_bits[(i * cpm.n_b_per_s + b) * 2 + bit_state] =
                  MAX(proba_msg_bits[(i * cpm.n_b_per_s + b) * 2 + bit_state], proba_msg_symb[i * cpm.m_order + tr]);
            }
    }
}
...
template<typename SIN, typename SOUT, typename Q, proto_max<Q> MAX>
void
CPM_BCJR<SIN, SOUT, Q, MAX>::compute_ext(Q* Le_N)
{
    // remove tail bits
...
template<typename SIN, typename SOUT, typename Q, proto_max<Q> MAX>
void
CPM_BCJR<SIN, SOUT, Q, MAX>::compute_ext(const Q* Ldec_N, Q* Le_N)
{
    // remove tail bits
    for (auto i = 0; i < ext_size; i++)
        // processing aposteriori and substracting a priori to directly obtain extrinsic
