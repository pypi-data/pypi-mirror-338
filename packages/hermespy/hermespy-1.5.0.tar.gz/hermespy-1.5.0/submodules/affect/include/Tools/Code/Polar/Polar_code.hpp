...
{
std::string
display_kernel(const std::vector<std::vector<bool>>& pattern_bits);
void
read_polar_MK_code(const std::string& code_path,
...
class Polar_code
{
  ...
  public:
    ...
    Polar_code(const std::vector<std::vector<std::vector<bool>>>& kernel_matrices, const std::vector<uint32_t>& stages);
