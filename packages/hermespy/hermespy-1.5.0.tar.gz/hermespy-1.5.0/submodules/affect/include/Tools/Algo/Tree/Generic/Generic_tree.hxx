...
{
template<typename T>
Generic_tree<T>::Generic_tree(const int depth, const int base)
  : sequence(std::vector<uint32_t>(depth, base))
  , depth(depth)
  , root(nullptr)
{
    this->init();
...
template<typename T>
Generic_tree<T>::Generic_tree(const Generic_tree<T>& bt)
...
template<typename T>
Generic_tree<T>::Generic_tree(const std::vector<uint32_t>& sequence)
  : sequence(sequence)
  , depth(sequence.size() + 1)
  , root(nullptr)
{
    this->init();
}
...
template<typename T>
void
Generic_tree<T>::recursive_copy(const Generic_node<T>* nref, Generic_node<T>* nclone)
{
    if (nref == nullptr) throw spu::tools::runtime_error(__FILE__, __LINE__, __func__, "'nref' can't be nullptr.");
    if (nclone == nullptr) throw spu::tools::runtime_error(__FILE__, __LINE__, __func__, "'nclone' can't be nullptr.");
...
template<typename T>
void
Generic_tree<T>::create_nodes(Generic_node<T>* cur_node,
                              int cur_depth,
                              ...
                              const std::vector<uint32_t>& sequence)
{
    if (cur_node->children.size())
        throw spu::tools::runtime_error(
          __FILE__, __LINE__, __func__, "'cur_node->children.size()' has to be equal to 0.");
    ...
    if (cur_depth < this->depth)
    {
        ...
        {
            auto child =
              new Generic_node<T>(cur_node, std::vector<Generic_node<T>*>(), nullptr, cur_depth, lanes[cur_depth]++, c);
...
template<typename T>
void
Generic_tree<T>::delete_nodes(Generic_node<T>* cur_node)
{
    if (cur_node != nullptr)
    {
...
template<typename T>
void
Generic_tree<T>::recursive_get_leaves(Generic_node<T>* cur_node)
{
    if (cur_node->is_leaf())
...
Generic_tree<T>::get_leaves() const
{
    return this->leaves;
}
...
template<typename T>
int
Generic_tree<T>::get_depth() const
{
    return this->depth;
}
}
