...
class Distribution
{
  ...
  public:
    Distribution(const std::vector<R>& _x_data,
                 const std::vector<R>& _y_data,
                 Distribution_mode mode = Distribution_mode::SUMMATION);
    Distribution(std::vector<R>&& _x_data,
                 std::vector<R>&& _y_data,
                 Distribution_mode mode = Distribution_mode::SUMMATION);
    Distribution(const std::vector<R>& _x_data,
                 const std::vector<std::vector<R>>& _y_data,
                 Distribution_mode mode = Distribution_mode::SUMMATION);
    Distribution(std::vector<R>&& _x_data,
                 std::vector<std::vector<R>>&& _y_data,
                 Distribution_mode mode = Distribution_mode::SUMMATION);
