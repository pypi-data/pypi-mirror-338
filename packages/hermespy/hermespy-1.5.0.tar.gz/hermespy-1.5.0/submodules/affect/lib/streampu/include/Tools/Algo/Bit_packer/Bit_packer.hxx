...
void
Bit_packer::pack(const std::vector<B, AB>& vec_in,
                 std::vector<S, AS>& vec_out,
                 const int n_frames,
                 const bool msb_to_lsb,
                 const bool pack_per_byte,
                 const int Nbps)
{
    if (n_frames <= 0)
    {
    ...
    if (pack_per_byte)
    {
        ...
        Bit_packer::pack(
          vec_in.data(), (unsigned char*)vec_out.data(), (int)(vec_in.size() / n_frames), n_frames, msb_to_lsb, Nbps);
    }
    else
    {
        ...
        Bit_packer::pack(vec_in.data(), vec_out.data(), (int)(vec_in.size() / n_frames), n_frames, msb_to_lsb, Nbps);
    }
}

template<typename B, typename S>
void
Bit_packer::pack(const B* vec_in,
                 S* vec_out,
                 const int n_bits_per_frame,
                 const int n_frames,
                 const bool msb_to_lsb,
                 const int Nbps)
{
    ...
    for (auto f = 0; f < n_frames; f++)
        Bit_packer::_pack(
          vec_in + f * n_bits_per_frame, vec_out + f * n_symbols_per_frame, n_bits_per_frame, msb_to_lsb, Nbps);
}

template<typename B, class AB>
void
Bit_packer::pack(std::vector<B, AB>& vec, const int n_frames, const bool msb_to_lsb, const int Nbps)
{
    if (n_frames <= 0)
    {
        std::stringstream message;
...
template<typename B>
void
Bit_packer::pack(B* vec, const int n_bits_per_frame, const int n_frames, const bool msb_to_lsb, const int Nbps)
{
    ...
    for (auto f = 0; f < n_frames; f++)
        Bit_packer::_pack(
          vec + f * n_bits_per_frame, symbs_out + f * n_symbs_per_frame, n_bits_per_frame, msb_to_lsb, Nbps);
}

template<typename B, class AB, typename S, class AS>
void
Bit_packer::unpack(const std::vector<S, AS>& vec_in,
                   std::vector<B, AB>& vec_out,
                   const int n_frames,
                   const bool msb_to_lsb,
                   const bool unpack_per_byte,
                   const int Nbps)
{
    if (n_frames <= 0)
    {
    ...
    if (unpack_per_byte)
    {
        ...
        Bit_packer::unpack(
          (unsigned char*)vec_in.data(), vec_out.data(), (int)(vec_out.size() / n_frames), n_frames, msb_to_lsb, Nbps);
    }
    else
    {
        ...
        Bit_packer::unpack(vec_in.data(), vec_out.data(), (int)(vec_out.size() / n_frames), n_frames, msb_to_lsb, Nbps);
    }
}

template<typename B, typename S>
void
Bit_packer::unpack(const S* vec_in,
                   B* vec_out,
                   const int n_bits_per_frame,
                   const int n_frames,
                   const bool msb_to_lsb,
                   const int Nbps)
{
    ...
    for (auto f = 0; f < n_frames; f++)
        Bit_packer::_unpack(
          vec_in + f * n_symbs_per_frame, vec_out + f * n_bits_per_frame, n_bits_per_frame, msb_to_lsb, Nbps);
}

template<typename B, class AB>
void
Bit_packer::unpack(std::vector<B, AB>& vec, const int n_frames, const bool msb_to_lsb, const int Nbps)
{
    if (n_frames <= 0)
    {
        std::stringstream message;
...
template<typename B>
void
Bit_packer::unpack(B* vec, const int n_bits_per_frame, const int n_frames, const bool msb_to_lsb, const int Nbps)
{
    if (Nbps > (int)CHAR_BIT || Nbps <= 0)
    {
        std::stringstream message;
    ...
    for (auto f = 0; f < n_frames; f++)
    {
        ...
        Bit_packer::_unpack(
          bytes_cpy.data() + f * n_symbs_per_frame, vec + f * n_bits_per_frame, n_bits_per_frame, msb_to_lsb, Nbps);
    }
}

template<typename B, typename S>
void
Bit_packer::_pack(const B* vec_in, S* symbs_out, const int n_bits, const bool msb_to_lsb, const int Nbps)
{
...
template<typename B, typename S>
void
Bit_packer::_unpack(const S* symbs_in, B* vec_out, const int n_bits, const bool msb_to_lsb, const int Nbps)
{
    const auto n_symbs = n_bits / Nbps;
    const auto rest = n_bits - n_symbs * Nbps;
