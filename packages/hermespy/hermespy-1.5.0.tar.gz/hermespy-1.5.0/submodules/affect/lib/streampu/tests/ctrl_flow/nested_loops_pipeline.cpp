...
main(int argc, char** argv)
{
    ...
    if (force_sequence)
    {
        ...
        for (auto cur_initializer :
             sequence_nested_loops->get_cloned_modules<module::Initializer<uint8_t>>(initializer))
        {
        ...
        for (auto& mod : sequence_nested_loops->get_modules<module::Module>(false))
    ...
    else
    {
        pipeline_chain.reset(
          new runtime::Pipeline(initializer("initialize"), // first task of the sequence
                                {
        ...
        auto tid = 0;
        for (auto cur_initializer :
             pipeline_chain->get_stages()[0]->get_cloned_modules<module::Initializer<uint8_t>>(initializer))
        {
            std::vector<std::vector<uint8_t>> init_data(n_inter_frames, std::vector<uint8_t>(data_length, 0));
            for (size_t f = 0; f < n_inter_frames; f++)
                std::fill(init_data[f].begin(), init_data[f].end(), tid * n_inter_frames + f);
    ...
    if (force_sequence)
    {
        for (auto cur_finalizer : sequence_nested_loops->get_cloned_modules<module::Finalizer<uint8_t>>(finalizer))
        {
            for (size_t f = 0; f < n_inter_frames; f++)
            {
                ...
                {
                    auto expected =
                      (int)(incs.size() * iterator.get_limit() * iterator2.get_limit() + (tid * n_inter_frames + f));
    ...
    else
    {
        for (auto cur_finalizer :
             pipeline_chain->get_stages()[2]->get_cloned_modules<module::Finalizer<uint8_t>>(finalizer))
        {
            for (size_t f = 0; f < n_inter_frames; f++)
            {
                const auto& final_data = cur_finalizer->get_final_data()[f];
