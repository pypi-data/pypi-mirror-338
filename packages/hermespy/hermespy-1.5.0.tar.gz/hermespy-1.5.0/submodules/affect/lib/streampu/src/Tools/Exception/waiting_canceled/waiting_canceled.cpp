...
waiting_canceled::waiting_canceled() noexcept
  : exception(DEFAULT_MESSAGE)
{
}

waiting_canceled::waiting_canceled(std::string&& message) noexcept
  : exception(message.empty() ? DEFAULT_MESSAGE : message)
{
}

waiting_canceled::waiting_canceled(std::string&& filename,
                                   int&& line_num,
                                   std::string&& funcname,
