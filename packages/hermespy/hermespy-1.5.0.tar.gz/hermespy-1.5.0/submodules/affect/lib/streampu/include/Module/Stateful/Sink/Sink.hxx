...
template<typename B>
runtime::Task&
Sink<B>::operator[](const snk::tsk t)
{
...
template<typename B>
runtime::Socket&
Sink<B>::operator[](const snk::sck::send s)
{
...
template<typename B>
runtime::Socket&
Sink<B>::operator[](const snk::sck::send_count s)
{
...
template<typename B>
runtime::Socket&
Sink<B>::operator[](const std::string& tsk_sck)
{
    return Module::operator[](tsk_sck);
}

template<typename B>
Sink<B>::Sink(const int max_data_size)
  : Stateful()
  , max_data_size(max_data_size)
{
    ...
    this->create_codelet(p1,
                         [p1s_in_data](Module& m, runtime::Task& t, const size_t frame_id) -> int
                         {
    ...
    this->create_codelet(p2,
                         [p2s_in_data, p2s_in_count](Module& m, runtime::Task& t, const size_t frame_id) -> int
                         {
                             auto& snk = static_cast<Sink<B>&>(m);
...
template<class A>
void
Sink<B>::send(const std::vector<B, A>& in_data, const int frame_id, const bool managed_memory)
{
...
template<typename B>
void
Sink<B>::send(const B* in_data, const int frame_id, const bool managed_memory)
{
    (*this)[snk::sck::send::in_data].bind(in_data);
    (*this)[snk::tsk::send].exec(frame_id, managed_memory);
}
...
template<class A>
void
Sink<B>::send_count(const std::vector<B, A>& in_data,
...
template<typename B>
void
Sink<B>::send_count(const B* in_data, uint32_t* in_count, const int frame_id, const bool managed_memory)
{
    (*this)[snk::sck::send_count::in_data].bind(in_data);
    (*this)[snk::sck::send_count::in_count].bind(in_count);
    (*this)[snk::tsk::send_count].exec(frame_id, managed_memory);
...
template<typename B>
void
Sink<B>::_send(const B* in_data, const size_t frame_id)
{
