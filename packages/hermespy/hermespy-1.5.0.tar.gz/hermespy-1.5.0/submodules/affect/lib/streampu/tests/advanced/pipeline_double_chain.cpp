...
main(int argc, char** argv)
{
    ...
    while (1)
    {
        const int opt = getopt_long(argc, argv, "t:f:s:d:e:u:o:i:j:cpbgqwh", longopts, 0);
        if (opt == -1) break;
        switch (opt)
        {
            case 't':
    ...
    std::vector<std::shared_ptr<module::Incrementer<uint8_t>>> incs_fwd(3);
    ...
    for (auto cur_finalizer : finalizer_list)
    {
        for (size_t f = 0; f < n_inter_frames; f++)
        {
            ...
            {
                auto expected = (int)(40 + n_inter_frames * tid + f + incs_fwd.size() + incs.size());
    ...
    (*incs_fwd[0])[module::inc::sck::incrementf::fwd].unbind(initializer[module::ini::sck::initialize::out]);
    for (size_t s = 0; s < incs_fwd.size() - 1; s++)
    {
        (*incs_fwd[s + 1])[module::inc::sck::incrementf::fwd].unbind((*incs_fwd[s])[module::inc::sck::incrementf::fwd]);
