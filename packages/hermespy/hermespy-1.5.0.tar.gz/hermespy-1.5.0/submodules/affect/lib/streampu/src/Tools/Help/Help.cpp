...
spu::tools::get_help(const spu::module::Module& mdl, const bool& verbose)
{
    ...
    if (!mdl.get_custom_name().empty()) message << "# |- custom name: " << mdl.get_custom_name() << std::endl;
    if (verbose) message << "# |- address: " << std::hex << static_cast<const void*>(&mdl) << std::endl;
...
spu::tools::get_help(const spu::runtime::Task& tsk, const bool& verbose)
{
    ...
    if (verbose) message << "|  |- address: " << std::hex << static_cast<const void*>(&tsk) << std::endl;
    ...
    if (tsk.sockets.size() > 0)
    {
        ...
        if (n_inputs > 0) message << message_inputs.str();
        if (n_outputs > 0) message << message_outputs.str();
    }
    return message.str();
}
