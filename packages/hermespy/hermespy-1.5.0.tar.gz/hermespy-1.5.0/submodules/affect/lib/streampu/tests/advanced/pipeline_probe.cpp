...
main(int argc, char** argv)
{
    ...
    std::cout << "#   - probes_filepath = " << (probes_filepath.empty() ? "[empty]" : probes_filepath.c_str())
              << std::endl;
    ...
    rep_timestamp_stats.register_probes(
      { &prb_ts_s1b, &prb_ts_s1e, &prb_ts_s2b, &prb_ts_s2e, &prb_ts_s3b, &prb_ts_s3e });
    ...
    {
        if (probes_file.is_open()) terminal_probes.temp_report(probes_file);
        return false;
    };
        ...
        pipeline_chain.reset(new runtime::Pipeline(
          { &prb_ts_s1b("probe") }, // first task of the sequence
          {
            ...
            // pipeline stage 1: first & last tasks
            {
              ...
              { &(*ts_s2e)("exec") },
              { &prb_ts_s3b("probe"), &sink("send_count"), &prb_ts_s2b("probe"), &prb_ts_s2e("probe") },
            },
            // pipeline stage 2: first task (no need to specify the last taks)
            {
              { &prb_ts_s3b("probe"),
                &sink("send_count"),
                &prb_bitvals_cnt("probe"),
                &prb_bitvals_data("probe"),
                &prb_ts_s2b("probe"),
                &prb_ts_s2e("probe") },
              {},
              {},
    ...
    if (probes_file.is_open()) terminal_probes.final_report(probes_file);
    ...
    auto theoretical_time =
      (n_frames * (rlys.size() * sleep_time_us * 1000) * n_inter_frames) / 1000.f / 1000.f / n_threads;
