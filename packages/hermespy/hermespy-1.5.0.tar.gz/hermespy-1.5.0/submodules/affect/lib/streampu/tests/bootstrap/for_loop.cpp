...
main(int argc, char** argv)
{
    ...
    auto tid = 0;
    for (auto cur_initializer : sequence_for_loop.get_cloned_modules<module::Initializer<uint8_t>>(initializer))
    {
    ...
    for (auto& mod : sequence_for_loop.get_modules<module::Module>(false))
    ...
    if (!dot_filepath.empty())
    {
        ...
        sequence_for_loop.export_dot(file);
    }
    ...
    if (!step_by_step)
    {
        ...
        sequence_for_loop.exec([&counter, n_exec]() { return ++counter >= n_exec; });
    }
    else
    {
        do
    ...
    auto theoretical_time =
      ((chain_sleep_time * n_exec * n_inter_frames) / 1000.f / 1000.f / n_threads) * iterator.get_limit();
    ...
    tid = 0;
    for (auto cur_finalizer : sequence_for_loop.get_cloned_modules<module::Finalizer<uint8_t>>(finalizer))
    {
        for (size_t f = 0; f < n_inter_frames; f++)
        {
