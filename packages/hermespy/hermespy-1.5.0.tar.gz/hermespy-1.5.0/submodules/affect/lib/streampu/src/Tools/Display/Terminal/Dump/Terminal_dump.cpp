...
Terminal_dump::Terminal_dump(const std::vector<std::unique_ptr<tools::Reporter>>& reporters)
  : Terminal_std(reporters)
{
}

void
Terminal_dump::report(std::ostream& stream, bool final)
{
    ...
    while (is_not_over)
    {
        ...
        for (unsigned r = 0; r < this->reporters.size(); r++)
            if (this->reporters[r] != nullptr)
            {
                ...
                {
                    if (report[g].size()) assert(report[g].size() % groups[g].second.size() == 0);
