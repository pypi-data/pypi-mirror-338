...
runtime::Task&
Unaryop<TI, TO, UOP>::operator[](const uop::tsk t)
{
...
template<typename TI, typename TO, tools::proto_uop<TI, TO> UOP>
runtime::Socket&
Unaryop<TI, TO, UOP>::operator[](const uop::sck::perform s)
{
...
template<typename TI, typename TO, tools::proto_uop<TI, TO> UOP>
runtime::Socket&
Unaryop<TI, TO, UOP>::operator[](const std::string& tsk_sck)
{
    return Module::operator[](tsk_sck);
}

template<typename TI, typename TO, tools::proto_uop<TI, TO> UOP>
Unaryop<TI, TO, UOP>::Unaryop(const size_t n_elmts)
  : Stateful()
  , n_elmts(n_elmts)
{
    ...
    this->create_codelet(
      p,
      [ps_in, ps_out](Module& m, runtime::Task& t, const size_t frame_id) -> int
      {
          auto& uop = static_cast<Unaryop&>(m);
          uop._perform(t[ps_in].template get_dataptr<const TI>(), t[ps_out].template get_dataptr<TO>(), frame_id);
...
template<typename TI, typename TO, tools::proto_uop<TI, TO> UOP>
size_t
Unaryop<TI, TO, UOP>::get_n_elmts() const
{
    return this->n_elmts;
}
...
void
Unaryop<TI, TO, UOP>::perform(const std::vector<TI, AI>& in,
...
template<typename TI, typename TO, tools::proto_uop<TI, TO> UOP>
void
Unaryop<TI, TO, UOP>::perform(const TI* in, TO* out, const int frame_id, const bool managed_memory)
{
    (*this)[uop::sck::perform::in].bind(in);
    (*this)[uop::sck::perform::out].bind(out);
    (*this)[uop::tsk::perform].exec(frame_id, managed_memory);
