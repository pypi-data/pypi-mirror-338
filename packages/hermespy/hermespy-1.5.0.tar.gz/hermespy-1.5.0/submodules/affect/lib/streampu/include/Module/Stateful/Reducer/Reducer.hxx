...
runtime::Task&
Reducer<TI, TO, BOP>::operator[](const red::tsk t)
{
...
template<typename TI, typename TO, tools::proto_bop<TI, TO> BOP>
runtime::Socket&
Reducer<TI, TO, BOP>::operator[](const red::sck::reduce s)
{
...
template<typename TI, typename TO, tools::proto_bop<TI, TO> BOP>
runtime::Socket&
Reducer<TI, TO, BOP>::operator[](const std::string& tsk_sck)
{
    return Module::operator[](tsk_sck);
}

template<typename TI, typename TO, tools::proto_bop<TI, TO> BOP>
Reducer<TI, TO, BOP>::Reducer(const size_t n_elmts)
  : Stateful()
  , n_elmts(n_elmts)
{
    ...
    this->create_codelet(
      ...
      [ps_in, ps_out](Module& m, runtime::Task& t, const size_t frame_id) -> int
      {
          ...
          red._reduce(t[ps_in].template get_dataptr<const TI>(), t[ps_out].template get_dataptr<TO>(), frame_id);
...
template<typename TI, typename TO, tools::proto_bop<TI, TO> BOP>
size_t
Reducer<TI, TO, BOP>::get_n_elmts() const
{
    return this->n_elmts;
}
...
void
Reducer<TI, TO, BOP>::reduce(const std::vector<TI, AI>& in,
...
template<typename TI, typename TO, tools::proto_bop<TI, TO> BOP>
void
Reducer<TI, TO, BOP>::reduce(const TI* in, TO* out, const int frame_id, const bool managed_memory)
{
    (*this)[red::sck::reduce::in].bind(in);
    (*this)[red::sck::reduce::out].bind(out);
    (*this)[red::tsk::reduce].exec(frame_id, managed_memory);
