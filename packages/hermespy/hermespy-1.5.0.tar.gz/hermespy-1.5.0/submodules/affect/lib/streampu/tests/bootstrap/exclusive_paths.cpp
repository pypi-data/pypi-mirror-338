...
main(int argc, char** argv)
{
    ...
    for (auto& mod : sequence_exclusive_paths.get_modules<module::Module>(false))
    ...
    auto theoretical_time =
      (((chain_sleep_time / multiplier[real_path]) * n_exec * n_inter_frames) / 1000.f / 1000.f / n_threads);
    ...
    for (auto cur_finalizer : sequence_exclusive_paths.get_cloned_modules<module::Finalizer<uint8_t>>(finalizer))
    {
        ...
        if (cyclic_path) real_path = ((*cur_finalizer)[module::fin::tsk::finalize].get_n_calls() - 1) % 3;
        for (size_t f = 0; f < n_inter_frames; f++)
        {
