...
struct Bit_packer
{
     ...
     * \brief Packs bits.
     *
     * \param vec_in:        an input vector of unpacked bits (only 1 bit per element is used to transport data) (used
     * as size reference). \param vec_out:       an output vector of packed bits in symbols \param msb_to_lsb:    pack
     * from MSB to LSB else from LSB to MSB \param pack_per_byte: pack bits in each byte (type char) of vec_out, else
     * pack per each symbol (of type S) of vec_out \param Nbps:          the number of bits packed in one symbol
     * (between 1 and (sizeof(S) * CHAR_BIT)).
     */
    template<typename B, class AB = std::allocator<B>, typename S, class AS = std::allocator<S>>
    static inline void pack(const std::vector<B, AB>& vec_in,
                            std::vector<S, AS>& vec_out,
                            const int n_frames = 1,
                            const bool msb_to_lsb = false,
                            const bool pack_per_byte = true,
    ...
    static inline void pack(const B* vec_in,
                            S* vec_out,
    ...
    static inline void pack(std::vector<B, AB>& vec,
                            const int n_frames = 1,
                            const bool msb_to_lsb = false,
                            const int Nbps = CHAR_BIT);

    template<typename B>
    static inline void pack(B* vec,
                            const int n_bits_per_frame,
                            const int n_frames = 1,
                            const bool msb_to_lsb = false,
                            const int Nbps = CHAR_BIT);
     ...
     * \param unpack_per_byte: unpack symbol in each byte (type char) of vec_out, else unpack per each symbol (of type
     * S) of vec_in \param Nbps:            the number of bits packed in one symbol (between 1 and (sizeof(S) *
     * CHAR_BIT)).
     */
    template<typename B, class AB = std::allocator<B>, typename S, class AS = std::allocator<S>>
    static inline void unpack(const std::vector<S, AS>& vec_in,
                              std::vector<B, AB>& vec_out,
                              const int n_frames = 1,
                              const bool msb_to_lsb = false,
                              const bool unpack_per_byte = true,
                              const int Nbps = CHAR_BIT);
     ...
     */
    template<typename B, typename S>
    static inline void unpack(const S* vec_in,
                              B* vec_out,
                              const int n_bits_per_frame,
                              const int n_frames = 1,
                              const bool msb_to_lsb = false,
     ...
     */
    template<typename B, class AB = std::allocator<B>>
    static inline void unpack(std::vector<B, AB>& vec,
                              const int n_frames = 1,
                              const bool msb_to_lsb = false,
                              const int Nbps = CHAR_BIT);

    template<typename B>
    static inline void unpack(B* vec,
                              const int n_bits_per_frame,
                              const int n_frames = 1,
                              const bool msb_to_lsb = false,
                              const int Nbps = CHAR_BIT);
...
#include "Tools/Algo/Bit_packer/Bit_packer.hxx"
#endif
