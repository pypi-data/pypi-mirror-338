...
template<typename B>
Encoder_turbo_legacy<B>::Encoder_turbo_legacy(const int& K,
                                              const int& N,
                                              const Encoder<B>& sub_enc,
...
Encoder_turbo_legacy<B>::clone() const
{
...
template<typename B>
void
Encoder_turbo_legacy<B>::deep_copy(const Encoder_turbo_legacy<B>& m)
{
...
template<typename B>
void
Encoder_turbo_legacy<B>::_encode(const B* U_K, B* X_N, const size_t frame_id)
{
...
template<typename B>
bool
Encoder_turbo_legacy<B>::is_codeword(const B* X_N)
{
    ...
    if (!sub_enc->is_codeword(X_N_n.data())) return false;
    ...
    if (!sub_enc->is_codeword(X_N_i.data())) return false;
...
template<typename B>
void
Encoder_turbo_legacy<B>::set_n_frames(const size_t n_frames)
{
