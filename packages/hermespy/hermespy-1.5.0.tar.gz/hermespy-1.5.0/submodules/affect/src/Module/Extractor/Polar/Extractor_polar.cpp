...
Extractor_polar<B, Q>::Extractor_polar(const int K, const int N, const std::vector<bool>& frozen_bits)
  : Extractor<B, Q>(K, N, 0)
...
template<typename B, typename Q>
Extractor_polar<B, Q>*
Extractor_polar<B, Q>::clone() const
{
...
template<typename B, typename Q>
void
Extractor_polar<B, Q>::set_frozen_bits(const std::vector<bool>& fb)
{
    std::copy(fb.begin(), fb.end(), this->frozen_bits.begin());
    int j = 0;
    for (auto i = 0; i < this->N; i++)
        if (!this->frozen_bits[i]) this->info_bits_pos[j++] = i;
}

template<typename B, typename Q>
const std::vector<bool>&
Extractor_polar<B, Q>::get_frozen_bits() const
{
...
Extractor_polar<B, Q>::get_info_bits_pos()
{
