...
template<typename B>
Encoder_turbo_DB<B>::Encoder_turbo_DB(const int& K,
                                      const int& N,
                                      const Encoder_RSC_DB<B>& enco_n,
                                      const Encoder_RSC_DB<B>& enco_i,
...
template<typename B>
Encoder_turbo_DB<B>*
Encoder_turbo_DB<B>::clone() const
{
...
template<typename B>
void
Encoder_turbo_DB<B>::deep_copy(const Encoder_turbo_DB<B>& m)
{
...
template<typename B>
void
Encoder_turbo_DB<B>::_encode(const B* U_K, B* X_N, const size_t frame_id)
{
...
template<typename B>
bool
Encoder_turbo_DB<B>::is_codeword(const B* X_N)
{
    auto& U_K_n = X_N_tmp;
    ...
    if (!enco_n->is_codeword(X_N_tmp.data())) return false;
...
template<typename B>
void
Encoder_turbo_DB<B>::set_n_frames(const size_t n_frames)
{
