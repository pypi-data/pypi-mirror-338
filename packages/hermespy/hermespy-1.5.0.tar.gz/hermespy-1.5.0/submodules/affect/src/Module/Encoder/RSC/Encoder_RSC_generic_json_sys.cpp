...
template<typename B>
Encoder_RSC_generic_json_sys<B>::Encoder_RSC_generic_json_sys(const int& K,
                                                              const int& N,
...
Encoder_RSC_generic_json_sys<B>::get_trellis()
{
...
template<typename B>
int
Encoder_RSC_generic_json_sys<B>::inner_encode(const int bit_sys, int& state)
{
    ...
    if (bit_counter == 0)
    {
        if (natural_domain)
        {
            ...
            for (auto i = 0; i < (int)trellis.size(); i++)
            {
                ...
                if (i != (int)trellis.size() - 1) stream << "," << std::endl;
            }
            stream << "]," << std::endl;
...
template<typename B>
void
Encoder_RSC_generic_json_sys<B>::set_n_frames(const size_t n_frames)
{
    if (n_frames != 1)
    {
    ...
    if (old_n_frames != n_frames) Encoder_RSC_generic_sys<B>::set_n_frames(n_frames);
}
