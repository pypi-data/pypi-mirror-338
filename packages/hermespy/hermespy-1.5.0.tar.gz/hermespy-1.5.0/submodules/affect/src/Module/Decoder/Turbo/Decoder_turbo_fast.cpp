...
Decoder_turbo_fast<B, R>::Decoder_turbo_fast(const int& K,
                                             const int& N,
                                             const int& n_ite,
...
template<typename B, typename R>
void
Decoder_turbo_fast<B, R>::_load(const R* Y_N, const size_t frame_id)
{
...
Decoder_turbo_fast<B, R>::clone() const
{
...
template<typename B, typename R>
int
Decoder_turbo_fast<B, R>::_decode_siho(const R* Y_N, B* V_K, const size_t frame_id)
{
    ...
    do
    {
        ...
        if (!stop)
        {
            ...
            if (ite == this->n_ite || stop) tools::hard_decide(this->l_e1n.data(), this->s.data(), this->K * n_frames);
        }
...
template<typename B, typename R>
void
Decoder_turbo_fast<B, R>::_store(B* V_K) const
{
