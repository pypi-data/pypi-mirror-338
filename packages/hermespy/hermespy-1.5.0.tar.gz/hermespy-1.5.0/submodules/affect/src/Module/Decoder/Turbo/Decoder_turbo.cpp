...
Decoder_turbo<B, R>::Decoder_turbo(const int& K,
                                   const int& N,
                                   const int& n_ite,
  ...
  , l_pn(((N - (siso_n.tail_length() + siso_i.tail_length()) - K) / 2 + (siso_n.tail_length() / 2)) *
           siso_n.get_n_frames_per_wave() +
         mipp::nElReg<R>())
  , l_pi(((N - (siso_n.tail_length() + siso_i.tail_length()) - K) / 2 + (siso_i.tail_length() / 2)) *
           siso_i.get_n_frames_per_wave() +
         mipp::nElReg<R>())
  ...
  , s((K)*siso_n.get_n_frames_per_wave())
{
    ...
    if (siso_n.get_K() != K)
    {
        ...
        message << "'siso_n.get_K()' has to be equal to 'K' ('siso_n.get_K()' = " << siso_n.get_K() << ", 'K' = " << K
                << ").";
    ...
    {
        std::stringstream message;
        message << "'siso_i.get_K()' has to be equal to 'K' ('siso_i.get_K()' = " << siso_i.get_K() << ", 'K' = " << K
                << ").";
        throw spu::tools::invalid_argument(__FILE__, __LINE__, __func__, message.str());
    }
    ...
    {
        std::stringstream message;
        message << "'N' - ('siso_n.tail_length()' + 'siso_i.tail_length()') has to be equal to 'K' * 3 ('N' = " << N
                << ", 'siso_n.tail_length()' = " << siso_n.tail_length()
...
template<typename B, typename R>
void
Decoder_turbo<B, R>::deep_copy(const Decoder_turbo<B, R>& m)
{
...
template<typename B, typename R>
void
Decoder_turbo<B, R>::add_post_processing(const tools::Post_processing_SISO<B, R>& post_processing)
{
...
template<typename B, typename R>
void
Decoder_turbo<B, R>::_load(const R* Y_N, const size_t frame_id)
{
...
template<typename B, typename R>
void
Decoder_turbo<B, R>::buffered_load(const R* Y_N, const size_t frame_id)
{
...
template<typename B, typename R>
void
Decoder_turbo<B, R>::standard_load(const R* Y_N, const size_t frame_id)
{
    ...
    else // inter frame => input reordering
    {
        ...
        for (auto i = 0; i < tail_n / 2; i++)
        {
            for (size_t j = 0; j < n_frames; j++)
            {
                l_sn[(this->K + i) * n_frames + j] =
                  Y_N[j * ((this->K * 3) + tail_n + tail_i) + this->K * 3 + 2 * i + 0];
                l_pn[(this->K + i) * n_frames + j] =
                  Y_N[j * ((this->K * 3) + tail_n + tail_i) + this->K * 3 + 2 * i + 1];
            }
        }
        ...
        for (auto i = 0; i < tail_i / 2; i++)
        {
            for (size_t j = 0; j < n_frames; j++)
            {
                l_si[(this->K + i) * n_frames + j] =
                  Y_N[j * ((this->K * 3) + tail_n + tail_i) + this->K * 3 + tail_n + 2 * i + 0];
                l_pi[(this->K + i) * n_frames + j] =
                  Y_N[j * ((this->K * 3) + tail_n + tail_i) + this->K * 3 + tail_n + 2 * i + 1];
            }
        }
    }
...
template<typename B, typename R>
void
Decoder_turbo<B, R>::_store(B* V_K) const
{
    if (this->get_n_frames_per_wave() == 1)
    {
...
template<typename B, typename R>
void
Decoder_turbo<B, R>::set_n_frames(const size_t n_frames)
{
