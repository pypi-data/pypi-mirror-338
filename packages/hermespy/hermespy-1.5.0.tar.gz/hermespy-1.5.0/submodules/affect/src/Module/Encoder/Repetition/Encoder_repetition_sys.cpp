...
template<typename B>
Encoder_repetition_sys<B>::Encoder_repetition_sys(const int& K, const int& N, const bool buffered_encoding)
  : Encoder<B>(K, N)
  , rep_count((N / K) - 1)
...
template<typename B>
bool
Encoder_repetition_sys<B>::is_codeword(const B* X_N)
{
    bool valid = true;
    ...
    if (buffered_encoding)
    {
        ...
        {
            auto k = 0;
            while (k < this->K && (X_N[k] == X_N[(r + 1) * this->K + k]))
                k++;
            valid = k == this->K;
            r++;
        }
    ...
    else
    {
        ...
        {
            auto r = 0;
            while (r < rep_count && (X_N[k * (rep_count + 1)] == X_N[k * (rep_count + 1) + r + 1]))
                r++;
            valid = r == rep_count;
            k++;
        }
