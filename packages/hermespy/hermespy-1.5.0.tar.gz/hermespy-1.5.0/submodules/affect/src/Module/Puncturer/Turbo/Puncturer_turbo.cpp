...
Puncturer_turbo<B, Q>::Puncturer_turbo(const int& K,
                                       const int& N,
                                       const int& tail_bits,
...
template<typename B, typename Q>
Puncturer_turbo<B, Q>*
Puncturer_turbo<B, Q>::clone() const
{
...
Puncturer_turbo<B, Q>::_puncture(const B* X_N1, B* X_N2, const size_t /*frame_id*/) const
{
    ...
    if (this->buff_enc)
    {
        ...
        for (auto j = 0; j < 3; j++)
        {
            ...
            {
                if (pattern_bits[j][p]) X_N2[k++] = X_N1[o + i];
                p = (p + 1) % period;
            }
...
template<typename B, typename Q>
std::string
Puncturer_turbo<B, Q>::display_pattern(const std::vector<std::vector<bool>>& pattern_bits)
{
    std::string m;
...
template<typename B, typename Q>
unsigned
Puncturer_turbo<B, Q>::get_period(const std::vector<std::vector<bool>>& pattern_bits)
{
    if (pattern_bits.size() == 0) return 0;
...
template<typename B, typename Q>
unsigned
Puncturer_turbo<B, Q>::get_bit_count(const std::vector<std::vector<bool>>& pattern_bits)
{
...
template<typename B, typename Q>
void
Puncturer_turbo<B, Q>::check_pattern(const int K, const std::vector<std::vector<bool>>& pattern_bits)
{
    if (pattern_bits.size() !=
        3) // pattern_bits[0] == bit systematic, pattern_bits[1] == parity 1, pattern_bits[2] == bit parity 2
    {
        std::stringstream message;
    ...
    {
        std::stringstream message;
        message << "'pattern' sets have to contains an equal number of bits ('pattern' = "
                << display_pattern(pattern_bits) << ", 'pattern_bits[0].size()' = " << pattern_bits[0].size()
...
template<typename B, typename Q>
int
Puncturer_turbo<B, Q>::compute_N(const int K, const int tail_bits, const std::vector<std::vector<bool>>& pattern_bits)
{
