...
Decoder_polar_MK_SCL_naive_CA_sys<B, R>::Decoder_polar_MK_SCL_naive_CA_sys(
  const int& K,
  const int& N,
  const int& L,
...
template<typename B, typename R>
Decoder_polar_MK_SCL_naive_CA_sys<B, R>::Decoder_polar_MK_SCL_naive_CA_sys(const int& K,
                                                                           const int& N,
                                                                           const int& L,
                                                                           const tools::Polar_code& code,
                                                                           const std::vector<bool>& frozen_bits,
...
Decoder_polar_MK_SCL_naive_CA_sys<B, R>::clone() const
{
...
template<typename B, typename R>
void
Decoder_polar_MK_SCL_naive_CA_sys<B, R>::select_best_path(const size_t frame_id)
{
    std::vector<B> U_test;
...
template<typename B, typename R>
void
Decoder_polar_MK_SCL_naive_CA_sys<B, R>::_store(B* V, bool coded) const
{
    if (!coded)
    {
        auto k = 0;
