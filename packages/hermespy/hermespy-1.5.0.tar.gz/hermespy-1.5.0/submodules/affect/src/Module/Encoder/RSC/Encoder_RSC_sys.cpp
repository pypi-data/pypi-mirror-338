...
template<typename B>
Encoder_RSC_sys<B>::Encoder_RSC_sys(const int& K, const int& N, const int n_ff, const bool buffered_encoding)
  : Encoder<B>(K, N)
  , n_ff(n_ff)
  , n_states(1 << n_ff)
...
template<typename B>
int
Encoder_RSC_sys<B>::get_n_ff()
{
    return n_ff;
}
template<typename B>
int
Encoder_RSC_sys<B>::tail_length() const
{
    return 2 * n_ff;
}
...
template<typename B>
void
Encoder_RSC_sys<B>::__encode(const B* U_K,
                             B* sys,
                             B* tail_sys,
                             B* par,
                             B* tail_par,
                             const int stride,
                             const int stride_tail)
{
    ...
    for (auto i = 0; i < this->n_ff; i++)
    {
        ...
        if (tail_sys != nullptr) tail_sys[i * stride_tail] = bit_sys; // systematic transmission of the bit

        auto p = inner_encode((int)bit_sys, state); // encoding block

        if (tail_par != nullptr) tail_par[i * stride_tail] = p;
    }
...
template<typename B>
bool
Encoder_RSC_sys<B>::_is_codeword(const B* sys,
                                 const B* tail_sys,
                                 const B* par,
                                 const B* tail_par,
                                 const int stride,
                                 const int stride_tail)
{
    ...
    for (auto i = 0; i < this->n_ff; i++)
    {
        ...
        if (tail_par != nullptr)
            if (tail_par[i * stride_tail] != p) return false;
    }
...
template<typename B>
bool
Encoder_RSC_sys<B>::is_codeword(const B* X_N)
{
    if (buffered_encoding)
