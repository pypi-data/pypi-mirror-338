...
Decoder_turbo_product<B, R>::Decoder_turbo_product(const int& n_ite,
  ...
  , Y_N_cha_i(pi.get_core().get_size())
{
    ...
    {
        std::stringstream message;
        message << "'alpha.size()' has to be twice 'n_ite' ('n_ite' = " << n_ite
                << " and 'alpha.size()' = " << alpha.size() << ").";
        throw spu::tools::invalid_argument(__FILE__, __LINE__, __func__, message.str());
    }
    ...
    {
        std::stringstream message;
        message << "'beta.size()' has to be twice 'n_ite' or null ('n_ite' = " << n_ite
                << " and 'beta.size()' = " << alpha.size() << ").";
        throw spu::tools::invalid_argument(__FILE__, __LINE__, __func__, message.str());
    }
    ...
    {
        std::stringstream message;
        message << "'N' has to be equal to 'cp_r.get_N()' * 'cp_c.get_N()' ('N' = " << this->N
                << ", 'cp_r.get_N()' = " << cp_r.get_N() << ", 'cp_c.get_N()' = " << cp_c.get_N() << ").";
        throw spu::tools::invalid_argument(__FILE__, __LINE__, __func__, message.str());
    }
    ...
    {
        std::stringstream message;
        message << "'K' has to be equal to 'cp_r.get_K()' * 'cp_c.get_K()' ('K' = " << this->K
                << ", 'cp_r.get_K()' = " << cp_r.get_K() << ", 'cp_c.get_K()' = " << cp_c.get_K() << ").";
...
Decoder_turbo_product<B, R>::clone() const
{
...
template<typename B, typename R>
int
Decoder_turbo_product<B, R>::_decode_siso(const R* Y_N1, R* Y_N2, const size_t frame_id)
{
...
template<typename B, typename R>
int
Decoder_turbo_product<B, R>::_decode_siho(const R* Y_N, B* V_K, const size_t frame_id)
{
    //	auto t_load = std::chrono::steady_clock::now(); // -----------------------------------------------------------
    // LOAD
...
template<typename B, typename R>
int
Decoder_turbo_product<B, R>::_decode_siho_cw(const R* Y_N, B* V_N, const size_t frame_id)
{
    //	auto t_load = std::chrono::steady_clock::now(); // -----------------------------------------------------------
    // LOAD
    std::copy(Y_N, Y_N + this->N, Y_N_i.data());
...
template<typename B, typename R>
int
Decoder_turbo_product<B, R>::_decode(const R* Y_N_cha, const size_t frame_id, int return_K_siso)
{
    ...
    for (int i = 0; i < n_ite; i++)
    {
        ...
        else if (return_K_siso == 0)
        {
            for (int j = 0; j < cp_c->get_K(); j++)
            {
                ...
                cp_r->decode_siho(Y_N_i.data(),
                                  V_K_i.data() + (j - pos) * cp_r->get_K(),
                                  pos); // decode pos-th row,
...
template<typename B, typename R>
void
Decoder_turbo_product<B, R>::set_n_frames(const size_t n_frames)
{
