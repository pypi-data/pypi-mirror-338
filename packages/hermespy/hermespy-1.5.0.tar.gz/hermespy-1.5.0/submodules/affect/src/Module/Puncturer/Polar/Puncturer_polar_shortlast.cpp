...
Puncturer_polar_shortlast<B, Q>::Puncturer_polar_shortlast(const int& K,
                                                           const int& N,
  ...
  , fb_generator(fb_generator)
{
    ...
    {
        std::stringstream message;
        message << "'fb_generator.get_N()' has to be equal to 'N_cw' ('fb_generator.get_N()' = " << fb_generator.get_N()
                << ", 'N_cw' = " << this->N_cw << ").";
        throw spu::tools::invalid_argument(__FILE__, __LINE__, __func__, message.str());
    }
}
...
Puncturer_polar_shortlast<B, Q>::clone() const
{
...
template<typename B, typename Q>
void
Puncturer_polar_shortlast<B, Q>::gen_frozen_bits(std::vector<bool>& frozen_bits)
{
...
template<typename B, typename Q>
void
Puncturer_polar_shortlast<B, Q>::_puncture(const B* X_N1, B* X_N2, const size_t /*frame_id*/) const
{
...
template<typename B, typename Q>
void
Puncturer_polar_shortlast<B, Q>::_depuncture(const Q* Y_N1, Q* Y_N2, const size_t /*frame_id*/) const
{
