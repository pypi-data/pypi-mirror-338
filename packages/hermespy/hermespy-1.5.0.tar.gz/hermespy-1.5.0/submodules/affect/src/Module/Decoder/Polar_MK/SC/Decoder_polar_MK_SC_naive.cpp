...
std::vector<uint32_t>
gen_sequence(const tools::Polar_code& code)
{
...
Decoder_polar_MK_SC_naive<B, R>::Decoder_polar_MK_SC_naive(
  const int& K,
  const int& N,
...
template<typename B, typename R>
Decoder_polar_MK_SC_naive<B, R>::Decoder_polar_MK_SC_naive(const int& K,
                                                           const int& N,
                                                           const tools::Polar_code& code,
                                                           const std::vector<bool>& frozen_bits)
...
Decoder_polar_MK_SC_naive<B, R>::clone() const
{
...
template<typename B, typename R>
void
Decoder_polar_MK_SC_naive<B, R>::recursive_deep_copy(const tools::Generic_node<Contents_MK_SC<B, R>>* nref,
...
template<typename B, typename R>
void
Decoder_polar_MK_SC_naive<B, R>::set_frozen_bits(const std::vector<bool>& fb)
{
...
Decoder_polar_MK_SC_naive<B, R>::get_frozen_bits() const
{
...
template<typename B, typename R>
void
Decoder_polar_MK_SC_naive<B, R>::_load(const R* Y_N)
{
...
template<typename B, typename R>
int
Decoder_polar_MK_SC_naive<B, R>::_decode_siho(const R* Y_N, B* V_K, const size_t /*frame_id*/)
{
...
template<typename B, typename R>
int
Decoder_polar_MK_SC_naive<B, R>::_decode_siho_cw(const R* Y_N, B* V_N, const size_t /*frame_id*/)
{
    //	auto t_load = std::chrono::steady_clock::now(); // -----------------------------------------------------------
    // LOAD
    this->_load(Y_N);
...
template<typename B, typename R>
void
Decoder_polar_MK_SC_naive<B, R>::_store(B* V, bool coded) const
{
    if (!coded)
    {
        auto k = 0;
...
template<typename B, typename R>
void
Decoder_polar_MK_SC_naive<B, R>::recursive_allocate_nodes_contents(tools::Generic_node<Contents_MK_SC<B, R>>* node_curr,
...
template<typename B, typename R>
void
Decoder_polar_MK_SC_naive<B, R>::recursive_initialize_frozen_bits(
  const tools::Generic_node<Contents_MK_SC<B, R>>* node_curr,
  const std::vector<bool>& frozen_bits)
{
...
template<typename B, typename R>
void
Decoder_polar_MK_SC_naive<B, R>::recursive_decode(const tools::Generic_node<Contents_MK_SC<B, R>>* node_curr)
{
    if (!node_curr->is_leaf()) // stop condition
    {
        const auto stage = polar_tree.get_depth() - node_curr->get_depth() - 2;
        ...
        for (auto child = 0; child < kern_size; child++)
        {
            ...
            for (auto k = 0; k < n_kernels; k++)
            {
                for (auto l = 0; l < kern_size; l++)
                    LLRs[l] = node_curr->get_c()->l[l * n_kernels + k];
                for (auto c = 0; c < child; c++)
                    bits[c] = node_curr->get_children()[c]->get_c()->s[k];
        ...
        for (auto k = 0; k < n_kernels; k++)
        {
            for (auto i = 0; i < kern_size; i++)
            {
                ...
                this->u[i] =
                  node_curr->get_children()[(this->idx[i] / n_kernels)]->get_c()->s[this->idx[i] % n_kernels];
            }
...
template<typename B, typename R>
void
Decoder_polar_MK_SC_naive<B, R>::recursive_store(const tools::Generic_node<Contents_MK_SC<B, R>>* node_curr,
                                                 B* V_K,
                                                 int& k) const
{
    if (!node_curr->is_leaf()) // stop condition
...
template<typename B, typename R>
void
Decoder_polar_MK_SC_naive<B, R>::recursive_deallocate_nodes_contents(
  tools::Generic_node<Contents_MK_SC<B, R>>* node_curr)
{
    for (auto c : node_curr->get_children())
