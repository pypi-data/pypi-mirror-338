...
template<typename B>
Encoder_polar<B>::Encoder_polar(const int& K, const int& N, const std::vector<bool>& frozen_bits)
  : Encoder<B>(K, N)
  , m((int)std::log2(N))
...
template<typename B>
void
Encoder_polar<B>::_encode(const B* U_K, B* X_N, const size_t /*frame_id*/)
{
...
template<typename B>
void
Encoder_polar<B>::light_encode(B* bits)
{
...
template<typename B>
void
Encoder_polar<B>::convert(const B* U_K, B* U_N)
{
    if (U_K == U_N)
    {
...
template<typename B>
bool
Encoder_polar<B>::is_codeword(const B* X_N)
{
...
template<typename B>
void
Encoder_polar<B>::set_frozen_bits(const std::vector<bool>& frozen_bits)
{
...
Encoder_polar<B>::get_frozen_bits() const
{
