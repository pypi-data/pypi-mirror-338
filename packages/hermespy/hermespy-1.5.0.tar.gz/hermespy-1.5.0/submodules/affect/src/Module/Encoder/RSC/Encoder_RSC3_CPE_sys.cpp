...
template<typename B>
Encoder_RSC3_CPE_sys<B>::Encoder_RSC3_CPE_sys(const int& K, const int& N, const bool buffered_encoding)
...
template<typename B>
int
Encoder_RSC3_CPE_sys<B>::inner_encode(const int bit_sys, int& state)
{
    auto symbol = 0;
...
template<typename B>
int
Encoder_RSC3_CPE_sys<B>::tail_bit_sys(const int& state)
{
