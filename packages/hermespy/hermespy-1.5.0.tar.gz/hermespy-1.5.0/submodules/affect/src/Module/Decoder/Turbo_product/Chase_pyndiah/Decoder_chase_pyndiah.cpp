...
Decoder_chase_pyndiah<B, R>::Decoder_chase_pyndiah(const int K,
                                                   const int N, // N includes the parity bit if any
  ...
  , beta_is_set(false)
{
    ...
    if (n_least_reliable_positions <= 0 || n_least_reliable_positions >= N_np)
    {
        ...
        message
          << "'n_least_reliable_positions' has to be positive and lower than 'N_np' ('n_least_reliable_positions' = "
    ...
    {
        std::stringstream message;
        message << "'n_test_vectors' has to be positive and lower than 2^'n_least_reliable_positions' "
    ...
    if (enc->get_K() != dec->get_K())
    {
        ...
        message << "'enc->get_K()' has to be equal to 'dec->get_K()' "
                << "('enc->get_K()' = " << enc->get_K() << ", 'dec->get_K()' = " << dec->get_K() << ").";
        throw spu::tools::invalid_argument(__FILE__, __LINE__, __func__, message.str());
    }
    ...
    if (enc->get_N() != dec->get_N())
    {
        ...
        message << "'enc->get_N()' has to be equal to 'dec->get_K()' "
                << "('enc->get_N()' = " << enc->get_N() << ", 'dec->get_N()' = " << dec->get_N() << ").";
        throw spu::tools::invalid_argument(__FILE__, __LINE__, __func__, message.str());
    }
...
Decoder_chase_pyndiah<B, R>::clone() const
{
...
Decoder_chase_pyndiah<B, R>::get_info_bits_pos()
{
...
template<typename B, typename R>
void
Decoder_chase_pyndiah<B, R>::set_beta(R b)
{
    if (b < (R)0.)
    {
        std::stringstream message;
...
template<typename B, typename R>
void
Decoder_chase_pyndiah<B, R>::clear_beta()
{
    beta_is_set = false;
}
...
template<typename B, typename R>
int
Decoder_chase_pyndiah<B, R>::_decode_siho(const R* Y_N, B* V_K, const size_t frame_id)
{
...
template<typename B, typename R>
int
Decoder_chase_pyndiah<B, R>::_decode_siho_cw(const R* Y_N, B* V_N, const size_t frame_id)
{
    auto status = decode_chase(Y_N, frame_id);
...
template<typename B, typename R>
int
Decoder_chase_pyndiah<B, R>::_decode_siso(const R* Y_N1, R* Y_N2, const size_t frame_id)
{
...
template<typename B, typename R>
int
Decoder_chase_pyndiah<B, R>::decode_chase(const R* Y_N, const size_t frame_id)
{
    ...
    std::cerr << std::endl;
    {
        std::vector<R> v(Y_N, Y_N + this->N);
        ft.display_real_vector(v);
    }
    ...
    {
        std::cerr << i << ". (" << least_reliable_pos[i].pos << ", " << least_reliable_pos[i].metric << ") -> "
                  << hard_Y_N[least_reliable_pos[i].pos] << std::endl;
    }
    ...
    std::cerr << std::endl;
    {
        std::vector<R> v(DW, DW + this->N);
        ft.display_bit_vector(v);
    }
#endif
...
template<typename B, typename R>
void
Decoder_chase_pyndiah<B, R>::find_least_reliable_pos(const R* Y_N)
{
...
template<typename B, typename R>
void
Decoder_chase_pyndiah<B, R>::compute_test_vectors(const size_t frame_id)
{
    ...
    for (int c = 0; c < n_test_vectors; c++)
    {
        ...
        dec->decode_hiho_cw(hard_Y_N.data() - dec_offset,
                            test_vect.data() + c * this->N - dec_offset,
                            frame_id); // parity bit is ignored by the decoder
        ...
        {
            std::vector<B> v(test_vect.data() + c * this->N, test_vect.data() + (c + 1) * this->N);
            ft.display_bit_vector(v);
        }
#endif
...
template<typename B, typename R>
void
Decoder_chase_pyndiah<B, R>::compute_metrics(const R* Y_N)
{
    ...
    for (int c = 0; c < n_test_vectors; c++)
    {
        ...
        for (int i = 0; i < this->N; i++)
            if (hard_Y_N[i] ^ test_vect[tv_off + i]) metrics[c] += std::abs(Y_N[i]);
    }
...
template<typename B, typename R>
void
Decoder_chase_pyndiah<B, R>::compute_reliability(const R* Y_N1, R* Y_N2)
{
    ...
    n_good_competitors =
      std::min(n_good_competitors, n_competitors); // if there is less than 'n_competitors' good competitors
    ...
    for (int i = 0; i < this->N; i++)
    {
        ...
        for (; j < n_good_competitors; j++)
            if (test_vect[competitors[j].pos + i] != DB) break;
...
#ifndef NDEBUG_TPC
        std::cerr << "Rel:  i = " << i << ", Y_N1 = " << Y_N1[i] << ", competitors[" << j
                  << "].metric = " << competitors[j].metric;
#endif
...
template<typename B, typename R>
void
Decoder_chase_pyndiah<B, R>::bit_flipping(B* hard_vect, const int c)
{
    for (int i = 0; i < n_least_reliable_positions; i++)
        hard_vect[least_reliable_pos[i].pos] =
          test_patterns[c][i] ? !hard_vect[least_reliable_pos[i].pos] : hard_vect[least_reliable_pos[i].pos];
}

template<typename B, typename R>
void
Decoder_chase_pyndiah<B, R>::generate_bit_flipping_candidates()
{
    ...
    else if (n_test_vectors <= (n_least_reliable_positions * (n_least_reliable_positions + 1) / 2 + 1))
    {
        ...
        // (2) always flip the least realiable position and do (1) on the 'n_least_reliable_positions' -1 left bits ->
        // 'n_least_reliable_positions' -1 test vectors (3) always flip the two least reliable position and do (1) on
        // the 'n_least_reliable_positions' -2 left bits -> 'n_least_reliable_positions' -2 test vectors (4) and so on
        // until reaching the wish number of test vectors with a maximum of the sum from 1 to
        // 'n_least_reliable_positions' + 1
