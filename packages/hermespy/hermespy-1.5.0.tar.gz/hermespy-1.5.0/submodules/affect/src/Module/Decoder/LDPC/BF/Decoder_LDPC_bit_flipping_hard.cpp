...
Decoder_LDPC_bit_flipping_hard<B, R>::Decoder_LDPC_bit_flipping_hard(const int& K,
                                                                     const int& N,
                                                                     const int& n_ite,
  ...
  , info_bits_pos(info_bits_pos)
{
    ...
    {
        std::stringstream message;
        message << "'N' is not compatible with the H matrix ('N' = " << N << ", 'H.get_n_rows()' = " << H.get_n_rows()
                << ").";
        throw spu::tools::invalid_argument(__FILE__, __LINE__, __func__, message.str());
    }
}
...
Decoder_LDPC_bit_flipping_hard<B, R>::clone() const
{
...
template<typename B, typename R>
int
Decoder_LDPC_bit_flipping_hard<B, R>::_decode_hiho(const B* Y_N, int8_t* CWD, B* V_K, const size_t frame_id)
{
...
template<typename B, typename R>
int
Decoder_LDPC_bit_flipping_hard<B, R>::_decode_hiho_cw(const B* Y_N, int8_t* CWD, B* V_N, const size_t frame_id)
{
    //	auto t_load = std::chrono::steady_clock::now();  // ----------------------------------------------------------
    // LOAD
    std::copy(Y_N, Y_N + this->N, var_nodes.data());
...
template<typename B, typename R>
int
Decoder_LDPC_bit_flipping_hard<B, R>::_decode_siho(const R* Y_N, int8_t* CWD, B* V_K, const size_t frame_id)
{
    //	auto t_load = std::chrono::steady_clock::now();  // ----------------------------------------------------------
    // LOAD
...
template<typename B, typename R>
int
Decoder_LDPC_bit_flipping_hard<B, R>::_decode_siho_cw(const R* Y_N, int8_t* CWD, B* V_N, const size_t frame_id)
{
    //	auto t_load = std::chrono::steady_clock::now();  // ----------------------------------------------------------
    // LOAD
    tools::hard_decide(Y_N, YH_N.data(), this->N);
...
template<typename B, typename R>
void
Decoder_LDPC_bit_flipping_hard<B, R>::_store(B* V_K, const size_t /*frame_id*/)
{
...
template<typename B, typename R>
void
Decoder_LDPC_bit_flipping_hard<B, R>::_store_cw(B* V_N, const size_t /*frame_id*/)
{
...
template<typename B, typename R>
bool
Decoder_LDPC_bit_flipping_hard<B, R>::decode(const B* Y_N, const size_t frame_id)
{
    ...
    for (auto ite = 0; ite < this->n_ite; ite++)
    {
        ...
        if (this->enable_syndrome && (synd = check_syndrome()))
        {
            ...
            if (this->cur_syndrome_depth >= this->syndrome_depth) break;
        }
        else
            this->cur_syndrome_depth = 0;
    ...
    if (!this->enable_syndrome || !synd) synd = check_syndrome();
...
template<typename B, typename R>
bool
Decoder_LDPC_bit_flipping_hard<B, R>::check_syndrome() const
{
    auto res = std::accumulate(check_nodes.begin(), check_nodes.end(), (B)0, [](const B a, B b) { return a || b; });
