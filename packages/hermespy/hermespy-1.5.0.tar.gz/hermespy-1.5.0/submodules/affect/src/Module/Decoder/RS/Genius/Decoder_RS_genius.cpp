...
Decoder_RS_genius<B, R>::Decoder_RS_genius(const int K,
                                           const int N,
...
template<typename B, typename R>
int
Decoder_RS_genius<B, R>::hamming_distance()
{
    int n_error = 0;
...
template<typename B, typename R>
int
Decoder_RS_genius<B, R>::_decode_hiho(const B* Y_N, B* V_K, const size_t frame_id)
{
    ...
    auto n_error =
      (int)tools::distance<B, tools::Bitwise_diff<B, true>>(this->X_Ns.data(), this->YH_N.data(), this->N_rs);
...
template<typename B, typename R>
int
Decoder_RS_genius<B, R>::_decode_hiho_cw(const B* Y_N, B* V_N, const size_t frame_id)
{
    auto& X_N = encoder.get_X_N(frame_id);
...
template<typename B, typename R>
int
Decoder_RS_genius<B, R>::_decode_siho(const R* Y_N, B* V_K, const size_t frame_id)
{
...
template<typename B, typename R>
int
Decoder_RS_genius<B, R>::_decode_siho_cw(const R* Y_N, B* V_N, const size_t frame_id)
{
    tools::hard_decide(Y_N, this->YH_Nb.data(), this->N);
...
template<typename B, typename R>
void
Decoder_RS_genius<B, R>::set_n_frames(const size_t n_frames)
{
