...
Decoder_BCH_std<B, R>::Decoder_BCH_std(const int& K, const int& N, const tools::BCH_polynomial_generator<B>& GF_poly)
...
template<typename B, typename R>
int
Decoder_BCH_std<B, R>::_decode(B* Y_N, const size_t frame_id)
{
    int i, j, syn_error = 0;
    ...
    for (i = 1; i <= t2; i++)
    {
        s[i] = 0;
        for (j = 0; j < this->N; j++)
            if (Y_N[j] != 0) s[i] ^= alpha_to[(i * j) % this->N_p2_1];
        ...
        do
        {
            ...
            { // search for words with greatest u_lu[q] for which d[q]!=0
                ...
                if (q > 0)
                {
                    ...
                    {
                        j--;
                        if ((discrepancy[j] != -1) && (u_lu[q] < u_lu[j])) q = j;
                    } while (j > 0);
                }
...
template<typename B, typename R>
int
Decoder_BCH_std<B, R>::_decode_hiho(const B* Y_N, int8_t* CWD, B* V_K, const size_t frame_id)
{
...
template<typename B, typename R>
int
Decoder_BCH_std<B, R>::_decode_hiho_cw(const B* Y_N, int8_t* CWD, B* V_N, const size_t frame_id)
{
    std::copy(Y_N, Y_N + this->N, this->YH_N.begin());
...
template<typename B, typename R>
int
Decoder_BCH_std<B, R>::_decode_siho(const R* Y_N, int8_t* CWD, B* V_K, const size_t frame_id)
{
...
template<typename B, typename R>
int
Decoder_BCH_std<B, R>::_decode_siho_cw(const R* Y_N, int8_t* CWD, B* V_N, const size_t frame_id)
{
    tools::hard_decide(Y_N, this->YH_N.data(), this->N);
