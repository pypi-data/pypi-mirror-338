...
Decoder_LDPC_BP_flooding_Gallager_E<B, R>::Decoder_LDPC_BP_flooding_Gallager_E(
  const int K,
  const int N,
  const int n_ite,
...
Decoder_LDPC_BP_flooding_Gallager_E<B, R>::clone() const
{
...
template<typename B, typename R>
void
Decoder_LDPC_BP_flooding_Gallager_E<B, R>::_initialize_var_to_chk(const B* Y_N,
...
template<typename B, typename R>
void
Decoder_LDPC_BP_flooding_Gallager_E<B, R>::_decode_single_ite(const std::vector<int8_t>& var_to_chk,
...
template<typename B, typename R>
void
Decoder_LDPC_BP_flooding_Gallager_E<B, R>::_make_majority_vote(const B* Y_N, std::vector<int8_t>& V_N)
{
