...
Decoder_LDPC_BP_flooding_Gallager_A<B, R>::Decoder_LDPC_BP_flooding_Gallager_A(
  const int K,
  const int N,
  const int n_ite,
...
Decoder_LDPC_BP_flooding_Gallager_A<B, R>::clone() const
{
...
template<typename B, typename R>
int
Decoder_LDPC_BP_flooding_Gallager_A<B, R>::_decode_hiho(const B* Y_N, int8_t* CWD, B* V_K, const size_t /*frame_id*/)
{
...
template<typename B, typename R>
int
Decoder_LDPC_BP_flooding_Gallager_A<B, R>::_decode_hiho_cw(const B* Y_N, int8_t* CWD, B* V_N, const size_t /*frame_id*/)
{
    //	auto t_load = std::chrono::steady_clock::now();  // ----------------------------------------------------------
    // LOAD
    if (this->transform_HY_N)
...
template<typename B, typename R>
int
Decoder_LDPC_BP_flooding_Gallager_A<B, R>::_decode_siho(const R* Y_N, int8_t* CWD, B* V_K, const size_t /*frame_id*/)
{
    //	auto t_load = std::chrono::steady_clock::now();  // ----------------------------------------------------------
    // LOAD
...
template<typename B, typename R>
int
Decoder_LDPC_BP_flooding_Gallager_A<B, R>::_decode_siho_cw(const R* Y_N, int8_t* CWD, B* V_N, const size_t /*frame_id*/)
{
    //	auto t_load = std::chrono::steady_clock::now();  // ----------------------------------------------------------
    // LOAD
    tools::hard_decide(Y_N, HY_N.data(), this->N);
...
template<typename B, typename R>
int
Decoder_LDPC_BP_flooding_Gallager_A<B, R>::_decode(const B* Y_N)
{
    auto ite = 0;
    auto synd = true;
    ...
    if (ite == this->n_ite) this->_make_majority_vote(Y_N, this->V_N);
...
template<typename B, typename R>
void
Decoder_LDPC_BP_flooding_Gallager_A<B, R>::_initialize_var_to_chk(const B* Y_N,
...
template<typename B, typename R>
void
Decoder_LDPC_BP_flooding_Gallager_A<B, R>::_decode_single_ite(const std::vector<int8_t>& var_to_chk,
                                                              std::vector<int8_t>& chk_to_var)
{
...
template<typename B, typename R>
void
Decoder_LDPC_BP_flooding_Gallager_A<B, R>::_make_majority_vote(const B* Y_N, std::vector<int8_t>& V_N)
{
