...
template<typename B>
Encoder_LDPC_from_IRA<B>::Encoder_LDPC_from_IRA(const int K, const int N, const tools::Sparse_matrix& _H)
  : Encoder_LDPC<B>(K, N)
{
...
Encoder_LDPC_from_IRA<B>::clone() const
{
...
template<typename B>
void
Encoder_LDPC_from_IRA<B>::_encode(const B* U_K, B* X_N, const size_t /*frame_id*/)
{
    int M = this->N - this->K;
    ...
    for (auto& l : this->H.get_rows_from_col(0))
        if (l < (unsigned)this->K) parity[0] ^= U_K[l];
...
template<typename B>
void
Encoder_LDPC_from_IRA<B>::_check_H_dimensions()
{
    ...
    if ((this->N - this->K) != (int)this->H.get_n_cols())
    {
        ...
        message << "The built H matrix has a dimension '(N-K)' different than the given one ('(N-K)' = "
                << (this->N - this->K) << ", 'H.get_n_cols()' = " << this->H.get_n_cols() << ").";
