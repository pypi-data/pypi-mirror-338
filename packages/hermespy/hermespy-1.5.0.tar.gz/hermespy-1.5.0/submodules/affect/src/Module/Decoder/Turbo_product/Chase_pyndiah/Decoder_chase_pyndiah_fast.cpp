...
Decoder_chase_pyndiah_fast<B, R>::Decoder_chase_pyndiah_fast(const int K,
...
Decoder_chase_pyndiah_fast<B, R>::clone() const
{
...
template<typename B, typename R>
void
Decoder_chase_pyndiah_fast<B, R>::compute_metrics(const R* Y_N)
{
    ...
    for (int c = 0; c < this->n_test_vectors; c++)
    {
        ...
        for (int i = N_loop_size; i < this->N; i++)
            if (this->hard_Y_N[i] ^ this->test_vect[tv_off + i]) this->metrics[c] += std::abs(Y_N[i]);
    }
    ...
    std::sort(
      this->competitors.begin(), this->competitors.end(), [](const I& a, const I& b) { return a.metric < b.metric; });
}

template<typename B, typename R>
void
Decoder_chase_pyndiah_fast<B, R>::compute_reliability(const R* Y_N1, R* Y_N2)
{
    ...
    if (!this->beta_is_set)
    {
        ...
        beta_applied = 0;
        int max_sum = this->cp_coef[4] ? std::min((int)this->cp_coef[4], this->n_least_reliable_positions)
                                       : this->n_least_reliable_positions;
    ...
    this->n_good_competitors =
      std::min(this->n_good_competitors,
               this->n_competitors); // if there is less than 'this->n_competitors' good this->competitors
    ...
    else
    {
        for (auto i = 0; i < N_loop_size; i += mipp::nElReg<R>())
        {
            ...
            mipp::Reg<R> r_reliability =
              mipp::abs(r_Y1) * (R)this->cp_coef[3] + r_beta; // reliability when no competitor with different bit
    ...
    for (int i = N_loop_size; i < this->N; i++)
    {
        ...
        for (; j < this->n_good_competitors; j++)
            if (this->test_vect[this->competitors[j].pos + i] != DB) break;
