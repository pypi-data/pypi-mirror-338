...
Decoder_LDPC_BP_flooding_Gallager_B<B, R>::Decoder_LDPC_BP_flooding_Gallager_B(
  const int K,
  const int N,
  const int n_ite,
...
Decoder_LDPC_BP_flooding_Gallager_B<B, R>::clone() const
{
...
template<typename B, typename R>
void
Decoder_LDPC_BP_flooding_Gallager_B<B, R>::_initialize_var_to_chk(const B* Y_N,
