...
Decoder_RS_std<B, R>::Decoder_RS_std(const int& K, const int& N, const tools::RS_polynomial_generator& GF)
...
template<typename B, typename R>
int
Decoder_RS_std<B, R>::_decode(S* Y_N, const size_t /*frame_id*/)
{
    ...
    for (auto i = 1; i <= t2; i++)
    {
        ...
        for (auto j = 0; j < this->N_rs; j++)
        {
            ...
            if (y_idx != -1) s[i] ^= this->alpha_to[(y_idx + i * j) % this->N_p2_1];
        }
        ...
        do
        {
            ...
            { // search for words with greatest u_lu[q] for which discrepancy[q]!=0
                ...
                for (auto i = 0; i <= l[q]; i++)
                    if (elp[q][i] != -1)
                        elp[u + 1][i + u - q] =
                          this->alpha_to[(discrepancy[u] + this->N_p2_1 - discrepancy[q] + elp[q][i]) % this->N_p2_1];
            ...
            if (u < t2)
            {
                ...
                for (auto i = 1; i <= l[u + 1]; i++)
                    if (s[u + 1 - i] != -1 && elp[u + 1][i] != 0)
                        discrepancy[u + 1] ^=
                          this->alpha_to[(s[u + 1 - i] + this->index_of[elp[u + 1][i]]) % this->N_p2_1];
        ...
        if (l[u] <= this->t) // Can correct errors
        {
            ...
            if (count == l[u]) // no. roots = degree of elp hence <= t errors
            {
                ...
                for (auto i = 0; i < l[u]; i++) // compute numerator of error term first
                {
                    ...
                    for (auto j = 1; j <= l[u]; j++)
                        if (z[j] != -1) err[loc[i]] ^= this->alpha_to[(z[j] + j * root[i]) % this->N_p2_1];
                    ...
                    if (err[loc[i]] != 0)
                    {
                        ...
                        for (auto j = 0; j < l[u]; j++)
                            if (j != i) q += this->index_of[1 ^ this->alpha_to[(loc[j] + root[i]) % this->N_p2_1]];
