...
Decoder_LDPC_bit_flipping<B, R>::Decoder_LDPC_bit_flipping(const int& K,
                                                           const int& N,
                                                           const int& n_ite,
        ...
        for (auto j = 0; j < (int)CN_to_VN[i].size(); j++)
        {
            ...
            if (connections[id_V] > (int)VN_to_CN[id_V].size())
            {
                ...
                message << "'connections[id_V]' has to be equal or smaller than 'VN_to_CN[id_V].size()' ('id_V' = "
                        << id_V << ", 'connections[id_V]' = " << connections[id_V]
...
Decoder_LDPC_bit_flipping<B, R>::clone() const
{
...
template<typename B, typename R>
void
Decoder_LDPC_bit_flipping<B, R>::_reset(const size_t frame_id)
{
...
template<typename B, typename R>
int
Decoder_LDPC_bit_flipping<B, R>::_decode_siso(const R* Y_N1, int8_t* CWD, R* Y_N2, const size_t frame_id)
{
    // actual decoding
...
template<typename B, typename R>
int
Decoder_LDPC_bit_flipping<B, R>::_decode_siho(const R* Y_N, int8_t* CWD, B* V_K, const size_t frame_id)
{
...
template<typename B, typename R>
int
Decoder_LDPC_bit_flipping<B, R>::_decode_siho_cw(const R* Y_N, int8_t* CWD, B* V_N, const size_t frame_id)
{
    //	auto t_load = std::chrono::steady_clock::now(); // -----------------------------------------------------------
    // LOAD
...
template<typename B, typename R>
bool
Decoder_LDPC_bit_flipping<B, R>::BF_decode(const R* Y_N, const size_t frame_id)
{
    ...
    for (auto ite = 0; ite < this->n_ite; ite++)
    {
        ...
        // stop criterion
        if (this->enable_syndrome && syndrome) break;
    }
...
template<typename B, typename R>
void
Decoder_LDPC_bit_flipping<B, R>::set_n_frames(const size_t n_frames)
{
