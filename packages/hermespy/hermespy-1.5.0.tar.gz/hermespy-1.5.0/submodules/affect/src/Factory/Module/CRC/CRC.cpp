...
CRC ::CRC(const std::string& prefix)
...
void
CRC ::get_description(cli::Argument_map_info& args) const
{
    ...
    tools::add_arg(args, p, class_name + "p+fra,F", cli::Integer(cli::Positive(), cli::Non_zero()));

    tools::add_arg(args, p, class_name + "p+type,p+poly", cli::Text());

    tools::add_arg(args, p, class_name + "p+implem", cli::Text(cli::Including_set("STD", "FAST", "INTER")));

    tools::add_arg(args, p, class_name + "p+size", cli::Integer(cli::Positive()));
}

void
CRC ::store(const cli::Argument_map_value& vals)
{
    auto p = this->get_prefix();
...
void
CRC ::get_headers(std::map<std::string, tools::header_list>& headers, const bool full) const
{
    auto p = this->get_prefix();
...
template aff3ct::module::CRC<B_8>*
aff3ct::factory::CRC::build<B_8>() const;
template aff3ct::module::CRC<B_16>*
aff3ct::factory::CRC::build<B_16>() const;
template aff3ct::module::CRC<B_32>*
aff3ct::factory::CRC::build<B_32>() const;
template aff3ct::module::CRC<B_64>*
aff3ct::factory::CRC::build<B_64>() const;
#else
template aff3ct::module::CRC<B>*
aff3ct::factory::CRC::build<B>() const;
#endif
// ==================================================================================== explicit template instantiation
