...
Polar_code ::Polar_code(const std::string& prefix)
...
void
Polar_code ::get_description(cli::Argument_map_info& args) const
{
...
void
Polar_code ::store(const cli::Argument_map_value& vals)
{
    auto p = this->get_prefix();
...
void
Polar_code ::get_headers(std::map<std::string, tools::header_list>& headers, const bool full) const
{
    auto p = this->get_prefix();
...
tools::Polar_code*
Polar_code ::build() const
{
    if (this->code_path.empty())
        return new tools::Polar_code(this->N_cw, this->kernel_matrix);
    else
        return new tools::Polar_code(this->code_path);
