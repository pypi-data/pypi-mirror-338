...
Channel ::get_description(cli::Argument_map_info& args) const
{
    ...
    tools::add_arg(args, p, class_name + "p+blk-fad", cli::Text(cli::Including_set("NO", "FRAME", "ONETAP")));

    tools::add_arg(args, p, class_name + "p+seed,S", cli::Integer(cli::Positive()));

    tools::add_arg(args, p, class_name + "p+add-users", cli::None());

    tools::add_arg(args, p, class_name + "p+complex", cli::None());

    tools::add_arg(args, p, class_name + "p+gain-occur", cli::Integer(cli::Positive(), cli::Non_zero()));
}

void
Channel ::store(const cli::Argument_map_value& vals)
{
    auto p = this->get_prefix();
...
void
Channel ::get_headers(std::map<std::string, tools::header_list>& headers, const bool full) const
{
    auto p = this->get_prefix();
...
template<typename R>
module::Channel<R>*
Channel ::build_event() const
{
    tools::Event_generator_implem impl;
    if (this->implem == "STD")
        impl = tools::Event_generator_implem::STD;
    else if (this->implem == "FAST")
        impl = tools::Event_generator_implem::FAST;
#ifdef AFF3CT_CHANNEL_MKL
    else if (this->implem == "MKL")
        impl = tools::Event_generator_implem::MKL;
#endif
#ifdef AFF3CT_CHANNEL_GSL
    else if (this->implem == "GSL")
        impl = tools::Event_generator_implem::GSL;
#endif
    else
...
template<typename R>
module::Channel<R>*
Channel ::build_gaussian() const
{
    tools::Gaussian_noise_generator_implem impl;
    if (this->implem == "STD")
        impl = tools::Gaussian_noise_generator_implem::STD;
    else if (this->implem == "FAST")
        impl = tools::Gaussian_noise_generator_implem::FAST;
#ifdef AFF3CT_CHANNEL_MKL
    else if (this->implem == "MKL")
        impl = tools::Gaussian_noise_generator_implem::MKL;
#endif
#ifdef AFF3CT_CHANNEL_GSL
    else if (this->implem == "GSL")
        impl = tools::Gaussian_noise_generator_implem::GSL;
#endif
    else
        throw spu::tools::cannot_allocate(__FILE__, __LINE__, __func__);
    ...
    if (type == "RAYLEIGH")
        return new module::Channel_Rayleigh_LLR<R>(this->N, this->complex, impl, this->seed, this->add_users);
    if (type == "RAYLEIGH_USER")
        return new module::Channel_Rayleigh_LLR_user<R>(
          this->N, this->complex, this->path, impl, this->seed, this->gain_occur, this->add_users);
...
template<typename R>
module::Channel<R>*
Channel ::build_userpdf(const tools::Distributions<R>& dist) const
{
    tools::User_pdf_noise_generator_implem impl;
    if (implem == "STD")
        impl = tools::User_pdf_noise_generator_implem::STD;
    else if (implem == "FAST")
        impl = tools::User_pdf_noise_generator_implem::FAST;
#ifdef AFF3CT_CHANNEL_MKL
    else if (implem == "MKL")
        impl = tools::User_pdf_noise_generator_implem::MKL;
#endif
#ifdef AFF3CT_CHANNEL_GSL
    else if (implem == "GSL")
        impl = tools::User_pdf_noise_generator_implem::GSL;
#endif
    else
        throw spu::tools::cannot_allocate(__FILE__, __LINE__, __func__);
...
template<typename R>
module::Channel<R>*
Channel ::build(const tools::Distributions<R>& dist) const
{
    try
    {
...
template aff3ct::module::Channel<R_32>*
aff3ct::factory::Channel::build<R_32>() const;
template aff3ct::module::Channel<R_64>*
aff3ct::factory::Channel::build<R_64>() const;
template aff3ct::module::Channel<R_32>*
aff3ct::factory::Channel::build<R_32>(const tools::Distributions<R_32>&) const;
template aff3ct::module::Channel<R_64>*
aff3ct::factory::Channel::build<R_64>(const tools::Distributions<R_64>&) const;
#else
template aff3ct::module::Channel<R>*
aff3ct::factory::Channel::build<R>() const;
template aff3ct::module::Channel<R>*
aff3ct::factory::Channel::build<R>(const tools::Distributions<R>&) const;
#endif
// ==================================================================================== explicit template instantiation
