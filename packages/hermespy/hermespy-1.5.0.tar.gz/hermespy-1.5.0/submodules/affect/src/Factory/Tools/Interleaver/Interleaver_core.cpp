...
Interleaver_core ::Interleaver_core(const std::string& prefix)
...
void
Interleaver_core ::get_description(cli::Argument_map_info& args) const
{
    ...
    tools::add_arg(args, p, class_name + "p+cols", cli::Integer(cli::Positive(), cli::Non_zero()));

    tools::add_arg(args, p, class_name + "p+uni", cli::None());

    tools::add_arg(args, p, class_name + "p+seed", cli::Integer(cli::Positive()));

    tools::add_arg(args,
                   p,
                   class_name + "p+read-order",
                   cli::Text(cli::Including_set("TOP_LEFT", "TOP_RIGHT", "BOTTOM_LEFT", "BOTTOM_RIGHT")));
}

void
Interleaver_core ::store(const cli::Argument_map_value& vals)
{
    auto p = this->get_prefix();
...
void
Interleaver_core ::get_headers(std::map<std::string, tools::header_list>& headers, const bool full) const
{
    auto p = this->get_prefix();
...
template<typename T>
tools::Interleaver_core<T>*
Interleaver_core ::build() const
{
    ...
    if (this->type == "RAND_COL")
        return new tools::Interleaver_core_random_column<T>(this->size, this->n_cols, this->seed, this->uniform);
    if (this->type == "ROW_COL")
        return new tools::Interleaver_core_row_column<T>(this->size, this->n_cols, this->read_order);
    if (this->type == "COL_ROW")
        return new tools::Interleaver_core_column_row<T>(this->size, this->n_cols, this->read_order);
...
template aff3ct::tools::Interleaver_core<uint32_t>*
aff3ct::factory::Interleaver_core::build<uint32_t>() const;
// ==================================================================================== explicit template instantiation
