...
Simulation ::Simulation(const std::string& name, const std::string& prefix)
...
void
Simulation ::get_description(cli::Argument_map_info& args) const
{
    ...
    tools::add_arg(args, p, class_name + "p+meta", cli::Text());

    tools::add_arg(args, p, class_name + "p+stop-time", cli::Integer(cli::Positive()), cli::arg_rank::ADV);

    tools::add_arg(args, p, class_name + "p+max-fra,n", cli::Integer(cli::Positive()), cli::arg_rank::ADV);

    tools::add_arg(args, p, class_name + "p+crit-nostop", cli::None(), cli::arg_rank::ADV);

    tools::add_arg(args, p, class_name + "p+dbg", cli::None());

    tools::add_arg(args, p, class_name + "p+dbg-hex", cli::None());

    tools::add_arg(args, p, class_name + "p+dbg-prec", cli::Integer(cli::Positive()));

    tools::add_arg(args, p, class_name + "p+dbg-limit,d", cli::Integer(cli::Positive(), cli::Non_zero()));

    tools::add_arg(args, p, class_name + "p+dbg-fra", cli::Integer(cli::Positive(), cli::Non_zero()));

    tools::add_arg(args, p, class_name + "p+stats", cli::None());

    tools::add_arg(args, p, class_name + "p+threads,t", cli::Integer(cli::Positive()));

    tools::add_arg(args, p, class_name + "p+seed,S", cli::Integer(cli::Positive()));

    tools::add_arg(args, p, class_name + "p+inter-fra,F", cli::Integer(cli::Positive(), cli::Non_zero()));
}

void
Simulation ::store(const cli::Argument_map_value& vals)
{
    ...
    {
        std::stringstream message;
        message << "'max_n_threads_global' has to be greater than 0 ('max_n_threads_global' = " << max_n_threads_global
                << ").";
...
void
Simulation ::get_headers(std::map<std::string, tools::header_list>& headers, const bool full) const
{
    ...
    if (this->debug)
    {
        ...
        if (this->debug_limit) headers[p].push_back(std::make_pair("Debug limit", std::to_string(this->debug_limit)));
    }

    headers[p].push_back(std::make_pair("Inter frame level", std::to_string(this->n_frames)));

    std::string threads = "unused";
    if (this->n_threads) threads = std::to_string(this->n_threads) + " thread(s)";
