...
bool
Sparse_matrix ::at(const size_t row_index, const size_t col_index) const
{
...
void
Sparse_matrix ::add_connection(const size_t row_index, const size_t col_index)
{
...
void
Sparse_matrix ::rm_connection(const size_t row_index, const size_t col_index)
{
    check_indexes(row_index, col_index);
    ...
    if (row_found != col_found)
    {
        ...
        message << "The connection has been found only in one of the two vectors 'row_to_cols' and 'col_to_rows' "
                ...
                << ", found in row = " << std::boolalpha << row_found << ", found in col = " << col_found
                << std::noboolalpha << ").";
        throw spu::tools::runtime_error(__FILE__, __LINE__, __func__, message.str());
    }
...
void
Sparse_matrix ::self_resize(const size_t n_rows, const size_t n_cols, Origin o)
{
...
void
Sparse_matrix ::sort_cols_per_density(Sort order)
{
    switch (order)
    {
        case Sort::ASCENDING:
            std::sort(this->col_to_rows.begin(),
                      this->col_to_rows.end(),
...
void
Sparse_matrix ::print(bool transpose, std::ostream& os) const
{
    if (transpose)
    {
...
Sparse_matrix
Sparse_matrix ::identity(const size_t n_rows, const size_t n_cols)
{
...
Sparse_matrix
Sparse_matrix ::zero(const size_t n_rows, const size_t n_cols)
{
