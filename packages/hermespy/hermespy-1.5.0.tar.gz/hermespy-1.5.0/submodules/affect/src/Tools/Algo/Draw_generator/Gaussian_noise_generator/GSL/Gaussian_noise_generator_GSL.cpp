...
template<typename R>
Gaussian_noise_generator_GSL<R>::Gaussian_noise_generator_GSL(const int seed)
  : Gaussian_noise_generator<R>()
  , rng((void*)gsl_rng_alloc(gsl_rng_mt19937))
{
    if (rng == nullptr) throw spu::tools::runtime_error(__FILE__, __LINE__, __func__, "'rng' can't be null.");

    this->set_seed(seed);
}

template<typename R>
Gaussian_noise_generator_GSL<R>::~Gaussian_noise_generator_GSL()
{
...
Gaussian_noise_generator_GSL<R>::clone() const
{
...
template<typename R>
void
Gaussian_noise_generator_GSL<R>::set_seed(const int seed)
{
...
template<typename R>
void
Gaussian_noise_generator_GSL<R>::generate(R* noise, const unsigned length, const R sigma, const R mu)
{
