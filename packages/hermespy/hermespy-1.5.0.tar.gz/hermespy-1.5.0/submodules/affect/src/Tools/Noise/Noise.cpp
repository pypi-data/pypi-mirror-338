...
template<typename R>
void
Noise<R>::copy(const Noise<R>& other)
{
...
template<typename R>
void
Noise<R>::set_value(const R value)
{
    this->value = value;
    this->check();
    this->callback_update.notify();
...
template<typename R>
bool
Noise<R>::is_of_type(Noise_type t) const noexcept
{
...
template<typename R>
void
Noise<R>::is_of_type_throw(Noise_type t) const
{
    if (!is_of_type(t))
    {
        std::stringstream message;
...
template<typename R>
uint32_t
Noise<R>::record_callback_update(std::function<void()> callback)
{
...
template<typename R>
bool
Noise<R>::unrecord_callback_update(const uint32_t id)
{
...
template<typename R>
void
Noise<R>::clear_callbacks_update()
{
