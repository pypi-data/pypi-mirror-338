  ...
  , trellis(new std::vector<std::vector<int>>())
{
    ...
    *trellis = encoder_RSC->get_trellis();
    ...
    this->set_extractor(
      new module::Extractor_RSC<B, Q>(enc_params.K, enc_params.N_cw, enc_params.tail_length, enc_params.buffered));
}

template<typename B, typename Q>
Codec_RSC<B, Q>*
Codec_RSC<B, Q>::clone() const
{
...
template<typename B, typename Q>
bool
Codec_RSC<B, Q>::is_buffered_encoding() const
{
...
Codec_RSC<B, Q>::get_trellis() const
{
    return *this->trellis;
}
