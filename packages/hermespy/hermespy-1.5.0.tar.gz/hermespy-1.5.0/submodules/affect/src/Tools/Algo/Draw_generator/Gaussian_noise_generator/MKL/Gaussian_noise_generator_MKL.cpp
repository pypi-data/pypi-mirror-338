...
template<typename R>
Gaussian_noise_generator_MKL<R>::Gaussian_noise_generator_MKL(const int seed)
...
template<typename R>
Gaussian_noise_generator_MKL<R>::~Gaussian_noise_generator_MKL()
{
...
Gaussian_noise_generator_MKL<R>::clone() const
{
...
template<typename R>
void
Gaussian_noise_generator_MKL<R>::set_seed(const int seed)
{
    if (is_stream_alloc) vslDeleteStream((VSLStreamStatePtr*)stream_state);
...
template<typename R>
void
Gaussian_noise_generator_MKL<R>::generate(R* noise, const unsigned length, const R sigma, const R mu)
{
    throw spu::tools::runtime_error(
      __FILE__, __LINE__, __func__, "Adding white Gaussian noise is impossible on this data type.");
}
...
template<>
void
Gaussian_noise_generator_MKL<float>::generate(float* noise, const unsigned length, const float sigma, const float mu)
{
    vsRngGaussian(VSL_RNG_METHOD_GAUSSIAN_BOXMULLER2, *(VSLStreamStatePtr*)stream_state, length, noise, mu, sigma);
    /*
...
template<>
void
Gaussian_noise_generator_MKL<double>::generate(double* noise,
                                               const unsigned length,
                                               const double sigma,
                                               const double mu)
{
    vdRngGaussian(VSL_RNG_METHOD_GAUSSIAN_BOXMULLER2, *(VSLStreamStatePtr*)stream_state, length, noise, mu, sigma);
    /*
