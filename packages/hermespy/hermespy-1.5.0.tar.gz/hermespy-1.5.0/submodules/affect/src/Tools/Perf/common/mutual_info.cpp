...
R
aff3ct::tools::mutual_info_histo(const B* ref, const R* llr, const unsigned size)
{
...
aff3ct::tools::mutual_info_histo(const B* ref, const R* llr, const unsigned size)
{
    ...
    if (std::is_same<R, double>::value) return mutual_info_histo_seq(ref, llr, size);
     ...
     *                              = sum( llr_sum2 - 2 * mean * llr_sum + llr_noninf_count * mean^2 ) /
     * llr_noninf_count = sum( llr_sum2 - 2 * mean * llr_sum + llr_sum * mean ) / llr_noninf_count = sum( llr_sum2 - 1 *
     * mean * llr_sum) / llr_noninf_count
     */
    ...
    for (unsigned i = 0; i < vec_loop_size; i += mipp::N<B>())
    {
        ...
        const auto m_ref0_infp =
          mipp::andnb(m_ref, m_llr_infp); // mask is true when ref is 0 and llr is infinite positive
        const auto m_ref1_infp =
          mipp::andb(m_ref, m_llr_infp); // mask is true when ref is 1 and llr is infinite positive

        const auto m_ref0_infn =
          mipp::andnb(m_ref, m_llr_infn); // mask is true when ref is 0 and llr is infinite negative
        const auto m_ref1_infn =
          mipp::andb(m_ref, m_llr_infn); // mask is true when ref is 1 and llr is infinite negative
    ...
    if (bit_1_count == 0 || bit_0_count == 0) return (R)0;
    ...
    if (llr_0_noninf_count && llr_1_noninf_count && llr_0_min <= llr_1_max && llr_1_min <= llr_0_max)
    {
        ...
        bin_width =
          (R)0.5 * ((R)3.49 * (R)std::sqrt(llr_0_variance) * (R)(std::pow(llr_0_noninf_count, (R)-1.0 / (R)3.0)) +
...
template<typename B, typename R>
R
aff3ct::tools::mutual_info_histo_seq(const B* ref, const R* llr, const unsigned size)
{
    const R inf = std::numeric_limits<R>::infinity();
    ...
    // compute the llr_variance as sum of (llr[i] - mean)^2 / llr_noninf_count
    //                              = sum( llr_sum2 - 2 * mean * llr_sum + llr_noninf_count * mean^2 ) /
    //                              llr_noninf_count = sum( llr_sum2 - 2 * mean * llr_sum + llr_sum * mean ) /
    //                              llr_noninf_count = sum( llr_sum2 - 1 * mean * llr_sum) / llr_noninf_count
    ...
    if (bit_0_count == 0 || bit_1_count == 0) return (R)0;
...
template Q_32
aff3ct::tools::mutual_info_histo<B_32, Q_32>(const B_32*, const Q_32*, const unsigned);
template Q_64
aff3ct::tools::mutual_info_histo<B_64, Q_64>(const B_64*, const Q_64*, const unsigned);
#else
template Q
aff3ct::tools::mutual_info_histo<B, Q>(const B*, const Q*, const unsigned);
#endif

#ifdef AFF3CT_MULTI_PREC
template Q_32
aff3ct::tools::mutual_info_histo_seq<B_32, Q_32>(const B_32*, const Q_32*, const unsigned);
template Q_64
aff3ct::tools::mutual_info_histo_seq<B_64, Q_64>(const B_64*, const Q_64*, const unsigned);
#else
template Q
aff3ct::tools::mutual_info_histo_seq<B, Q>(const B*, const Q*, const unsigned);
#endif
// ==================================================================================== explicit template instantiation
