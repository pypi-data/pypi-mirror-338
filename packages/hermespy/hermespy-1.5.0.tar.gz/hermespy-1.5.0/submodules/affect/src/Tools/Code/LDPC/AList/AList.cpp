...
void
AList ::write(const Sparse_matrix& matrix, std::ostream& stream, bool zero_padding)
{
...
void
AList ::read_matrix_size(std::istream& stream, int& H, int& N)
{
    std::string line;
...
std::vector<unsigned>
AList ::read_info_bits_pos(std::istream& stream)
{
    std::string line;
...
std::vector<unsigned>
AList ::read_info_bits_pos(std::istream& stream, const int K, const int N)
{
...
void
AList ::write_info_bits_pos(const std::vector<unsigned>& info_bits_pos, std::ostream& stream)
{
...
Sparse_matrix
AList ::read_format1(std::istream& stream)
{
    ...
    if (n_rows > 0 && n_cols > 0 && rows_max_degree > 0 && cols_max_degree > 0)
    {
        ...
        for (unsigned i = 0; i < n_rows; i++)
        {
            for (unsigned j = 0; j < rows_max_degree; j++)
            {
                ...
                if ((col_index > 0 && j < rows_degree[i]) || (col_index == 0 && j >= rows_degree[i]))
                {
                    if (col_index) matrix.add_connection(i, col_index - 1);
                }
                else
                {
                    std::stringstream message;
        ...
        //						{
        //							// everything is normal if the code passes through the
        // catch
        //						}
        //					}
        //				}
    ...
    else
    {
        ...
        message << "'n_rows', 'n_cols', 'rows_max_degree' and 'cols_max_degree' have to be greater than 0 "
                << "('n_rows' = " << n_rows << ", 'n_cols' = " << n_cols << ", 'rows_max_degree' = " << rows_max_degree
...
Sparse_matrix
AList ::read_format2(std::istream& stream)
{
    std::string line;
    ...
    {
        std::stringstream message;
        message << "'n_rows' and 'n_cols' have to be greater than 0 ('n_rows' = " << n_rows << ", 'n_cols' = " << n_cols
                << ").";
        throw spu::tools::runtime_error(__FILE__, __LINE__, __func__, message.str());
    }
...
// format ANR NAND
Sparse_matrix
AList ::read_format3(std::istream& stream)
{
    std::string line;
