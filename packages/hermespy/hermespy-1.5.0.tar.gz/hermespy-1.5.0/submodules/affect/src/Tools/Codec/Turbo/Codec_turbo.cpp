...
Codec_turbo<B, Q>::Codec_turbo(const factory::Encoder_turbo<>& enc_params,
  ...
  , trellis(new std::vector<std::vector<int>>())
{
    ...
    *trellis = encoder_RSC->get_trellis();
    ...
    {
        std::unique_ptr<module::Decoder_SISO<B, Q>> sub_dec(
          dec_params.sub1->build_siso<B, Q>(*trellis, *json_stream, dec_params.n_ite));
        decoder_turbo.reset(
          dec_params.build<B, Q>(*sub_dec, *sub_dec, this->get_interleaver_llr(), &this->get_encoder()));
    ...
    if (decoder_turbo)
    {
        ...
        if (dec_params.fnc->enable)
        {
            ...
            post_pros.push_back(
              std::unique_ptr<Post_processing_SISO<B, Q>>(new CRC_checker<B, Q>(*crc, dec_params.crc_start_ite)));

        if (dec_params.self_corrected)
            post_pros.push_back(std::unique_ptr<Post_processing_SISO<B, Q>>(
              new Self_corrected<B, Q>(dec_params.K, dec_params.n_ite, 4, dec_params.n_ite)));

        for (auto i = 0; i < (int)post_pros.size(); i++)
            if (post_pros[i] != nullptr) decoder_turbo->add_post_processing(*post_pros[i]);
    }
}

template<typename B, typename Q>
Codec_turbo<B, Q>::~Codec_turbo()
{
...
Codec_turbo<B, Q>::get_trellis() const
{
    return *this->trellis;
}
