...
std::vector<bool>
init_fb(const int K, const int N)
{
...
Frozenbits_generator_GA ::Frozenbits_generator_GA(const int K,
                                                  const int N,
...
Frozenbits_generator_GA ::clone() const
{
...
void
Frozenbits_generator_GA ::evaluate()
{
    this->check_noise();
...
double
Frozenbits_generator_GA ::phi(double t)
{
    if (t < phi_pivot)
...
double
Frozenbits_generator_GA ::phi_inv(double t)
{
    if (t > phi_inv_pivot)
...
void
Frozenbits_generator_GA ::check_noise()
{
...
void
Frozenbits_generator_GA ::recursive_override_frozen_bits(
  const Generic_node<module::Contents_MK_SC<int64_t, double>>* node_curr)
{
...
void
Frozenbits_generator_GA ::recursive_store_DE(const Generic_node<module::Contents_MK_SC<int64_t, double>>* node_curr,
                                             double* z) const
{
    if (!node_curr->is_leaf()) // stop condition
        for (auto c : node_curr->get_children())
...
double
Frozenbits_generator_GA ::square_plus_DE(const double& zl, const double& zr)
{
