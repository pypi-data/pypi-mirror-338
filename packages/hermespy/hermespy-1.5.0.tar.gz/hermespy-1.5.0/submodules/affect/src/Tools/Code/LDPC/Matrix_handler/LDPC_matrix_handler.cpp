...
LDPC_matrix_handler::Matrix_format
LDPC_matrix_handler ::get_matrix_format(const std::string& filename)
{
...
LDPC_matrix_handler::Matrix_format
LDPC_matrix_handler ::get_matrix_format(std::ifstream& file)
{
    file.seekg(0);
...
Sparse_matrix
LDPC_matrix_handler ::read(const std::string& filename, Positions_vector* info_bits_pos, std::vector<bool>* pct_pattern)
{
    std::ifstream file(filename);
    if (!file.is_open())
    {
...
Sparse_matrix
LDPC_matrix_handler ::read(std::ifstream& file, Positions_vector* info_bits_pos, std::vector<bool>* pct_pattern)
{
    ...
    switch (format)
    {
        case Matrix_format::QC:
        {
            ...
            if (pct_pattern != nullptr) *pct_pattern = tools::QC::read_pct_pattern(file);
...
void
LDPC_matrix_handler ::read_matrix_size(const std::string& filename, int& H, int& N)
{
    std::ifstream file(filename);
    if (!file.is_open())
    {
...
void
LDPC_matrix_handler ::read_matrix_size(std::ifstream& file, int& H, int& N)
{
    auto format = get_matrix_format(file);
...
bool
LDPC_matrix_handler ::check_info_pos(const Positions_vector& info_bits_pos, int K, int N, bool throw_when_wrong)
{
    if (info_bits_pos.size() != (unsigned)K)
    {
        if (!throw_when_wrong) return false;
...
Sparse_matrix
LDPC_matrix_handler ::transform_H_to_G_decomp_LU(const Sparse_matrix& H, Positions_vector& info_bits_pos)
{
...
Sparse_matrix
LDPC_matrix_handler ::transform_H_to_G_identity(const Sparse_matrix& H, Positions_vector& info_bits_pos)
{
    H.is_of_way_throw(Matrix::Way::HORIZONTAL);
    LDPC_matrix Hf = sparse_to_full<LDPC_matrix::value_type>(H);
...
void
swap_columns(LDPC_matrix_handler::LDPC_matrix& mat, size_t idx1, size_t idx2)
{
    auto n_row = mat.get_n_rows();
    std::vector<LDPC_matrix_handler::LDPC_matrix::value_type> tmp(n_row);
    for (size_t l = 0; l < n_row; l++)
        tmp[l] = mat[l][idx1];
    for (size_t l = 0; l < n_row; l++)
        mat[l][idx1] = mat[l][idx2];
    for (size_t l = 0; l < n_row; l++)
        mat[l][idx2] = tmp[l];
}

LDPC_matrix_handler::LDPC_matrix
LU_decomp(const LDPC_matrix_handler::LDPC_matrix& Hp)
{
        ...
        for (size_t d = 0; d < n_min; d++)
        {
                ...
                if (!found)
                {
                        ...
                        throw spu::tools::runtime_error(__FILE__, __LINE__, __func__,
                                                        "The matrix H is rank deficient, the SPARSE method can't handle
it for now."); break;
                }
        ...
        for (size_t r = 0; r < X.get_n_rows(); r++)
                for (size_t d = n_min - 1; d > 0; d--)
                        for (size_t c = d; c < n_min; c++)
                                X[r][d - 1] = X[r][d - 1] != ((mat[d - 1][c] != (V)0) && (X[r][c] != (V)0)); // row of
'mat' * row of 'X'
...
LDPC_matrix_handler::LDPC_matrix
LDPC_matrix_handler ::LU_decomposition(const Sparse_matrix& H)
{
...
LDPC_matrix_handler::LDPC_matrix
LDPC_matrix_handler ::LU_decomposition(const LDPC_matrix& H)
{
    H.is_of_way_throw(Matrix::Way::HORIZONTAL);
...
// Benjamin's version
LDPC_matrix_handler::LDPC_matrix
LDPC_matrix_handler ::transform_H_to_G_decomp_LU(const LDPC_matrix& H, Positions_vector& info_bits_pos)
{
    auto M = H.get_n_rows();
    auto N = H.get_n_cols();
    auto K = N - M;
...
// Valentin's version
LDPC_matrix_handler::LDPC_matrix
LDPC_matrix_handler ::transform_H_to_G_identity(const LDPC_matrix& H, Positions_vector& info_bits_pos)
{
    H.is_of_way_throw(Matrix::Way::HORIZONTAL);
    auto G = H;
...
LDPC_matrix_handler::Positions_pair_vector
LDPC_matrix_handler ::form_diagonal(LDPC_matrix& mat, Matrix::Origin o)
{
        ...
        case Matrix::Origin::TOP_RIGHT:
            for (size_t i = 0; i < n_row; i++)
            {
                ...
                if (found)
                {
                    ...
                    for (auto j = i + 1; j < n_row; j++)
                        if (mat[j][ref_col])
                            std::transform(mat[i].begin(),
                                           mat[i].begin() + ref_col + 1, // ref
                                           mat[j].begin(),
                                           mat[j].begin(),
                                           std::not_equal_to<LDPC_matrix::value_type>());
                }
            ...
            for (auto i = n_row; i > 0; i--)
            {
                ...
                if (found)
                {
                    ...
                    for (auto j = ref_row; j > 0; j--)
                    {
                        auto tested_row = j - 1;
                        if (mat[tested_row][ref_col])
                            std::transform(mat[ref_row].begin() + ref_col,
                                           mat[ref_row].end(), // ref
                                           mat[tested_row].begin() + ref_col,
                                           mat[tested_row].begin() + ref_col,
                                           std::not_equal_to<LDPC_matrix::value_type>());
                    }
                }
...
void
LDPC_matrix_handler ::form_identity(LDPC_matrix& mat, Matrix::Origin o)
{
    mat.self_turn(Matrix::Way::HORIZONTAL);
    ...
    switch (o)
    {
        case Matrix::Origin::TOP_LEFT:
            for (auto c = n_row - 1; c > 0; c--)
            {
                ...
                for (auto r = c; r > 0; r--)
                    if (mat[r - 1][c])
                        std::transform(mat[ref_row].begin() + c,
                                       mat[ref_row].end(), // ref
                                       mat[r - 1].begin() + c,
                                       mat[r - 1].begin() + c,
                                       std::not_equal_to<LDPC_matrix::value_type>());
        ...
        case Matrix::Origin::TOP_RIGHT:
            for (auto c = diff; c < (n_col - 1); c++)
            {
                ...
                for (auto r = ref_row; r > 0; r--)
                    if (mat[r - 1][c])
                        std::transform(mat[ref_row].begin(),
                                       mat[ref_row].begin() + c + 1, // ref
                                       mat[r - 1].begin(),
                                       mat[r - 1].begin(),
                                       std::not_equal_to<LDPC_matrix::value_type>());
            }
...
Sparse_matrix
LDPC_matrix_handler ::interleave_matrix(const Sparse_matrix& mat, Positions_vector& old_cols_pos)
{
    if (mat.get_n_cols() != old_cols_pos.size())
    {
        std::stringstream message;
        message << "'mat.get_n_cols()' has to be equal to interleaver length 'old_cols_pos.size()' "
                << "('mat.get_n_cols()' = " << mat.get_n_cols() << ", 'old_cols_pos.size()' = " << old_cols_pos.size()
                << ").";
...
LDPC_matrix_handler::Positions_vector
LDPC_matrix_handler ::interleave_info_bits_pos(const Positions_vector& info_bits_pos, Positions_vector& old_cols_pos)
{
...
bool
LDPC_matrix_handler ::check_GH(const Sparse_matrix& H, const Sparse_matrix& G)
{
    Sparse_matrix GH;
            ...
            switch (G.get_way())
            {
                ...
                case Matrix::Way::VERTICAL:
                    throw spu::tools::runtime_error(
                      __FILE__, __LINE__, __func__, "G and H can't be both in VERTICAL way.");
                    break;
            }
            break;
    }
...
bool
LDPC_matrix_handler ::check_GH(const LDPC_matrix& H, const LDPC_matrix& G)
{
    LDPC_matrix GH;
