...
Encoder_CPE_Rimoldi<SIN, SOUT>::Encoder_CPE_Rimoldi(const int N, const CPM_parameters<SIN, SOUT>& cpm)
...
template<typename SIN, typename SOUT>
SOUT
Encoder_CPE_Rimoldi<SIN, SOUT>::inner_encode(const SIN new_symbol, int& state)
{
...
template<typename SIN, typename SOUT>
SIN
Encoder_CPE_Rimoldi<SIN, SOUT>::tail_symb(const int& state)
{
...
template<typename SIN, typename SOUT>
void
Encoder_CPE_Rimoldi<SIN, SOUT>::generate_mapper(std::vector<SIN>& transition_to_binary,
                                                ...
                                                const std::string& mapping)
{
    if ((int)transition_to_binary.size() != (this->cpm.m_order * this->cpm.n_b_per_s))
    {
        ...
        message << "'transition_to_binary.size()' has to be equal to 'cpm.m_order' * 'cpm.n_b_per_s' "
                ...
                << ", 'cpm.m_order' = " << this->cpm.m_order << ", 'cpm.n_b_per_s' = " << this->cpm.n_b_per_s << ").";
    ...
    else if (mapping == "GRAY")
    {
        for (auto tr = 0; tr < this->cpm.m_order; tr++)
        {
            ...
            for (auto b = 0; b < this->cpm.n_b_per_s; b++)
                transition_to_binary[tr * this->cpm.n_b_per_s + this->cpm.n_b_per_s - 1 - b] =
                  (gray_val & (1 << b)) ? 1 : 0;
        }

        for (auto tr = 0; tr < this->cpm.m_order; tr++)
            binary_to_transition[this->merge_bits(
              transition_to_binary.data() + tr * this->cpm.n_b_per_s, this->cpm.n_b_per_s, true)] = tr;
    }
    else
        throw spu::tools::runtime_error(
          __FILE__, __LINE__, __func__, "Unknown BCJR mapping scheme ('mapping' = " + mapping + ").");
}

template<typename SIN, typename SOUT>
void
Encoder_CPE_Rimoldi<SIN, SOUT>::generate_allowed_states(std::vector<int>& allowed_states)
{
...
template<typename SIN, typename SOUT>
void
Encoder_CPE_Rimoldi<SIN, SOUT>::generate_allowed_wave_forms(std::vector<SOUT>& allowed_wave_forms)
{
...
template<typename SIN, typename SOUT>
void
Encoder_CPE_Rimoldi<SIN, SOUT>::generate_tail_symb_transition()
{
    ...
    while ((int)state_found.size() != this->cpm.n_st)
    {
        ...
        for (auto st = 0; st < this->cpm.n_st; ++st)
        {
            ...
            if (std::find(state_found.begin(), state_found.end(), cur_state) == state_found.end()) // not in the list
            {
                ...
                if (pos_in_vect < (int)state_found.size()) state_found_aux.push_back(cur_state);
            }
        }
    ...
    {
        std::stringstream message;
        message << "'n_tail_symb' has to be equal to 'cpm.tl' ('n_tail_symb' = " << n_tail_symb
                << ", 'cpm.tl' = " << this->cpm.tl << ").";
        throw spu::tools::length_error(__FILE__, __LINE__, __func__, message.str());
    }
}
