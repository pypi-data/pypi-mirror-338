...
Codec_uncoded<B, Q>::Codec_uncoded(const factory::Encoder_NO& enc_params, const factory::Decoder_NO& dec_params)
  : Codec_SISO<B, Q>(enc_params.K, enc_params.N_cw, enc_params.N_cw)
{
    ...
    {
        std::stringstream message;
        message << "'K' has to be equal to 'N' ('K' = " << enc_params.K << ", 'N' = " << enc_params.N_cw << ").";
        throw spu::tools::invalid_argument(__FILE__, __LINE__, __func__, message.str());
    }
