...
Event_generator_MKL<R, E>::Event_generator_MKL(const int seed)
  : Event_generator<R, E>()
...
template<typename R, typename E>
Event_generator_MKL<R, E>::~Event_generator_MKL()
{
...
Event_generator_MKL<R, E>::clone() const
{
...
template<typename R, typename E>
void
Event_generator_MKL<R, E>::set_seed(const int seed)
{
    if (is_stream_alloc) vslDeleteStream((VSLStreamStatePtr*)stream_state);
...
template<typename R, typename E>
void
Event_generator_MKL<R, E>::generate(E* draw, const unsigned length, const R event_probability)
{
    throw spu::tools::runtime_error(
      __FILE__, __LINE__, __func__, "The MKL viRngBernoulli random generator does not support this type.");
}
...
template<>
void
Event_generator_MKL<R_32, B_32>::generate(B_32* draw, const unsigned length, const R_32 event_probability)
{
...
template<>
void
Event_generator_MKL<R_64, B_64>::generate(B_64* draw, const unsigned length, const R_64 event_probability)
{
    std::vector<int> draw_i(length);
    viRngBernoulli(
      VSL_RNG_METHOD_BERNOULLI_ICDF, *(VSLStreamStatePtr*)stream_state, length, draw_i.data(), event_probability);
