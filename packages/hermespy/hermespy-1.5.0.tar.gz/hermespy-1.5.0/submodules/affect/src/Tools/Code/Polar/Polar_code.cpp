...
std::string
aff3ct::tools::display_kernel(const std::vector<std::vector<bool>>& pattern_bits)
{
    std::string m = "{";
...
void
aff3ct::tools::read_polar_MK_code(const std::string& code_path,
                ...
                for (auto k = 0; k < n_kernels; k++)
                {
                    ...
                    if (values.size() > 0)
                    {
                        ...
                        else
                        {
                            ...
                            std::stringstream message;
                            message << "'kernel_size' should be greater than 1 ('kernel_size' = " << kernel_size
                                    << ").";
                            throw spu::tools::runtime_error(__FILE__, __LINE__, __func__, message.str());
                        }
            ...
            if (values.size() > 0)
            {
                ...
                if (n_stages > 0)
                {
                    ...
                    else
                    {
                        ...
                        std::stringstream message;
                        message << "'values.size()' should be equal to 'n_stages' ('values.size()' = " << values.size()
                                << ", 'n_stages' = " << n_stages << ").";
                        throw spu::tools::runtime_error(__FILE__, __LINE__, __func__, message.str());
                    }
                }
...
void
get_cofactor(const std::vector<std::vector<T>>& mat,
...
template<typename T = int32_t>
int
determinant_of_matrix(const std::vector<std::vector<T>>& mat, const int n)
{
...
template<typename T = int32_t>
bool
is_invertible(const std::vector<std::vector<T>>& mat)
{
    return determinant_of_matrix(mat, (int)mat.size()) != 0;
}

Polar_code ::Polar_code(const std::string& code_path)
  : N()
  , kernel_matrices()
  , stages()
{
...
Polar_code ::Polar_code(const int N, const std::vector<std::vector<bool>>& kernel_matrix)
  : N(N)
...
Polar_code ::Polar_code(const std::vector<std::vector<std::vector<bool>>>& kernel_matrices,
...
bool
Polar_code ::can_be_systematic() const
{
    if (!this->is_mono_kernel())
    {
        const auto n_stages_2 = this->get_stages().size() / 2;
        for (size_t sl = 0; sl < n_stages_2; sl++)
            if (this->get_stages()[sl] != this->get_stages()[this->get_stages().size() - 1 - sl]) return false;
    }
...
size_t
Polar_code ::is_mono_kernel() const
{
    auto type = stages[0];
    for (size_t s = 1; s < stages.size(); s++)
        if (stages[s] != type) return 0;
    return this->get_kernel_matrices()[0].size();
}

int
Polar_code ::get_codeword_size() const
{
    return this->N;
}

const std::vector<std::vector<std::vector<bool>>>&
Polar_code ::get_kernel_matrices() const
{
    return this->kernel_matrices;
}

const std::vector<uint32_t>&
Polar_code ::get_stages() const
{
    return this->stages;
}

size_t
Polar_code ::get_biggest_kernel_size() const
{
