...
Encoder_CPE<SIN, SOUT>::Encoder_CPE(const int N, const CPM_parameters<SIN, SOUT>& cpm)
  : N(N)
  , cpm(cpm)
{
    ...
    if ((int)sizeof(SIN) * 256 < cpm.m_order)
    {
        ...
        message << "'cpm.m_order' has to be equal or smaller than 'sizeof(SIN)' * 256 ('cpm.m_order' = " << cpm.m_order
...
template<typename SIN, typename SOUT>
int
Encoder_CPE<SIN, SOUT>::get_n_ff() const
{
    return cpm.L;
}
...
template<typename SIN, typename SOUT>
int
Encoder_CPE<SIN, SOUT>::tail_length() const
{
    return cpm.tl;
}
...
template<typename SIN, typename SOUT>
void
Encoder_CPE<SIN, SOUT>::encode(const std::vector<SIN>& U_N, std::vector<SOUT>& X_N)
{
    if ((int)U_N.size() != N)
    {
        std::stringstream message;
    ...
    {
        std::stringstream message;
        message << "'X_N.size()' has to be equal to 'N' + 'cpm.tl' ('X_N.size()' = " << X_N.size() << ", 'N' = " << N
                << ", 'cpm.tl' = " << cpm.tl << ").";
        throw spu::tools::length_error(__FILE__, __LINE__, __func__, message.str());
    }
...
template<typename SIN, typename SOUT>
void
Encoder_CPE<SIN, SOUT>::generate_trellis(std::vector<int>& trellis_next_state,
                                         std::vector<SOUT>& trellis_related_wave_form)
{
    if ((int)trellis_next_state.size() != cpm.max_st_id * cpm.m_order)
    {
        ...
        message << "'trellis_next_state.size()' has to be equal to 'cpm.max_st_id' * 'cpm.m_order' "
                ...
                << ", 'cpm.max_st_id' = " << cpm.max_st_id << ", 'cpm.m_order' = " << cpm.m_order << ").";
        throw spu::tools::length_error(__FILE__, __LINE__, __func__, message.str());
    }
...
template<typename SIN, typename SOUT>
void
Encoder_CPE<SIN, SOUT>::generate_anti_trellis(std::vector<int>& anti_trellis_original_state,
...
template<typename SIN, typename SOUT>
SIN
Encoder_CPE<SIN, SOUT>::merge_bits(const SIN* in_bit, const int number_of_bits, const bool msb_to_lsb)
{
    SIN res = 0;
