...
Codec_turbo_DB<B, Q>::Codec_turbo_DB(const factory::Encoder_turbo_DB& enc_params,
    ...
    *trellis = encoder_RSC->get_trellis();
    ...
    if (decoder_turbo)
    {
        ...
        if (dec_params.fnc->enable)
        {
            ...
            post_pros.push_back(
              std::unique_ptr<Post_processing_SISO<B, Q>>(new CRC_checker_DB<B, Q>(*crc, dec_params.crc_start_ite)));
...
template<typename B, typename Q>
Codec_turbo_DB<B, Q>*
Codec_turbo_DB<B, Q>::clone() const
{
...
Codec_turbo_DB<B, Q>::get_trellis() const
{
    return *this->trellis;
}
