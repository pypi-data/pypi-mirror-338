...
Simulation_BFER<B, R>::Simulation_BFER(const factory::BFER& params_BFER)
  : Simulation()
  , params(params_BFER)
  ...
  , dumper(params_BFER.n_threads)
{
    ...
    if (!params_BFER.noise->pdf_path.empty())
        this->distributions.reset(
          new tools::Distributions<R>(params_BFER.noise->pdf_path, tools::Distribution_mode::SUMMATION));
    try
    {
...
template<typename B, typename R>
std::unique_ptr<module::Monitor_MI<B, R>>
Simulation_BFER<B, R>::build_monitor_mi()
{
...
template<typename B, typename R>
std::unique_ptr<module::Monitor_BFER<B>>
Simulation_BFER<B, R>::build_monitor_er()
{
...
template<typename B, typename R>
std::vector<std::unique_ptr<spu::tools::Reporter>>
Simulation_BFER<B, R>::build_reporters(const tools ::Noise<>* noise,
...
template<typename B, typename R>
std::unique_ptr<spu::tools::Terminal>
Simulation_BFER<B, R>::build_terminal(const std::vector<std::unique_ptr<spu::tools::Reporter>>& reporters)
{
...
Simulation_BFER<B, R>::create_modules()
{
    ...
    if (params_BFER.mnt_mi != nullptr) this->monitor_mi = this->build_monitor_mi();
}

template<typename B, typename R>
void
Simulation_BFER<B, R>::configure_sequence_tasks()
{
    ...
    for (auto& mod : sequence->get_modules<spu::module::Module>())
        ...
        {
            if (this->params.statistics) tsk->set_stats(true);
            // enable the debug mode in the modules
            if (this->params.debug)
            {
                tsk->set_debug(true);
                tsk->set_debug_hex(this->params.debug_hex);
                if (this->params.debug_limit) tsk->set_debug_limit((uint32_t)this->params.debug_limit);
                if (this->params.debug_precision) tsk->set_debug_precision((uint8_t)this->params.debug_precision);
                if (params.debug_frame_max) tsk->set_debug_frame_max((uint32_t)this->params.debug_frame_max);
            }
...
template<typename B, typename R>
void
Simulation_BFER<B, R>::create_monitors_reduction()
{
...
Simulation_BFER<B, R>::launch()
{
    if (!params_BFER.err_track_revert)
    {
        ...
        this->reporters =
          this->build_reporters(this->noise.get(), this->monitor_er_red.get(), this->monitor_mi_red.get());
    ...
    for (auto noise_idx = noise_begin; noise_idx != noise_end; noise_idx += noise_step)
    {
        ...
        params_BFER.noise->template update<>(
          *this->noise, params_BFER.noise->range[noise_idx], bit_rate, params_BFER.mdm->bps, params_BFER.mdm->cpm_upf);
        ...
        if (params_BFER.mpi_rank == 0)
            ...
            {
                if (params_BFER.debug) terminal->legend(std::cout);
...
template<typename B, typename R>
bool
Simulation_BFER<B, R>::stop_time_reached()
{
...
template<typename B, typename R>
bool
Simulation_BFER<B, R>::stop_condition()
{
