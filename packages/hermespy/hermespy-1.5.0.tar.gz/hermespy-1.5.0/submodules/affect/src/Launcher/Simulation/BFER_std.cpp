...
BFER_std<B, R, Q>::BFER_std(const int argc, const char** argv, std::ostream& stream)
...
template<typename B, typename R, typename Q>
void
BFER_std<B, R, Q>::get_description_args()
{
...
BFER_std<B, R, Q>::store_args()
{
    ...
    if (!this->arg_vals.exist({ psim + "-inter-fra", "F" }) && params.mdm->type == "SCMA") params.n_frames = 6;
