...
class Launcher
{
  private:
    std::unique_ptr<simulation::Simulation>
      simu; /*!< A generic simulation pointer to allocate a specific simulation. */
    std::string cmd_line;
    std::vector<std::string> cmd_warn;

  protected:
    cli::Argument_handler
      ah; /*!< An argument reader to manage the parsing and the documentation of the command line parameters. */
  ...
  public:
     ...
     */
    Launcher(const int argc, const char** argv, factory::Simulation& params_common, std::ostream& stream = std::cout);
