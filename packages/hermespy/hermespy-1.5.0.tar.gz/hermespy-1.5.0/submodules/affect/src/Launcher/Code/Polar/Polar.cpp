...
Polar<L, B, R, Q>::Polar(const int argc, const char** argv, std::ostream& stream)
  : L(argc, argv, stream)
  , params_cdc(new factory::Codec_polar("cdc"))
{
...
template<class L, typename B, typename R, typename Q>
void
Polar<L, B, R, Q>::get_description_args()
{
...
template<class L, typename B, typename R, typename Q>
void
Polar<L, B, R, Q>::store_args()
{
    auto dec_polar = dynamic_cast<factory::Decoder_polar*>(params_cdc->dec.get());

    params_cdc->store(this->arg_vals);

    if (dec_polar->simd_strategy == "INTER") this->params.n_frames = mipp::N<Q>();
    ...
    if (!this->arg_vals.exist({ pfbg + "-gen-method" }))
        if (this->params.chn->type == "BEC") this->params_cdc->fbg->type = "BEC";
}
