...
Turbo<L, B, R, Q>::Turbo(const int argc, const char** argv, std::ostream& stream)
  : L(argc, argv, stream)
  , params_cdc(new factory::Codec_turbo("cdc"))
{
...
template<class L, typename B, typename R, typename Q>
void
Turbo<L, B, R, Q>::get_description_args()
{
...
template<class L, typename B, typename R, typename Q>
void
Turbo<L, B, R, Q>::store_args()
{
    auto dec_tur = dynamic_cast<factory::Decoder_turbo<>*>(params_cdc->dec.get());

    params_cdc->store(this->arg_vals);

    if (dec_tur->sub1->simd_strategy == "INTER") this->params.n_frames = mipp::N<Q>();
    if (dec_tur->sub1->simd_strategy == "INTRA") this->params.n_frames = (int)std::ceil(mipp::N<Q>() / 8.f);
