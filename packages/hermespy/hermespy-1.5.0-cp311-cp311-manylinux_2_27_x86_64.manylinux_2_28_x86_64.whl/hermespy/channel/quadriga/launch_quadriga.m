...
addpath(path_quadriga_src);  %<<<Python 
...
numb_snap = channels(1,1,1).no_snap; %number of snapshots
for irx = 1:layout.no_rx    
    for itx = 1:layout.no_tx
        ...
        cirs(irx, itx).coefficients = channels(irx, itx, ifrq).coeff;
        cirs(irx, itx).delays = channels(irx, itx, ifrq).delay;
    end
end