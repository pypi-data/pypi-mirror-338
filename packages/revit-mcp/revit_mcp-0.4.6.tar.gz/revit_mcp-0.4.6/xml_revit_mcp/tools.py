...
def call_func(ctx: Context, method: str = "CallFunc", params: List[dict[str, any]] = None) -> dict:
    """
    调用Revit函数服务，支持多种功能操作，包括内置功能和插件目录下MCP文件夹中的功能，遵循JSON-RPC 2.0规范。

    特性:
    - 支持批量函数调用
    - 自动去重处理相同函数调用
    - 支持从插件目录下MCP文件夹加载并执行实现IMCPMethod接口的功能
    - 事务化操作确保数据一致性
    - 严格遵循JSON-RPC 2.0规范
    ...
    参数:
        ...
        params (List[Dict]): 功能参数列表，每个字典包含:
            - func (List[str]): 要调用的功能列表，支持以下功能：
                - "ClearDuplicates": 清理重复元素
                - "DeleteZeroRooms": 清理面积为0或未放置的房间
    ...
    示例:
        ...
        response = call_func(ctx, params=[
            {"func": ["ClearDuplicates", "DeleteZeroRooms"]},
            {"func": ["DimensionViewPlanGrids"]},
            {"func": ["ExternalFunction1", "ExternalFunction2"]}
        ])
    ...
    try:
        ...
        for param in params:
            ...
            if not param.get("func") or not isinstance(param["func"], list):
                raise ValueError("'func'字段必须是非空的字符串列表")

            validated_params.append({
                "func": [str(f) for f in param["func"]]  # 确保所有函数名为字符串
            })
