#! /usr/bin/python3
# This script exercises the LDK-Sample implementation
...
LDK_SRC = os.path.join(os.getcwd(), os.getenv("LDK_SRC", ".."))
...
class Runner(lnprototest.Runner):
    def __init__(self, config: Any):
        ...
        for o in opts:
            ...
            elif k == "AnchorsNonzeroFeeHtlcTx":
                self.options["option_anchors_non_zero_fee_htlc_tx"] = val[v]
    ...
    def start(self) -> None:
        ...
        key = bytes([0] * 32)  # Equivalent to [0; 32] in Rust
    ...
    def runner_features(
        ...
        if additional_features is None:
            if globals:
                ...
                return bitfield(0, 8, 12, 14)
        else:
            if globals:
                return bitfield(*(additional_features))
            else:
                return bitfield(*(additional_features + list((0, 8, 12, 14))))
