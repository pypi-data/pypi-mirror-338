...
This module provides functions to compress and decompress files using Zstandard (zstd) with parallel support.
It includes functions to compress a file into chunks if it exceeds a specified size, and to decompress files or combine split parts back into a single file.
I wrote these functions for compressing/splitting files that are larger than Gihub's upload limit (100MB) and decompressing them later. I inted to use it for files up to 2~3 GB max that would results to 20-30 parts at most. I don't see a point to store larger files in GitHub.
