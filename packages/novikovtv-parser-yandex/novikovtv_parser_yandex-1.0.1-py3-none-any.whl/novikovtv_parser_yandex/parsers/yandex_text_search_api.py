...
from parsers.models.operation import Operation
from parsers.models.site_data import SiteParserData
from parsers.search_links_parser import YandexSearchLinksParser
from parsers.site_parser import SiteParser
from parsers.yandex_api_lib.constants import PAGE_RANGE, RPS_V1, MAX_RETRY_ATTEMPTS
from parsers.yandex_api_lib.exceptions import HTTPBadResponseError, YandexRetryError
from parsers.yandex_api_lib.yandex_api import YandexAPI
...
async def wait_until(target_time: dt_time):
    ...
    logging.debug(f"Ожидание {wait_seconds} секунд до {target_time} (UTC+3)")
    await asyncio.sleep(wait_seconds)


# async def add_request_result_to_db(*, file_path: str, page: int, search_id: int) -> None:
#     """
...
#         repository = RequestResultRepository(uow)
#         request_result = RequestResult(
#             search_id=search_id,
#             html_filename=file_path,
#             page=page,
#         )
#         await repository.add(request_result)
...
class YandexSearchTextApiV1(YandexAPI):
    ...
    @classmethod
    async def create(cls) -> YandexSearchTextApiV1:
        """
    ...
    def __init__(self, api_key_v1: str):
        super().__init__()
    ...
    async def search(self, query_text: str, p_type: ParserType, search_id: int = None) -> Tuple[
        ...
        if p_type.value == ParserType.Night.value:
            logging.debug("Ночной тип парсинга")
            await wait_until(dt_time(0, 0))
        elif p_type.value == ParserType.Day.value and dt_time(0, 0) <= now_utc3_time < dt_time(7, 0):
            logging.debug("Дневной тип парсинга")
            await wait_until(dt_time(7, 0))

        folder_abs_path = os.path.abspath(f'../files/{query_text}_{round(time.time())}').replace(" ", "_").strip()
        ...
        tasks: list = [self.__try_search(query_text, page, folder_abs_path, search_id=search_id)
                       for page in PAGE_RANGE]
    ...
    async def __fetch(self, query_text: str, page: int, folder_abs_path: str, *, search_id: int = 0,
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(self.get_url_v1(query_text, page=page)) as response:
                    ...
                    if text.startswith("<?xml"):
                        ...
                        logging.warning(f"{response.status} {await response.text()}")
                        return None
    ...
    async def __try_search(self, *args, iteration: int = 0, **kwargs) -> Optional[list[SiteParserData]]:
        try:
            ...
            # logging.warning(f"Ошибка с повторной попыткой. Итерация = {yre.iteration}. Ожидание {1 / RPS_V1} сек.")
            await asyncio.sleep(1 / RPS_V1)
