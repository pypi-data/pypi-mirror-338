# -*- coding: utf-8 -*-
"""TLKT_20250402_00.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/15OMyF6fmH8I2T2nsIrxnw_1Ge0UmfyO9
...
class ToolKit():
    ...
    def executor( self, cmd , shell = False, log = False ):
        """
        ...
        """
        if shell:
            self.debug.log_step(f"コマンド: { cmd }", success=None, back = 1)
        else:
            self.debug.log_step(f"コマンド: {' '.join(cmd)}", success=None, back = 1)
    ...
    def executor_sync(self, cmd, shell=False):
        ...
        args:
            cmd( str [] ): 実行するコマンド
            shell( bool ) : コマンドの書き方。Trueだと文字列で、Falseだと文字配列で
        """
        self.log_step(f"cmd: { cmd }", success=None, back = 1 )
        ...
        def log_stdout(pipe):
            for line in iter(pipe.readline, ''):
                self.log_step(line.rstrip(), success = None )
            pipe.close()

        # stderrも同様にログ出力する（必要に応じて）
        def log_stderr(pipe):
            for line in iter(pipe.readline, ''):
                self.log_step(f"ERR: {line.rstrip()}", success = None )
            pipe.close()
        ...
        self.log_step( f"result: {process.returncode}", success=None, back = 1 )

        # 完了後、出力をまとめて返す場合は communicate() の代わりに、
        # 今回はすでに各行をログ出力しているので、必要に応じて空文字列などを返す
        return subprocess.CompletedProcess( args=cmd, returncode=process.returncode, stdout="", stderr="")