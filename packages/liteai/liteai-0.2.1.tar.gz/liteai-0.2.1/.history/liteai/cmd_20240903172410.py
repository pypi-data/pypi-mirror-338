...
@click.option("--overwrite", default=False, help="是否覆盖原文件")
...
def batch(data_path: str, model: str, work_num=4, input_column="input", image_column: str = None, image_dir=None,
          overwrite: True = False, system: str = None, temperature=0.7, top_p=.7, log_level="INFO"):
    """
    批量处理数据
    """
    st = time.time()
    ...
    def _func(item):
        ...
        if _system:
            messages.insert(0, Message(role="system", content=_system))

        resp = chat(model=model, messages=messages, stream=False, top_p=top_p, temperature=temperature, log_level=log_level)
        item[f"{model}_response"] = resp.content

    batch_func = multi_thread(work_num=work_num, return_list=True)(_func)
    batch_func(data=data)
    base, ext = os.path.splitext(data_path)[-1].lower()
    if overwrite:
