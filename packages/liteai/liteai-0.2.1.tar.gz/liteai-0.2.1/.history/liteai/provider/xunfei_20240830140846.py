...
def send_voice(ws: WebSocket,  voice: Voice, common_args: dict, business_args: dict):
    ...
    for buf in voice.byte_stream:
        logger.debug(f"get {len(buf)} bytes")
        # 文件结束
        if not buf:
 ...
 wss://ws-api.xfyun.cn/v2/iat?authorization=YXBpX2tleT0iZGZiNDc0YWViOTFmMDU2NzIzOGViZTMyNjgwN2I4YWIiLCBhbGdvcml0aG09ImhtYWMtc2hhMjU2IiwgaGVhZGVycz0iaG9zdCBkYXRlIHJlcXVlc3QtbGluZSIsIHNpZ25hdHVyZT0iUlJ5SHdIWGZXQ2tDRmdVekoyOUpyOWtBcnB3YlhvcUNWLzFlL25TbDdlVT0i&date=Fri%2C+30+Aug+2024+06%3A08%3A20+GMT&host=ws-api.xfyun.cn
...
class XunfeiProvider(BaseProvider):
    ...
    def __init__(self, app_id: str, api_key: str = None, api_secret: str = None, ** kwargs):
    ...
    def asr(self, voice: Voice, model: ModelCard, **kwargs) -> str:
        ...
        def onopen(ws: WebSocket):
            _thread.start_new_thread(send_voice, (ws, voice,  {"app_id": self.app_id}, kwargs.get("business_args", self.DEFAULT_BUSINESS_ARGS)))
...
if __name__ == "__main__":
    ...
    xunfei_provider = XunfeiProvider(app_id="9b5cc933")
