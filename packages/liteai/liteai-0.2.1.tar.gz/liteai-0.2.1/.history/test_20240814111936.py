...
def play_bytes(byte_stream: Iterable[bytes], min_len=1000, buffer_size=1024*10):
    audio_buffer = io.BytesIO()
    ...
    while not producer_finished.is_set() or audio_buffer.tell() > 0:
        logger.info(f"{producer_finished.is_set()=}, {audio_buffer.tell()=}")
        if producer_finished.is_set() and audio_buffer.tell() < min_len:
            logger.info(f"waiting for more data, {audio_buffer.tell()=}")
            time.sleep(1)
            continue
        # if audio_buffer.tell() > buffer_size:  # 例如，每4KB播放一次
        raw_size = audio_buffer.tell()
        audio_buffer.seek(0)

        audio_segment = AudioSegment.from_file(audio_buffer, format="mp3")
        duration = len(audio_segment)
        logger.debug(f"play audio segment with size:{len(audio_segment.raw_data)}, raw size:{raw_size}, duration:{duration}")

        play(audio_segment)
        time.sleep(duration/1000*0.95)
    thread.join()
