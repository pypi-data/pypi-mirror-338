...
def play_bytes(byte_stream: Iterable[bytes], min_len=1000, buffer_size=1024*10):
    ...
    while True:
        ...
        # logger.info(f"{producer_finished.is_set()=}, {raw_size=}, {offset=}, {end_offset=}")
        # if producer_finished.is_set() and raw_size <= 0:
        #     break
        ...
        logger.debug(f"play audio segment with size:{len(audio_segment.raw_data)}, duration:{duration}")
