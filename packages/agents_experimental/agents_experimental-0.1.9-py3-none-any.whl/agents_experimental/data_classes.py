...
class DocumentSummaryTool(Tool):
    ...
    type: Literal["documentSummary"] = field(default="documentSummary", kw_only=True)
    configuration: Dict[str, Any] = field(default_factory=dict, kw_only=True)
...
class DataModelReference:
    """
    Represents a reference to a data model view.
    """
    space: str
    external_id: str
    version: str
...
@dataclass
class QueryDataModelToolConfiguration:
    """
    Configuration specific to the QueryDataModelTool.
    """
    data_models: List[DataModelReference] # Renamed from dataModels
    # instanceSpaces is not included as per user request to simplify
...
class QueryDataModelTool(Tool):
    """
    Tool for querying data models.
    """
    # configuration is non-default here, only type needs kw_only
    configuration: QueryDataModelToolConfiguration 
    type: Literal["queryDataModel"] = field(default="queryDataModel", kw_only=True)
