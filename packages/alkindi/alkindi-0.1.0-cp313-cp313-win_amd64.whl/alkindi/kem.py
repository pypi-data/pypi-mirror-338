...
STANDARDIZED_KEM_ALGORITHMS: Final[tuple[str, ...]] = (
    "ML-KEM-512",
    "ML-KEM-768",
    "ML-KEM-1024",
)
...
class KEM:
    ...
    def __init__(self, alg_name: str) -> None:
        ...
        self._kem = core.liboqs.OQS_KEM_new(alg_name.encode("utf-8"))
        if self._kem == core.ffi.NULL:
            raise ValueError(f"Algorithm {alg_name} is not supported by liboqs")

        self.method_name = core.ffi.string(self._kem.method_name).decode("utf-8")
        self.alg_version = core.ffi.string(self._kem.alg_version).decode("utf-8")
        self.claimed_nist_level = int(self._kem.claimed_nist_level)
        self.ind_cca = bool(self._kem.ind_cca)
    ...
    def __exit__(self, exc_type, exc_value, traceback) -> None:
        ...
        if self._kem is not None:
            core.liboqs.OQS_KEM_free(self._kem)
            self._kem = None
    ...
    def generate_keypair(self) -> Tuple[bytes, bytes]:
        ...
        result = core.liboqs.OQS_KEM_keypair(self._kem, public_key, secret_key)
    ...
    def encaps(self, public_key: bytes) -> Tuple[bytes, bytes]:
        ...
        ciphertext = core.ffi.new("uint8_t[]", self.length_ciphertext)
        if ciphertext == core.ffi.NULL:
            raise MemoryError("Failed to allocate memory for ciphertext")

        shared_secret = core.ffi.new("uint8_t[]", self.length_shared_secret)
        if shared_secret == core.ffi.NULL:
            raise MemoryError("Failed to allocate memory for shared secret")

        result = core.liboqs.OQS_KEM_encaps(
            self._kem, ciphertext, shared_secret, core.ffi.from_buffer(public_key)
        )

        if result != OQS_SUCCESS:
            raise RuntimeError(
                f"Failed to encapsulate shared secret for {self.method_name}"
            )
    ...
    def decaps(self, ciphertext: bytes, secret_key: bytes) -> bytes:
        ...
        result = core.liboqs.OQS_KEM_decaps(
            self._kem,
            shared_secret,
            core.ffi.from_buffer(ciphertext),
            core.ffi.from_buffer(secret_key),
        )

        if result != OQS_SUCCESS:
            raise RuntimeError(
                f"Failed to decapsulate shared secret for {self.method_name}"
            )
