...
def print_act(fname, output = 'last'):
    # output var can be first, last, or all
    if fname[-3:] == 'mat':
        act_cal = loadmat(fname)['act_cal']
        act_cal = {'channel': act_cal['channel'][0][0].flatten(),
                   'type': act_cal['type'][0][0].flatten(),
                   'min_usec': act_cal['min_usec'][0][0].flatten(),
                   'mid_usec': act_cal['mid_usec'][0][0].flatten(),
                   'max_usec': act_cal['max_usec'][0][0].flatten(),
                  }
    else:
        rg = nc.Dataset(fname,'r')
        act_cal = rg['/ACTUATORS_CALIBRATION_vec'][:]
    ...
    row = ['Ch','Surface Type','Min','Mid','Max']
    print("{: >2} {: >20} {: >6} {: >6} {: >6}".format(*row))
    for ch in np.unique(act_cal['channel']):
        idx = np.where(act_cal['channel'] == ch)[0]
        if output == 'first':
            idx = [idx[0]]
        if output == 'last':
            idx = [idx[-1]]
        ...
        for i in idx:
            row = [ch,act_type[act_cal['type'][i]][4:],
                   act_cal['min_usec'][i], act_cal['mid_usec'][i],act_cal['max_usec'][i]]
            print("{: >2} {: >20} {: >6} {: >6} {: >6}".format(*row))
...
def get_hs_list(fname,fname2=None,idx=-1):
  fname = '/home/stachura/tmp/vtol/LOG_050_S01001.nc'
...
def get_surface_list(fname,fname2=None,idx=-1):
  act_type = [
    'ACT_UNUSED',
    'ACT_L_AILERON',
    'ACT_L_ELEVATOR',
...
def print_act_cal(fname,idx=-1):
  act_list = get_surface_list(fname,idx)
  row =  "{:<8} {:<20} {:<6} {:>6} {:>6}"
  hrow = "{:<8} {:<20} {:<6} {:>6} {:>6}"
...
def plot_ap_modes(fname,ax_share=None):
    ...
    packet_names = [
       ...
       'FlightMode_t',
       'LandingStatus_t',
       'AltitudeControlMode_t',
       'LateralControlMode_t',
       'NavigationControllerMode_t',
       'TrueFalse_t',
       'TrueFalse_t',
    ]

    fig,gs,ax_share,ax = create_subplot(len(varnames)+2,
                                        ttl = 'Autopilot Modes',
                                        ax_share = ax_share)
    add_flying_bar(ax,fname)

    for i,(var,packet) in enumerate(zip(varnames,packet_names)):
        ax = fig.add_subplot(gs[i+1],sharex=ax_share)
        idx = np.where(((com['t_'+var] < 1e16)) & ((abs(com[var]) < 256)))[0] #FIXME
        ax.step(com['t_'+var][idx],com[var][idx],color='xkcd:dark grey',where='post')
        setup_subplot(ax,ylabel=var)
        make_ticks(ax, packet_type = packet)

    ax = fig.add_subplot(gs[i+2],sharex=ax_share)
    ...
    ax = fig.add_subplot(gs[i+3],sharex=ax_share)
    ax.step(tsys_status[idx],sys_status['lost_gps'][idx],color='xkcd:dark grey')
    setup_subplot(ax,ylabel='lost_gps')
    make_ticks(ax, packet_type = 'TrueFalse_t')
    return ax_share

def plot_surfaces(fname,idx=-1,ax_share=None,fname2=None,xmlname=None):
    act_list = get_surface_list(fname,idx=idx,fname2=fname2)

    if len(act_list) > 0:

        fig,gs,ax_share,ax = create_subplot(len(act_list),
                                            ttl = 'Control Surfaces',
                                            ax_share = ax_share)
        add_flying_bar(ax,fname)

        for i,stype in enumerate(act_list):
            ax = fig.add_subplot(gs[i+1],sharex=ax_share)
            ax.plot(act_list[stype]['system_time'],act_list[stype]['usec'],'.',color='xkcd:dark gray')
            ax.set_ylim([act_list[stype]['min_usec'],act_list[stype]['max_usec']])
...
def plot_fw_control(fname,ax_share=None):
    state,tstate = get_var(fname,'state')
    gps,tgps = get_var(fname,'gps')
    r,p,y = bae.quat2ang_vec(state['q'])
    com = get_log_command(fname)

    fig,gs,ax_share,ax = create_subplot(4,ttl = 'Fixedwing Control',ax_share = ax_share)
    add_flying_bar(ax,fname)

    ax = fig.add_subplot(gs[1],sharex=ax_share)
    ax.plot(tstate,r*180/pi,color='xkcd:dark gray')
    ax.plot(com['t_ROLL'],com['ROLL']*180/pi,'--',color='xkcd:red')
    setup_subplot(ax,ylabel='Roll')

    ax = fig.add_subplot(gs[2],sharex=ax_share)
    ax.plot(tstate,p*180/pi,color='xkcd:dark gray')
    ax.plot(com['t_PITCH'],com['PITCH']*180/pi,'--',color='xkcd:red')
    setup_subplot(ax,ylabel='Pitch')

    ax = fig.add_subplot(gs[3],sharex=ax_share)
    ax.plot(tstate,state['ias'],color='xkcd:dark grey')
    ax.plot(com['t_SPEED'],com['SPEED'],'--',color='xkcd:red')
    ax.plot(tgps,gps['speed'],color='xkcd:green')
    setup_subplot(ax,ylabel='IAS')

    ax = fig.add_subplot(gs[4],sharex=ax_share)
    ax.plot(tstate,state['altitude'],color='xkcd:dark grey')
    ax.plot(com['t_ALTITUDE'],com['ALTITUDE'],'--',color='xkcd:red')
    ax.set_ylim([1580,1650])
    setup_subplot(ax,ylabel = 'Altitude')

    return ax

def plot_gps(fname,ax_share = None):
    com = get_log_command(fname)
    gps,tgps = get_var(fname,'gps')

    fig,gs,ax_share,ax = create_subplot(9,ttl = 'GPS Sensor',ax_share = ax_share)
    add_flying_bar(ax,fname)
    ...
    var = 'satellites'
    ax = fig.add_subplot(gs[6],sharex=ax_share)
    ax.step(tgps,gps[var],color='xkcd:dark gray',where='post')
    setup_subplot(ax,ylabel=var)

    var = 'pdop'
    ax = fig.add_subplot(gs[7],sharex=ax_share)
    ax.step(tgps,gps[var],color='xkcd:dark gray',where='post')
    setup_subplot(ax,ylabel=var)

    var = 'fix_type'
    ax = fig.add_subplot(gs[8],sharex=ax_share)
    ax.step(tgps,gps[var],color='xkcd:dark gray',where='post')
    setup_subplot(ax,ylabel=var)
    make_ticks(ax,packet_type='GPSFixType_t')

    ax = fig.add_subplot(gs[9],sharex=ax_share)
...
def plot_mr_control(fname,ax_share=None):
    ...
    subfig,gs,ax_share,ax = create_subplotN([6,4],ttl = 'Multirotor Control',ax_share = ax_share)
    add_flying_bar(ax,fname)

    ax = subfig[0].add_subplot(gs[0][1],sharex=ax_share)
    ax.plot(tstate,r*180/pi,color='xkcd:dark gray')
    ax.plot(com['t_ROLL'],com['ROLL']*180/pi,'--',color='xkcd:red')
    setup_subplot(ax,ylabel='Roll')

    ax = subfig[0].add_subplot(gs[0][2],sharex=ax_share)
    ax.plot(tgyr,gyr['x']*180/pi,color='xkcd:dark gray')
    ax.plot(com['t_ROLL_RATE'],com['ROLL_RATE']*180/pi,'--',color='xkcd:red')
    ax2 = ax.twinx()
    ax2.plot(com['t_MOMENT_X'],com['MOMENT_X'])
    ...
    ax = subfig[0].add_subplot(gs[0][3],sharex=ax_share)
    ax.plot(tstate,p*180/pi,color='xkcd:dark gray')
    ax.plot(com['t_PITCH'],com['PITCH']*180/pi,'--',color='xkcd:red')
    setup_subplot(ax,ylabel='Pitch')

    ax = subfig[0].add_subplot(gs[0][4],sharex=ax_share)
    ax.plot(tgyr,gyr['y']*180/pi,color='xkcd:dark gray')
    ax.plot(com['t_PITCH_RATE'],com['PITCH_RATE']*180/pi,'--',color='xkcd:red')
    ax2 = ax.twinx()
    ax2.plot(com['t_MOMENT_Y'],com['MOMENT_Y'])
    ...
    ax = subfig[0].add_subplot(gs[0][5],sharex=ax_share)
    ax.plot(tstate,y*180/pi,color='xkcd:dark gray')
    ax.plot(com['t_YAW'],com['YAW']*180/pi,'--',color='xkcd:red')
    setup_subplot(ax,ylabel='Yaw')

    ax = subfig[0].add_subplot(gs[0][6],sharex=ax_share)
    ax.plot(tgyr,gyr['y']*180/pi,color='xkcd:dark gray')
    ax.plot(com['t_YAW_RATE'],com['YAW_RATE']*180/pi,'--',color='xkcd:red')
    ax2 = ax.twinx()
    ax2.plot(com['t_MOMENT_Z'],com['MOMENT_Z'])
    ...
    ax = subfig[1].add_subplot(gs[1][0],sharex=ax_share)
    add_flying_bar(ax,fname)

    ax = subfig[1].add_subplot(gs[1][1],sharex=ax_share)
    ax.plot(tgps,gps['velocity.x'],color='xkcd:green',label='GPS')
    ax.plot(tstate,state['vx'],color='xkcd:dark gray',label='State')
    ax.plot(com['t_X_VEL'],com['X_VEL'],'--',color='xkcd:red',label='Command')
    ax.legend()
    setup_subplot(ax,ylabel='X-Vel')

    ax = subfig[1].add_subplot(gs[1][2],sharex=ax_share)
    ax.plot(tgps,gps['velocity.y'],color='xkcd:green')
    ax.plot(tstate,state['vy'],color='xkcd:dark gray')
    ax.plot(com['t_Y_VEL'],com['Y_VEL'],'--',color='xkcd:red')
    setup_subplot(ax,ylabel='Y-Vel')

    ax = subfig[1].add_subplot(gs[1][3],sharex=ax_share)
    ax.plot(tgps,gps['velocity.z'],color='xkcd:green')
    ax.plot(tstate,-state['h_dot'],color='xkcd:dark gray')
    ax.plot(com['t_VRATE'],-com['VRATE'],'--',color='xkcd:red')
    ax2 = ax.twinx()
    ax2.plot(com['t_THRUST'],com['THRUST'])
    ...
    ax = subfig[1].add_subplot(gs[1][4],sharex=ax_share)
    ax.plot(tstate,state['altitude'],color='xkcd:dark grey')
    ax.plot(com['t_ALTITUDE'],com['ALTITUDE'],'--',color='xkcd:red')
    ax.plot(tgps,gps['altitude'],color='xkcd:green')
    ax.set_ylim([1580,1650])
    setup_subplot(ax,ylabel='Alt')

    return ax

def plot_vt_control(fname,ax_share=None):
    ...
    subfig,gs,ax_share,ax = create_subplotN([6,5],ttl = 'VTOL Control',ax_share = ax_share)
    add_flying_bar(ax,fname)
    ...
    ax.plot(com['t_X_VEL'],com['X_VEL'],'--',color='xkcd:red')
    setup_subplot(ax,ylabel='X-Vel')
    ...
    ax = subfig[1].add_subplot(gs[1][5],sharex=ax_share)
    ax.plot(tstate,state['ias'],color='xkcd:dark grey')
    ax.plot(com['t_SPEED'],com['SPEED'],'--',color='xkcd:red')
    ax.plot(tgps,gps['speed'],color='xkcd:green')
    setup_subplot(ax,ylabel='IAS')
...
def plot_sys_status(fname,ax_share = None,i_factor=1):
    sys_status,tsys_status = get_var(fname,'sys_status')
    sys_status['batt_current'] *= i_factor

    fig,gs,ax_share,ax = create_subplot(6,ttl = 'System Status',ax_share = ax_share)
    add_flying_bar(ax,fname)
...
def get_gps_info(fname):
    ...
    for i in range(0, len(filt_lats)):
        ...
        coords.append([filt_lats[i], filt_lons[i], filt_alts[i]])

    return start_time, end_time, avg_lat, avg_lon, coords
