...
def add_time_err(fname):
    # Variables to run
    tvars = ['/SENSORS_GPS_time','/SENSORS_ACCELEROMETER_time','/SENSORS_GYROSCOPE_time','/SENSORS_MAGNETOMETER_time','/SENSORS_DYNAMIC_PRESSURE_time','/SENSORS_STATIC_PRESSURE_time']
    name_key = ['gps_','acc_','gyr_','mag_','dynp_','statp_']
    hf = h5py.File(fname, 'r')
    for idx,var in enumerate(tvars):
        err_data = get_time_err(hf,var,name_key[idx])
        if idx == 0:
...
def get_time_err(hf,var,key):
    if hf.__contains__(var):
        t = hf[var]
        if len(t) > 1:
            dt = np.diff(hf[var])[1:]
...
def performance_plot(DF=None, repo_ncname=None,fromFile=None,fdir = '/home/stachura/usb/flight_testing/'):
    ...
    lu.plot_traj(fdir+df['filename'].item(),ax=ax,bmap=True)
...
def err_calc(Y1,Y2):
    ...
    idx = np.where(~np.isnan(Y1))
    Y1 = Y1[idx]
    Y2 = Y2[idx]
    idx = np.where(~np.isnan(Y2))
    Y1 = Y1[idx]
    Y2 = Y2[idx]
