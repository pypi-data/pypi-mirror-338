...
def get_sucursal(modulo='VTAS', mov='Servicio', sucursal=0, using=None):
    from isapilib.models import Sucursal
    if issubclass(type(sucursal), Sucursal): sucursal = sucursal.pk
    pk = execute_fn("fnCA_GeneraSucursalValida", [modulo, mov, sucursal], using=using)
    return Sucursal.objects.get(pk=pk)


def get_almacen(modulo='VTAS', mov='Servicio', sucursal=0, using=None):
    from isapilib.models import Sucursal, Almacen
    if issubclass(type(sucursal), Sucursal): sucursal = sucursal.pk
    pk = execute_fn("fnCA_GeneraAlmacenlValido", [modulo, mov, sucursal], using=using)
...
def get_param_empresa(interfaz, clave, default=None, using=None):
    result = execute_fn('fnCA_BusquedaClaveParametroInterfazEmpresa', [interfaz, clave], using=using)
...
def get_param_sucursal(sucursal, clave, default=None, using=None):
    from isapilib.models import Sucursal
    if issubclass(type(sucursal), Sucursal): sucursal = sucursal.pk
    result = execute_fn('fnCA_CatParametrosSucursalValor', [sucursal, clave], using=using)
    return result if result is not None and result != '' else default
