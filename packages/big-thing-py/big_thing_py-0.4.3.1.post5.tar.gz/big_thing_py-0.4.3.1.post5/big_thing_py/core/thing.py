...
class MXThing:
    ...
    def __init__(
        ...
        self._value_update_callback_table: Dict[str, List[MXValue]] = {}
    ...
    def add_dependency(
        ...
        if not isinstance(function, MXFunction):
            raise MXTypeError(f'function must be MXFunction object')
        ...
        value_update_callback_info = dict(
            value=value,
            callback=callback,
        )
        if function.name not in self.value_update_callback_table:
            self._value_update_callback_table[function.name] = [value_update_callback_info]
        else:
            self._value_update_callback_table[function.name].append(value_update_callback_info)
    ...
    @property
    def value_update_callback_table(self) -> Dict[str, List[MXValue]]:
        return self._value_update_callback_table
