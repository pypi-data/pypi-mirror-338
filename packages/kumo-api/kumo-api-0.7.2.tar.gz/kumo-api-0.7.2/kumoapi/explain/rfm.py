...
TimeMetric = Dict[str, Tuple[float, TimeUnit]]
IntTimeMetric = Dict[str, Tuple[int, TimeUnit]]
...
class BaseRFM:
    ...
    most_recent_rel_time: IntTimeMetric = field(default_factory=dict)
...
class NodePredRFM(BaseRFM):
    ...
    """
    unique_counts: Dict[str, Dict[str, int]] = field(default_factory=dict)
...
class LinkPredRFM(BaseRFM):
    ...
    Args:
        num_hist_items (int): The number of historical items for each entity.
        num_pred_hist_items (int): The number of predicted and historical
            overlapped items for each entity.
        num_true_items (int): The number of truth items for each entity.
        num_true_hist_items (int): The number of true and historical overlapped
            items for each entity.
    """
    num_hist_items: int = 0
    num_pred_hist_items: int = 0
    num_true_items: Optional[int] = None
    num_true_hist_items: Optional[int] = None
