...
class TestSim3DSplitCombine(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.struct = tg.struct3d.StructDiscretizedCubic3D(
            discretization_config=tg.struct3d.cuboid(l=10, w=3, h=3),
            step=30,
    ...
    def test_gpm_vs_effpola_model(self):
        for device in self.devices:
            ...
            sim_gpm = tg.Simulation(
                structures=s_gpm1,
                linearsystem=tg.linearsystem.LinearSystemFullInverse(),
                **sim_conf,
            )
            ...
            wl = self.wls[0]
            r_probe = tg.tools.geometry.coordinate_map_2d_square(1000, 11, r3=100)
            ...
            assert torch.abs((nf_eff.efield - nf_gpm.efield).mean()) < 0.0003
    ...
    def test_gpm_translation(self):
        for device in self.devices:
            ...
            r_gpm_0 = torch.as_tensor(
                [[-150, 0, 0], [0, 0, 0], [150, 0, 0]], device=device
            )
            ...
            with warnings.catch_warnings():
                ...
                # compare fields
                r_probe = tg.tools.geometry.coordinate_map_2d_square(1000, 3, r3=800)
                for wl in self.wls:
                    ...
                    # print(nf1.efield)
                    # print(nf2.efield)
                    torch.testing.assert_close(nf1.efield, nf2.efield)
                    torch.testing.assert_close(nf1.hfield, nf2.hfield)
    ...
    def test_gpm_rotation(self):
        for device in self.devices:
            ...
            with warnings.catch_warnings():
                ...
                # compare fields
                r_probe = tg.tools.geometry.coordinate_map_2d_square(500, 3, r3=300)
                for wl in self.wls:
                    nf1 = sim_gpm_a.get_nearfield(wl, r_probe, progress_bar=False)[
                        "sca"
                    ]
...
if __name__ == "__main__":
    print("testing global polarizability matrix simulations...")
