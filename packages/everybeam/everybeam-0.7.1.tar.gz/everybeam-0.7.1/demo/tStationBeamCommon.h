...
void calculateStationBeams(
  ...
  loop.Run(
      0, stations.size(),
      [&, subgrid_size, n_stations](size_t start_station, size_t end_station) {
        for (size_t s = start_station; s < end_station; ++s) {
          for (unsigned y = 0; y < subgrid_size; y++) {
            for (unsigned x = 0; x < subgrid_size; x++) {
              ...
              stations[s]->Response(&gainMatrix, time, std::span(&frequency, 1),
                                    direction, freq_beamformer,
