...
BOOST_AUTO_TEST_CASE(element_response_dipole_zero) {
  const everybeam::OSKARElementResponseDipole element_response;
  const aocommon::MC2x2 zero_response = element_response.Response(0, 0, 0);
  for (int i = 0; i < 4; ++i) {
    BOOST_CHECK(std::abs(zero_response.Get(i).real()) < 1.0e-9);
    BOOST_CHECK(std::abs(zero_response.Get(i).imag()) < 1.0e-9);
  }
}
...
BOOST_AUTO_TEST_CASE(element_response_dipole_real) {
  ...
  const std::array<double, 4> expected_response_real = {
      -0.2251034054922027, 0.22760323166697446, 0.12765011577270144,
      0.43246524595460084};
