...
BOOST_AUTO_TEST_CASE(element_response) {
  ...
  station.ComputeElementResponse(&element_response, time, frequency_list,
                                 direction, false, true);

  // Check whether element_response and target_element_response are "equal"
  for (size_t i = 0; i != 4; ++i) {
    BOOST_CHECK_LT(
        std::abs(element_response.Get(i) - target_element_response.Get(i)),
        1e-6);
  }
  ...
  aocommon::MC2x2 station63_response;
  station63.Response(&station63_response, time, frequency_list, direction_s63,
                     frequency, station0_dir, tile0_dir);
  ...
  for (size_t i = 0; i != 4; ++i) {
    BOOST_CHECK_LT(
        std::abs(station63_response.Get(i) - target_station_response.Get(i)),
        1e-6);
  }
}

BOOST_AUTO_TEST_CASE(element_response_multi) {
  const LOFAR& lofartelescope = static_cast<const LOFAR&>(*telescope.get());
  // Compute and check the Station::ComputeElementResponse for
  // a "randomly selected"  station (station 11)
  // NOTE: this is a regression test in the sense that we only check whether
  ...
  aocommon::MC2x2 target_element_response(
      {-0.164112, -0.000467162}, {-0.843709, -0.00123631},
      {-0.892528, -0.00126278}, {0.0968527, -6.7158e-05});
  const std::vector<double> frequency_list({frequency, frequency});
  const Station& station = lofartelescope.GetStation(11);

  std::vector<aocommon::MC2x2> element_response(frequency_list.size());
  station.ComputeElementResponse(element_response.data(), time, frequency_list,
                                 direction, false, true);
  ...
  for (size_t f = 0; f < frequency_list.size(); f++) {
    for (size_t i = 0; i != 4; ++i) {
      BOOST_CHECK_LT(
          std::abs(element_response[f].Get(i) - target_element_response.Get(i)),
          1e-6);
    }
  }
  ...
  // Check the array factor
  std::vector<aocommon::MC2x2Diag> multi_array_factor(frequency_list.size());
  station63.ArrayFactor(multi_array_factor.data(), time, frequency_list,
                        direction_s63, frequency, station0_dir, tile0_dir);

  aocommon::MC2x2Diag single_array_factor;
  std::span single_frequency(&frequency, 1);
  station63.ArrayFactor(&single_array_factor, time, single_frequency,
                        direction_s63, frequency, station0_dir, tile0_dir);

  // Check whether computing the array factor for one or multiple frequency is
  // equivalent
  for (size_t f = 0; f < frequency_list.size(); f++) {
    for (size_t i = 0; i < 2; ++i) {
      BOOST_CHECK_LT(
          std::abs(multi_array_factor[f].Get(i) - single_array_factor.Get(i)),
          1e-6);
    }
  }

  std::vector<aocommon::MC2x2> station63_response(frequency_list.size());
  station63.Response(station63_response.data(), time, frequency_list,
                     direction_s63, frequency, station0_dir, tile0_dir);
  ...
  for (size_t f = 0; f < frequency_list.size(); ++f) {
    for (size_t i = 0; i != 4; ++i) {
      BOOST_CHECK_LT(std::abs(station63_response[f].Get(i) -
                              target_station_response.Get(i)),
                     1e-6);
    }
  }
}
...
BOOST_AUTO_TEST_CASE(point_response_caching_multi) {
  BOOST_CHECK_EQUAL(point_response->HasTimeUpdate(), true);
  std::vector<double> frequencies({frequency, frequency});
  std::vector<aocommon::MC2x2F> point_buffer_1(2);
  const BeamMode beam_mode = BeamMode::kFull;
  point_response->Response(beam_mode, point_buffer_1.data(), coord_system.ra,
                           coord_system.dec, frequencies, 23, 0);
  ...
  point_response->UpdateTime(time + 199);
  BOOST_CHECK_EQUAL(point_response->HasTimeUpdate(), false);
  std::vector<aocommon::MC2x2F> point_buffer_2(2);
  point_response->Response(beam_mode, point_buffer_2.data(), coord_system.ra,
                           coord_system.dec, frequencies, 23, 0);

  for (size_t i = 0; i != point_buffer_1.size(); ++i) {
    for (size_t p = 0; p != 4; ++p)
      BOOST_CHECK_CLOSE(point_buffer_1[i].Get(p), point_buffer_2[i].Get(p),
                        1e-6);
  }
  ...
  for (size_t i = 0; i != point_buffer_1.size(); ++i) {
    BOOST_CHECK_PREDICATE(std::not_equal_to<aocommon::MC2x2F>(),
                          (point_buffer_1[i])(point_buffer_2[i]));
  }
}
...
BOOST_AUTO_TEST_CASE(gridded_response_array_factor) {
  ...
  aocommon::MC2x2 full_beam_p13;
  station.Response(&full_beam_p13, time, std::span(&frequency, 1),
                   direction_p13, frequency, station0, station0);
  aocommon::MC2x2 element_beam_p13;
  station.ComputeElementResponse(&element_beam_p13, time,
                                 std::span(&frequency, 1), direction_p13, false,
                                 true);
  aocommon::MC2x2Diag array_factor_p13;
  station.ArrayFactor(&array_factor_p13, time, std::span(&frequency, 1),
                      direction_p13, frequency, station0, station0);
  aocommon::MC2x2 full_beam_product_p13 = array_factor_p13 * element_beam_p13;
  for (std::size_t i = 0; i < 4; ++i) {
    // Tolerance is a percentage, so 1e-2 --> 1e-4
    BOOST_CHECK_CLOSE(full_beam_product_p13.Get(i), full_beam_p13.Get(i), 1e-2);
  }

  vector3r_t direction_p22 = {0.408326, 0.527345, 0.745102};
  aocommon::MC2x2 full_beam_p22;
  station.Response(&full_beam_p22, time, std::span(&frequency, 1),
                   direction_p22, frequency, station0, station0);
  aocommon::MC2x2 element_beam_p22;
  station.ComputeElementResponse(&element_beam_p22, time,
                                 std::span(&frequency, 1), direction_p22, false,
                                 true);
  aocommon::MC2x2Diag array_factor_p22;
  station.ArrayFactor(&array_factor_p22, time, std::span(&frequency, 1),
                      direction_p22, frequency, station0, station0);
  aocommon::MC2x2 full_beam_product_p22 = array_factor_p22 * element_beam_p22;

  for (std::size_t i = 0; i < 4; ++i) {
    // Tolerance is a percentage, so 1e-2 --> 1e-4
    BOOST_CHECK_CLOSE(full_beam_product_p22.Get(i), full_beam_p22.Get(i), 1e-2);
  }
}
