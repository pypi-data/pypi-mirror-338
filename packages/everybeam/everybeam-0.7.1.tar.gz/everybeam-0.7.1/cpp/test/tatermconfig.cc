  ...
  for (size_t freq_index = 0; freq_index < n_channels; ++freq_index) {
    for (size_t time_index = 0; time_index < n_times; ++time_index) {
      ...
      if (!is_updated) {
        ...
        previous_image_index = (time_index - 1) * n_channels + freq_index;
        std::complex<float>* previous_buffer =
            aterm_buffer.data() + previous_image_index * single_buffer_size;
        std::copy_n(previous_buffer, single_buffer_size, buffer);
      }
    }
  }
}
...
BOOST_AUTO_TEST_CASE(combine_aterms) {
  ...
  for (size_t j = 0; j != aterm_buffer_size; j += kAtermMatrixSize) {
    ...
    for (size_t i = 0; i < 4; ++i) {
      BOOST_CHECK_CLOSE(product.Get(i), aterm_buffer_combined[j + i], 1.0e-4);
    }
  }
}
