...
JonesMatrix Beam2016Implementation::CalcJones(double az_deg, double za_deg,
                                              int freq_hz_param,
                                              bool bZenithNorm) {
  return CalcJones(az_deg, za_deg, freq_hz_param, _delays, _amps, mutex_,
                   bZenithNorm);
}
...
JonesMatrix Beam2016Implementation::CalcZenithNormMatrix(
    int freq_hz, std::recursive_mutex& lock) {
...
const std::vector<std::vector<double>>& Beam2016Implementation::GetDataSet(
    const DataSetIndex& index, std::recursive_mutex& lock) {
  std::lock_guard glock(lock);
