...
class [[gnu::visibility("default")]] Station {
 public:
  ...
  aocommon::MC2x2 Response(double time, double freq,
    ...
    Response(&result, time, std::span(&freq, 1), direction, freq0, station0,
             tile0, rotate);
    return result;
  }
   ...
   * \see CorrectionMode.
   */
  void Response(aocommon::MC2x2 * result, CorrectionMode mode, double time,
                const std::span<const double>& freqs,
                const vector3r_t& direction, double freq0,
                const vector3r_t& station0, const vector3r_t& tile0,
                const bool is_local = false, const bool rotate = true) const;

  aocommon::MC2x2 Response(CorrectionMode mode, double time, double freq,
                           const vector3r_t& direction, double freq0,
                           const vector3r_t& station0, const vector3r_t& tile0,
                           const bool is_local = false,
                           const bool rotate = true) const {
    aocommon::MC2x2 result;
    Response(&result, mode, time, std::span(&freq, 1), direction, freq0,
             station0, tile0, is_local, rotate);
    return result;
  }
   ...
   */
  void ComputeElementResponse(aocommon::MC2x2 * result, double time,
                              const std::span<const double>& freqs,
