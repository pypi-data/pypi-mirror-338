...
def read_telescope_center(oskar_telescope_dir: str):
    ...
    oskar_telescope_center = pd.read_csv(
        f"{oskar_telescope_dir}/position.txt", header=None, sep=" "
    )

    if len(oskar_telescope_center.columns) == 2:
        oskar_telescope_center["Height"] = [0.0]

    oskar_telescope_center.columns = ["Lon", "Lat", "Height"]
    telescope_center_itrf = geo.xyz_from_geographic(
        np.deg2rad(oskar_telescope_center["Lon"][0]),
...
def fix_antenna(oskar_ms_name: str, telescope_center_itrf: np.array):
    ...
    if np.isclose(norm(anttable[0]["POSITION"]), 0, atol=1e-3):
        anttable.putcol(
            "POSITION", anttable.getcol("POSITION") + telescope_center_itrf
        )
    else:
...
def add_array_center(oskar_ms_name: str, telescope_center_itrf: np.array):
    ...
    obstable = pt.table(
        f"{oskar_ms_name}::OBSERVATION", readonly=False, ack=False
    )
...
def add_phased_array_table(oskar_ms_name: str):
    ...
    anttable = pt.table(f"{oskar_ms_name}::ANTENNA", ack=False)
    phasedarraytable = pt.table(
        f"{oskar_ms_name}/PHASED_ARRAY",
        pt.maketabdesc([]),
        nrow=anttable.nrows(),
    )
    ...
    coordinate_system_coldesc = pt.makearrcoldesc(
        "COORDINATE_AXES",
        0.0,
        shape=[3, 3],
    ...
    element_offset_coldesc = pt.makearrcoldesc(
        "ELEMENT_OFFSET",
        0.0,
        ndim=2,
    ...
    element_flag_coldesc = pt.makearrcoldesc(
        "ELEMENT_FLAG",
        0.0,
        ndim=2,
        comment="Offset per element",
...
def fill_phased_array(oskar_ms_name: str, oskar_telescope_dir: str):
    ...
    element_locations = pd.read_csv(
        f"{oskar_telescope_dir}/station/layout.txt", header=None, sep=" "
    )
    ...
    normal_vector_ellipsoid = geo.normal_vector_ellipsoid(
        np.deg2rad(oskar_telescope_center["Lon"][0]),
        np.deg2rad(oskar_telescope_center["Lat"][0]),
    )

    local_to_itrf_projection_matrix = geo.projection_matrix(
        telescope_center_itrf, normal_vector_ellipsoid
    )

    element_locations_itrf = (
        local_to_itrf_projection_matrix @ element_locations.to_numpy().T
    )
    nr_pol = 2
    all_unflagged = np.zeros((nr_pol, element_locations_itrf.shape[1]))

    phasedarraytable = pt.table(
        f"{oskar_ms_name}::PHASED_ARRAY", readonly=False, ack=False
    )

    for stationnr in range(len(phasedarraytable)):
        phasedarraytable.putcell(
            "COORDINATE_AXES", stationnr, local_to_itrf_projection_matrix.T
        )
        phasedarraytable.putcell(
            "ELEMENT_OFFSET", stationnr, element_locations_itrf
        )
