...
def form_merge(form: Form, language="English (en)") -> pd.DataFrame:
    ...
    for k, v in reps.items():
        out = pd.merge(left=out.set_index(f"{'' if rstruct[k[len(o.form)+1:]] == None else rstruct[k[len(o.form)+1:]]+'-'}KEY", drop=False), right=v.rename(
            columns={"KEY": f"{k[len(o.form)+1:]}-KEY"}).set_index('PARENT_KEY'), how='outer', left_index=True, right_index=True)
        drops = []
