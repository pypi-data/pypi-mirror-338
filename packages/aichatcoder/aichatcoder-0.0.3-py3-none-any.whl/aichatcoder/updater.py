...
def check_for_updates(current_version):
    ...
    try:
        ...
        if version.parse(latest_version) > version.parse(current_version):
            click.echo(f"[UPDATE AVAILABLE] New version {latest_version} is available! You are using {current_version}.")
            ...
            click.echo(f"[OK] You are using the latest version {current_version}.")
        return False
    except (requests.RequestException, KeyError, ValueError):
        click.echo("[FAILED] Could not check for updates. Proceeding with current version.")
        return False
