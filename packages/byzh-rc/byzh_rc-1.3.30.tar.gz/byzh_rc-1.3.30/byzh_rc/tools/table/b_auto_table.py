...
class BAutoTable:
    ...
    def read_txt(self, path):
        ...
        row_name, col_name = temp[0][0].split(' \\ ') if ('\\' in temp[0][0]) else ("x", "y")
        row_sidebar = [var[0] for var in temp[1:]]
        col_sidebar = temp[0][1:]
        ...
        for i in range(len(row_sidebar)):
            for j in range(len(col_sidebar)):
                self[row_sidebar[i]][col_sidebar[j]] = temp[i+1][j+1]
...
if __name__ == '__main__':
    my_table = BAutoTable("x", "y", auto_adaptive=False)
