...
def compute_sasa_pymol(pdb_path, remove_ligand=False):
    ...
    if USE_PYMOL2:
        try:
            ...
            logging.error(Fore.RED + Style.BRIGHT + f"[ERROR] Failed to start PyMOL2: {e}")
            return None
    else:
        try:
            ...
            logging.error(Fore.RED + Style.BRIGHT + f"[ERROR] Failed to initialize PyMOL: {e}")
            return None
    logging.info("Loading " + pdb_file + " in pymol for SASA computation...")
    try:
        cmd.load(pdb_file, "protein")
    except Exception as e:
        logging.error(Fore.RED + Style.BRIGHT + f"[ERROR] Failed to load PDB file '{pdb_file}': {e}")
        return None
    if remove_ligand:
        cmd.remove("hetatm")
...
def evaluate_sasa_contact(pocket_atom_indices, sasa_before, sasa_after, ligand_coords, protein_coords, threshold=SASA_CONTACT_THRESHOLD):
    contact_atoms = []
