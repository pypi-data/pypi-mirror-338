...
import logging
from config import SIMPLE_SPLIT, ATOM_SPLITTING, ATOM_SPLITTING_MODE, SAVE_ALPHA, VERSION_TAG, OUTPUT_DIR,LIGAND_THRESHOLD,BETA_RANKING,ALPHA_RANKING
import os
import pandas as pd

def compute_ligand_contact_distance(protein_coords, ligand_coords, threshold=LIGAND_THRESHOLD):
...
def compute_mouth_parameters(ranked_pockets, simplices, protein_coords, ligand_coords=None):
    ligand_center = None
    ...
    for pocket in ranked_pockets:
        ...
        for face in boundary_faces:
            ...
            total_circumference += sum(np.linalg.norm(protein_coords[e[0]] - protein_coords[e[1]]) for e in edges)
            rim_atoms.update(face)
        mouth_center = np.mean(face_centroids, axis=0) if face_centroids else np.zeros(3)
        if ligand_coords is not None and len(ligand_coords) > 0:
        ...
        mouth_params_list.append({
            ...
            "ligand_distance": round(ligand_distance, 2) if ligand_distance is not None else None
        })
    return mouth_params_list



def compute_ranking_scores(pocket_volumes, pockets, alpha_ranking=ALPHA_RANKING, beta_ranking=BETA_RANKING):
    ranking_scores = []
...
def merge_nearby_clusters(pockets, tetra_positions, threshold):
    ...
    while changed:
        ...
        for i in range(len(clusters)):
            ...
            if not cluster_i:
                continue
            tetra_coords_i = np.concatenate([tetra_positions[idx] for idx in cluster_i], axis=0)
            ...
            for j in range(i + 1, len(clusters)):
                ...
                if not cluster_j:
                    continue
                tetra_coords_j = np.concatenate([tetra_positions[idx] for idx in cluster_j], axis=0)
...
def save_pocket_properties(result: dict, pdb_id: str, output_dir: str):
    ...
    for i in range(n):
        data.append({
            ...
            "min_ligand_mouth_dist": result.get("ligand_mouth_min_distance", [None])[i],
            ...
            if common_atoms:
                ...
                for atom in common_atoms:
                    ...
                    if ATOM_SPLITTING_MODE.upper() == "CAST":
                        frac1, frac2 = split_atom_contribution(atom_coord, atom_radius, pocket_center1, pocket_center2)
                    else:
                        frac1, frac2 = 0.5, 0.5
                    logging.info(f"Atom {atom} shared between pockets {i} and {j}: split fractions {frac1:.2f}, {frac2:.2f}")
    return pockets
