...
def compute_ranking_scores(volumes, pockets, flow_steps, connectivity_graph, tetra_positions):
    import numpy as np
    import logging
    scores = []
    for i, (volume, pocket) in enumerate(zip(volumes, pockets)):
        ...
        logging.debug(f"Pocket {i+1}: volume={volume:.2f}, steps={n_steps}, connections={n_connections}, score={score:.2f}")
        scores.append(score)
    return scores
