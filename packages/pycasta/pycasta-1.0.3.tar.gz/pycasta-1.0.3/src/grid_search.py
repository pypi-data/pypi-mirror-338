...
from config import OUTPUT_DIR, VERSION_TAG, MANUAL_ALPHA, MIN_POCKET_VOLUME, MERGE_THRESHOLD, FLOW_KWARGS
...
def evaluate_result(result, target_min=1, target_max=3):
    ...
    if target_min <= num_pockets <= target_max:
        ...
        penalty = abs(num_pockets - (target_min if num_pockets < target_min else target_max))
        return penalty
...
for combo in parameter_grid:
    ...
    logging.info(f"Testing parameters: alpha={alpha_val}, volume_threshold={vol_thresh}, merge_threshold={merge_thresh}, flow_tol={flow_tol}, max_steps={flow_max}, adaptive_factor={flow_adapt}")
    result = process_pdb(test_pdb)
    score = evaluate_result(result)
    logging.info(f"Result: {len(result.get('ranked_pockets', []))} pockets, score={score}")
...
output_file = os.path.join(OUTPUT_DIR, f"parameter_grid_search_summary_{VERSION_TAG}.json")
