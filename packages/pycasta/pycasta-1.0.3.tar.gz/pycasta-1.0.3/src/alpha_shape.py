...
import numpy as np
import logging
from config import SIMPLE_SPLIT, ATOM_SPLITTING, ATOM_SPLITTING_MODE, SAVE_ALPHA, VERSION_TAG, OUTPUT_DIR, FILTER_AFTER_MERGE,SASA_CONTACT_THRESHOLD
...
def filter_alpha_complex_by_sasa(tetrahedra, alpha_values, protein_coords, pdb_file, sasa_threshold):
    ...
    logging.info(f"Tetrahedra: {total_tetra} total, {len(filtered_indices)} pass the SASA filter (threshold={sasa_threshold}).")
...
def compute_alpha_complex_from_tetrahedra(simplices, tetra_positions, alpha_value, molecule_name=None, 
                                            protein_coords=None, filter_by_sasa=False, sasa_threshold=SASA_CONTACT_THRESHOLD, pdb_file=None):
    """
    ...
    """
    assert tetra_positions.ndim == 3 and tetra_positions.shape[1:] == (4, 3), \
    ...
    if SAVE_ALPHA and molecule_name and protein_coords is not None:
        ...
        np.savez(out_file, alpha_values=radii, alpha_mask=alpha_mask, 
                 protein_coords=protein_coords, tetrahedra=tetra_positions)
    ...
    if filter_by_sasa:
        if pdb_file is None:
            ...
            filtered_tetra, filtered_alpha_values, filtered_mask = filter_alpha_complex_by_sasa(
            ...
            version_dir = os.path.join(OUTPUT_DIR, VERSION_TAG)
            os.makedirs(version_dir, exist_ok=True)
            out_file_filtered = os.path.join(version_dir, f"{molecule_name}.alpha_filtered.npz")
            np.savez(out_file_filtered, alpha_values=radii, alpha_mask=alpha_mask, 
                     protein_coords=protein_coords, tetrahedra=tetra_positions)
...
def flow_detection(simplices, alpha_mask, tetra_positions, protein_coords, flow_params=None):
    """
    ...
    for i, simplex in enumerate(simplices):
        faces = [tuple(sorted(simplex[[a, b, c]])) 
    ...
    for i in empty_indices:
        faces = [tuple(sorted(simplices[i, [a, b, c]]))
                 for (a, b, c) in [(0, 1, 2), (0, 1, 3), (0, 2, 3), (1, 2, 3)]]
        neighbors = set()
        for face in faces:
    ...
    for i in empty_indices:
        ...
        while steps < max_steps:
            ...
            lower_neighbors = [j for j in neighbor_dict[current] 
    ...
    final_pockets = apply_splitting_to_shared_atoms(final_pockets, protein_coords, flow_params.get('atom_radii'))
