...
import trimesh
from config import SAVE_EXTRUDED_MESHES, OUTPUT_DIR,MESH_EXTRUSION_DISTANCE,LIGAND_THRESHOLD
...
def validate_ligand_in_extruded_mesh(mouth_coords, ligand_coords, extrusion_distance=MESH_EXTRUSION_DISTANCE):
...
def log_radii_statistics(radii):
    stats = {"min": float(np.min(radii)), "max": float(np.max(radii)), "mean": float(np.mean(radii))}
    import logging
...
def monte_carlo_pocket_volume(tetra_positions, pocket_indices, num_samples=10000):
    ...
    random_points = np.random.uniform(low=min_bounds, high=max_bounds, size=(num_samples, 3))
    count_inside = 0
...
def compute_circumcenter(tetra):
    ...
    rhs = 0.5 * np.array([np.dot(B - A, B - A), np.dot(C - A, C - A), np.dot(D - A, D - A)])
    try:
        sol = np.linalg.solve(M, rhs)
        circumcenter = A + sol
    except np.linalg.LinAlgError:
