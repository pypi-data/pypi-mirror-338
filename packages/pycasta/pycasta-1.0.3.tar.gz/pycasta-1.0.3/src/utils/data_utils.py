...
def summarize_and_print_results(results, analysis_type="single", output_file=None, csv_output_file=None):
    ...
    valid_steps = [res["step_to_ligand"] for res in results if isinstance(res.get("step_to_ligand"), int)]
    ...
    pocket_volumes = [vol for res in results for vol in res.get("pocket_volumes", []) if isinstance(vol, (int, float))]
    pocket_depths = [depth for res in results for depth in res.get("pocket_depths", []) if isinstance(depth, (int, float))]
    mouth_areas = [area for res in results for area in res.get("mouth_area", []) if isinstance(area, (int, float))]
    mouth_perimeters = [perim for res in results for perim in res.get("mouth_perimeter", []) if isinstance(perim, (int, float))]
    ...
    if is_paired:
        header = ["File Pair", "Top1", "Top3", "Top5", "Rank (B/U)", "#Pockets", "#Validated", "Validation", "Contact", "Min. Dist. (Å)", "RMSD (Å)"]
        for r in results:
            ...
            table.append([file_pair, top1_val, top3_val, top5_val, rank_info, num_pockets, num_validated, validation_used, contact, min_distance, rmsd])

        print("\nPaired Analysis Summary:")
        print(tabulate(table, headers=header, tablefmt="github"))

    else:
        header = ["File", "Top1", "Top3", "Top5", "Top All", "Rank", "Validation", "#Pockets", "#Validated", "Contact", "Min. Dist. (Å)", "Avg. Volume", "Avg. Depth"]
        for res in results:
            ...
            table.append([filename, top1_val, top3_val, top5_val, top_all, rank, validation_used, num_pockets, num_validated, contact, min_distance, avg_volume, avg_depth])
...
def save_pocket_results_csv(result, pdb_id, output_dir="results"):
    ...
    for i in range(num_pockets):
        rows.append({
            ...
            "ligand_mouth_distance": result.get("ligand_mouth_min_distance", [None])[i],
