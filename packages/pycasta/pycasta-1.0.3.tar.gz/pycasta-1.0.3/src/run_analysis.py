...
from config import (
    BOUNDED_DIR, UNBOUNDED_DIR, DATASET_DIR, OUTPUT_DIR,
    INCLUDE_WATER, MIN_POCKET_VOLUME, FLOW_KWARGS,
    MERGE_CLUSTERS, MERGE_THRESHOLD, USE_EXISTING_RESULTS, VERSION_TAG,
    CORRESPONDENCE_FILE, VOLUME_METHOD, SIMPLE_SPLIT, USE_CGAL, USE_SASA,
    FILTER_ALPHA_BY_SASA, SASA_THRESHOLD, SASA_METHOD, LIGAND_CONTACT_METHOD,
    LIGAND_CONTACT_THRESHOLD, NUM_POCKETS_TO_SAVE, STRICT_DISTANCE_THRESHOLD,
    USE_SASA_CONTACT_VALIDATION, SASA_CONTACT_THRESHOLD,
    MESH_EXTRUSION_DISTANCE, NUM_POCKETS_TO_VALIDATE, VALIDATION_METHOD,
    FAKE_SPHERE_RADIUS, SAVE_EXTRUDED_MESHES
)
...
from utils.data_utils import (
    save_pocket_results_csv, save_json, load_json,
    convert_numpy_types, summarize_and_print_results
)
...
from utils.sasa_utils import (
    compute_sasa, compute_ligand_contact_sasa, evaluate_sasa_contact
)
from utils.pocket_utils import (
    apply_splitting_to_shared_atoms, compute_volume_for_tetra_group,
    merge_nearby_clusters, compute_mouth_parameters, save_pocket_properties
)
...
def match_atoms_by_residue_and_name(fixed_atoms, moving_atoms, allowed_atom_names=["CA"]):
    ...
    logging.info(f"Matched {len(common_keys)} atoms for alignment based on {allowed_atom_names}.")
    return fixed_coords, moving_coords



def make_dummy_atom(coord):
    return Atom(name="X", coord=coord, bfactor=0.0, occupancy=1.0, altloc=" ", fullname=" X  ", serial_number=1, element="X")
...
def align_using_superimposer(fixed_coords, moving_coords, return_transform=False):
    ...
    if len(fixed_coords) != len(moving_coords):
        logging.warning(f"Different number of atoms: fixed {len(fixed_coords)} vs moving {len(moving_coords)}. Using the first {n} atoms.")
...
def run_analysis_mode(pdb_sources, analysis_type="single", correspondence_df=None):
    ...
    if analysis_type == "paired" and correspondence_df is not None:
        for _, row in correspondence_df.iterrows():
            ...
            if not os.path.exists(bounded_path) or not os.path.exists(unbounded_path):
                logging.warning(f"Skipping pair {bounded_name} & {unbounded_name}: Missing file(s).")
                continue
            ...
            aligned_coords, rmsd, (R, t) = align_using_superimposer(fixed_coords, moving_coords, return_transform=True)
            ...
            logging.info(f"Alignment RMSD (filtered on {ALIGN_ATOM_NAMES}): {rmsd:.3f} Å")
            ...
            "ligand_to_pocket_distances": bounded_result.get("ligand_to_pocket_distances", []),
            "ligand_mesh_distances": bounded_result.get("ligand_mesh_distances", []),
            "ligand_containment_mesh": bounded_result.get("ligand_containment_mesh", []),
            "ligand_containment_strict": bounded_result.get("ligand_containment_strict", []),
            ...
            save_json(bounded_result, os.path.join(versioned_output_dir, f"bounded_{bounded_name}.json"))
            save_json(unbounded_result, os.path.join(versioned_output_dir, f"unbounded_{unbounded_name}.json"))
    ...
    # Single (bounded) mode
        ...
        for pdb_file in pdb_sources:
            ...
            if not result or "ranked_pockets" not in result or len(result["ranked_pockets"]) == 0:
                logging.warning(f"Skipping {pdb_file}: No pockets found.")
                continue
            pdb_name = os.path.splitext(os.path.basename(pdb_file))[0]
            json_path = os.path.join(versioned_output_dir, f"{pdb_name}.json")
            save_json(result, json_path)
            rank_sasa = result.get("step_to_ligand", None)
            rank_mesh = result.get("step_to_ligand_mesh", None)
            chosen_rank = rank_mesh if VALIDATION_METHOD == "mesh_extrusion" else rank_sasa
            result_entry = {
                "pdb_path": pdb_file,
                ...
                "is_top1": (chosen_rank == 1) if isinstance(chosen_rank, int) else False,
                "is_top3": (chosen_rank <= 3) if isinstance(chosen_rank, int) else False,
                "is_top5": (chosen_rank <= 5) if isinstance(chosen_rank, int) else False,
...
def process_pdb(pdb_path):
    ...
    out_file = os.path.join(OUTPUT_DIR, f"{base}_pockets.{VERSION_TAG}.json")

    if USE_EXISTING_RESULTS and os.path.exists(out_file):
        logging.info(f"Using existing results for {pdb_path}")
        cached_result = load_json(out_file)
        if cached_result:
            return cached_result
        logging.warning(f"Existing results for {pdb_path} invalid. Reprocessing.")
    ...
    try:
        simplices = cgal_weighted_delaunay(protein_coords, radii)[0] if USE_CGAL else Delaunay(protein_coords).simplices
    except Exception as e:
    ...
    alpha_mask, radii, tetra_positions, simplices = compute_alpha_complex_from_tetrahedra(
        simplices, tetra_positions, alpha, base, protein_coords, FILTER_ALPHA_BY_SASA,
        SASA_CONTACT_THRESHOLD, pdb_path
    )
    ...
    for idx, pocket in enumerate(ranked_pockets):
        ...
        if valid_indices:
            ...
            logging.warning(f"⚠️ Pocket {idx+1}: No valid tetrahedra. Volume and depth set to 0.")
    ...
    if method == "mesh_extrusion":
        for mouth in mouth_params:
            ...
            in_contact, mesh, min_dist = validate_ligand_in_extruded_mesh(
                mouth_coords,
                ligand_coords,
                extrusion_distance=MESH_EXTRUSION_DISTANCE
            )
            ...
            if SAVE_EXTRUDED_MESHES and mesh:
                ...
                mesh.export(os.path.join(mesh_dir, f"{base}_pocket_{len(validation_methods)}.ply"))
        ...
        for pocket in ranked_pockets:
            ...
            valid = evaluate_sasa_contact(
                unique_atoms, sasa_before, sasa_after,
    ...
    first_valid = next((i+1 for i, m in enumerate(validation_methods) if m != "None"), None)
...
def main():
    ...
    if UNBOUNDED_DIR and os.path.exists(UNBOUNDED_DIR) and CORRESPONDENCE_FILE and os.path.exists(CORRESPONDENCE_FILE):
        ...
        logging.error("Invalid configuration: check UNBOUNDED_DIR and CORRESPONDENCE_FILE settings.")
        sys.exit(1)
    logging.info(f"=== Analysis completed. Results saved in: {versioned_output_dir} ===")
