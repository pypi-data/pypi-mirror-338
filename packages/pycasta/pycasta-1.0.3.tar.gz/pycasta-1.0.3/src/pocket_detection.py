...
def detect_pockets(protein_coords, simplices=None, tetra_positions=None, alpha_mask=None,
                   min_volume_threshold=None, flow_params=None,
                   merge_clusters=None, merge_threshold=None,
                   molecule_name=None, radii=None):
    ...
    if np.max(simplices) > max_index:
        raise ValueError(f"ERROR: Some simplex indices exceed protein_coords size ({max_index}).")
    ...
    logging.info(f"Alpha shape retained {int(np.sum(alpha_mask))} tetrahedra out of {len(alpha_mask)}.")
    ...
    if merge_clusters:
        ...
        logging.info(f"After merging (threshold={merge_threshold}), {len(pockets)} pockets remain.")
    else:
        pockets = valid_pockets
    pocket_volumes = [compute_analytic_pocket_volume(tetra_positions, pocket) for pocket in pockets]
    filtered = [(p, v) for p, v in zip(pockets, pocket_volumes) if v >= min_volume_threshold]
    if not filtered:
        logging.warning(f"No pockets exceed the minimum volume threshold of {min_volume_threshold} Å³.")
        return {
            "ranked_pockets": [],
    ...
    ranking_scores = compute_ranking_scores(pocket_volumes, pockets, flow_steps, connectivity_graph, tetra_positions)
    ranked_data = sorted(zip(pockets, pocket_volumes, ranking_scores), key=lambda x: x[2], reverse=True)
    ranked_pockets, pocket_volumes, ranking_scores = zip(*ranked_data)
    
    for pocket in ranked_pockets:
        dual_info = compute_dual_set_for_pocket(pocket, tetra_positions, radii,
                                                alpha_value=alpha_value, decimals=3)
    representative_points = [np.mean(tetra_positions[p].reshape(-1, 3), axis=0) for p in ranked_pockets]
    logging.info(f"Final number of pockets in {molecule_name}: {len(ranked_pockets)}")
    # Compute dual set information for each pocket:
    dual_sets_info = []  # Will store the dual set info (closure, dual set, boundary, etc.)
    ...
    for pocket in ranked_pockets:
        ...
        dual_info = compute_dual_set_for_pocket(pocket, tetra_positions, radii, alpha_value=alpha_value, decimals=3)
