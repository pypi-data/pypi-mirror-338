...
# import AppIndicator3 as appindicator
...
class Application(Gtk.Application):
    ...
    def print_time_totals(self = None, widget = None):
        ...
        l = ''
        for t in db_query("SELECT DISTINCT id, parent_id, label, parent_label FROM tasks WHERE id IN(SELECT DISTINCT task_id FROM sessions "+utils.sessions_timeframe_sql()+") ORDER BY parent_label ASC "):
    ...
    def mark_done(self, w=None, task = None):
        ...
        try:
            ...
            try:
                self.taskwindow.refresh_search_cache()
            except Exception:
                pass
    ...
    def async_refresh(self, w=None):
        ...
        try:
            self.taskwindow.tasks_box.foreach(lambda child: child.destroy())
            taskwindow_refreshing = Gtk.Label("Checking all todo lists for updates") 
            self.taskwindow.tasks_box.add(taskwindow_refreshing)
            self.taskwindow.tasks_box.show_all()
            self.taskwindow.shown_tasks.clear()
        except Exception:
            pass
