...
def test_embedded_templates_disabled(template: str, expected: t.Any, mocker: pytest_mock.MockerFixture) -> None:
    ...
    # without conditional warnings control, the resulting warning would just be noise. To get warnings properly in the current impl,
    # we'd have to `access_const` *all* string templates, which would be very expensive. Might be something to revisit in the future
    # before removing the deprecated version, either with a hypothetical granular warning control mechanism, or an opt-in "extra checks"
    # mode.
