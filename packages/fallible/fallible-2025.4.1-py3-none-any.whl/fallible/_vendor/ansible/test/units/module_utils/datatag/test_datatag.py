...
# DTFIX-RELEASE: query all available str methods and ensure they're implemented and tested or on an ignore list
...
def test_tagged_str_overrides(method_name: str) -> None:
    plain_value = ' hi mom '
    tagged_value = ExampleSingletonTag().tag(plain_value)
    ...
    if isinstance(plain_result, c.Iterable) and not isinstance(plain_result, (str, bytes)):
        assert all(ExampleSingletonTag.is_tagged_on(value) for value in tagged_result)
        assert AnsibleTagHelper.tags(tagged_result) is _empty_frozenset
    else:
        assert ExampleSingletonTag.is_tagged_on(tagged_result)


def test_tagged_bytes_decode() -> None:
    value = ExampleSingletonTag().tag(b'hi mom').decode()

    assert value == 'hi mom'
    assert ExampleSingletonTag.is_tagged_on(value)
