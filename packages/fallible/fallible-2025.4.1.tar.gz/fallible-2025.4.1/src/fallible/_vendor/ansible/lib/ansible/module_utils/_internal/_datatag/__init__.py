...
class AnsibleSerializableDataclass(AnsibleSerializable, metaclass=abc.ABCMeta):
    ...
    def _as_dict(self) -> t.Dict[str, t.Any]:
        ...
        # DTFIX-MERGE: this isn't recursive like dataclasses.asdict- it works the way we want for JSON serialization, since `default` will iteratively call back
        #          on nested serializable types until the complete object graph has been converted, but other callers need a way to do it recursively upfront.
...
class _AnsibleTaggedStr(str, AnsibleTaggedObject):
    ...
    # DTFIX-MERGE: figure out a way to eliminate these; they pre-date trust inversion and set a bad precedent for the future. static/runtime detection instead?
