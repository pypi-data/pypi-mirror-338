...
class _JSONSerializationProfile(t.Generic[_T_encoder, _T_decoder]):
    ...
    def __init_subclass__(cls, **kwargs) -> None:
        ...
        cls._unwrapped_json_types = (
            frozenset(obj for obj in cls.serialize_map if not issubclass(obj, _json_types)) |  # custom types that do not extend JSON-native types
            frozenset(obj for obj in _json_scalar_types if obj not in cls.serialize_map)  # JSON-native scalars lacking custom handling
        )
...
# DTFIX-MERGE:JSON: all the implementation details and support classes for profiles should probably move to an internal package; even for type hinting,
#                   the public API should only need json.encoder and json.decoder, not our derived base classes.
...
class AnsibleJSONEncoder(json.JSONEncoder):
    ...
    def default(self, o):
        ...
        if isinstance(o, Tripwire):
            ...
            o = o._native_copy()
        elif isinstance(o, AnsibleSerializable):
            if self._preserve_datatags:
                o = o._serialize()
            else:
                o = o._as_dict()
        ...
        elif isinstance(o, self._wrap_container_types):
...
class AnsibleJSONDecoder(json.JSONDecoder):
    """JSON decoder capable of handling Ansible internal types."""
    ...
    def object_hook(pairs) -> object:
        value = AnsibleSerializable._deserialize(pairs)
