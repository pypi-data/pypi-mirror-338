...
class NereidStaticFile(sequence_ordered(), ModelSQL, ModelView):
    ...
    convert_format = fields.Selection('get_image_formats',
        'Convert image format',
        states={
            'invisible': Bool(Eval('mimetype')),
            },
        help='Convert images to the selected format.\n'
        '(Choose .webp for best web performance.)')
    ...
    def get_image_formats(cls):
        extensions = list(Image.registered_extensions().items())
        # Prepare a list that is later parseable for the chosen extension for
        # subtype
        l = [(None, '')]
        for e in extensions:
            value = ''.join([e[0], ' (', e[1], ')'])
    ...
    def _set_file_binary(self, value):
        ...
        if convert_format:
            ...
            new_ext, save_subtype = convert_format.split(' ')
            save_subtype = save_subtype[+1:-1]
            if (mime_type != f'image/{save_subtype.lower()}'
                    and mtype_name == 'image'):
                base, ext = self.name.split('.')
                self.name = f'{base}{new_ext}'
                data = io.BytesIO()
                ...
                try:
                    ...
                    key = 'image_convert_error.%s' % save_subtype.lower()
