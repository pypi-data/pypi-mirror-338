...
class TestDrawFunction(unittest.TestCase):
    ...
    def test_draw(self, mock_vlines, mock_fill_between, mock_bar, mock_plot, mock_show):
        ...
        # Assert the plot methods are called with expected arguments
        mock_plot.assert_called_with(
            [timestamp], [200], label="line", color="green", lw=2
        )  # Line chart
        print("DEBUG: mock_plot call args ->", mock_plot.call_args_list)

        # Assert histogram bar is drawn
        mock_bar.assert_called_with(
            timestamp, 100, label="histogram", color="blue", width=0.5
        )
