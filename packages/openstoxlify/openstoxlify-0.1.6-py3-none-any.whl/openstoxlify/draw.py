...
from .models import MarketData, PlotType
from .fetch import fetch
from .plotter import PLOT_DATA


def draw():
    """Draw all charts from the PLOT_DATA based on the specified plot types."""
    ...
    for plot in PLOT_DATA.get(PlotType.LINE, []):
        ...
        print(
            f"DEBUG: Plotting Line Chart -> timestamps: {timestamps}, values: {values}"
        )
    ...
    # Handle Candlestick (HLOC) Plot
    for plot in PLOT_DATA.get(PlotType.CANDLESTICK, []):  # FIXED: Use correct key
        for item in plot["data"]:
