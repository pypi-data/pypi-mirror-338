...
class MaktabkhoonehCrawler:
    ...
    def _handle_video(self, response_text: str, chapter_directory: str, unit_name: str):
        ...
        subtitle_url = self._extract_subtitle_link(response_text)
        if subtitle_url:
            # Construct full subtitle URL by combining base URL with subtitle path
            full_subtitle_url = f"{self.BASE_URL}{subtitle_url}"
            logging.info(f"Found subtitle link: {full_subtitle_url}")
            subtitle_path = f"{chapter_directory}{os.sep}{unit_name}.srt"
            subtitle_res = self._download_subtitle(
                subtitle_url=full_subtitle_url,
                output_file=subtitle_path,
            )
            if subtitle_res:
                logging.info(f"Subtitle downloaded successfully to {subtitle_path}")
            else:
                logging.warning(f"Failed to download subtitle for {unit_name}")
        else:
            logging.info(f"No subtitle found for {unit_name}")
