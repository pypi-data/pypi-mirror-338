...
class Command:
    def __init__(self, config:dict, command_name:str):
        ...
        self.args_help.update({
            ...
            'export-test-json':    'export, and save a JSON file per target',
        })
        self.modified_args = {}
...
class CommandDesign(Command):
    ...
    def __init__(self, config:dict, command_name:str):
        ...
        self.args_help.update({
            ...
            'all-sv': 'Maintain .sv and .v in single file list. (False: .sv flist separate from .v flist)',
        })
        self
        self.defines = dict()
        self.incdirs = list()
    ...
    def add_file(self, filename, use_abspath=True, add_to_non_sources=False, caller_info:str=''):
        ...
        elif file_ext == '.v' and not self.args['all-sv']:
            ...
            if os.path.exists(token):
                ...
                if file_ext == '':
                    ...
                    if self.args['top'] == "" and \
                       file_ext in self.config['inferred_top_file_ext']:
...
class ToolVivado(Tool):
    ...
    def get_versions(self) -> str:
        ...
        if not xilinx_vivado or not os.path.exists(xilinx_vivado):
            self.error(f"environment for XILINX_VIVADO is not set or set to a non-existent path: {xilinx_vivado=}")
