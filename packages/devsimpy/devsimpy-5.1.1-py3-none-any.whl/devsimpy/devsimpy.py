...
#			self.SetAcceleratorTable(aTable)
	...
	def WriteDefaultConfigFile(self, cfg):
		"""
		Writes the default configuration settings to the provided configuration file.
		This method initializes various settings and writes them to the configuration
		file specified by `cfg`. It also sends a message to indicate the start of the
		writing process and outputs progress to the standard output.
		Args:
			cfg: The configuration file object where the settings will be written.
		Attributes:
			exportPathsList (list): List of export paths.
			openFileList (list): List of last opened files.
			language (str): Default language, 'fr' for French if the current locale is French, otherwise 'en'.
			perspectives (dict): Dictionary to store perspectives.
			last_position (None): Placeholder for the last position setting.
			last_size (None): Placeholder for the last size setting.
		Writes:
			version (str): Version of the main application.
			exportPathsList (str): Serialized empty list of export paths.
			ChargedDomainList (str): Serialized empty list of charged domains.
			openFileList (str): Serialized list of last opened files.
			language (str): Serialized default language.
			active_plugins (str): Serialized empty list of active plugins.
			perspectives (str): Serialized empty dictionary of perspectives.
			settings (str): Serialized current user settings.
	...
	def GetCurrentUserSetting(self):
		"""
		Retrieves the current user settings.

		This method returns a dictionary containing the current user settings
		by extracting the values from the built-in namespace for the keys
		specified in the USER_SETTINGS list.

		Returns:
			dict: A dictionary where the keys are the user setting names and
	...
	def SetConfig(self, cfg):
		...
		if self.cfg.Exists('version'):
			...
			if not rewrite:
				...
				# if os.path.exists(settings_dict['ICON_PATH']):
				# 	builtins.__dict__.update(settings_dict)
	...
	def OnDragInit(self, event):
		...
		if not tree.ItemHasChildren(item) and flag:
			text = tree.GetItemData(event.GetItem())
			try:
