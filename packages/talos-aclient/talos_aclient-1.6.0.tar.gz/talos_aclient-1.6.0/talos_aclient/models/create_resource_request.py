...
class CreateResourceRequest(BaseModel):
    """
    创建资源请求类  Attributes:     type (ResourceType): 资源类型，枚举值，默认值为 \"file\"     dry_run (bool): 是否为试运行，布尔值，默认值为 False     name (str): 资源名称，字符串，必填项     data (ResourceTypeData): 资源数据，根据资源类型不同而不同，默认值为 ResourceDataPlain()     parse_config (ParseConfig | None): 解析配置，可选参数，默认值为 None     index_config (DatasetResourceIndexConfig | None): 索引配置，可选参数，默认值为 None     async_mode (bool): 是否为异步模式，布尔值，默认值为 True     should_parser (bool): 是否应该解析，布尔值，默认值为 True  Methods:     check_model_before(cls, values: Any) -> Any: 在模型验证之前检查和处理数据     check_model_after(self) -> Any: 在模型验证之后检查和处理数据     _get_file_extension(filename: str) -> str: 获取文件名的扩展名
    """ # noqa: E501
    ...
    __properties: ClassVar[List[str]] = ["queue_config", "trace_info", "type", "dry_run", "name", "data", "parse_config", "index_config", "async_mode", "should_parser"]
