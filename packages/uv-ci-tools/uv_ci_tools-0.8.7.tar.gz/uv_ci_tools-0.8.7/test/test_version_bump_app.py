...
from uv_ci_tools.app.version.bump import app
...
def test_gitlab_no_project_name():
    with pytest.raises(RuntimeError, match='CI_PROJECT_NAME'):
...
def test_gitlab_no_project_path():
    with pytest.raises(RuntimeError, match='CI_PROJECT_PATH'):
        run_app('simple', ci_type=ci.Type.GITLAB, project_path=None)


def test_gitlab_no_commit_ref_name():
    with pytest.raises(RuntimeError, match='CI_COMMIT_REF_NAME'):
        run_app('simple', commit_ref_name=None)


def test_gitlab_no_repository_host():
    with pytest.raises(RuntimeError, match='CI_SERVER_HOST'):
