...
def DocumentMatchScore(data: dict):
    ...
    invalid_documents = sum(
        [1 for doc_list in document_names for name in doc_list if name is None]
    )
    document_names = [
        [normalize_name(name) for name in doc_list if name is not None]
        for doc_list in document_names
    ]
    ...
    test_document_names = [
        [normalize_name(doc) for doc in doc_list] for doc_list in data["test_documents"]
    ]

    # Metric 1: Mean Document Match
    mean_match_scores = []
    for retrieved, test in zip(document_names, test_document_names):
    ...
    for retrieved, test in zip(document_names, test_document_names):
        ...
        match_score = max(
            (weights[i] for i, name in enumerate(retrieved) if name in test), default=0
        )
