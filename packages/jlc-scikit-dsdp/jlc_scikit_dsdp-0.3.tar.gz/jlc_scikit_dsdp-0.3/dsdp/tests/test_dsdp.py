...
target = dict(
    zip(
        ["truss1", "control1", "vibra1", "mcp100"],
        [8.999996, -17.784626, -40.819012, -226.1573],
    )
)


def test_dsdp():
    A = matrix([[10, 4, 4, 0], [0, 0, 0, -8], [0, -8, -8, -2]])
    b = matrix([[48], [-8], [20]])
    c = matrix([[-11], [0], [0], [23]])
...
def test_control1():
    result = dsdp_readsdpa(
        os.path.dirname(os.path.abspath(__file__)) + "/examples/control1.dat-s", ""
    )
...
def test_truss1():
    result = dsdp_readsdpa(
        os.path.dirname(os.path.abspath(__file__)) + "/examples/truss1.dat-s", ""
    )
...
def test_vibra1():
    result = dsdp_readsdpa(
        os.path.dirname(os.path.abspath(__file__)) + "/examples/vibra1.dat-s", ""
    )
...
def test_mcp100():
    result = dsdp_readsdpa(
        os.path.dirname(os.path.abspath(__file__)) + "/examples/mcp100.dat-s", ""
    )
