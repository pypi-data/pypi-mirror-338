...
class Simulator:
    ...
    def has_converged(self, threshold=1e-6):
        if self.prev_positions is None or self.prev_velocities is None:
            return False
        else:
            prev_states =  np.concatenate([self.prev_positions, self.prev_velocities], axis=1)

        if np.linalg.norm(self.get_states() - prev_states) < threshold:
            return True
        else:
            return False
    ...
    def _set_states(self):
        ...
        # Calculate velocity as v_k = (x_{k} - x_{k-1}) / dt
