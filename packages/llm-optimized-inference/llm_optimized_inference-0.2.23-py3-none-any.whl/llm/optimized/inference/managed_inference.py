...
def get_processed_input_data_for_chat_completion(
    ...
    tokenizer_path = str(
        os.path.join(os.getenv("AZUREML_MODEL_DIR", ""), DEFAULT_TOKENIZER_PATH)
    )
    ...
    try:
        # processor class for multimodal models, to apply chat template
        processor = AutoProcessor.from_pretrained(tokenizer_path,
                                                  use_default_system_prompt=False,
                                                  trust_remote_code=True)
        chat_conv = processor.apply_chat_template(dialog, tokenize=False, add_generation_prompt=add_generation_prompt)
    except Exception as e:
        logger.info(f"Falling back to standard tokenizer chat-template:\
                    Chat-completion task does not support multimodal inputs. Error: {str(e)}")
