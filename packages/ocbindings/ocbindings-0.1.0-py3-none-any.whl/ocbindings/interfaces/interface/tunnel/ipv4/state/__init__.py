...
depends on the interface's type.
  ...
  def _get_gratuitous_arp_accepted(self):
    """
    Getter method for gratuitous_arp_accepted, mapped from YANG variable /interfaces/interface/tunnel/ipv4/state/gratuitous_arp_accepted (boolean)
...
the ARP table will be updated.
  ...
  def _set_gratuitous_arp_accepted(self, v, load=False):
    """
    Setter method for gratuitous_arp_accepted, mapped from YANG variable /interfaces/interface/tunnel/ipv4/state/gratuitous_arp_accepted (boolean)
    If this variable is read-only (config: false) in the
...
the ARP table will be updated.
    ...
    if hasattr(v, "_utype"):
      v = v._utype(v)
    try:
      t = YANGDynClass(v,base=YANGBool, is_leaf=True, yang_name="gratuitous-arp-accepted", parent=self, path_helper=self._path_helper, extmethods=self._extmethods, register_paths=True, namespace='http://openconfig.net/yang/interfaces/tunnel', defining_module='openconfig-if-tunnel', yang_type='boolean', is_config=False)
    except (TypeError, ValueError):
      raise ValueError({
