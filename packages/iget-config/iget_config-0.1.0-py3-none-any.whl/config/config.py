...
class Config:
    ...
    def __init__(self, reload: bool = False):
        ...
        if getattr(self, "_initialized", False) and not reload:
            sys.stdout.write(
                f"DEBUG [Config]: Skipping init for already initialized instance {id(self)}\n"
            )
            sys.stdout.flush()
            return
        ...
        sys.stdout.write(
            f"DEBUG [Config]: Resetting Train/Data/Optimizer Config objects for {id(self)}\n"
        )
        sys.stdout.flush()
        ...
        sys.stdout.write(
            f"DEBUG [Config]: Calling _load_hierarchical_config for {id(self)} with mode {Config.mode}\n"
        )
        sys.stdout.flush()
        ...
        sys.stdout.write(
            f"DEBUG [Config]: Setting mode-specific attributes for {id(self)} with mode {Config.mode}\n"
        )
        sys.stdout.flush()
        ...
        sys.stdout.write(
            f"DEBUG [Config __init__ EXIT] id:{id(self)}, Loaded mode: {self._loaded_mode}\n"
        )
        sys.stdout.flush()
    ...
    def _load_hierarchical_config(self):
        ...
        mode = Config.mode
        sys.stdout.write(
            f"DEBUG [Config]: _load_hierarchical_config ENTER - Mode='{mode.name if mode else 'None'}', Prefix='{self.prefix}', InstanceID={id(self)}\n"
        )
        sys.stdout.flush()
        ...
        if mode and mode != Mode.TEST:
            sys.stdout.write(
                f"DEBUG [Config]: _load_hierarchical_config - Attempting base mode file default_{mode.value}.yaml\n"
            )
            sys.stdout.flush()
            self._load_from_file(f"default_{mode.value}.yaml")
        else:
            sys.stdout.write(
                f"DEBUG [Config]: _load_hierarchical_config - Skipping base mode file for mode {mode.name if mode else 'None'}\n"
            )
            sys.stdout.flush()
        ...
        if self.prefix:
            sys.stdout.write(
                f"DEBUG [Config]: _load_hierarchical_config - Attempting prefix file {self.prefix}.yaml\n"
            )
            sys.stdout.flush()
        ...
        if self.prefix and mode and mode != Mode.TEST:
            sys.stdout.write(
                f"DEBUG [Config]: _load_hierarchical_config - Attempting prefix mode file {self.prefix}_{mode.value}.yaml\n"
            )
            sys.stdout.flush()
            self._load_from_file(f"{self.prefix}_{mode.value}.yaml")
        else:
            sys.stdout.write(
                f"DEBUG [Config]: _load_hierarchical_config - Skipping prefix mode file for prefix '{self.prefix}' and mode {mode.name if mode else 'None'}\n"
            )
            sys.stdout.flush()

        sys.stdout.write(
            f"DEBUG [Config]: _load_hierarchical_config EXIT - Final batch_size={getattr(getattr(self,'train', None),'batch_size','N/A')}, InstanceID={id(self)}\n"
        )
        sys.stdout.flush()
    ...
    def _load_from_file(self, filename: str):
        ...
        if os.path.exists(file_path):
            sys.stdout.write(
                f"DEBUG [Config]: _load_from_file - Loading: {filename} (InstanceID={id(self)})\n"
            )
            sys.stdout.flush()
            try:
                with open(file_path, "r", encoding="utf-8") as f:
                    ...
                    if yaml_data:
                        ...
                        sys.stdout.write(
                            f"DEBUG [Config]: _load_from_file - Skipped empty file: {filename}\n"
                        )
                        sys.stdout.flush()
            ...
            # 保留调试打印
            sys.stdout.write(
                f"DEBUG [Config]: _load_from_file - File not found, skipping: {filename}\n"
            )
            sys.stdout.flush()
    ...
    def save_version(self, version: str):
        ...
        文件名格式: {prefix}_{mode}_{version}.yaml (基础 Config 为 default_{mode}_{version}.yaml)。
        此操作使用当前的全局模式。

        参数:
            version (str): 配置版本标识符 (例如 'v1.0', 'best_run')，不应包含模式。
        """
        prefix_part = self.prefix or "default"
        mode_part = (
            Config.mode.value if Config.mode else "test"
        )  # Use 'test' if mode is None
        file_name = (
            f"{prefix_part}_{mode_part}_{version}.yaml"  # Include mode in filename
        )
    ...
    def load_version(self, version: str):
        ...
        参数:
            version (str): 要加载的版本标识符，**必须包含模式和版本** (例如 'dev_v1.0', 'prod_best_run')。

        返回:
            bool: 如果加载成功则返回 True，否则返回 False。
        """
        # The 'version' parameter now includes the prefix and mode, e.g., "default_dev_v1"
        # We need to parse it or assume the caller provides the full expected filename part.
        # Simpler approach: Assume 'version' contains the full part after config_folder.
        # Example: user calls load_version("cnn_dev_v1.0") -> filename becomes cnn_dev_v1.0.yaml
        # Example: user calls load_version("default_prod_final") -> filename becomes default_prod_final.yaml

        # Let's keep the original logic but update the expectation in the docstring.
        # The user provides the *combined* mode+version string.
        prefix_part = self.prefix or "default"
        # The file_name IS the version string provided by the user + .yaml
        file_name = f"{version}.yaml"
        file_path = os.path.join(Config.config_folder, file_name)

        print(
            f"加载版本快照: {file_path}"
        )  # Uses the user-provided version string directly
