    ...
    def run(self):
        ...
        for idx, file_path in enumerate(files):
            try:
                ...
                results.append((str(file_path), doc_filename, content))
                self.progress_signal.emit(doc_filename, int((idx + 1) / total_files * 100))
...
class FileProcessorApp(widget.OWWidget):
    ...
    class Inputs:
        data = Input("Path", Orange.data.Table)
    ...
    def set_data(self, data):
        ...
        if self.data is not None:
            ...
            for i, meta_var in enumerate(self.data.domain.metas):
                if meta_var.name.lower() == 'path':
                    path_index = i
                    break

            if path_index is not None:
                self.input_path = self.data.metas[0][path_index]
                print("Extracted Path:", self.input_path)
                self.startProcessing()
            else:
                print("No 'path' column found in input data. Available columns:", [m.name for m in self.data.domain.metas])
    ...
    def startProcessing(self):
        ...
        output_dir = input_dir.parent / (input_dir.name + "_md")
        self.progress_bar.setValue(0)
        self.start_button.setEnabled(False)
        self.status_label.setText("Traitement en cours...")

        self.thread = MarkdownConversionThread(input_dir, output_dir)
    ...
    def send_output(self, results):
        domain = Domain([], metas=[StringVariable('path'), StringVariable('name'), StringVariable('content')])
        metas = [[path, name, content] for path, name, content in results] if results else [["", "", ""]]
