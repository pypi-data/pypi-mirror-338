...
class AuthProxy:
    ...
    async def _handle_websocket(
        ...
        if "Upgrade" not in headers or headers["Upgrade"].lower() != "websocket":
            logger.error("Invalid WebSocket upgrade request: missing or invalid Upgrade header")
            writer.write(b"HTTP/1.1 400 Bad Request\r\n\r\n")
            await writer.drain()
            return
            
        if "Connection" not in headers or "upgrade" not in headers["Connection"].lower():
            logger.error("Invalid WebSocket upgrade request: missing or invalid Connection header")
            writer.write(b"HTTP/1.1 400 Bad Request\r\n\r\n")
            await writer.drain()
            return
            
        if "Sec-WebSocket-Key" not in headers:
            logger.error("Invalid WebSocket upgrade request: missing Sec-WebSocket-Key header")
            writer.write(b"HTTP/1.1 400 Bad Request\r\n\r\n")
            await writer.drain()
            return
        ...
        try:
            ...
            client_to_backend = asyncio.create_task(
                self._forward_websocket_data(reader, backend_writer, "client -> backend")
            )
            backend_to_client = asyncio.create_task(
                self._forward_websocket_data(backend_reader, writer, "backend -> client")
            )
    ...
    async def handle_request(
        ...
        try:
            ...
            if is_websocket:
                await self._handle_websocket(reader, writer, request_line, headers, authenticated, auth_headers)
                return
