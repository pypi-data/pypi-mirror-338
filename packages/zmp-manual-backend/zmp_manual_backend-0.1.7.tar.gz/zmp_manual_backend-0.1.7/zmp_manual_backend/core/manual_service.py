...
class ManualService:
    ...
    async def _ensure_repository(self) -> bool:
        ...
        try:
            ...
            if should_clone:
                ...
                try:
                    ...
                    possible_credential_paths = [
                        "/git-credentials/.git-credentials",  # From init container
                        "/root/.git-credentials",  # Legacy/symlink location
                        os.path.expanduser("~/.git-credentials"),  # User home location
                    ]

                    credential_found = False
                    for path in possible_credential_paths:
                        if os.path.exists(path):
                            logger.info(f"Git credentials file found at {path}")
                            credential_found = True

                            # Read first few characters to validate format (without exposing token)
                            try:
                                with open(path, "r") as f:
                                    content = f.read().strip()
                                    if (
                                        content.startswith("https://git:")
                                        and "@github.com" in content
                                    ):
                                        logger.info(
                                            f"Git credentials file at {path} has correct format"
                                        )
                                    else:
                                        logger.warning(
                                            f"Git credentials file at {path} may have incorrect format"
                                        )
                            except Exception as read_err:
                                logger.warning(
                                    f"Failed to read credentials file {path}: {str(read_err)}"
                                )

                    if not credential_found:
                        logger.warning(
                            "No Git credentials file found in any standard location"
                        )

                    # Check HOME environment variable - critical for Git credential helper
                    logger.info(
                        f"HOME environment variable: {os.environ.get('HOME', 'not set')}"
                    )

                    # Try to explicitly set HOME if it's not correctly pointing to credential location
                    if (
                        os.path.exists("/git-credentials")
                        and os.environ.get("HOME") != "/git-credentials"
                    ):
                        logger.info(
                            "Setting HOME to /git-credentials for git operations"
                        )
                        git_env["HOME"] = "/git-credentials"

                    # First attempt: use environment-configured credentials
                    logger.info(
                        f"Attempting to clone repository {self.github_repo_url} with credential helper"
                    )

                    # Clone with credentials from git credential helper
                    ...
                    logger.info("Successfully cloned repository with credential helper")
                except Exception as e:
                    # Log the error for debugging
                    logger.warning(f"Initial clone attempt failed: {str(e)}")

                    # Try to recover: if this is likely a credential error, check for GitHub token in env
                    github_token = os.environ.get("GITHUB_TOKEN")
                    if github_token:
                        try:
                            ...
                            logger.info(
                                f"Retrying with token from environment (starts with {github_token[:4]})"
                            )

                            # Extract username and domain from GitHub URL
                            url_parts = self.github_repo_url.split("//")
                            if len(url_parts) > 1:
                                # Rebuild URL with embedded credentials
                                auth_url = (
                                    f"{url_parts[0]}//git:{github_token}@{url_parts[1]}"
                                )
                                logger.info(
                                    "Retrying clone with embedded token authentication"
                                )

                                # Clone with the token-based URL
                                await asyncio.to_thread(
                                    git.Repo.clone_from,
                                    auth_url,
                                    repo_path,
                                ...
                                logger.info(
                                    "Successfully cloned repository with embedded token"
                                )
                            else:
                                logger.error(
                                    f"Invalid GitHub URL format: {self.github_repo_url}"
                                )
                                raise ValueError(
                                    f"Invalid GitHub URL format: {self.github_repo_url}"
                                )
                            ...
                            logger.error(
                                f"Failed to clone with token authentication: {str(inner_error)}"
                            )
                            # Try one more approach - direct git command
                            try:
                                logger.info("Attempting direct git command with token")
                                os.makedirs(repo_path, exist_ok=True)
                                # Extract the repo path from URL
                                repo_path_match = self.github_repo_url.split(
                                    "github.com/"
                                )
                                if len(repo_path_match) > 1:
                                    repo_path_str = repo_path_match[1]
                                    if repo_path_str.endswith(".git"):
                                        repo_path_str = repo_path_str[:-4]

                                    # Use direct git command with token (more reliable)
                                    cmd = f"git clone https://git:{github_token}@github.com/{repo_path_str}.git {repo_path}"
                                    ...
                                    if process.returncode == 0:
                                        logger.info(
                                            "Successfully cloned using direct git command"
                                        )
                                    else:
                                        stderr_text = (
                                            stderr.decode()
                                        ...
                                        logger.error(
                                            f"Direct git command failed: {stderr_text}"
                                        )

                                        # One last attempt with git config
                                        logger.info(
                                            "Final attempt: manually configuring git"
                                        )
                                        # Try to configure git manually
                                        config_cmds = [
                                            "mkdir -p ~/.git",
                                            f"echo 'https://git:{github_token}@github.com' > ~/.git-credentials",
                                            "chmod 600 ~/.git-credentials",
                                            "git config --global credential.helper store",
                                            "git config --global http.sslVerify false",
                                            f"git clone {self.github_repo_url} {repo_path}",
                                        ]

                                        cmd_script = " && ".join(config_cmds)
                                        process = await asyncio.create_subprocess_shell(
                                            cmd_script,
                                            stdout=subprocess.PIPE,
                                            stderr=subprocess.PIPE,
                                        ...
                                        if process.returncode == 0:
                                            logger.info(
                                                "Successfully cloned after manual git configuration"
                                            )
                                        else:
                                            stderr_text = (
                                                stderr.decode()
                                            ...
                                            logger.error(
                                                f"Manual git configuration failed: {stderr_text}"
                                            )
                                            # Re-raise the original error since we couldn't recover
                                            raise e
                                else:
                                    logger.error(
                                        f"Could not extract repository path from URL: {self.github_repo_url}"
                                    )
                                    raise e
                            except Exception as cmd_error:
                                logger.error(
                                    f"All clone methods failed: {str(cmd_error)}"
                                )
                                # Create detailed diagnostics
                                try:
                                    diag_cmd = "env | grep -E 'GIT|HOME' && ls -la ~/.git-credentials /git-credentials /root/.git-credentials 2>/dev/null || echo 'No credentials files found'"
                                    diag_process = (
                                    ...
                                    (
                                        diag_stdout,
                                        diag_stderr,
                                    ) = await diag_process.communicate()
                                    logger.error(
                                        f"Diagnostics: {diag_stdout.decode() if diag_stdout else 'No output'}"
                                    )
                                except Exception as diag_error:
                                    logger.error(
                                        f"Failed to run diagnostics: {str(diag_error)}"
                                    )

                                raise e
                    else:
                        # No token available for recovery, re-raise the original error
                        logger.error(
                            "No GITHUB_TOKEN environment variable found for fallback authentication"
                        )
                        raise e
            ...
            def update_repo():
                try:
                    # Use GitHub token from environment if available for authentication
                    github_token = os.environ.get("GITHUB_TOKEN")
                    if github_token:
