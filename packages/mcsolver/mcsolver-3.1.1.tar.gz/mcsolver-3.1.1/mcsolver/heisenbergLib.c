...
PyObject * MCMainFunction(PyObject* self, PyObject* args){
    ...
    //printf("totOrbs=%d s0=%f D0x=%f nther=%d nst=%d tau=%d maxLink=%d link0=%d J0x=%f\n",totOrbs,initSpin[0],initD[0],nthermal,nsweep,ninterval,maxNLinking,nlink[0],linkStrength[0]);
    ...
    *p_energy/=2; // double counting
    ...
    for(unsigned long long isweep=0;isweep<nsweep;isweep++){
        ...
        for(unsigned long long j=0;j<nLat;j++){
            ...
            spin_i_h_avg+=lattice[corrOrbPair[j*2+0]].spin.z;
            spin_j_h_avg+=lattice[corrOrbPair[j*2+1]].spin.z;
        }

        double topological_q_local=0;
        for(unsigned long long icircuit=0;icircuit<nLocalCircuits;icircuit++){
            topological_q_local+=calcSignedArea(lattice[localCircuits[icircuit*3+0]].spin, lattice[localCircuits[icircuit*3+1]].spin, lattice[localCircuits[icircuit*3+2]].spin);
        }
    ...
    for(unsigned long long iOrbGroup=0;iOrbGroup<(nOrbGroup+1);iOrbGroup++){
        for(unsigned long long jOrbGroup=0;jOrbGroup<(nOrbGroup+1);jOrbGroup++){
            unsigned long long index=iOrbGroup*(nOrbGroup+1)+jOrbGroup;
    ...
    for(unsigned long long iOrbGroup=0;iOrbGroup<(nOrbGroup+1);iOrbGroup++){
        unsigned long long index=(nOrbGroup+1)*(nOrbGroup+1)+iOrbGroup;
