...
class Interceptor(ABC):
    ...
    @classmethod
    def RequestInterceptor(cls, interceptor_paths: set[str]) -> callable:
        if not isinstance(interceptor_paths, set):
            raise ValueError('RequestInterceptor: interceptor_paths should be a set')

        def decorator_request_func(other_cls) -> callable:
            def wrapper(*args, **kwargs) -> object:
                return other_cls(*args, **kwargs)

            wrapper.__decorator__ = "RequestInterceptor"
            wrapper.__decorator_params__ = interceptor_paths
            return wrapper
