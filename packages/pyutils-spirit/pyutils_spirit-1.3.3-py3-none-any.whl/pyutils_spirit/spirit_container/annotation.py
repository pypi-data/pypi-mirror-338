...
def RequestInterceptor(interceptor_paths: set[str]) -> callable:
    if not isinstance(interceptor_paths, set):
        raise ValueError('RequestInterceptor: interceptor_paths should be a set')
    if len(interceptor_paths) == 0:
        raise ValueError('RequestInterceptor: interceptor_paths should be')

    def decorator_request_func(func) -> callable:
        func.__decorator__ = "RequestInterceptor"
        func.__decorator_params__ = interceptor_paths
        return func
...
def InterceptorBefore() -> callable:
    def decorator_request_func(func) -> callable:
        ...
        def interceptor_before(request: BaseHTTPRequestHandler) -> tuple[int, bool]:
            ...
            response_code, response_status = func(request)
            if not isinstance(response_code, int) or not isinstance(response_status, bool):
                raise ValueError('InterceptorBefore: return value must be'
                                 ' (response_code: int, response_status: bool)')
            return response_code, response_status

        func.__decorator__ = "InterceptorBefore"
...
def InterceptorAfter() -> callable:
    def decorator_request_func(func) -> callable:
        def interceptor_after(request: BaseHTTPRequestHandler) -> None:
            if not isinstance(request, BaseHTTPRequestHandler):
                raise ArgumentTypeError('InterceptorAfter: argument should be BaseHTTPRequestHandler Type')
            func(request)
            return None

        func.__decorator__ = "InterceptorAfter"
