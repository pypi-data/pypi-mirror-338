...
import time
import webullpaysdkcore
import logging
import platform
import json
from webullpaysdkcore import compat
from webullpaysdkcore.exception import error_code
from webullpaysdkcore.exception.exceptions import ClientException, ServerException
from webullpaysdkcore.retry.retry_policy_context import RetryPolicyContext
from webullpaysdkcore.vendored.requests import codes
from webullpaysdkcore.vendored.requests.adapters import HTTPAdapter
from webullpaysdkcore.auth.signers.signer_factory import SignerFactory
from webullpaysdkcore.endpoint.default_endpoint_resolver import DefaultEndpointResolver
from webullpaysdkcore.vendored.requests.structures import CaseInsensitiveDict
from webullpaysdkcore.vendored.requests.structures import OrderedDict
from webullpaysdkcore.request import BaseRequest
from webullpaysdkcore.http.response import Response
...
import webullpaysdkcore.headers as hd
import webullpaysdkcore.retry.retry_policy as retry_policy
from webullpaysdkcore.common.api_type import DEFAULT as HTTP_API_TYPE
...
class ApiClient:
    ...
    def default_user_agent():
        ...
        default_agent['Core'] = __import__('webullpaysdkcore').__version__
        default_agent['python-requests'] = __import__(
            'webullpaysdkcore.vendored.requests.__version__', globals(), locals(),
    ...
    def _handle_single_request(self, endpoint, request, read_timeout, connect_timeout, signer):
        ...
        try:
            ...
            msg = "HttpError occurred. Host:%s SDK-Version:%s ClientException:%s" % (
                         endpoint, webullpaysdkcore.__version__, exception)
    ...
    def _parse_error_info_from_response_body(body_obj):
        ...
        error_msg_to_return = ""
        if body_obj and body_obj.get('code'):
            error_code_to_return = body_obj.get('code')
        if body_obj and body_obj.get('msg'):
            error_msg_to_return = body_obj.get('msg')
    ...
    def set_stream_logger(self, log_level=logging.DEBUG, logger_name='webullpaysdkcore', stream=None,
                          format_string=None):
    ...
    def set_file_logger(self, path, log_level=logging.DEBUG, logger_name='webullpaysdkcore', format_string=None):
        log = logging.getLogger(logger_name)
        log.setLevel(log_level)
