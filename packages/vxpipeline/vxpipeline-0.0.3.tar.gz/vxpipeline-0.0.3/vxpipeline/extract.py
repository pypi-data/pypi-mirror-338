...
def extract_from_mongo(
    ...
    try:
        ...
        logging.info(f"Extração concluída: {len(docs)} documentos extraídos da coleção '{collection}' in {elapsed:.2f}s")
        return docs
...
def extract_from_secrets(secret_name: str, **kwargs: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        logging.info(f" Segredo '{secret_name}' recuperado com sucesso in {elapsed:.2f}s")
        return secret
    except Exception as e:
