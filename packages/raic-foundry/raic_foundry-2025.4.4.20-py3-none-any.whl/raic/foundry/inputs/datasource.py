...
def prepare(source_path: Path,
    ...
    unpacked_datasource_file_paths = []
    for file_path in [Path(root_path, relative_file_path) for relative_file_path in available_relative_file_paths]:
        ...
        for geospatial_file_path in raic.foundry.inputs.geospatial.prepare(file_path, root_path, max_size_px):
            unpacked_datasource_file_paths.append(geospatial_file_path)
            processed = True
        ...
        if image_artifact is not None:
            unpacked_datasource_file_paths.append(image_artifact.info.local_path)
        else:
            print(f'Data source file {file_path.name} not recognized as valid image type')

    manifest = DataSourceManifest(root_path=root_path, relative_paths=[file_path.relative_to(root_path) for file_path in unpacked_datasource_file_paths])
    manifest.save()
    return manifest   
