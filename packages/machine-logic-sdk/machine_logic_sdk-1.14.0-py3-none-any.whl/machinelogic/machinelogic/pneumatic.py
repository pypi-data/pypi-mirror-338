...
class Pneumatic(IPneumatic):
    def __init__(
        self, configuration: PneumaticConfiguration, api: Api, mqtt_client: MqttClient
    ) -> None:
    ...
    def _update_pin_state(self, pin_states: List[int]) -> None:
        if self.configuration.input_pin_pull is not None:
            self._pin_state["input_pin_pull"] = bool(
                pin_states[self.configuration.input_pin_pull]
            )
        if self.configuration.input_pin_push is not None:
            self._pin_state["input_pin_push"] = bool(
                pin_states[self.configuration.input_pin_push]
            )
