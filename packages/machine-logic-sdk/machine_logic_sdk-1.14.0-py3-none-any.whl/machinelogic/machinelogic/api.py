...
class Routes(Enum):
    ...
    ESTOP = "/v1/python/estop"
    TRY_CLEAR_DRIVE_ERRORS = "/v1/python/try_clear_drive_errors"
...
class Api:
    ...
    def _make_route(self, route: Routes, params: str = "") -> str:
        return self._conn_str + str(route.value) + "/" + params

    def get_estop_status(self) -> Dict[Literal["status"], estop_status.EstopStatus]:
        """Get the status of the estop

        Returns:
            EstopStatus: Estop status
        """
        response = requests.get(
            self._make_route(Routes.ESTOP),
            timeout=DEFAULT_TIMEOUT_SECONDS,
        )
        if not response.ok:
            raise ApiException(f"Failed to get the estop status: {str(response)}")
        return cast(Dict[Literal["status"], estop_status.EstopStatus], response.json())

    def try_clear_drive_errors(self) -> bool:
        """Try to clear drive errors

        Returns:
            bool: True if the drive errors were cleared, otherwise False
        """
        response = requests.get(
            self._make_route(Routes.TRY_CLEAR_DRIVE_ERRORS),
            timeout=DEFAULT_TIMEOUT_SECONDS,
        )
        if not response.ok:
    ...
    def get_scene_assets(
        ...
        return cast(
            Dict[Literal["assets"], List[scene_assets.SceneAssets]], response.json()
        )
    ...
    def get_calibration_frame(
        ...
        if not response.ok:
            raise ApiException(
                f"Failed to fetch the calibration frame: {str(response)}"
            )
        return cast(
            Dict[
                Literal["robotCalibration"],
