...
class ActuatorGroupState:
    ...
    def move_in_progress(self) -> bool:
        """bool: The boolean is True if the ActuatorGroup has at least one Actuator in progress"""
...
class IActuatorGroup(ABC):
    ...
    def state(self) -> ActuatorGroupState:
        """ActuatorGroupState: The state of the ActuatorGroup."""
        return self._state
    ...
    def move_relative(
        ...
        Args:
            ...
            timeout (float, optional): The timeout in seconds after which an exception is thrown. Defaults to DEFAULT_MOVEMENT_TIMEOUT_SECONDS.
    ...
    def lock_brakes(self) -> None:
        """
        Locks the brakes for all Actuators in the group.

        Raises:
            ActuatorGroupException: If the brakes failed to lock on a single Actuator in the group.
        """

    @abstractmethod
    def unlock_brakes(self) -> None:
        """
        Unlocks the brakes on all Actuators in the group.

        Raises:
            ActuatorGroupException: If the brakes failed to unlock on a single Actuator in the group.
        """

    @abstractmethod
    def set_speed(self, speed: float) -> None:
        """
        Sets the speed on all Actuators in the group.

        Args:
            speed (float): The speed to set on all Actuators in the group.

        Raises:
            ActuatorGroupException: If the speed failed to set on any Actuator in the group.
        """

    @abstractmethod
    def set_acceleration(self, acceleration: float) -> None:
        """
        Sets the acceleration on all Actuators in the group.

        Args:
            acceleration (float): The acceleration to set on all Actuators in the group.

        Raises:
            ActuatorGroupException: If the acceleration failed to set on any Actuator in the group.
        """
