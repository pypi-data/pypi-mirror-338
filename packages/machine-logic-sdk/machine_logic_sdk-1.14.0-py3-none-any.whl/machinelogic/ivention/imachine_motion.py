...
from .ibag_gripper import BagGripperConfiguration, IBagGripper
...
class MachineMotionConfiguration:
    ...
    def __init__(
        self, uuid: str, name: str, ip_address: str, mqtt_conn_str: str
    ) -> None:
        """
        Args:
        ...
        self._bag_gripper_configuration_list: list[BagGripperConfiguration] = []
        self._ac_motor_configuration_list: list[ACMotorConfiguration] = []

    @property
    def name(self) -> str:
        """str: The name of the MachineMotion."""
        return self._name

    @property
    def uuid(self) -> str:
        """str: The ID of the MachineMotion."""
        return self._uuid

    @property
    def ip_address(self) -> str:
        """str: The IP address of the MachineMotion."""
        return self._ip_address

    @property
    def mqtt_conn_str(self) -> str:
        """str: The connection string for the MQTT client."""
...
class IMachineMotion(ABC):
    ...
    is comprised of many actuators, inputs, outputs, pneumatics, ac motors,
    and bag grippers. It keeps a persistent connection to MQTT as well.
    ...
    def __init__(self, configuration: MachineMotionConfiguration) -> None:
        ...
        self._bag_gripper_list: list[IBagGripper] = []
    ...
    def get_bag_gripper(self, name: str) -> IBagGripper:
        ...
        Raises:
            MachineMotionException: If it is not found.
        """
        for bag_gripper in self._bag_gripper_list:
            if bag_gripper.configuration.name == name:
                return bag_gripper

        raise MachineMotionException(f"Failed to find bag gripper with name: {name}")
    ...
    def list_components(
        ...
        Returns:
            Tuple[list[IActuator], list[IRobot], list[IDigitalInput], list[IOutput], list[IPneumatic], list[IACMotor], list[IBagGripper], list[ICamera]]:
                Tuple containing actuators, robots, inputs, outputs, pneumatics, AC motors and bag grippers
        """
        return (
            self._actuator_list,
            self._robot_list,
