...
from machinelogic.decorators.deprecated import deprecated_property
...
def log_alarm_callback(
    ...
    def should_trigger_alarm(
        alarm_level: RobotAlarm.Level, trigger_level: RobotAlarm.Level | None
    ) -> bool:
    ...
    def get_instance(
        ...
        if RosRobotSingleton.__instance is None:
            ...
            RosRobotSingleton.__instance = RosRobotClient(
                pendant_ip=parsed_ros_address.host,
...
class RobotState(IRobotState):
    def __init__(self, ros_robot: RosRobotClient):
    ...
    def tcp_offset(self) -> CartesianPose:
        return self._ros_robot.get_tcp_offset()
...
class Robot(IRobot):
    ...
    def __init__(self, configuration: RobotConfiguration, api: Api):
        ...
        self._state = RobotState(self._robot)

        configuration._set_cartesian_velocity_limit(
            self._robot.get_cartesian_velocity_limit()
        )
    ...
    def compute_forward_kinematics(
        ...
        if len(joint_angles) != 6:
            raise ValueError(
                "Joint angles must be a list of 6 joint angles, in degrees."
            )
    ...
    def compute_inverse_kinematics(
        ...
        joint_angles = self._robot.inverse_kinematics(
            cartesian_position, joint_constraints, seed_position=seed_position
        )
    ...
    def set_tcp_offset(self, tcp_offset: CartesianPose) -> bool:
        ...
        response = self._api.set_tcp_offset(
            self._configuration.uuid, tcp_offset_payload
        )
        return response
    ...
    def movel_async(
        ...
        self._robot.movel(
            target, velocity, acceleration, reference_frame, is_async=True
        )
    ...
    def reconnect(self, timeout: float | None = 15) -> bool:
        return self._api.reconnect_robot(
            robot_uuid=self._configuration.uuid, timeout=timeout
        )
