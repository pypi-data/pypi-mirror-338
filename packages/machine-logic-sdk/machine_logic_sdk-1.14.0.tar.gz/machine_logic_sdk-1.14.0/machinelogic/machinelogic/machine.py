...
class MachineState(IMachineState):
    ...
    def operational_state(self) -> MachineOperationalState:
        estop_status_result = self._api.get_estop_status()
        estop_status = estop_status_result["status"]
        are_smart_drives_ready = estop_status["areSmartDrivesReady"]
        return (
            MachineOperationalState.NORMAL
            if are_smart_drives_ready is True
            else MachineOperationalState.NON_OPERATIONAL
        )
    ...
    def safety_state(self) -> MachineSafetyState:
        estop_status_result = self._api.get_estop_status()
        estop_status = estop_status_result["status"]
        safety_state = estop_status["estop"]
        if safety_state == "Unknown":
            raise MachineException("Safety state is unknown")
        return (
            MachineSafetyState.NORMAL
            if safety_state == "Released"
            else MachineSafetyState.EMERGENCY_STOP
        )
...
class Machine(IMachine):
    def __init__(self, ip_address: Union[str, None] = None) -> None:
        ...
        machine_motions = _create_machine_motions(
            machine_configuration, self._api, mqtt_client
        )
    ...
    def _on_machine_state_update(
        self, topic: str, message: Optional[str]  # pylint: disable=unused-argument
    ) -> None:
        # topic: "execution-engine/estop"
        # message: {estop: "Unkown" | "Triggered" | "Released", areSmartDrivesReady: "true" | "false", detectedNetworks: dict[] | undefined}
        ...
        incoming_operational_state = parsed_message["areSmartDrivesReady"]
        incoming_safety_state = parsed_message["estop"]

        operational_state_enum = (
            MachineOperationalState.NORMAL
            if incoming_operational_state is True
            else MachineOperationalState.NON_OPERATIONAL
        )

        safety_state_enum = (
            MachineSafetyState.NORMAL
            if incoming_safety_state == "Released"
            else MachineSafetyState.EMERGENCY_STOP
        )

        if incoming_safety_state == "Unknown":
            raise MachineException("Safety state is unknown")

        if self._on_state_change_callback is not None:
            self._on_state_change_callback(operational_state_enum, safety_state_enum)
