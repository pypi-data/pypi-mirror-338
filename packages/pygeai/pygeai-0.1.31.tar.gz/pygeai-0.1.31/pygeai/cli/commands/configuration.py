...
def configure(option_list: list[str, str] = None):
    if not any(option_list):
        ...
        studio_url = str(input("-> Insert your GEAI API STUDIO URL (Leave empty to keep current value): "))
        if studio_url:
            configure_studio_url(studio_url, alias)
        ...
        for option_flag, option_arg in option_list:
            ...
            if option_flag.name == "studio_url":
                configure_studio_url(studio_url=option_arg, alias=alias)
...
def configure_eval_url(eval_url: str, alias: str = "default"):
    ...
    sys.stdout.write(f"GEAI API EVAL URL for alias '{alias}' saved successfully!\n")


def configure_studio_url(studio_url: str, alias: str = "default"):
    settings = get_settings()
    settings.set_studio_url(studio_url, alias)
    sys.stdout.write(f"GEAI API STUDIO URL for alias '{alias}' saved successfully!\n")
...
configuration_options = (
    ...
    Option(
        "studio_url",
        ["--studio-url", "--su"],
        "Set GEAI API STUDIO URL for the AI Studio",
        True
    ),
    Option(
        "alias",
