...
class SettingsManager:
    ...
    def get_setting_value(self, setting_key: str, alias: str):
        ...
        if setting_key not in self.config[alias]:
            ...
            if setting_key.lower() == "GEAI_API_STUDIO_URL".lower():
                self.set_studio_url("", alias)
    ...
    def get_studio_url(self, alias: str = "default"):
        studio_url = os.environ.get("GEAI_API_STUDIO_URL") if alias == "default" else None
        if not studio_url:
            studio_url = self.get_setting_value("GEAI_API_STUDIO_URL", alias)

        return studio_url

    def set_studio_url(self, studio_url, alias: str = "default"):
        self.set_setting_value("GEAI_API_STUDIO_URL", studio_url, alias)
