...
class DAG:
    def __init__(self, input_text_arr, grid_mapping: dict[str, tuple[WorldObj, tuple[int, int]]]):
        """
        Initialize the DAG using an input array of arrays of strings.
        """
        self.adj_list, self.nodes = self._create_dag(input_text_arr, grid_mapping)
    ...
    def _create_dag(self, input_text_arr, grid_mapping: dict[str, tuple[WorldObj, tuple[int, int]]]) -> tuple[dict[int, list[int]], dict[int, DagNode]]:
        """
        ...
        for i in range(len(input_text_arr)):
            for j in range(len(input_text_arr[i])):
                ...
                if (pred, obj) not in pred_obj_dict:
                    ...
                    worldObj, coord = grid_mapping[obj]
