...
class HomeLanUtils:
    ...
    def hlan_checkurl_shutdown_bj(
        ...
        *,
        ...
        @reboot python3 -c "from dknovautils import *; HomeLanUtils.hlan_checkurl_shutdown_bj(cmd='pve_stop_all');" >>/dev/null
        ...
        if cmd == "pve_stop_all":
            ...
            """
            CMD_pve_stop_all = """ for vm in $(qm list | grep running | awk '{print $1}'); do qm shutdown $vm; done; """.strip()
        ...
        def cb(rss: List[str | None]) -> bool:
            ...
            rss = rss[-NL:]
            rss = [("OK" if rs else None) for rs in rss]
            shutdown = (len(rss) == NL) and all(rs is None for rs in rss)
            if shutdown:
                if verbose:
                    print(f"{ts} connect url:{url} bad")
                # cmd = "shutdown -P now"
                run_simple_a(cmd)
                return False
            else:
