...
@asset(
    ...
    ins={"b_df_lazy": AssetIn("b_df_lazy", metadata={"columns": ["a"]})},
)
...
def test_not_supported_type(io_manager):
    resource_defs = {"io_manager": io_manager}
...
def load_partitioned(daily_partitioned: pl.DataFrame) -> pl.DataFrame:
...
def test_dynamic_partition(tmp_path, io_manager):
    with instance_for_test() as instance:
        ...
        instance.add_dynamic_partitions(dynamic_fruits.name, ["apple"])  # type: ignore
        ...
        instance.add_dynamic_partitions(dynamic_fruits.name, ["orange"])  # type: ignore
