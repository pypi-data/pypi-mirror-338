...
from ydata_profiling.model import BaseAnalysis, BaseDescription as Base
...
from ydata.profiling.model.pandas.duplicates import get_potential_duplicates, get_duplicates, get_near_duplicates
...
from ydata.profiling.model.scores import describe_scores
from ydata.profiling.model.description import BaseDescription
from ydata.profiling.model.alerts import get_alerts
...
def describe(
    ...
    with tqdm(
        ...
        duplicates_df = progress(get_potential_duplicates, pbar, "Detecting duplicates")(
            config, df, supported_columns
        )

        metrics, duplicates = progress(get_duplicates, pbar, "Calculate exact duplicates")(
            config, duplicates_df, len_df
        )
        table_stats.update(metrics)

        # Near duplicates
        metrics, near_duplicates = progress(get_near_duplicates, pbar, "Calculate near duplicates")(
            config, duplicates_df, len_df
        )
        table_stats.update(metrics)
    ...
    #might need a few more metrics for this logic
    scores = describe_scores(config=config,
