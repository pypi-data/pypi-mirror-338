...
class SetSocks5Tunnel():
    ...
    def is_tunnel_active(self, local_port):
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            self.logger.debug(f"Checking if tunnel is active, local-port: {local_port}, status: {s.connect_ex(("localhost", local_port)) == 0}")
    ...
    async def tunnel_status(self, timeout, test_port, local_port):
        self.logger.debug(f"Checking the tunnel status at local-port {local_port}, test-port {test_port}, timeout {timeout}")
        self.logger.info(f"Tunnel status {self.is_tunnel_active(local_port=local_port)}")
        if self.is_tunnel_active(local_port=local_port):
    ...
    def test_proxy(self, timeout, test_port, local_port):
        ...
        print ("bh",self.cfg.get("bastion_host"))
        try:
