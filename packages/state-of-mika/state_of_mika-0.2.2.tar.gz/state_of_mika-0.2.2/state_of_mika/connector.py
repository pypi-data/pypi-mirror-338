...
try:
    ...
    logger.warning(f"Could not import MCP components: {e}. Will try again when needed.")

from .registry import Registry
from .installer import Installer

def ensure_mcp_imports():
    """
    Ensure that MCP components are imported.
    
    This function attempts to import MCP components if they weren't imported at module load time.
    It's called before any server connection to make sure we have the required components.
    
    Returns:
        bool: True if imports are successful, False otherwise
    """
    global HAS_MCP, ClientSession, StdioServerParameters, stdio_client
    ...
    try:
        ...
        from mcp import StdioServerParameters as SSP
        ...
        logger.info("Successfully imported MCP components on demand")
        return True
    except ImportError as e:
...
class Connector:
    ...
    async def connect_to_server(self, server_name: str) -> 'ClientSession':
        ...
        # Ensure MCP components are imported
        if not ensure_mcp_imports():
            raise RuntimeError(f"Cannot connect to server {server_name}: MCP components could not be imported")
    ...
    async def execute_capability(
        ...
        try:
            ...
            if not installed_servers:
                ...
                if auto_install and matching_servers:
                    ...
                    for server_to_install in matching_servers:
                        ...
                        if installed:
                            logger.info(f"✅ Successfully installed server '{server_name}' for capability: {capability}")
                            # Only verify the installation once to avoid redundant checks
                            if self.registry.is_server_installed(server_name):
                                ...
                                # If verification fails despite successful installation, provide detailed error
                                logger.warning(f"⚠️ Server '{server_name}' reports as installed but verification failed")
                                return {
                                    "status": "error",
                                    "error": f"Server '{server_name}' was installed but verification failed",
                                    "error_type": "Verification Error",
                                    "suggestion": "The server may not be properly installed. Try installing it manually or check the logs for details.",
                                    "capability": capability,
                                    "tool_name": tool_name,
            ...
            # Create a structured error response that MikaAdapter can analyze
            structured_error = {
                "status": "error",
                "error": error_message,
                "capability": capability,
            ...
            if "API key" in error_message or "Authentication" in error_message or "Unauthorized" in error_message:
                ...
                if capability == "weather":
                    ...
                    if not os.environ.get(key_name):
                        structured_error["missing_api_key"] = key_name
                        structured_error["suggestion"] = f"Set the {key_name} environment variable with your AccuWeather API key. You can get one at https://developer.accuweather.com/"
                    else:
                        structured_error["suggestion"] = f"Check if your {key_name} is valid and has not expired. You may need to regenerate it at https://developer.accuweather.com/"
                else:
                    # Generic API key error
                    structured_error["suggestion"] = "Check if the required API key is set in the environment variables."
            elif "Connection" in error_message or "Timeout" in error_message:
                error_type = "Network Error"
                structured_error["suggestion"] = "Check your internet connection and try again."
            elif "No such file or directory" in error_message:
                error_type = "File Not Found"
                structured_error["suggestion"] = "A required file or tool is missing. Try reinstalling the server."
            elif "ImportError" in error_message or "ModuleNotFoundError" in error_message:
                error_type = "Missing Dependency"
                # Extract the missing module name if possible
                missing_module = None
                if "No module named '" in error_message:
                    parts = error_message.split("No module named '")
                    if len(parts) > 1:
                        missing_module = parts[1].split("'")[0]
                        structured_error["missing_dependency"] = missing_module
                
                structured_error["suggestion"] = "A required Python module is missing. Try reinstalling the server."
                if missing_module:
                    structured_error["suggestion"] = f"Install the missing dependency with: pip install {missing_module}"
            elif "'str' object has no attribute 'get'" in error_message:
                error_type = "Server Configuration Error"
                structured_error["suggestion"] = "There was an issue finding a server for this capability. Try setting AUTO_INSTALL_SERVERS=true."
            elif "'NoneType' object is not callable" in error_message:
                error_type = "MCP Configuration Error"
                # This is often due to issues with MCP components not being properly initialized
                structured_error["suggestion"] = "There was an issue with the MCP server connection setup. Try installing the mcp package with: pip install mcp"
                structured_error["error"] = "Failed to initialize MCP connection components. The MCP package may not be properly installed."
            else:
                # Default suggestion for unknown errors
                structured_error["suggestion"] = "An unexpected error occurred. Please check the logs for more details."
                
            # Add the error type to the response
            structured_error["error_type"] = error_type
            
            # Return the structured error that can be passed to MikaAdapter.analyze_error
