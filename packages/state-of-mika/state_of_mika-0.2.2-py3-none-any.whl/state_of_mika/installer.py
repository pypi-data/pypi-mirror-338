...
class Installer:
    ...
    async def install_server(self, server_data: Union[str, Dict[str, Any]]) -> bool:
        ...
        # Clear any previous installation flags for this server
        for attr in dir(self.registry):
            if attr.startswith(f"_just_installed_{server_name}"):
                delattr(self.registry, attr)
            ...
            try:
                ...
                if github_parts and len(github_parts) >= 2:
                    ...
                    if process.returncode != 0:
                        ...
                        # Record what was actually installed by analyzing the npm output
                        # Look for "added X packages" or similar patterns in stdout
                        stdout_text = stdout.decode()
                        
                        # Extract any installed package names from the output if available
                        installed_packages = []
                        for line in stdout_text.splitlines():
                            if "+ " in line:  # npm typically shows installed packages with a + sign
                                package_info = line.strip().split("+ ")[1].split("@")[0] if "@" in line else line.strip()
                                installed_packages.append(package_info)
                        
                        if installed_packages:
                            logger.info(f"Detected installed packages: {', '.join(installed_packages)}")
                            
                            # Save the list of installed packages to help with verification
                            setattr(self.registry, f"_installed_packages_{server_name}", installed_packages)
                        
                        # Create a record of where the tarball was installed to help with verification
                        try:
                            # Find installed location using npm list
                            ...
                            for line in npm_list.stdout.splitlines():
                                if line.strip().startswith("├──") or line.strip().startswith("└──"):
                                    installed_pkg = line.strip().split(" ")[1]
                                    if "@" in installed_pkg:
                                        pkg_name = installed_pkg.split("@")[0]
                                        # If the package name contains the server name or vice versa
                                        if server_name in pkg_name or pkg_name in server_name:
                                            logger.info(f"Found likely match for {server_name}: {installed_pkg}")
                                            setattr(self.registry, f"_installed_npm_package_{server_name}", pkg_name)
                        except Exception as e:
                            logger.warning(f"Unable to determine exact installed package name: {e}")
