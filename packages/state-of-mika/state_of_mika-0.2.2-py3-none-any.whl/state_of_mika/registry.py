...
class Registry:
    ...
    def is_server_installed(self, server_name: str) -> bool:
        ...
        # For npm packages, check if the package is installed with npm list
            ...
            try:
                # Get package name from installation info
                package_name = install_info.get("package", server_name)
                
                # Try multiple approaches to detect npm packages
                import subprocess
                import os
                import glob
                
                # Method 1: Check if there's a binary in PATH
                ...
                if which_result.returncode == 0 and which_result.stdout.strip():
                    logger.info(f"✓ Found npm package '{server_name}' at {which_result.stdout.strip()} ✅")
                    return True
                
                # Method 2: Check global npm packages
                npm_list_global = subprocess.run(
                    ["npm", "list", "-g", "--depth=0"], 
                    capture_output=True, 
                    text=True
                )
                ...
                if package_name in npm_list_global.stdout:
                    logger.info(f"✓ Found npm package '{package_name}' in global packages ✅")
                    return True
                    
                # Method 3: Check local npm packages
                npm_list_local = subprocess.run(
                    ["npm", "list", "--depth=0"], 
                    capture_output=True, 
                    text=True
                )
                ...
                # Check if the package name appears in the output
                if package_name in npm_list_local.stdout:
                    logger.info(f"✓ Found npm package '{package_name}' in local packages ✅")
                    return True
                    
                # Method 4: Look for the package in node_modules directories
                # First check in current directory
                current_dir_modules = glob.glob("node_modules/*")
                for module_path in current_dir_modules:
                    if os.path.basename(module_path) == package_name:
                        logger.info(f"✓ Found npm package '{package_name}' in local node_modules ✅")
                        return True
                        
                # Then check in global node_modules directories
                    ...
                    for path in glob.glob(path_pattern):
                        if os.path.isdir(path):
                            ...
                            for module_path in modules:
                                if os.path.basename(module_path) == package_name:
                                    logger.info(f"✓ Found npm package '{package_name}' in global node_modules at {path} ✅")
                                    return True
                
                # Check if we have a record of installed npm packages for this server
                installed_packages_attr = f"_installed_packages_{server_name}"
                if hasattr(self, installed_packages_attr):
                    installed_packages = getattr(self, installed_packages_attr)
                    logger.info(f"Found record of installed npm packages for {server_name}: {installed_packages}")
                    return True
                
                # Check if we have a record of an exact npm package name for this server
                installed_npm_attr = f"_installed_npm_package_{server_name}"
                if hasattr(self, installed_npm_attr):
                    installed_pkg = getattr(self, installed_npm_attr)
                    logger.info(f"Found record of npm package for {server_name}: {installed_pkg}")
                    return True
                
                # If we get here, the package was not found
                logger.info(f"✗ npm package '{package_name}' is NOT installed ❌")
                return False
            except Exception as e:
