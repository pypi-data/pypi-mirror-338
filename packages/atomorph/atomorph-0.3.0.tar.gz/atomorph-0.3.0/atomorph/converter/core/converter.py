                        ...
                        if constraints == 'fixed':
                            ...
                            if constraints[0] == 'elements':
                                ...
                                for layer_range in constraints[1:]:
                                    try:
                                        # 检查是否有fractional_前缀
                                        if isinstance(layer_range, str) and layer_range.startswith('fractional_'):
                                            use_fractional = True
                                            # 去除前缀
                                            layer_range = layer_range[11:]  # 移除 'fractional_'
                                            
                                        if use_fractional:
                                            # 使用分数坐标
                                            cell_inv = np.linalg.inv(structure.cell)
                                            frac_pos = np.dot(pos, cell_inv)
                                            frac_z = frac_pos[2]  # 使用分数坐标的Z值
