...
class StructureConverter:
    ...
    def _write_vasp(self, structure: Atoms, output_path: Union[str, Path], constraints: Optional[Union[str, List[str]]] = None) -> None:
        ...
        print(f"调用_write_vasp，约束：{constraints}")
                            ...
                            if ',' in layer_range:
                                ...
                                if use_fractional:
                                    ...
                                    for i, frac_pos in enumerate(frac_positions):
                                        ...
                                        if start <= frac_z <= end:
                                            fixed_atoms.append(i)
                                            print(f"固定原子 {i+1} ({elements[i]}): 分数Z坐标 {frac_z:.5f} 在范围 {start}-{end} 内")
                                else:
                                    # 使用笛卡尔坐标
                                    z_values = [pos[2] for pos in structure.positions]
                                    print(f"实际Z坐标范围: {min(z_values):.5f} - {max(z_values):.5f}")
                                    
                                    for i, pos in enumerate(structure.positions):
                                        z = pos[2]
                                        if start <= z <= end:
                                            fixed_atoms.append(i)
                                            print(f"固定原子 {i+1} ({elements[i]}): 实际Z坐标 {z:.5f} 在范围 {start}-{end} 内")
                            else:
                                print(f"警告: 层范围参数格式错误 '{layer_range}'，应为 'start,end'")
                                
                        except ValueError as e:
                            print(f"警告: 无法解析层范围 '{layer_range}': {str(e)}")
                    else:
                        print("警告: 没有提供层范围参数")
                    
                    fixed_count = len(fixed_atoms)
                    print(f"总共固定了 {fixed_count} 个原子")
                    ...
                    for idx_str in constraints[1:]:
                        try:
                            if '-' in str(idx_str):
                                ...
                                for idx in range(start-1, end):  # 转换为0-based索引
                                    if 0 <= idx < total_atoms:
                                        fixed_atoms.append(idx)
                            else:
                                idx = int(idx_str) - 1  # 转换为0-based索引
                                if 0 <= idx < total_atoms:
                                    fixed_atoms.append(idx)
                        except ValueError as e:
                            print(f"警告: 无法解析索引 '{idx_str}': {str(e)}")
                    
                    fixed_count = len(fixed_atoms)
                    print(f"固定索引，共 {fixed_count} 个原子")
        ...
        print(f"VASP文件已写入: {output_path}")
        print(f"总原子数: {total_atoms}, 固定原子数: {fixed_count}")