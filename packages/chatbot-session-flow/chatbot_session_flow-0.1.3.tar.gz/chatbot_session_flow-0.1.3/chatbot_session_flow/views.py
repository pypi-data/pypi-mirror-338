...
def validateOptionMessage(request):
    ...
    if stepper:
        ...
        call_dymanic_method(session, stepper.page.prev, message)
        if message:
...
# def sendTextMessages(request, session: Session, stepper: Stepper, currentPage: Page):
#     message = currentPage.message
#     sendWhatsappMessageViaNgumzo(
...
def sendWhatsappMessageViaNgumzo(session, phoneNumber, message):
    url = "https://ngumzo.com/v1/send-message"
...
def perform_dynamic_action(session, page:Page, message:str, *args, **kwargs):
    """
    Dynamically calls a function from views based on `page.task`.
    """
    task = getattr(page, "task", None)  # Get task safely

    if not task:
        print("Task is not found !")
        return

    # Fetch function dynamically from the global scope
    action_function = globals().get(task)

    if callable(action_function):  # Ensure it's a valid function before calling
        response = action_function(session, page, message, args, kwargs)
    else:
        response = f"No valid action found for task: {task}"
        print(response)

def add_to_cart(session, page:Page, message:str, *args, **kwargs):
    try:
        print("adding to cart -> " + str(page.id))
    except Exception as e:
        print("Exception -> {}".format(e))