    ...
    try:
        ...
        if message_id:
                ...
                if 'filew_id' not in POST_MEDIA[index]:
                    ...
                    if BOT_TOKEN:
                        ...
                        try:
                            ...
                            padding = 10
                            bg_radius = 15
                            bg_color = (0, 0, 0, 128)  # Чёрный с 50% прозрачностью

                            x = width - text_w - 40
                            y = height - text_h - 40

                            # Создаём фон с закруглёнными краями
                            ...
                            #
                            # truetype_ = os.path.join(EXTRA_D, 'Roboto-Regular.ttf')
                            # font = ImageFont.truetype(truetype_, 25)
                            #
                            # draw.text((width - len(POST_WATER) * 12 - 40, height - 40), POST_WATER, font=font,
                            #           fill=(255, 255, 255))
                            # image.save(POST_FNAME_COPY)
            ...
            try:
                ...
                result = await bot.send_video(chat_id=chat_id, video=POST_MEDIA[index]['file_id_note'], caption=POST_TEXT,
                                          parse_mode=ParseMode.HTML,
                                          show_caption_above_media=POST_CHKBOX['POST_ISPREVIEW'],
                                          has_spoiler=POST_CHKBOX['POST_ISSPOILER'],
                                          disable_notification=not POST_CHKBOX['POST_ISSOUND'],
