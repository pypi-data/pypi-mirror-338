...
async def post_save(bot, data_user, data_web, MEDIA_D, BASE_P, KEYS_JSON, PROJECT_USERNAME, PROJECT_TYPE):
    ...
    try:
        ...
        elif POST_ISPAY and POST_STARS and BOT_TOKEN:
            try:
                extra_bot = Bot(token=BOT_TOKEN)
                try:
        ...
        result['post'] = {
            ...
            'POST_LZ': POST_LZ, 'POST_TZ': POST_TZ, 'POST_DT': POST_DT, 'POST_TR': POST_TR,
        }
...
async def post_pub(bot, lz, chat_id, ENT_TID, post, MEDIA_D, BASE_S, BASE_D, PROJECT_USERNAME, is_private=True,
    ...
    try:
        ...
        print(f"========================")
        if len(POST_MEDIA) > 1 and POST_ISGALLERY and not POST_ISCOLLAPSE and not POST_ISPAY and POST_TYPE not in [
            'sticker']:
            POST_TEXT = '' if POST_TEXT == str_empty and POST_TYPE != 'text' else POST_TEXT
                ...
                if 'filew_id' not in POST_MEDIA[index]:
                    ...
                    if BOT_TOKEN:
                        ...
                        try:
                            ...
                            POST_FNAME_COPY = os.path.join(MEDIA_D, str(ENT_TID),
                                                           f"{os.path.basename(POST_FNAME1)}_copy{ext}")
                            POST_FNAME_COPY = await add_water_to_photo(POST_FNAME, POST_FNAME_COPY, POST_WATER, EXTRA_D)
