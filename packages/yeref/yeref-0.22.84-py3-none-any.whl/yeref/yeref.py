...
async def save_posts(chat_id, lz, username, bot, data, tz, MEDIA_D, BASE_D, KEYS_JSON, PROJECT_USERNAME, PROJECT_TYPE):
    result = {'result': True, 'bot': True, 'POST_BLOG': None, 'POST_WEB': None}
    try:
        posts = data['posts']
        ENT_TID = str(data['ENT_TID'])
        POST_TID = posts['POST_TID']
        POST_TYPE = posts['POST_TYPE']
        POST_TEXT = posts['POST_TEXT'].strip()
        BOT_TOKEN = posts['BOT_TOKEN'] if 'BOT_TOKEN' in posts else None
        os.makedirs(os.path.join(MEDIA_D, str(ENT_TID)), exist_ok=True, mode=0o777)
        ...
        POST_STARS = posts['POST_STARS'] if 'POST_STARS' in POST_CHKBOX else ''
        POST_ISWATER = posts['POST_ISWATER'] if 'POST_ISWATER' in POST_CHKBOX else False
        POST_WATER = posts['POST_WATER'] if 'POST_WATER' in POST_CHKBOX else ''
        ...
        print(f"{POST_DT=}, {POST_TARGET=}, {POST_TOKEN=}")
        ...
        result = {'result': True, 'bot': True, 'POST_BLOG': POST_BLOG, 'POST_WEB': POST_WEB}
        print(f"{result=}")
        if POST_TOKEN and len(json.loads(POST_MEDIA)):
            POST_MEDIA, res_bot = await update_media(bot, chat_id, lz, ENT_TID, POST_TOKEN, POST_MEDIA, MEDIA_D, PROJECT_USERNAME)
            result = {'result': True, 'bot': res_bot, 'POST_BLOG': POST_BLOG, 'POST_WEB': POST_WEB}

            print(f"before {POST_ISWATER=}, {POST_WATER=}, {BOT_TOKEN=}, {POST_MEDIA=}")
            if POST_TYPE == 'photo' and BOT_TOKEN:
                for it in POST_MEDIA:
                    if not (POST_ISWATER and POST_WATER and 'filew_id' not in it): continue
                    ...
                    extra_bot = Bot(token=BOT_TOKEN)
                    try:
                        file = await extra_bot.get_file(it['file_id'])
        ...
        if len(data_media):
            ...
            if POST_MEDIA_OLD:
                ...
                new_ids = {media["file_id"] for media in json.loads(POST_MEDIA)}
...
async def post_pub(bot, lz, chat_id, ENT_TID, item, MEDIA_D, BASE_S, BASE_D, PROJECT_USERNAME, is_private=True, message_id=None, cur_=1):
    result = False
    try:
