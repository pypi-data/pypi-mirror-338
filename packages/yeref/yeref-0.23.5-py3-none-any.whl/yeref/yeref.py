...
async def pst_gen_ent2(bot, chat_id, lc, lz, page, POST_TID, POST_TYPE, POST_MEDIA, ENT_TID, PROJECT_TYPE, ENT_USERNAME, ENT_TITLE, MEDIA_D, EXTRA_D, BASE_D, prompt='', ENT_TOKEN_=None, request_app=None):
    result = dst = None
    try:
        ...
        if POST_TYPE == 'text':
            ...
            if POST_MEDIA and len(POST_MEDIA) and POST_MEDIA[0]['file_type'] == POST_TYPE:
                POST_MEDIA.append({"file_id": POST_FID, "file_id_note": POST_FIDNOTE, "fileb_id": POST_FID, "fileb_id_note": POST_FIDNOTE, "file_type": POST_TYPE, "file_name": POST_FILENAME, "file_link": POST_LNK, "file_link2": POST_LNK2})
            else:
                POST_MEDIA = [{"file_id": POST_FID, "file_id_note": POST_FIDNOTE, "fileb_id": POST_FID, "fileb_id_note": POST_FIDNOTE, "file_type": POST_TYPE, "file_name": POST_FILENAME, "file_link": POST_LNK, "file_link2": POST_LNK2}]
            ...
            if POST_MEDIA and len(POST_MEDIA) and POST_MEDIA[0]['file_type'] == POST_TYPE:
                POST_MEDIA.append(
                    {"file_id": POST_FID, "file_id_note": POST_FIDNOTE, "fileb_id": POSTB_FID, "fileb_id_note": POST_FIDNOTE, "file_type": POST_TYPE, "file_name": POST_FILENAME,
                ...
                try:
                    ...
                    if MSG_TYPE == 'photo':
                        ...
                        if F_NAME.lower().endswith('.webm') and not IS_LINK:
                            ...
                            POST_FNAME_COPY = os.path.join(MEDIA_D, str(BOT_TID), f"{os.path.basename(POST_FNAME1)}_copy.mp4")
...
async def return_file_link(bot, chat_id, FILE_NAME, KEYS_JSON, MSG_VID, MSG_TYPE, IS_LINK):
    ...
    try:
        ...
        if IS_LINK:
            ...
            if MSG_TYPE in ['document', 'web'] and not FILE_NAME.endswith(('.png', '.jpg', '.jpeg', '.gif', '.mp4', '.webp', '.webm', '.mp3')): return
            ...
            try:
                ...
                if not result or os.path.getsize(FILE_NAME) < 5242880:
                    ...
                    res = await extra_bot.send_audio(chat_id=OWNER_TID, audio=types.FSInputFile(file_name_video),
                                                     thumbnail=thumb, title=title, performer=performer, duration=duration,
                                                     disable_notification=True)
                    await extra_bot.delete_message(OWNER_TID, res.message_id)
                    # if os.path.exists(file_name_video): os.remove(file_name_video)
        ...
        if OFFER_ISTGPH and OFFER_TGPHLINK and '[' not in OFFER_TGPHLINK:
            ...
            if OFFER_ISGALLERY:
                ...
                buttons = [
                    ...
                    types.InlineKeyboardButton(text="→", callback_data=f'pst_{chat_id}_{offer_id}_{cur_}_{len_}_next'), ]
                reply_markup.row(*buttons)
...
async def edit_simple(bot, chat_id, post_id, message_id, cur_, BASE_D):
    ...
    try:
        ...
        if POST_ISGALLERY:
            ...
            buttons = [types.InlineKeyboardButton(text="←", callback_data=f'pst_{chat_id}_{post_id}_0_{cur_}_{len_}_prev'),
                       types.InlineKeyboardButton(text=f"{cur_}/{len_}",
        ...
        if POST_BUTTONS:
            if ENT_TYPE == 'PST':
                ...
                for k, v in dic_btns.items():
                    ...
                    btn_ix = extra_id
                    btn_knd = 'like' if v[-1] == 'pst_' else 'link'
...
async def get_user_vars(PROJECT_USERNAME, BASE_P, ENT_TID, chat_id, lc, username, full_name, is_premium, usr_hash='', page=''):
    USER_TID = chat_id
...
async def save_posts(chat_id, lz, username, bot, data, tz, MEDIA_D, BASE_D, KEYS_JSON, PROJECT_USERNAME, PROJECT_TYPE):
    result = {'result': True, 'bot': True, 'post': data['post']}
    try:
        post = data['post']
        ...
        BOT_TOKEN = data['BOT_TOKEN'] if 'BOT_TOKEN' in data else None
        os.makedirs(os.path.join(MEDIA_D, str(ENT_TID)), exist_ok=True, mode=0o777)
        ...
        POST_BLOG = await region_blog2(bot, ENT_TID, POST_TYPE, POST_TEXT, POST_MEDIA, BASE_D, PROJECT_TYPE)
        ...
        POST_STARS = POST_CHKBOX['POST_STARS'] if 'POST_STARS' in POST_CHKBOX else ''
        ...
        POST_TARGETTYPE = post['POST_TARGETTYPE'] if 'POST_TARGETTYPE' in post else ''
        ...
        if POST_ISPAY and POST_STARS and BOT_TOKEN:
            try:
                ...
                try:
                    ...
                    amount = 1
                prices = [LabeledPrice(label="XTR", amount=amount)]
                POST_PAY = await extra_bot.create_invoice_link(title='Post',
                                                                   description='ent_title',
        ...
        if BOT_TOKEN:
            ...
            if PROJECT_USERNAME == 'FereyChannelBot':
                ...
                await db_change_pg(sql, (BOT_TOKEN, ENT_TID,), BASE_D)
            elif PROJECT_USERNAME == 'FereyGroupBot':
        ...
        await db_change_pg(sql, (POST_TID,), BASE_D)

        sql = f"SELECT POST_TID, POST_MEDIA FROM {schema_prefix}_{ENT_TID}.POST WHERE POST_TID=$1"
        data_media = await db_select_pg(sql, (POST_TID,), BASE_D)
        if len(data_media):
        ...
        result['post'] = {
            ...
            'POST_DT': POST_DT, 'POST_TR': POST_TR,
            'POST_BLOG': POST_BLOG, 'POST_WEB': POST_WEB
        }
        ...
        sql = f"""
            INSERT INTO {schema_prefix}_{ENT_TID}.POST (
                ...
                POST_BUTTONS, POST_CHKBOX, POST_WEB, POST_PAY, POST_BLOG, POST_ISPRIVATE, 
                POST_TZ, POST_DT, POST_TR
            )
        ...
        await db_change_pg(sql, (
            ...
            POST_ISPRIVATE or None, POST_TZ or None, POST_DT or None, POST_TR or None
        ), BASE_D)
        # endregion
    except TelegramRetryAfter as e:
        logger.info(log_ % f"TelegramRetryAfter {e.retry_after}")
...
async def post_pub(bot, lz, chat_id, ENT_TID, post, MEDIA_D, BASE_S, BASE_D, PROJECT_USERNAME, is_private=True, message_id=None, cur_=1):
    result = False
    try:
        ...
        if len(POST_MEDIA) > 1 and POST_ISGALLERY:
            len_ = len(POST_MEDIA)
            if message_id:
                index = cur_ - 1  # POST_MEDIA = [POST_MEDIA[cur_ - 1]]  # else:  # POST_MEDIA = [POST_MEDIA[index]]
        if message_id:
            POST_ISCOLLAPSE = False
        ...
        if POST_ISCOLLAPSE and not message_id and not POST_ISPAY:
        ...
        if message_id:
            ...
            if POST_TYPE == 'text':
                ...
                try:
                    ...
                    media = types.InputMediaDocument(media=POST_MEDIA[index]['file_id'],
                                                     parse_mode=ParseMode.HTML,
                                                     caption=POST_TEXT)
                    await bot.edit_message_media(media=media, chat_id=chat_id, message_id=message_id,
                                                 disable_content_type_detection=True,
        ...
        elif POST_ISPAY and POST_TYPE in ['photo', 'video']:
            ...
            for i in range(0, len(POST_MEDIA)):
                caption = POST_TEXT if i == 0 else None
                if caption: caption = caption if caption not in ['', str_empty] else None
                print(f"not gallery {POST_MEDIA[i]['file_type']=}")
                ...
                if 'filew_id' not in POST_MEDIA[index]:
                    ...
                    if BOT_TOKEN:
                        ...
                        try:
                            ...
                            POST_FNAME_COPY = os.path.join(MEDIA_D, str(ENT_TID), f"{os.path.basename(POST_FNAME1)}_copy{ext}")
                            POST_FNAME_COPY = await add_water_to_photo(POST_FNAME, POST_FNAME_COPY, POST_WATER, EXTRA_D)
            ...
            try:
                ...
                result = await bot.send_audio(chat_id=chat_id, audio=POST_MEDIA[index]['file_id_note'], caption=POST_TEXT,
                                              parse_mode=ParseMode.HTML,
                                              disable_notification=not POST_CHKBOX['POST_ISSOUND'],
                                              protect_content=POST_CHKBOX['POST_ISPROTECT'],
                                              message_effect_id=POST_CHKBOX['POST_EFFECT'],
...
async def get_ent_rm(chat_id, reply_markup, ENT_TID, POST_USERTUN, POST_TID, POST_BUTTONS, BASE_D, PROJECT_USERNAME, cur_='1'):
    try:
        schema_name = 'USER'
        if PROJECT_USERNAME == 'FereyBotBot':
            schema_name = 'Bot'
