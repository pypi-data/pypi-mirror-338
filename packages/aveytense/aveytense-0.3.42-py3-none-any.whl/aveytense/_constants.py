...
from . import _init_types
from . import util as _util
from platform import architecture as _architecture

_qualifier = ("alpha", "beta", "candidate", "final")
_qualifier_type = _init_types.Literal["alpha", "beta", "candidate", "final"]
...
class _VERSION_INFO:
    ...
    def __init__(self, major: int, minor: int, micro: int, releaselevel: _qualifier_type, serial: int, /): # 0.3.40
        ...
        if _counter_version_type == 1:
            error = TypeError("cannot instantiate internal the class '{}'".format(type(self).__name__))
            raise error
    ...
    def __le__(self, other: _version_info_type): # 0.3.40
    ...
    def __ge__(self, other: _version_info_type): # 0.3.40
...
VERSION_INFO = _VERSION_INFO(0, 3, 42, _qualifier[3], 0)
"""
\\@since 0.3.26b3
...
class AbroadHexMode(_init_types.Enum):
    "\\@since 0.3.26rc2"
...
class BisectMode(_init_types.Enum):
    "\\@since 0.3.26rc2"
    
    LEFT = 0
    RIGHT = 1
    
class InsortMode(_init_types.Enum):
    "\\@since 0.3.26rc2"
    
    LEFT = 0
    RIGHT = 1

class ProbabilityLength(_init_types.Enum):
    "\\@since 0.3.26rc2"
...
class ModeSelection(_init_types.Enum):
    "\\@since 0.3.36"
