...
class finalproperty(_types.Generic[_T, _T_return]):
    """
    \\@since 0.3.37
    ...
    Since 0.3.42 this decorator will no longer provide deduction to `typing.Any`. \\
    It is also worth noticing that the property itself is a class property, which \\
    creation is removed since Python 3.13. However, it is advisable to access final \\
    property via instance.
    """
        
    def __init__(self, f: _types.Callable[[_T], _T_return], /):
        
        self.func = f
        self.__doc__ = f.__doc__
        self.__isabstractmethod__ = bool(getattr(f, "__isabstractmethod__", False))
        self.__finalproperty__ = True
        
    def __str__(self):
        
        import re
        
        return "<final property " + re.sub(r"^<function '|'>", "", self.func)
    
    def __get__(self, instance: _T, ownerclass = None, /):
...
class ParamVar:
    ...
    def __init__(self, f: _types.Callable[..., _types.Any], i = 0, /): # 0.3.42
        ...
        def _inspect_func():
            ...
            return (isinstance(self.__func, _types.MethodType) and not isinstance(self.__func, staticmethod)) or (_reckon(_types.get_overloads(func)) > 0 or func.__module__ in _typing._overload_registry and
    ...
    def universal(self): # 0.3.42
        ...
        return self.func.__code__.co_varnames[_left : self.func.__code__.co_argcount]
...
class MutableString:
    ...
    def __imul__(self, other): # 0.3.42
        ...
        if type(other) is int:
            ...
            if other > 0:
                self.__str = list(self.__str__() * other)
    ...
    @_builtins_types.DynamicClassAttribute
