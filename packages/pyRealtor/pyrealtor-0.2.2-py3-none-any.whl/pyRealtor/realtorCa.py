...
class RealtorCa(Realtor):
    ...
    def set_transaction_type(self, transaction_type: str = 'for_sale'):
        ...
        if transaction_type.lower() == "for_sale":
            keep_col = ["Price"]
            remove_col = ["Rent", "Total Rent"]
        else:
            keep_col = ["Rent", "Total Rent"]
            remove_col = ["Price"]

        if remove_col[0] in self.report_obj.column_lst:
            #self.report_obj.column_lst.remove(remove_col)
            self.report_obj.column_lst = [col for col in self.report_obj.column_lst if col not in remove_col]
        if keep_col[0] not in self.report_obj.column_lst and keep_col[1] not in self.report_obj.column_lst:
            self.report_obj.column_lst.extend(keep_col)
    ...
    def transform(self, df):
        if "Total Rent" in self.report_obj.column_lst:
            df["Rent"] = df["Total Rent"]
        return df
