...
logging = importlib.import_module(name = 'logging')
import traceback
        ...
        try:
            ...
            for idx, query in enumerate(queries):
                if query.strip():  # Skip empty queries
                    ...
                    logger.info(f"Executed query {idx + 1} in file: {filename}")
...
def insert_into_table(SCHEMA: str, df: pd.DataFrame,
                      table_name: str,
                      cursor: mysql.connector,
                      connection: mysql.connector,
                      logger: Logger
    ...
    try:    
        ...
        update_null_values(SCHEMA = SCHEMA,
    ...
    try:
        ...
        if connection.is_connected():
            ...
            for table_name, df in df_dict.items():
                insert_into_table(SCHEMA = schema,
                                  df = df,
                                  table_name = table_name,
                                  cursor = cursor,
                                  connection = connection,
