...
class CCDDeidentifier:
    def __init__(self, secret_salt: str = "some_super_secret_salt"):
        ...
        try:
            spacy.cli.download(f"{self.ner_model}", quiet=False)
        except Exception:
            print(
                f"Could not download {self.ner_model}. Continuing without it. Assuming Inital Download worked and will attempt to load")
        ...
        self.mask_map: Dict[str, List[Dict]] = {}
        self.file_name = None
        self.file_content = None

    def mask_map_to_data_frame(self):
        records = []
        for dict_key, items in self.mask_map.items():
            for item in items:
                row = {
    ...
    def mask_value(self, original: str, category: str = "GENERAL", xpath: str = "", file_name: str = "") -> str:
        """
        ...
        existing_items = self.mask_map.get(dict_key, [])
        ...
        self.mask_map.setdefault(dict_key, []).append({
            "filename": file_name,
    ...
    def _mask_name_elements(self, root: etree._Element, ns: dict):
        ...
        for elem in name_elems:
            if elem.text:
                ...
                elem.text = self.mask_value(elem.text, "NAME", xp)
    ...
    def _mask_id_elements(self, root: etree._Element, ns: dict):
        ...
        for elem in id_elems:
            ...
            if val:
                ...
                masked_val = self.mask_value(val, cat, xp)
    ...
    def _mask_birth_time(self, root: etree._Element, ns: dict):
        ...
        for elem in birth_elems:
            ...
            if val:
                xp = self._generate_specific_xpath(elem)
                masked_val = self.mask_value(val, "DATE", xp)
        ...
        for addr in addr_elems:
            for child in addr:
                ...
                if child.text:
                    ...
                    child.text = self.mask_value(child.text, "ADDR", xp)
        ...
        for text_elem in text_elems:
            ...
            for ent in doc.ents:
                if ent.label_ in entities_to_mask:
                    ...
                    masked_text = self.mask_value(ent.text, cat, xp)
            ...
            def apply_regex_patterns(text, pattern, category):
                ...
                for match in pattern.finditer(text):
                    ...
                    masked_text = self.mask_value(match.group(), category, xp)
                    result += masked_text
                    last_pos = e
                result += text[last_pos:]
                return result

            masked_result = apply_regex_patterns(masked_result, self.phone_pattern, "PHONE")
            masked_result = apply_regex_patterns(masked_result, self.ssn_pattern, "SSN")
            masked_result = apply_regex_patterns(masked_result, self.email_pattern, "EMAIL")
    ...
    def deidentify_ccd_xml(self, xml_path: str) -> str:
        ...
        """
        self.file_name = xml_path
        parser = etree.XMLParser(remove_blank_text=False)
        xml_string = self._get_file_contents(xml_path)
        try:
            tree = etree.parse(io.StringIO(xml_string), parser)
            root = tree.getroot()
        ...
        self._mask_name_elements(root, tree, ns)
        self._mask_id_elements(root, tree, ns)
        self._mask_birth_time(root, tree, ns)
        self._mask_address(root, tree, ns)
        self._mask_telecom(root, tree, ns)
