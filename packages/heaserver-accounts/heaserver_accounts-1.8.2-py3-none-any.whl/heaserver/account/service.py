...
@action('heaserver-accounts-awsaccount-get-searchitem', rel='hea-search hea-context-menu', path='awsaccounts/{id}/search')
...
@routes.get('/awsaccounts/{id}/search/')
@action(name='heaserver-accounts-awsaccount-searchitem-get-actual', rel='hea-actual', path='{+actual_object_uri}')
    ...
    async with DesktopObjectActionLifecycle(request=request,
        ...
        try:
            ...
            for sr_dict in search_items:
                ...
                account_url = str(URL(external_base) / 'awsaccounts' / id_)
                if sr_dict['bucket_id'] and sr_dict['id']:
                    ...
                    internal_url = URL(bucket_base)/volume_id/"buckets"/sr_dict['bucket_id']
                    urls = await _get_context_path(request, sr_dict['path'], internal_url, external_base_url)
                    ext_urls = []
                    for i in range(len(urls)):
                        external, internal_id, type = urls[i]
                        if i == len(urls) - 1:
                            sr_dict['actual_object_uri'] = str(actual_object_bucket_uri/internal_id)
                            sr_dict['actual_object_type_name'] = type
                        ext_urls.append(external)
                    cxt_paths =[account_url, external_base_url]
                    cxt_paths.extend(ext_urls)
                    sr_dict['context_dependent_object_path'] = [encode_key(c) for c in cxt_paths]
                else:
            ...
            async with SQSClientContext(request=request,
                ...
                while True:
                    try:
                        ...
                        logger.info("Messages inserted into index %s" % inserted  )
...
async def _get_context_path(request: web.Request, path: str, internal_url: str, external_url: str)\
    ...
    #ignore leading slash and bucket name at root in all paths
    if not path:
    ...
    # file check
    if len(folders) > 0 and folders[-1]:
        current_encoded_key = encode_key('/'.join(folders))
        key_name = 'awss3files'
        hierarchical_paths.append((f"{external_url}/{key_name}/{current_encoded_key}",
                                   f"{key_name}/{current_encoded_key}",
                                   AWSS3FileObject.get_type_name()))
        folders.pop()
    # create folder/project encoded paths
    while len(folders) > 0:
        current_encoded_key = encode_key(('/'.join(folders) + '/'))
        key_name = 'awss3projects'
        if await client.has(request.app, f"{internal_url}/{key_name}/{current_encoded_key}", AWSS3Project):
            hierarchical_paths.append((f"{external_url}/{key_name}/{current_encoded_key}",
                                       f"{key_name}/{current_encoded_key}",
    ...
    return list(reversed(hierarchical_paths))
