...
def test_new_command_warehouse_basic(
    ...
    assert "compute_pool=" in cmd_args
    ...
    env_content = yaml.safe_load((tmp_path / "environment.yml").read_text())
...
def test_new_command_compute_pool_basic(
    ...
    cmd_args = mock_snow_info.call_args[0][0]
    assert "app" in cmd_args
    assert "compute_pool=TEST_POOL" in cmd_args
    assert "query_warehouse=" in cmd_args
