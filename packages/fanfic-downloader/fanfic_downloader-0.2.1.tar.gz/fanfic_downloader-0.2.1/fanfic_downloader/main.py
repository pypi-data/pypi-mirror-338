...
def download_stories(
    ...
    # Get URLs passed directly from another function
    if direct_urls:
    ...
    console.print(f"[bold]Downloading [cyan]{len(all_urls)}[/cyan] stories to [blue]{target_folder}[/blue][/bold]")
    ...
    with Progress(
        ...
        download_task = progress.add_task("[bold]Downloading stories...[/bold]", total=len(all_urls))
        success_count = 0
        error_count = 0
...
def extract_story_urls(
    ...
    if download and all_extracted_urls:
        if Confirm.ask(f"Do you want to download all {len(all_extracted_urls)} extracted stories?"):
            ...
            download_stories(direct_urls = all_extracted_urls)
