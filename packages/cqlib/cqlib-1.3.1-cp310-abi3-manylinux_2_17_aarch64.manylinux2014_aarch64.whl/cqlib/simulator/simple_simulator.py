...
class SimpleSimulator:
    ...
    def __init__(
        ...
        try:
            ...
            raise ImportError("PyTorch is not installed. `pip install torch` "
                              "Please install it to use the SimpleSimulator. ") from e
        if device is None:
    ...
    def statevector(self, dict_format=True) -> dict[str, complex] | list[complex]:
        ...
        if dict_format:
            return {np.binary_repr(i, width=self.nq): val for i, val in enumerate(self._state)}
        return self._state
    ...
    def probs(self, dict_format=True) -> dict[str, float] | list[float]:
        ...
        if dict_format:
            return {np.binary_repr(i, width=self.nq): val for i, val in enumerate(self._probs)}
        return self._probs
    ...
    def measure(self, dict_format=True) -> dict[str, float] | list[float]:
        ...
        if dict_format:
            return {np.binary_repr(i, width=len(self._mq)): val
                    for i, val in enumerate(self._measure)}
    ...
    def sample(
        ...
        """
        return self._runner.sample(self._mq, shots, is_sorted, dict_format)
