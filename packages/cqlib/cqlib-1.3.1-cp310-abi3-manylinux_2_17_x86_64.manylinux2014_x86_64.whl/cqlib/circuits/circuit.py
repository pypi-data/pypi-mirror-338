...
class Circuit:
    ...
    def assign_parameters(
        ...
        Example:
            ...
            >>> circuit = Circuit(2, ['theta', 'phi'])
            >>> circuit.assign_parameters({'theta': 3.14159})
            >>> circuit.assign_parameters(theta=0.5, phi=0.25)
        ...
        if values is None:
            values = {}
        _t = type(values)
        if isinstance(values, (Sequence, np.ndarray)) or \
                (hasattr(_t, "__module__") and _t.__module__ == 'torch'
                 and _t.__name__ == 'Tensor'):
            # list like type, list/tuple/np.ndarray/torch.Tensor
            if len(values) != len(target.parameters_value):
                raise ValueError(f"Length of values {len(values)} does not match "
