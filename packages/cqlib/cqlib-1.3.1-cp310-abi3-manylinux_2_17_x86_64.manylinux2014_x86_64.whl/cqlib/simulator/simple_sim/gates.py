...
def xy2p_mat(theta, backend: TorchBackend):
    ...
    row1 = backend.hstack([one, -i * exp_neg_i_theta])
    row2 = backend.hstack([-i * exp_i_theta, one])

    # Combine rows and apply normalization
    matrix = sqrt2_inv * backend.vstack([row1, row2])
