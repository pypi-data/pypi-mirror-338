...
class SimpleRunner:
    ...
    def SWAP(self, i: int, j: int, k: int, mat: np.ndarray):
        """Apply SWAP gate."""
    ...
    def sample(
        ...
        if not dict_format:
            return "".join([np.binary_repr(v, width=len(mq)) for v in r])
        result = {np.binary_repr(k, width=len(mq)): v for k, v in Counter(r).items()}
        if is_sorted:
