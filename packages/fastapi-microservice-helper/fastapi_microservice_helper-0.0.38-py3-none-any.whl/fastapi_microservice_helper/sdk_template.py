...
class SdkTemplate:
  ...
  def generate_method(self, method_detail: MethodDetail, is_mock: bool = False):
    ...
    return GenerationMethodDetail(
      ...
      body=self.get_method_body(method_detail, is_mock),
      params=list(
  ...
  def get_method_body(self, method_detail: MethodDetail, is_mock: bool = False):
    if is_mock:
      return f'return MockHelper.sample({self.get_response(method_detail)})'
  ...
  def generate_class(self, name, methods: list[MethodDetail], is_mock: bool = False):
    class_detail = GenerationClassDetail(class_name=name if not is_mock else f'Mock{name}', parent_name='BaseMicroserviceClient')
    for method in methods:
      class_detail.methods.append(self.generate_method(method, is_mock))
