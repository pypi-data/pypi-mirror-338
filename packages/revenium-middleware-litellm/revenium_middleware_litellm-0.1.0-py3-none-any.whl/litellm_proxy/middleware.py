...
class MiddlewareHandler(CustomLogger):
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        ...
        completion_args = {
            ...
            "time_to_first_token": (end_time - start_time).total_seconds() * 1000,  # Time to first token in milliseconds
            "stop_reason": "END",
            ...
            "subscriber_identity": metadata['user_api_key_alias'],
            ...
            "ai_provider_key_name": metadata['user_api_key_alias'],
            "agent": headers.get("x-revenium-agent"),
            "is_streamed": metadata['hidden_params']['optional_params']['stream'],
    ...
    async def async_log_failure_event(self, kwargs, response_obj, start_time, end_time):
        ...
        completion_args = {
            ...
            "task_id": headers.get("x-revenium-task-id"),
            "task_type": headers.get("x-revenium-task-type"),
            "subscriber_identity": metadata.get('user_api_key_alias', ''),
            "organization_id": metadata.get('user_api_key_team_alias', '') or headers.get("x-revenium-organization-id"),
            "subscription_id": headers.get("x-revenium-subscription-id"),
            "product_id": headers.get("x-revenium-product-id"),
            "ai_provider_key_name": metadata.get('user_api_key_alias', ''),
            "agent": headers.get("x-revenium-agent"),
