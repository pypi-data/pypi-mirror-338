...
class FKBase:
    ...
    def _generate_form_signature(
        self, order_id: str, amount: float, currency: Currency
    ) -> str:
        """
        ...
        """
        return hashlib.md5(
            f"{self.shop_id}:{amount}:{self.secret_word_1}:{currency}:{order_id}".encode(
                "utf-8"
            )
        ).hexdigest()
    ...
    def _generate_signature(self, params: Dict[str, Any]) -> str:
        ...
        return hmac.new(
            self.api_key.encode("utf-8"), sign_string.encode("utf-8"), hashlib.sha256
        ).hexdigest()

    def request(self, route: str, data: Dict[str, Any] = {}) -> FreeKassaResponse:
        """
...
class FreeKassa(FKBase):
    ...
    def create_payment_link(
        ...
        return "https://pay.fk.money/?" + "&".join(
    ...
    def get_orders(
        ...
        fk_id: str = None,
        order_id: int = None,
        order_status: OrderStatus | int = None,
        ...
        return OrdersResponse(
            type=response.type, pages=response.pages, orders=response.orders
        )
    ...
    def create_order(
        ...
        recurrent: bool = None,
        recurrent_period: str = None,
        recurrent_description: str = None,
        recurrent_order_id: int = None,
    ) -> OrderResponse:
        """
    ...
    def refund_order(self, fk_id: int = None, order_id: str = None) -> RefundResponse:
        """
        ## Возврат заказа.
        ...
        response = self.request(
            "orders/refund", {"orderId": fk_id, "paymentId": order_id}
        )
    ...
    def get_balance(self) -> BalanceResponse:
        ...
        return BalanceResponse(type=response.type, balance=response.balance)
    ...
    def get_currencies(self) -> PaymentSystemsResponse:
        ...
        return PaymentSystemsResponse(
            type=response.type, currencies=response.currencies
        )
    ...
    def get_avaliable_payment_systems(self) -> PaymentSystemsResponse:
        ...
        response.currencies = [x for x in response.currencies if x.is_enabled]
        return response

    def check_payment_system_status(
        self, payment_system: PaymentSystem | int
    ) -> PaymentSystemResponse:
        """
        ...
        return PaymentSystemResponse(
            type=response.type, description=getattr(response, "description", None)
        )
    ...
    def get_withdrawal_currencies(self) -> WithdrawalPaymentSystemsResponse:
        ...
        return WithdrawalPaymentSystemsResponse(
            type=response.type, currencies=response.currencies
        )
    ...
    def get_shops(self) -> ShopsResponse:
        ...
        return ShopsResponse(type=response.type, shops=response.shops)
