...
class PQueryAPI:
    ...
    def get_if_exists(self, name_or_id: str) -> Optional[PQueryResource]:
        ...
        resp = self._client._get(f'{self._base_endpoint}/{name_or_id}')
    ...
    def list(self, *,
        ...
        resp = self._client._get(self._base_endpoint, params=params)
        raise_on_error(resp)
        return parse_response(List[PQueryResource], resp)

    def delete(self, name: str) -> None:
        resp = self._client._delete(f'{self._base_endpoint}/{name}')
        raise_on_error(resp)
    ...
    def infer_task_type(
        ...
        resp = self._client._post(
            f'{self._base_endpoint}/infer_task_type',
            params={
    ...
    def validate(self, pquery: PQueryResource) -> ValidationResponse:
        ...
        resp = self._client._post(f"{self._base_endpoint}/validate",
                                  json=to_json_dict(pquery))
        raise_on_error(resp)
    ...
    def suggest_training_table_plan(
        ...
        resp = self._client._post(
            f'{self._base_endpoint}/train_table_generation_plan',
    ...
    def suggest_model_plan(
        ...
        resp = self._client._post(
            f'{self._base_endpoint}/model_plan',
            json=to_json_dict(req),
        )
        raise_on_error(resp)
...
def filter_model_plan(
    ...
    for section_field in fields(plan):
        ...
        for field in fields(section):
            ...
            if field.name == 'weight_mode':
                if getattr(section, field.name) is MissingType.VALUE:
                    continue

            # Remove `MissingType` from type annotation:
            if not metadata.hidden and get_origin(field.type) is Union:
        ...
        Section = make_dataclass(
            section_field.type.__name__,  # type: ignore
            new_opt_fields,
            bases=(PlanMixin, ),
            repr=False,
        )
        Section = dataclass(  # type: ignore
