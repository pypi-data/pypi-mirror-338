...
class TrainingJobResult:
    ...
    def holdout_df(self) -> pd.DataFrame:
        ...
        if global_state.is_spcs:
            ...
            return _get_session().read.parquet(self.holdout_url())

        return pd.read_parquet(self.holdout_url())
...
class BatchPredictionJobResult:
    ...
    def export(
        ...
        Args:
            output_type: The type of output that should be export by the job.
                Can be either ``'predictions'`` or ``'embeddings'``.
            output_connector: The output data source that Kumo should write
                batch predictions to.
    ...
    def predictions_urls(self) -> List[str]:
        r"""Returns a URL for downloading or reading the predictions.

        If Kumo is deployed as a SaaS application, the returned URL will be a
        presigned AWS S3 URL. If Kumo is deployed as a Snowpark Container
        Services application, the returned URL will be a Snowflake stage
        path that can be directly accessed within a Snowflake worksheet.
        """
...
class ArtifactExportResult:
    r"""Represents a completed prediction export job."""
