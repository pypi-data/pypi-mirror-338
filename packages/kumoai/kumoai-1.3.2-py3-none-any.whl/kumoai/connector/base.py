...
class Connector(ABC):
    ...
    def has_table(self, name: str) -> bool:
        ...
        table_list: List[str] = []
        try:
            table_list = self._list_tables().table_names
        except HTTPException:
            # TODO(dong) use has_table api in the backend.
            # Return True in case of an exception caused by too many tables.
            return True
    ...
    def _validate_table(self, table_name: str) -> SourceTableValidateResponse:
        if table_name in self._validated_tables:
            return SourceTableValidateResponse(is_valid=True)
