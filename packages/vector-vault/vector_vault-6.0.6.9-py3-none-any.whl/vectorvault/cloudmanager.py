...
class VaultStorageManager:
    ...
    def create_item(self, path: str, value: str) -> None:
        ...
        Also ensures any parent directories have directory markers.
        """
        # Create directory markers for parent directories
        parts = path.split('/')
        if len(parts) > 1:
            # Create parent directory if it doesn't exist yet
            parent_dir = '/'.join(parts[:-1])
            directory_marker_path = f"{self.vault}/{parent_dir}/.directory"
            self.cloud_manager.upload_text_to_cloud(directory_marker_path, "")
    ...
    def list_labels(self, path: str) -> list[dict]:
        """
        Lists all items and sub-directories under the given path.
        Return structure: [ {"name": "invoice_1", "type": "item"}, {"name": "clients", "type": "directory"} ]
        Implementation detail depends on how you retrieve file listings from the cloud manager.
        """
        directory_path = f"{self.vault}/{path}/" if path else f"{self.vault}/"

        all_objects = self.cloud_manager.list_objects(directory_path)
        # all_objects might return something like ["some_folder/.directory", "some_folder/item1", ...]
        
        result = []
        # Distill the immediate children (subdirs, items)
        for obj in all_objects:
            # remove the prefix (directory_path) from the object name
            sub_path = obj.replace(directory_path, "")
            # Check if it is a ".directory" marker
            if sub_path.endswith(".directory"):
                # e.g. sub_path == "subfolder/.directory"
                name_only = sub_path.split("/")[0]  # "subfolder"
                result.append({"name": name_only, "type": "directory"})
            else:
                # If there's a slash in sub_path, it's deeper, skip or handle carefully
                # If it's the immediate file, add it to the result
                if "/" not in sub_path:
                    result.append({"name": sub_path, "type": "item"})
        return result
    ...
    def delete_label(self, path: str) -> None:
        ...
        if not objects_to_delete:
            ...
            self.cloud_manager.delete_object(base_path)
        else:
            # If we have multiple objects, it's definitely a directory
            for obj in objects_to_delete:
                self.cloud_manager.delete_object(obj)
            # Also delete the directory marker
            self.cloud_manager.delete_object(base_path + "/.directory")
