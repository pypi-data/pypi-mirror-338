...
class VaultStorageManager:
    ...
    def list_labels(self, path: str = None) -> list[dict]:
        ...
        Automatically detects directories based on path patterns.
        """
        ...
        # First pass: identify directories from path patterns
        for obj in all_objects:
            # Remove the prefix to get relative path
            rel_path = obj.replace(directory_path, "")
            
            # Skip .directory markers in this analysis
            if rel_path.endswith(".directory"):
                continue
                
            # If there's a slash, it means there's a subdirectory
            if "/" in rel_path:
                # Get the first segment as a directory
                dir_name = rel_path.split("/")[0]
                directories.add(dir_name)
            else:
                # It's an immediate item
                result.append({"name": rel_path, "type": "item"})
        
        # Add discovered directories to the result
