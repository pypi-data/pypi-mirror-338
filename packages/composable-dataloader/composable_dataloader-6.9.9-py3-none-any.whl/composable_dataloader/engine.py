...
def get_spark(query_engine: QueryEngine = QueryEngine.SPARK):
    ...
    try:
        ...
        raise ImportError(
            f"{query_engine} requested but required package is not installed: {str(e)}"
        )


def get_connection(query_engine: QueryEngine = QueryEngine.DUCKDB) -> ibis.BaseBackend:
    """
    ...
    if query_engine == QueryEngine.DUCKDB:
        ...
        try:
            ...
            raise ImportError(
                f"Required packages for {query_engine} are not installed: {str(e)}"
            )
    elif query_engine == QueryEngine.DEFER:
        return None
