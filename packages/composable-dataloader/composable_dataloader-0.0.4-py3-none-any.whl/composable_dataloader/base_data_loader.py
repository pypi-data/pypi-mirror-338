...
class DataLoader(DynamicCliMixin, ABC):
    ...
    def __post_init__(self):
        ...
        logger.info(
            f"Initializing {self.__class__.__name__} with {self.query_engine} engine"
        )
    ...
    query_engine: Annotated[
        ...
        Option(
            ...
            "--query-engine",
            help="Query engine to use (DEFER,DUCKDB, SPARK, or SPARK_CONNECT)",
    ...
    def write_results_to_stdout(self) -> None:
        ...
        """
        logger.info(
            f"Streaming results from {self.output_location} to stdout for Observable Framework"
        )
    ...
    def execute_and_write(self) -> None:
        ...
        logger.info(
            f"Writing results to {self.output_location} ({self.format}, mode={self.write_mode})"
        )
