...
def create_tamarin_cmd_hdr(id=0, cmd=0, request=0, data=0, idle_cycles=0):
    """
...
class FaulLink(object):
    ...
    def set_swd_frequency(self, f):
        print("Setting of SWD frequency currently ignored.")
        # self.start_queue()
...
class FindFaultier(object):
    ...
    VID_PID_CLASS = (0x2B3E, 0x2343, 0xef)  # Match for a Faultier
    ...
    def __call__(self, dev):
        ...
        # Check if vid, pid and the device class are valid ones for Faultier.
        # print(f"{hex(dev.idVendor)} {hex(dev.idProduct)} {hex(dev.bDeviceClass)}")
...
class Faultier(DebugProbe):
    ...
    SAFESWD_OPTION = 'Faultier.safeswd'
    ...
    def swj_sequence(self, length, bits):
        ...
        tamarin_cmd = create_tamarin_cmd_hdr(0, TAMARIN_LINE_RESET, 0, 0, 0)
        # print(f"reag reg Tamarin cmd {tamarin_cmd}")
    ...
    def swd_sequence(self, sequences):
        print("SWD sequence currently ignored.")
        return
    ...
    def _read_reg(self, addr, APnDP):
        # print(f"read {hex(addr)} {APnDP}")
        command_byte = self._swd_command_byte(self.READ, APnDP, addr)
        tamarin_cmd = create_tamarin_cmd_hdr(0, TAMARIN_READ, command_byte, 0, 0)
        # print(f"reag reg Tamarin cmd {tamarin_cmd}")
    ...
    def _write_reg(self, addr, APnDP, value):
        ...
        tamarin_cmd = create_tamarin_cmd_hdr(0, TAMARIN_WRITE, command_byte, value, 16)
...
class FaultierPlugin(Plugin):
    ...
    def options(self):
        ...
        return [
            OptionInfo(Faultier.SAFESWD_OPTION, bool, True,
