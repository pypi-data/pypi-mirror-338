...
def cli():
    """HotPod CLI - 京东云GPU云服务命令行工具"""
    # 检查环境变量
    # 仅在非开发模式下检查环境变量
    if not os.environ.get('HOTPOD_DEV_MODE'):
        ...
        if missing_env_vars:
            ...
            click.secho("\n开发模式下可设置 HOTPOD_DEV_MODE=1 跳过环境变量检查", fg='cyan')
            sys.exit(1)
...
def info():
    ...
    click.secho("京东云GPU云服务命令行工具", fg='white')
    click.secho("\n开发者: 京东云AIDC团队", fg='cyan')
    click.secho("版本: 0.1.0", fg='cyan')
