...
def delete_proxy_thread(id_thread: int, script_name: str) -> None:
    ...
    if hasattr(action_obj, "loaded_obj"):
        if id_thread in action_obj.loaded_obj.keys():
            ...
            core.PROXY_ACTIONS_DICT[id_thread].remove(script_name)

            """ if hasattr(obj_, "ifaceCtx"):
                LOGGER.warning("Deleting ifaceCtx from %s" % script_name)
                obj_.ifaceCtx = None """
            check_delete(obj_, "proxy.widget_%s" % script_name)
