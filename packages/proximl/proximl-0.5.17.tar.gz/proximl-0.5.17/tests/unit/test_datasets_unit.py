...
import proximl.datasets as specimen
...
@fixture
def datasets(mock_proximl):
    yield specimen.Datasets(mock_proximl)


@fixture
def dataset(mock_proximl):
...
class DatasetsTests:
    ...
    @mark.asyncio
    async def test_list_public_datasets(self, datasets, mock_proximl):
        api_response = dict()
        mock_proximl._query = AsyncMock(return_value=api_response)
    ...
    @mark.asyncio
    async def test_create_dataset_simple(self, datasets, mock_proximl):
...
class DatasetTests:
    ...
    def test_dataset_bool(self, dataset, mock_proximl):
        empty_dataset = specimen.Dataset(mock_proximl)
        assert bool(dataset)
    ...
    @mark.asyncio
    async def test_dataset_get_details(self, dataset, mock_proximl):
        api_response = {
            "type": "directory",
            "name": "/",
            "count": "8",
    ...
    @mark.asyncio
    async def test_dataset_attach(self, dataset, mock_proximl):
        api_response = None
    ...
    @mark.asyncio
    async def test_dataset_attach_immediate_return(self, mock_proximl):
        dataset = specimen.Dataset(
            mock_proximl,
            dataset_uuid="1",
            name="first one",
    ...
    @mark.asyncio
    async def test_dataset_refresh(self, dataset, mock_proximl):
        api_response = {
            "customer_uuid": "cus-id-1",
            "dataset_uuid": "data-id-1",
            "name": "new dataset",
    ...
    @mark.asyncio
    async def test_dataset_wait_for_successful(self, dataset, mock_proximl):
        api_response = {
            "customer_uuid": "cus-id-1",
            "dataset_uuid": "data-id-1",
            "name": "new dataset",
    ...
    @mark.asyncio
    async def test_dataset_wait_for_current_status(self, mock_proximl):
        dataset = specimen.Dataset(
            mock_proximl,
            dataset_uuid="1",
            name="first one",
    ...
    @mark.asyncio
    async def test_dataset_wait_for_with_delay(self, dataset, mock_proximl):
    ...
    @mark.asyncio
    async def test_dataset_wait_for_timeout(self, dataset, mock_proximl):
        api_response = dict(
            dataset_uuid="1",
            name="first one",
            status="new",
