...
import proximl.gpu_types as specimen

pytestmark = [mark.sdk, mark.unit, mark.gpu_types]


@fixture()
def gpu_types(mock_proximl):
    yield specimen.GpuTypes(mock_proximl)


@fixture()
def gpu_type(mock_proximl):
...
class GpuTypesTests:
    ...
    @mark.asyncio
    async def test_project_refresh_gpu_types(self, gpu_types, mock_proximl):
        api_response = dict()
        mock_proximl._query = AsyncMock(return_value=api_response)
...
class GpuTypeTests:
    ...
    def test_gpu_type_repr(self, gpu_type):
        ...
        regex = (
            r"^GpuType\( proximl , \*\*{.*'id': '" + gpu_type.id + r"'.*}\)$"
        )
        assert isinstance(string, str)
        assert re.match(regex, string)
