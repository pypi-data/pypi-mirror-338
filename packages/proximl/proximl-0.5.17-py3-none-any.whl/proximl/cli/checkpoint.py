...
def create(config, attach, connect, source, name, path):
    ...
    if source == "local":
        checkpoint = config.proximl.run(
            config.proximl.client.checkpoints.create(
...
def list_public(config):
    ...
    checkpoints = config.proximl.run(
        config.proximl.client.checkpoints.list_public()
    )
...
def rename(config, checkpoint, name):
    ...
    try:
        checkpoint = config.proximl.run(
            config.proximl.client.checkpoints.get(checkpoint)
        )
    ...
    return config.proximl.run(checkpoint.rename(name=name))
