...
VOLUME_NAME = "my-custom-result-volume"
...
class Config(BaseModel):
    ...
    def create_volume(self, url, volume_name: str = "input"):
        self.volume = Volume(
    ...
    def create_results_volume(self, url, result_volume_name: str = VOLUME_NAME):
    ...
    def _get_name_result_volume(self):
        return self.results_volume.metadata.name if self.results_volume else VOLUME_NAME

    def create_report(self, report_name: str = "report", report_type: str = "basic"):
        results = Results(
    ...
    def get_signal_metrics(self, name: str, avatarization_ref: str) -> str:
    ...
    def get_privacy_metrics(self, name: str, avatarization_ref: str) -> str:
        time_series = self.time_series or None
    ...
    def get_parameters(
        self,
        avatarization_name: str,
        privacy_metrics_name: str,
        signal_metrics_name: str,
        report_name: str,
    ) -> str:
        return aggregate_yamls(
            self.get_avatarization(name=avatarization_name),
            self.get_privacy_metrics(
                name=privacy_metrics_name, avatarization_ref=avatarization_name
            ),
            self.get_signal_metrics(
                name=signal_metrics_name, avatarization_ref=avatarization_name
            ),
            self.get_report(name=report_name),
        )

    def get_report(self, name: str) -> str:
        if self.report is None:
            self.create_report(report_name=name)
    ...
    def get_result_volume(self) -> str:
        if self.results_volume is None:
            self.create_results_volume(url=None)
        if self.results_volume is None:
            raise ValueError("Results volume is not set")
    ...
    def get_yaml(
        ...
        yaml = aggregate_yamls(
            ...
            self.get_parameters(
                avatarization_name, privacy_metrics_name, signal_metrics_name, report_name
            ),
        )
        if path:
