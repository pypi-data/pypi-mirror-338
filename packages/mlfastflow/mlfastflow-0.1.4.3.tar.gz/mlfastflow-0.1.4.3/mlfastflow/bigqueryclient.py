...
class BigQueryClient:
    ...
    def sql2gcs_raw(self, sql: str,
                   destination_uri: str,
                   format: str = 'PARQUET',
                   compression: str = 'SNAPPY',
                   chunk_size: int = 500000000) -> bool:
        """
        Alternative method to export BigQuery query results to GCS using direct API access.
        This function uses a different approach by creating partitioned tables to control output size.
        ...
        Args:
            ...
            format (str): Export format ('PARQUET', 'CSV', 'JSON', 'AVRO')
            compression (str): Compression type ('NONE', 'GZIP', 'SNAPPY', 'DEFLATE')
            chunk_size (int): Target chunk size in bytes (default: 500MB)
        ...
        # Create a unique job ID suffix
        job_suffix = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
        
        try:
            # Step 1: Create temporary table with results
            tmp_table_id = f"temp_export_{job_suffix}"
            tmp_table_ref = f"{self.project_id}.{self.dataset_id}.{tmp_table_id}"
            
            print(f"Creating temporary table {tmp_table_ref} for query results...")
            
            # Set up the query job
            query_job_config = bigquery.QueryJobConfig(
                destination=tmp_table_ref,
                write_disposition="WRITE_TRUNCATE"
            )
            ...
            # Step 2: Create extract job with custom configuration
            table_ref = f"{self.project_id}.{self.dataset_id}.{tmp_table_id}"
            
            # Set format and compression
            format = format.upper()
            compression = compression.upper()
            
            # Make destination URI have a wildcard if needed
            if '*' not in destination_uri:
                base, ext = os.path.splitext(destination_uri)
                destination_uri = f"{base}-*.{ext.lstrip('.')}" if ext else f"{destination_uri}-*"
                
            print(f"Preparing to export to {destination_uri}")
            
            # Create the job directly using the REST API structure
            extract_job_config = bigquery.ExtractJobConfig()
            
            # Set format and compression through the proper API
            extract_job_config.destination_format = format
            if compression != 'NONE':
                extract_job_config.compression = compression
                
            # Try to set max_bytes_per_file in multiple ways to ensure it takes effect
            try:
                # First try setting it directly if supported
                extract_job_config.max_bytes_per_file = chunk_size
            except (AttributeError, TypeError):
                pass
                
            # Also set it in the internal properties dictionary if possible
            try:
                if hasattr(extract_job_config, '_properties'):
                    ...
                    # Set the property
                    extract_job_config._properties['configuration']['extract']['maxBytesPerFile'] = str(chunk_size)
            except Exception as e:
                print(f"Note: Could not set maxBytesPerFile in properties: {str(e)}")
            
            # Use the public API to create the extract job
            table_ref = self.client.dataset(self.dataset_id).table(tmp_table_id)
            ...
            self.client.delete_table(tmp_table_ref)
            print(f"Cleaned up temporary table {tmp_table_ref}")
            
            return True
            
        except Exception as e:
            print(f"Error in raw GCS export: {str(e)}")
            return False

    def sql2gcs_via_query(self, sql: str,
                         destination_uri: str,
                         format: str = 'PARQUET',
                         compression: str = 'SNAPPY') -> bool:
        """
        Export BigQuery query results to GCS using the SQL EXPORT DATA statement.
        This is a completely different approach that uses BigQuery's SQL capabilities
        rather than the extract job API.
        ...
        format = format.upper()
        if format not in ['PARQUET', 'CSV', 'JSON', 'AVRO']:
            raise ValueError(f"Format must be one of: PARQUET, CSV, JSON, AVRO")
            
        # Validate and standardize compression
        compression = compression.upper()
        if compression not in ['NONE', 'GZIP', 'SNAPPY', 'DEFLATE']:
            raise ValueError(f"Compression must be one of: NONE, GZIP, SNAPPY, DEFLATE")
        
        try:
            # Format options for the EXPORT DATA statement
            options = []
            
            # Format-specific options
            if format == 'PARQUET':
                options.append(f"format = 'PARQUET'")
                if compression != 'NONE':
                    options.append(f"compression = '{compression}'")
            elif format == 'CSV':
                options.append(f"format = 'CSV'")
                options.append("header = true")
                options.append("field_delimiter = ','")
                if compression != 'NONE':
                    options.append(f"compression = '{compression}'")
            elif format == 'JSON':
                options.append(f"format = 'JSON'")
                if compression != 'NONE':
                    options.append(f"compression = '{compression}'")
            elif format == 'AVRO':
                options.append(f"format = 'AVRO'")
                if compression != 'NONE':
                    options.append(f"compression = '{compression}'")
            
            # Construct the export query
            # Try to set a high value for max_file_size to get larger files
            # Note: This is only supported in recent BigQuery versions
            options.append("max_file_size = '5GB'")
            
            # Create the EXPORT DATA statement
            export_options = ",\n  ".join(options)
            export_sql = f"""
            EXPORT DATA OPTIONS(
              uri = '{destination_uri}',
            ...
            print(f"Executing SQL EXPORT DATA statement...")
            
            # Execute the export query
            query_job = self.client.query(export_sql)
            result = query_job.result()  # Wait for the query to complete
            
            print(f"Export completed successfully to {destination_uri}")
            return True
            
        except Exception as e:
            print(f"Error in SQL-based export: {str(e)}")
            return False