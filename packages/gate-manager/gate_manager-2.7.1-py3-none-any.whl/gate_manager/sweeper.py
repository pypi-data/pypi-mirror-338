...
class Sweeper:
    ...
    def _log_params_start(self, sweep_type: str = 'voltage', status: str = 'start') -> None:
        ...
        """
        with open(f"log_{self.base_filename}.txt", 'a') as file:
            ...
            if sweep_type == 'voltage':
                file.write(f"{'Start Voltage:':<16} {self.convert_si_value(self.X_start_volt, 'V')} \n")
                file.write(f"{'End Voltage:':<16} {self.convert_si_value(self.X_end_volt, 'V')} \n")
                file.write(f"{'Step Size:':<16} {self.convert_si_value(self.X_step, 'V')} \n")
                file.write("\n")
            if self.is_2d_sweep:
                file.write(f"{'Y Swept Gates:':<16} {self.y_label} \n")
                file.write(f"{'Start Voltage:':<16} {self.convert_si_value(self.Y_start_volt, 'V')} \n")
                file.write(f"{'End Voltage:':<16} {self.convert_si_value(self.Y_end_volt, 'V')} \n")
    ...
    def sweep1D(self, 
        ...
        try:
            ...
            total_steps = round(abs(self.X_end_volt - self.X_start_volt) / self.X_step + 1)
            ...
            for i in tqdm(range(total_steps), desc="Sweeping", ncols=80):
                ...
                self.curr_list[i] = measured_inputs.gates[0].read_current() * self.curr_scale
    ...
    def _set_initial_state(self, initial_state, swept_outputs = None):
        ...
        if swept_outputs is not None:
            while not all([gate.is_at_target_voltage(self.X_start_volt)
                            for gate in swept_outputs.gates]):
                time.sleep(0.001)
    ...
    def _setup_1d_plot(self):
        ...
        # Axes labels
        self.ax.set_xlabel(f"{self.x_label} [{self.X_volt_unit}]", color='#2C3E50', fontsize=32)
        self.ax.set_ylabel(f"{self.z_label} [{self.curr_unit}]", color='#2C3E50', fontsize=32)

        #Ticks
        self.ax.tick_params(axis='y', direction='in', width=4, length=10 , pad=10 , right=True, labelsize=24)
        self.ax.tick_params(axis='x', direction='in', width=4, length=10 , pad=10 , top=False, labelsize=24)
    ...
    def _update_plot(self, index):
        ...
        self.data_lines.set_data(self.X_volt_list[:index+1], self.curr_list[:index+1])
        plt.draw()
        plt.pause(0.001)
    ...
    def _write_measurement_data(self, index):
        ...
        try:
            with open(f"data/{self.filename}.txt", 'a') as file:
                if self.is_2d_sweep:
                    file.write(f"{self.Y_volt * self.X_volt_scale:>16.4f} "
                             f"{self.X_volt_list[index]:>16.4f} "
                             f"{self.curr_list[index]:>16.8f}\n")
                else:
                    file.write(f"{self.X_volt_list[index]:>16.4f} "
                             f"{self.curr_list[index]:>16.8f}\n")
        except IOError as e:
    ...
    def _write_2d_data_header(self):
        ...
        try:
            with open(f"data/{self.filename}.txt", 'a') as file:
                header = (f"{self.y_label} [{self.Y_volt_unit}]".rjust(16) +
                         f"{self.x_label} [{self.X_volt_unit}]".rjust(16) +
                         f"{self.z_label} [{self.curr_unit}]".rjust(16))
                file.write(header + "\n")
        except IOError as e:
    ...
    def _setup_2d_plot(self):
        ...
        # Axes labels
        self.ax.set_xlabel(f"{self.x_label} [{self.X_volt_unit}]", color='#2C3E50', fontsize=32)
        self.ax.set_ylabel(f"{self.y_label} [{self.Y_volt_unit}]", color='#2C3E50', fontsize=32)
    ...
    def _update_time_sweep_plot(self, time_points, currents, current_idx):
        ...
        # Update plot data
        self.data_lines.set_data(time_points[:current_idx+1], currents[:current_idx+1])
        plt.draw()
        plt.pause(0.001)
