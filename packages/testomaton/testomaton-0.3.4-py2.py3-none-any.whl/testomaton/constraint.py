...
def report_parsing_error(context, tag, message):
    ...
    path = path + '::' + context.path if context.path is not None else path
    path = path + '::' + 'logic' + '::' + context.name if context.name is not None else path
...
def parse_expression(expression_tokens, aliases, context):
    ...
    if expression_tokens[0] == 'NOT':
        ...
        operator = expression_tokens.pop()
        lval_tokens = expression_tokens
        return PrimitiveStatement(lval_tokens, operator, rval_tokens, context=context)
...
class PrimitiveStatement:
    ...
    """
    def __init__(self, lval, operator, rval, context):
        """
        ...
        if context.function.get_parameter(self.lval) is not None:
            ...
            report_parsing_error(context, ParsingErrorTag.CONSTRAINT, f'"{self.lval}" not found in model')
    ...
    def to_cnf(self, choice_mapping, top_id):
        ...
        # do something to check for choice values. so that choice s that arent in a parameter are caught and report an error. 
...
class Invariant:
    ...
    def negated(self):
        expression = NegatedExpression(expr=self.expression)
...
class Parser:
    ...
    def parse_statement_alias(self, statement_alias_string, aliases, context):
        try:
            ...
            report_parsing_error(context, ParsingErrorTag.ALIAS, f'Syntax error: {str(e)}')
    ...
    def parse_constraint(self, constraint_string, aliases, context):
        ...
        try:
            ...
            report_parsing_error(context, ParsingErrorTag.CONSTRAINT, f'Syntax error: {str(e)}')
        if len(tokens) == 1:
    ...
    def parse_assignment(self, assignment_string, context, aliases):
        ...
        try:
            ...
            report_parsing_error(context, ParsingErrorTag.ASSIGNMENT, f'Syntax error: {str(e)}')
