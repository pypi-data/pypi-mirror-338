...
class TTYDManager:
    ...
    def __init__(self, config: TTYDConfig, force_reinstall_ttyd: bool = None):
        ...
        if self.config.ttyd_options.force_https:
            logger.info("HTTPS support is enabled (force_https=True in ttyd_options)")
    ...
    def _build_command(self, script_config: ScriptConfig) -> List[str]:
        ...
        # Add force_https flag if enabled
        if self.config.ttyd_options.force_https:
            # Use ttyd's theme option to set forceHttps instead of --ssl flag
            cmd.extend(['-t', 'forceHttps=true'])
            logger.debug(f"Added forceHttps=true for script {script_config.route_path} (force_https=True)")
        ...
        # Log the full command in debug mode with force_https status
        if self.config.debug:
            cmd_str = ' '.join(cmd)
            logger.debug(f"ttyd command for {script_config.route_path} (force_https={self.config.ttyd_options.force_https}): {cmd_str}")
    ...
    def start(self) -> None:
        ...
        https_mode = "HTTPS enabled" if self.config.ttyd_options.force_https else "HTTP mode"
        logger.info(f"Starting {script_count} ttyd processes ({mode_type} mode via {entry_mode} API, {https_mode})")
    ...
    def start_process(self, script_config: ScriptConfig) -> None:
        ...
        try:
            # Pass protocol information in environment variables for the child process
            env = os.environ.copy()
            if self.config.ttyd_options.force_https:
                env["TERMINAIDE_FORCE_HTTPS"] = "1"
            ...
            for _ in range(checks):
                ...
                if self.is_process_running(route_path):
                    ...
                    proto = "HTTPS" if self.config.ttyd_options.force_https else "HTTP"
                    logger.info(f"Started ttyd: {route_path} (port:{port}, pid:{process.pid}, mode:{proto})")
                    return
    ...
    def check_health(self) -> Dict[str, Any]:
        ...
        https_mode = "HTTPS enabled" if self.config.ttyd_options.force_https else "HTTP mode"
        logger.debug(f"Health check: {running_count}/{len(processes_health)} processes running ({https_mode})")
