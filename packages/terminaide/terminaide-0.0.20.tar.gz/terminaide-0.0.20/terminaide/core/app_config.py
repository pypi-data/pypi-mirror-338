...
def configure_routes(
    ...
    for script_config in config.script_configs:
        ...
        async def terminal_interface(
            ...
            try:
                # Get protocol from request (will be 'https' if middleware detected HTTPS)
                protocol = request.url.scheme
                logger.debug(f"Rendering template with protocol={protocol}, preview_image={preview_image}")
                
                # Log the detected protocol for debugging
                if protocol == 'https':
                    logger.debug(f"HTTPS detected for request path: {request.url.path}")
                ...
                return templates.TemplateResponse(
                    template_file,
                    {
                        ...
                        "title": title,
                        "preview_image": preview_image,
                        "protocol": protocol  # Explicitly pass protocol to template
                    }
                )
            except Exception as e:
        ...
        async def terminal_ws(websocket: WebSocket, route_path=route_path):
            # Get protocol (explicitly mark for websocket protocol)
            protocol = websocket.url.scheme
            websocket_protocol = 'wss' if protocol == 'https' else 'ws'
            logger.debug(f"WebSocket connecting with protocol: {websocket_protocol}")
            
            # Pass the protocol information to the proxy manager
...
def configure_app(app: FastAPI, config: TTYDConfig):
    ...
    # Apply the proxy header middleware by default to ensure proper protocol detection
    ...
    # Mount static files with an explicit name for better debugging
...
def convert_terminaide_config_to_ttyd_config(config: TerminaideConfig, script_path: Path = None) -> TTYDConfig:
    ...
    ttyd_options_dict = config.ttyd_options or {}
    
    # Set force_https to True if we're behind a proxy and trust_proxy_headers is True
    if config.trust_proxy_headers and 'force_https' not in ttyd_options_dict:
        ttyd_options_dict['force_https'] = True
        logger.debug("Setting force_https=True for ttyd behind proxy")
    
    ttyd_options_config = TTYDOptions(**ttyd_options_dict)
    ...
    ttyd_config = TTYDConfig(
        ...
        trust_proxy_headers=config.trust_proxy_headers  # Transfer this field
    )
