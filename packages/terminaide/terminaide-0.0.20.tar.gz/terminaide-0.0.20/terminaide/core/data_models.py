...
class TTYDOptions(BaseModel):
    ...
    force_https: bool = Field(
        default=False,
        description="Force HTTPS protocol for all terminaide resources."
        " Prevents mixed content errors when running behind an HTTPS proxy."
    )
    ...
    def validate_credentials(self) -> 'TTYDOptions':
        ...
        # Auto-detect HTTPS environments
        if not self.force_https:
            # Check common environment variables that indicate HTTPS environments
            for env_var in ['HTTPS', 'FORCE_HTTPS', 'TERMINAIDE_FORCE_HTTPS']:
                if os.environ.get(env_var, '').lower() in ('1', 'true', 'yes', 'on'):
                    self.force_https = True
                    logger.info(f"Auto-enabled force_https=True based on {env_var} environment variable")
                    break
            
            # Check if we're in a common cloud environment that defaults to HTTPS
            if any(env_var in os.environ for env_var in [
                'AWS_EXECUTION_ENV', 'HEROKU_APP_ID', 'DYNO', 'RAILWAY_ENVIRONMENT',
                'VERCEL', 'NETLIFY', 'RENDER', 'ELASTIC_BEANSTALK'
            ]):
                self.force_https = True
                logger.info("Auto-enabled force_https=True based on detected cloud environment")
        
        if self.force_https:
            logger.info("HTTPS mode is enabled (force_https=True)")
...
class ScriptConfig(BaseModel):
    ...
    protocol: Optional[str] = None  # Added explicit protocol field
    ...
    def validate_protocol(cls, v: Optional[str]) -> Optional[str]:
        ...
        if v not in ('http', 'https'):
            raise ConfigurationError(f"Invalid protocol: {v}. Must be 'http' or 'https'.")
        return v
...
class TTYDConfig(BaseModel):
    ...
    trust_proxy_headers: bool = True  # Added this field to match TerminaideConfig
    ...
    def validate_script_configs(self) -> 'TTYDConfig':
        ...
        # Propagate force_https to the url scheme if set
        if self.ttyd_options.force_https:
            for config in self.script_configs:
    ...
    def is_https_enabled(self) -> bool:
        """Return whether HTTPS is enabled via force_https setting."""
        return self.ttyd_options.force_https
        
    def get_url_with_protocol(self, path: str) -> str:
        """
        Return a URL with the appropriate protocol based on configuration.
        """
        protocol = "https" if self.ttyd_options.force_https else "http"
        if path.startswith('/'):
    ...
    def get_health_check_info(self) -> Dict[str, Any]:
        ...
        for config in self.script_configs:
            script_info.append({
                ...
                "protocol": config.protocol or ("https" if self.ttyd_options.force_https else "http")
            })
        return {
            ...
            "protocol": "https" if self.ttyd_options.force_https else "http",
            "force_https": self.ttyd_options.force_https,
...
def create_script_configs(
    ...
    for route_path, script_spec in terminal_routes.items():
        ...
        if isinstance(script_spec, dict) and "client_script" in script_spec:
            ...
            # Handle protocol if provided in the script_spec
            if "protocol" in script_spec:
                cfg_data["protocol"] = script_spec["protocol"]
