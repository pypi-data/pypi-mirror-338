...
def configure_routes(
    ...
    # Add middleware for HTTPS detection at startup if not already added
    from .middleware import ProxyHeaderMiddleware
    if config.trust_proxy_headers and not any(m.cls.__name__ == "ProxyHeaderMiddleware" for m in getattr(app, "user_middleware", [])):
    ...
    # Generate absolute_url helper function
    def absolute_url(request: Request, path: str) -> str:
        """Create an absolute URL with the correct protocol based on the request."""
        # Ensure path starts with a slash for joining
        if not path.startswith('/'):
            path = '/' + path
            
        # Get the base URL including the scheme
    ...
    for script_config in config.script_configs:
        ...
        async def terminal_interface(
            ...
            try:
                # Get the correct scheme from request
                scheme = request.url.scheme
                
                # Log the actual request info to help with debugging
                logger.debug(f"Rendering template for request: {request.method} {request.url.path}")
                ...
                response = templates.TemplateResponse(
                    template_file,
                    {
                        ...
                        "title": title,
                        "preview_image": preview_image,
                        "absolute_url": absolute_url,  # Add the helper function
                    }
                )
        ...
        async def terminal_ws(websocket: WebSocket, route_path=route_path):
            ...
            logger.debug(f"WebSocket connection: {websocket.url}")
            logger.debug(f"   - Scheme: {websocket.url.scheme}")
