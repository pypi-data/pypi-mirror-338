...
def configure_routes(
    ...
    # Check if trust_proxy_headers exists before using it
    # This maintains compatibility with existing TTYDConfig objects
    use_proxy_headers = getattr(config, 'trust_proxy_headers', True)
    ...
    for script_config in config.script_configs:
        ...
        async def terminal_interface(
            ...
            try:
                ...
                # Log the actual scheme being used (at debug level to avoid spamming logs)
                if hasattr(config, 'debug') and config.debug:
                    logger.debug(f"Rendering template with scheme: {scheme}")
                
                # Create a context dict with base URL for use in the template
                context = {
                    "request": request,
                ...
                response = templates.TemplateResponse(template_file, context)
