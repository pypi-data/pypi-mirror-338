...
import logging
from robyn.debug.allocator_test.allocator import BudgetAllocator
...
class AllocatorVisualizer(BaseVisualizer):
    ...
    def _plot_response_spend_comparison(self) -> go.Figure:
        ...
        subplot_titles = [
            f"Initial\nSpend: {self._format_num(0)}%\n"
            f"Resp: {self._format_num(0)}%\n"
            f"{self.metric}: {round(init_total_cpa if self.metric == 'CPA' else init_total_roi, 2)}",
            f"Bounded{'^' if optm_topped_bounded else ''}\n"
            f"Spend: {self._format_num(100 * (optm_total_spend_bounded - init_total_spend) / init_total_spend)}%\n"
            f"Resp: {self._format_num(100 * self.resp_metric['total_response_lift'].iloc[1])}%\n"
            f"{self.metric}: {round(optm_total_cpa_bounded if self.metric == 'CPA' else optm_total_roi_bounded, 2)}",
            f"Bounded x{bound_mult}{'^' if optm_topped_unbounded else ''}\n"
            f"Spend: {self._format_num(100 * (optm_total_spend_unbounded - init_total_spend) / init_total_spend)}%\n"
            f"Resp: {self._format_num(100 * self.resp_metric['total_response_lift'].iloc[2])}%\n"
        ...
        colors = ["grey", "steelblue", "darkgoldenrod"]
        ...
        fig.update_layout(
            title=f"Total Budget Optimization Result (scaled up to {self.dt_optim_out['periods'].iloc[0]})",
            showlegend=False,
            height=500,
            yaxis=dict(range=[0, y_max], showticklabels=False),
            yaxis2=dict(range=[0, y_max], showticklabels=False),
            yaxis3=dict(range=[0, y_max], showticklabels=False),
        )
    ...
    def _plot_response_curves(self):
        ...
        # Update layout
        fig.update_layout(
            title=f"Simulated Response Curve per {self.budget_allocator.mmm_data.mmmdata_spec.interval_type}",
            showlegend=True,
            height=200 * ((len(plotDT_scurve["constr_label"].unique()) + 2) // 3),
            width=900,
