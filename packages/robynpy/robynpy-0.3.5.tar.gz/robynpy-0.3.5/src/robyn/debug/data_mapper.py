...
def export_to_r_input_collect(
    ...
    """
    Convert Python MMM objects back to R InputCollect format

    Args:
        mmm_data: Core MMM data and specifications
        holidays_data: Holiday-related data
        hyperparameters: Model hyperparameters
        featurized_mmm_data: Processed/featurized data

    Returns:
        Dictionary matching R's InputCollect structure
    """
    # Core data
    input_collect = {
        "dt_input": mmm_data.data.to_dict("list"),
        "dt_mod": featurized_mmm_data.dt_mod.to_dict("list"),
        "dt_modRollWind": featurized_mmm_data.dt_mod_roll_wind.to_dict("list"),
        # Specification fields
        "dep_var": [mmm_data.mmmdata_spec.dep_var],
        "dep_var_type": [mmm_data.mmmdata_spec.dep_var_type.value],
        "date_var": [mmm_data.mmmdata_spec.date_var],
        ...
        "context_vars": mmm_data.mmmdata_spec.context_vars,
        "factor_vars": mmm_data.mmmdata_spec.factor_vars,
        # Window specifications
        "window_start": [mmm_data.mmmdata_spec.window_start],
        "window_end": [mmm_data.mmmdata_spec.window_end],
        "rollingWindowLength": [mmm_data.mmmdata_spec.rolling_window_length],
        "rollingWindowStartWhich": [mmm_data.mmmdata_spec.rolling_window_start_which],
        "rollingWindowEndWhich": [mmm_data.mmmdata_spec.rolling_window_end_which],
        # Holiday data
        "dt_holidays": holidays_data.dt_holidays.to_dict("list"),
        "prophet_vars": [v.value for v in holidays_data.prophet_vars],
        "prophet_country": holidays_data.prophet_country,
        "prophet_signs": [s.value for s in holidays_data.prophet_signs],
        # Hyperparameters
        "hyperparameters": {
            channel: {
        ...
        "adstock": hyperparameters.adstock.value,
        "lambda_": hyperparameters.lambda_,
        "train_size": hyperparameters.train_size,
        # Additional fields that might be needed
        "all_media": mmm_data.mmmdata_spec.all_media,
        "intervalType": "week",  # or derive from data
        "modNLS": featurized_mmm_data.mod_nls,
    }
