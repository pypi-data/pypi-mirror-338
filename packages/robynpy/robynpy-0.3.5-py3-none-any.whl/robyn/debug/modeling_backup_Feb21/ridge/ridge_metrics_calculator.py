...
class RidgeMetricsCalculator:
    ...
    def _calculate_lambda(
        ...
        """Calculate lambda value based on lambda_hp using R's exact formula."""
        # Use R's exact lambda_max value
        lambda_max = 65481090.2534
        
        # Use R's formula: lambda = lambda_min + (lambda_max - lambda_min) * lambda_hp
        lambda_min = lambda_max * 0.0001  # R's minimum ratio
        lambda_value = lambda_min + (lambda_max - lambda_min) * lambda_hp
        ...
        if debug:
            ...
            self.logger.debug(f"lambda_value: {lambda_value:.6f}")
            self.logger.debug("===========================\n")
        
        return lambda_value, lambda_max
