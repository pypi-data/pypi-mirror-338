...
class AllocatorPlotter(BaseVisualizer):
    ...
    def _plot_response_curves(self) -> plt.Figure:
        ...
        # Log input data for comparison
        logger.info("=== Response Curves Input Data ===")
        logger.info("Channels: %s", self.dt_optimOut.channels)
        logger.info("Initial spend units: %s", self.dt_optimOut.init_spend_unit)
        logger.info("Spend points shape: %s", self.mainPoints.spend_points.shape)
        logger.info("Spend points:\n%s", self.mainPoints.spend_points)
        logger.info("Response points:\n%s", self.mainPoints.response_points)
        logger.info(
            "Carryover values: %s",
        ...
        try:
            ...
            scenarios = ["Initial", "Bounded", "Bounded x3"]
            scenario_colors = ["white", "grey", "steelblue", "darkgoldenrod"]
            ...
            for i, channel in enumerate(self.dt_optimOut.channels):
                ...
                carryover_spend = self.mainPoints.spend_points[
                    0, i
                ]  # Initial point is carryover
                ...
                # Plot main curve
                ax.plot(spend_range, response, color="black", alpha=0.7, linewidth=0.5)

                # Get constraint values and adjust for carryover
                lower_bound = self.dt_optimOut.init_spend_unit[i] * 0.7
                upper_bound = self.dt_optimOut.init_spend_unit[i] * 1.2
                lower_bound_ext = self.dt_optimOut.init_spend_unit[i] * 0.1
                upper_bound_ext = self.dt_optimOut.init_spend_unit[i] * 1.6

                # Add constraint label to subplot title
                title = (
                    f"{channel}\n"
                    f"[{format_k(lower_bound, None)} - {format_k(upper_bound, None)}] & "
                    f"[{format_k(lower_bound_ext, None)} - {format_k(upper_bound_ext, None)}]"
                )
                ax.set_title(title, fontsize=10)

                # Plot points and error bars for each scenario
                for scenario_idx, scenario in enumerate(scenarios):
                    spend = self.mainPoints.spend_points[scenario_idx, i]
                    response = self.mainPoints.response_points[scenario_idx, i]

                    # Adjust bounds based on scenario
                    if scenario == "Bounded":
                        plot_lb = lower_bound + carryover_spend
                        plot_ub = upper_bound + carryover_spend
                    elif scenario == "Bounded x3":
                        plot_lb = lower_bound_ext + carryover_spend
                        plot_ub = upper_bound_ext + carryover_spend
                    else:
                        plot_lb = plot_ub = None

                    # Plot error bars for constraints
                    if plot_lb is not None and plot_ub is not None:
                        # Use vlines to create the dotted lines instead of errorbar
                        ax.hlines(
                            y=response,
                            xmin=plot_lb,
                            xmax=plot_ub,
                        ...
                        # Add diamond markers at constraint bounds
                        ax.scatter(
                            [plot_lb, plot_ub],
                            [response, response],
                            marker="D",
                    ...
                    ax.scatter(
                        ...
                        response,
                        color="black",
                        facecolor=scenario_colors[scenario_idx],
                        s=50,
                        zorder=5,
                        marker="o",
                        linewidth=1,
