...
class XTSMDSocket_io(socketio.Client):
    """A Socket.IO client.
    ...
    def __init__(self, token, userID,reconnection=True, reconnection_attempts=0, reconnection_delay=1,
                 reconnection_delay_max=50000, randomization_factor=0.5, logger=False, binary=False, json=None,URL=None,
                 **kwargs):
        ...
        self.userID = userID
        publishFormat = 'JSON'
        self.broadcastMode = "Full"#configParser.get('root_url', 'broadcastMode')
        self.token = token
