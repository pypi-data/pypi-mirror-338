...
while len(val_months) >= cv.val_size:
    ...
    clf.fit(X_train[clf_index], y_train_flag, eval_set=[(X_train[clf_index], y_train_flag)], verbose = False)

    mask = np.array(y_train) > 0
    if mask.sum() > 0:
        reg = xgb.XGBRegressor(**params)
        reg.fit(X_train[reg_index][mask], np.array(y_train)[mask], eval_set = [(X_train[reg_index][mask], np.array(y_train)[mask])], verbose = False)
    else:
        reg = None

    sale_prob_train_raw = clf.predict_proba(X_train[clf_index])[:, 1]
    ...
    if reg is not None:
        reg_pred_train = reg.predict(X_train[reg_index])
    else:
    ...
    for i in range(cv.val_size):
        ...
        if index_data_val:
            ...
            sale_prob_val_raw = clf.predict_proba(X_val[clf_index])[:, 1]
            ...
            if reg is not None:
                reg_pred_val = reg.predict(X_val[reg_index])
            else:
...
if index_data:
    ...
    if mask.sum() > 0:
        reg = xgb.XGBRegressor(**params)
        reg.fit(X_train[reg_index], np.array(y_train), eval_set=[(X_train[reg_index], np.array(y_train))], verbose=False)
...
if not test_months.empty:
    ...
    for m in test_months:
        ...
        if index_data_test:
            ...
            sale_prob_test_raw = clf.predict_proba(X_test[clf_index])[:, 1]
            ...
            if reg is not None:
                reg_pred_test = reg.predict(X_test[reg_index])
            else:
