    ...
    def generate_config(
        ...
        if version == "latest" and len(self.attachments) >= 3:
            ...
            if self.test_run:
                ...
                # Check InjCap is actually measured during wafer probing, otherwise leave empty.
                # In case no InjCap is measured, the value should be -1*scaling so -1e-15
                ...
                chip_template[chip_type]["Parameter"]["KSenseShuntA"] = (
                    self.test_run.get_result("CURR_MULT_FAC_A") * self.ratio_kshunt_kin
                )
                chip_template[chip_type]["Parameter"]["KSenseShuntD"] = (
                    self.test_run.get_result("CURR_MULT_FAC_D") * self.ratio_kshunt_kin
                )
                chip_template[chip_type]["Parameter"][
                    "KShuntA"
...
class LocalModule:
    ...
    def generate_config(
        ...
        for chip_index, chip in enumerate(self.chips):
            ...
            if not config_id:
                msg = (
                    f"No {suffix} chip configs were found for {chip} @ {current_stage}"
                )
                raise RuntimeError(msg)

            config_info = config_api.get_info(config_id)
            if not config_info:
                msg = "Unable to find configuration information for {config_id} for {suffix} chip configs for {chip} @ [current_stage}"
                raise RuntimeError(msg)
