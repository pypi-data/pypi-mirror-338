...
class Observation(object):
    ...
    def thermal_noise(self) -> Optional[Union[u.Quantity, dict[str, u.Quantity]]]:
        ...
        if len(self.sources()) == 0 or self.times is None:
            ...
            sefds = np.array([stat.sefd(self.band).value for stat in self.stations])
            bandwidths = np.ones(len(sefds))*128
            bandwidth_min = np.minimum.outer(bandwidths, bandwidths)
            return np.sum((bandwidth_min/np.outer(sefds, sefds)) * np.triu(np.ones_like(bandwidth_min), k=1))
        ...
        for source in self.sources():
            ...
            # TODO: data rate should check the maximum data rates from each station
            temp = 1.0/np.sqrt(temp*self.ontarget_fraction)
