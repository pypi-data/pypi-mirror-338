...
def load_dataset(
    ...
    # Check if the language_csv_path is valid
    ...
    for _, i in tqdm(uml_df_metadata.iterrows(), total=uml_df_metadata.shape[0], desc="Loading models"):
        ...
        json_data = json.load(open(os.path.join(graph_data_dir_path, model_id, f"{model_name}.json")))
        if json_data is None:
            print(f"Error loading JSON data for model ID: {model_id}")
            continue
        text_data = open(os.path.join(text_data_dir_path, model_id, f"{model_name}.txt")).read()
        if text_data is None:
            print(f"Error loading text data for model ID: {model_id}")
            continue
        xmi_data = open(os.path.join(xmi_data_dir_path, model_id)).read()
        if xmi_data is None:
