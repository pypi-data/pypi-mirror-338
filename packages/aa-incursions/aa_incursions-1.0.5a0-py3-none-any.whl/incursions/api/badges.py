    ...
    def __init__(self, api: NinjaAPI) -> None:
        ...
        def get_badge_members(request, badge_id: int) -> tuple[int, list[CharacterBadgesSchema] | dict]:
            ...
            if not characters.exists():
                logger.error(f"No members found or invalid badge ID: {badge_id}")
        ...
        def assign_badge(request, badge_id: int, character_id: int) -> tuple[int, dict]:
            if not request.user.has_perm("incursions.manage_badges"):
            ...
            if CharacterBadges.objects.filter(character=character, badge=badge).exists():
                logger.warning(f"Character {character_id} already has badge {badge_id}")
            ...
            logger.info(f"Assigned badge {badge_id} to character {character_id} by user {request.user}")
