...
class FakeEnforcer:
    def __init__(self, rules, config, namespace):
        self.rules = rules
        self.conf = None
...
def tool(policy_file, access_file, apply_rule, is_admin=False,
         target_file=None, enforcer_config=None, enforcer_namespace=None):
    ...
    enforcer = FakeEnforcer(rules, enforcer_config, enforcer_namespace)
...
def main():
    ...
    conf.register_cli_opt(cfg.StrOpt(
        'enforcer_namespace',
        help='namespace for the oslopolicy-checker enforcer'))

    conf()

    tool(conf.policy, conf.access, conf.rule, conf.is_admin,
         conf.target, conf.enforcer_config, conf.enforcer_namespace)
