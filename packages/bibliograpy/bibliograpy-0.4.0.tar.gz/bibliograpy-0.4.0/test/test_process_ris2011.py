...
def test_process_ris2011_yml_to_yml():
    ...
    _process(Namespace(CMD='process',
                       ...
                       output_file=_output('test_process_ris2011_yml_to_yml.yml'),
                       encoding='utf-8',
                       output_dir='.',
                       format='ris2011'))

    with open(_output('test_process_ris2011_yml_to_yml.yml'), encoding='utf-8') as s:
...
def test_process_ris2011_yml_to_ris():
    """test process from a yml bibliography to a RIS 2001 bibliography"""

    _process(Namespace(CMD='process',
                       file=_ris2001_resource('sample1.yml'),
                       output_file=_output('test_process_ris2011_yml_to_ris.ris'),
                       encoding='utf-8',
                       output_dir='.',
                       format='ris2011'))

    with open(_output('test_process_ris2011_yml_to_ris.ris'), encoding='utf-8') as s:
        content = s.read()
...
def test_process_ris2011_yml_to_json():
    """test process from a yml bibliography to a RIS 2001 bibliography"""

    _process(Namespace(CMD='process',
                       file=_ris2001_resource('sample1.yml'),
                       output_file=_output('test_process_ris2011_yml_to_json.json'),
                       encoding='utf-8',
                       output_dir='.',
                       format='ris2011'))

    with open(_output('test_process_ris2011_yml_to_json.json'), encoding='utf-8') as s:
        content = s.read()
        assert content == ('[{"TY": "JOUR", '
                           '"A1": ["Baldwin,S.A.", "Fugaccia,I.", "Brown,D.R.", "Brown,L.V.", "Scheff,S.W."], '
                           '"T1": "Blood-brain barrier breach following cortical contusion in the rat", '
...
def test_process_ris2011_json_to_yml():
    """test process from a json bibliography to a yml bibliography"""

    _process(Namespace(CMD='process',
                       file=_ris2001_resource('sample1.json'),
                       output_file=_output('test_process_ris2011_json_to_yml.yml'),
                       encoding='utf-8',
                       output_dir='.',
                       format='ris2011'))

    with open(_output('test_process_ris2011_json_to_yml.yml'), encoding='utf-8') as s:
        content = yaml.safe_load(s)
        assert content == [
            {"TY": "JOUR",
             "A1": ["Baldwin,S.A.", "Fugaccia,I.", "Brown,D.R.", "Brown,L.V.", "Scheff,S.W."],
...
def test_process_ris2011_json_to_ris():
    """test process from a json bibliography to a RIS 2001 bibliography"""

    _process(Namespace(CMD='process',
                       file=_ris2001_resource('sample1.json'),
                       output_file=_output('test_process_ris2011_json_to_ris.ris'),
                       encoding='utf-8',
                       output_dir='.',
                       format='ris2011'))

    with open(_output('test_process_ris2011_json_to_ris.ris'), encoding='utf-8') as s:
        content = s.read()
        assert content == """TY  - JOUR
A1  - Baldwin,S.A.
A1  - Fugaccia,I.
...
def test_process_ris2011_json_to_json():
    """test process from a json bibliography to a json bibliography"""

    _process(Namespace(CMD='process',
                       file=_ris2001_resource('sample1.json'),
                       output_file=_output('test_process_ris2011_json_to_json.json'),
                       encoding='utf-8',
                       output_dir='.',
                       format='ris2011'))

    with open(_output('test_process_ris2011_json_to_json.json'), encoding='utf-8') as s:
        content = s.read()
        assert content == ('[{"TY": "JOUR", '
                           '"A1": ["Baldwin,S.A.", "Fugaccia,I.", "Brown,D.R.", "Brown,L.V.", "Scheff,S.W."], '
                           '"T1": "Blood-brain barrier breach following cortical contusion in the rat", '
...
def test_process_ris2011_ris_to_yml():
    """test process from a bib bibliography to a yml bibliography"""

    _process(Namespace(CMD='process',
                       file=_ris2001_resource('sample1.ris'),
                       output_file=_output('test_process_ris2011_ris_to_yml.yml'),
                       encoding='utf-8',
                       output_dir='.',
                       format='ris2011'))

    with open(_output('test_process_ris2011_ris_to_yml.yml'), encoding='utf-8') as s:
        content = yaml.safe_load(s)
        assert content == [
            {"TY": "JOUR",
             "A1": ["Baldwin,S.A.", "Fugaccia,I.", "Brown,D.R.", "Brown,L.V.", "Scheff,S.W."],
...
def test_process_ris2011_ris_to_ris():
    """test process from a RIS 2001 bibliography to a RIS 2001 bibliography"""

    _process(Namespace(CMD='process',
                       file=_ris2001_resource('sample1.ris'),
                       output_file=_output('test_process_ris2011_ris_to_ris.ris'),
                       encoding='utf-8',
                       output_dir='.',
                       format='ris2011'))

    with open(_output('test_process_ris2011_ris_to_ris.ris'), encoding='utf-8') as s:
        content = s.read()
        assert content == """TY  - JOUR
A1  - Baldwin,S.A.
A1  - Fugaccia,I.
...
def test_process_ris2011_ris_to_json():
    """test process from a RIS 2001 bibliography to a json bibliography"""

    _process(Namespace(CMD='process',
                       file=_ris2001_resource('sample1.ris'),
                       output_file=_output('test_process_ris2011_ris_to_json.json'),
                       encoding='utf-8',
                       output_dir='.',
                       format='ris2011'))

    with open(_output('test_process_ris2011_ris_to_json.json'), encoding='utf-8') as s:
        content = s.read()
        assert content == ('[{"TY": "JOUR", '
                           '"A1": ["Baldwin,S.A.", "Fugaccia,I.", "Brown,D.R.", "Brown,L.V.", "Scheff,S.W."], '
                           '"T1": "Blood-brain barrier breach following cortical contusion in the rat", '
...
def test_process_ris2011_yml_to_py():
    """test process from a yml bibliography to a py source bibliography"""

    _process(Namespace(CMD='process',
                       file=_ris2001_resource('mini.yml'),
                       output_file=_sibbling_module('test_process_ris2011_yml_to_py.py'),
                       encoding='utf-8',
                       output_dir='.',
                       format='ris2011'))
    ...
    from test_process_ris2011_yml_to_py import IAU, NASA
...
def test_process_ris2011_json_to_py():
    """test process from a yml bibliography to a py source bibliography"""

    _process(Namespace(CMD='process',
                       file=_ris2001_resource('mini.json'),
                       output_file=_sibbling_module('test_process_ris2011_json_to_py.py'),
                       encoding='utf-8',
                       output_dir='.',
                       format='ris2011'))
    ...
    from test_process_ris2011_json_to_py import IAU, NASA
...
def test_process_ris2011_ris_to_py():
    """test process from a yml bibliography to a py source bibliography"""

    _process(Namespace(CMD='process',
                       file=_ris2001_resource('mini.ris'),
                       output_file=_sibbling_module('test_process_ris2011_ris_to_py.py'),
                       encoding='utf-8',
                       output_dir='.',
                       format='ris2011'))
    ...
    from test_process_ris2011_ris_to_py import IAU, NASA
