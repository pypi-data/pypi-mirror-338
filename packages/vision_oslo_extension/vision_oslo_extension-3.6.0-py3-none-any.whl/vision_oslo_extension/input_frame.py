...
class S03(BasePage):
    def __init__(self, parent, controller):
        ...
        choice3 = tk.Label(master=self.optionframe, text = 'Option 2: DC Extra Oslo Creator',font = controller.text_font)
        choice3.grid(row = 4, column = 0, sticky = "w", padx= 10, pady=5)

        filename3 = 'extra_bat_oslo_creator.xlsm'
        button3 = tk.Button(master=self.optionframe, text="Import Battery Extra OSLO Creator",command = lambda: SharedMethods.copy_example_files(filename3),width = 30, height =1)
