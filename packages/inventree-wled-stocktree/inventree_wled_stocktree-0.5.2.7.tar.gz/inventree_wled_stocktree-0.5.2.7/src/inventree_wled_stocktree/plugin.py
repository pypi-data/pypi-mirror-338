...
class WledInventreePlugin(UrlsMixin, LocateMixin, SettingsMixin, InvenTreePlugin):
    ...
    def turn_off_led(base_url, target_led):
        ...
        color_black = [0, 0, 0]  # RGB black
        requests.post(
            base_url,
            json={"seg": [{"i": [[target_led, target_led]], "col": [color_black]}]},
            timeout=3,
        )
    ...
    def _set_led(self, target_led: int = None, request=None):
        ...
        color_black = [0, 0, 0]  # RGB black
        color_marked = [255, 0, 0]  # RGB red

        # 🛑 Turn off ALL LEDs first
        requests.post(
            base_url,
            json={"seg": [{"i": [[0, max_leds - 1]], "col": [color_black]}]},
            timeout=3,
        )
        ...
        if target_led is not None:
            requests.post(
                base_url,
                json={"seg": [{"i": [[target_led, target_led]], "col": [color_marked]}]},
                timeout=3,
            )

            # 🔄 Schedule the LED to turn off after 10 seconds
            threading.Thread(target=turn_off_led, args=(base_url, target_led), daemon=True).start()
