...
from textparse.types import SMART_QUOTES_TABLE, ColorAttrs, ColorName


class TextParse(StrEnum):
    ...
    def escape(self, text: str) -> str:
        ...
        if self is TextParse.MARKDOWN or self is TextParse.MARKDOWN_V2:
            return self._escape_markdown(text)
        return self._escape_html(text) if self is TextParse.HTML else text

    def clean(self, text: str) -> str:
        """Remove all formatting based on the current text type."""
        if self is TextParse.MARKDOWN or self is TextParse.MARKDOWN_V2:
            return self._strip_markdown(text)
        return self._strip_html(text) if self is TextParse.HTML else text

    def _escape_markdown(self, text: str) -> str:
        if self not in {TextParse.MARKDOWN, TextParse.MARKDOWN_V2}:
            return text
    ...
    def _strip_html(self, text: str) -> str:
        return text if self != TextParse.HTML else re.sub(r"<[^>]*>", "", text)
    ...
    def split_message(message: str, max_length: int = 4096) -> list[str]:
        ...
        for pattern in split_points.values():
            split_point = TextParse._find_split_point(message, pattern, max_length)
            if split_point:
                break

        if not split_point:
            split_point, code_block_language = TextParse._split_by_code_blocks(message, max_length)

        part1 = message[:split_point].rstrip()
        part2 = message[split_point:].lstrip()

        if not TextParse._is_balanced_code_blocks(part1):
            part1 += "```"
            part2 = f"```{code_block_language}\n{part2}"

        return [part1, *TextParse.split_message(part2, max_length)]
    ...
    def _split_by_code_blocks(text: str, max_len: int) -> tuple[int, str]:
        ...
        for index in code_block_indices:
            if index < max_len:
                ...
                if TextParse._is_balanced_code_blocks(text[:split_point]):
                    break
    ...
    def truncate(
        ...
        if strict:  # In strict mode, truncate the text exactly to the specified limit
            truncated_text = TextParse._truncate_strict(text, chars, from_middle)
        else:  # In non-strict mode, truncate at sentence or word boundaries
            truncated_text = TextParse._truncate_at_boundaries(
    ...
    def _truncate_at_boundaries(
        ...
        if from_middle:
            truncated_text = TextParse._truncate_from_middle(chars, text, condensed)
        else:  # For standard (non-middle) truncation, find the optimal single truncation point
            split_index = TextParse._find_index(text, chars)
    ...
    def _truncate_from_middle(chars: int, text: str, condensed: bool) -> str:
        ...
        first_half_index = TextParse._find_index(text, first_half_limit)
        second_half_index = len(text) - TextParse._find_index(text[::-1], second_half_limit)
    ...
    def pluralize(*args: Any, **kwargs: Any) -> str:
        """Pluralize (for backward compatibility; use `TextParse.plural` now)."""
        import warnings

        warnings.warn(
            "'pluralize' is deprecated, use 'plural' instead", DeprecationWarning, stacklevel=2
        )
        return TextParse.plural(*args, **kwargs)

    @staticmethod
    def format_duration(hours: int = 0, minutes: int = 0, seconds: int = 0) -> str:
        """Print a formatted time duration."""
        sec_str = TextParse.plural("second", seconds, with_count=True)
        min_str = TextParse.plural("minute", minutes, with_count=True)
        hour_str = TextParse.plural("hour", hours, with_count=True)
    ...
    def num_to_word(
        ...
        if word_to_pluralize:
            word_to_pluralize = TextParse.plural(word_to_pluralize, number)
    ...
    def format_number(
        ...
        *,
        ...
        if as_ordinal:  # Convert number to appropriate form
            num_str = TextParse.ordinal_num(number)
        ...
        if word:  # Handle word if provided
            if as_ordinal:
                ...
                pluralized = TextParse.plural(word, number)
    ...
    def normalize(text: str) -> str:
        ...
        text = TextParse.straighten_quotes(text)
        text = text.strip()
    ...
    def extract_first_emoji(text: str) -> str:
        ...
        return "" if not text or not TextParse.starts_with_emoji(text) else text[0]
