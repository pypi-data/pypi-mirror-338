...
from grond.targets.phase_pick.target import PhasePickTarget
...
parameter bounds and shows the model space of the optimsation. %s''' % sref)
    ...
    def draw_figures(self, history, optimiser):
        ...
        if nselected < 2:
            logger.warning(
                'Cannot draw joinpar figures with less than two parameters '
                'selected.')
            return []
...
''' % (', cluster results' if self.cluster_attribute else ''))
    ...
    def draw_figures(self, history):
        ...
        if models.size == 0:
            logger.warning('Empty models vector.')
            return []
...
class LocationPlot(SectionPlotConfig):
    '''Location plot of the best solutions in three cross-sections. '''
    name = 'location'
    marker_size = Int.T(
        default=2,
        help='Size of the markers [points].')
...
Symbols show best locations and colors indicate low (red) and high (blue)
misfit.
''')
    ...
    def draw_figures(self, history, color_p_axis=False):
        ...
        for axes, xparname, yparname in [
            ...
            for ix, x in enumerate(models):
                ...
                axes.plot(
                    sx, sy, 'o', color=color, alpha=alpha, ms=self.marker_size)
...
class MTFuzzyPlot(PlotConfig):
    ...
    cluster_attribute = meta.StringID.T(
        optional=True,
        help='name of attribute to use as cluster IDs')
    plot_polarities = Bool.T(
        default=True,
        help='show observed polarities used in inversion')

    plot_polarities_model = StringChoice.T(
        choices=['mean', 'best', 'all'],
        default='mean',
        help='choice of model to use for plotting polarities')
...
''' % ('cluster' if self.cluster_attribute is not None else 'global'))
    ...
    def draw_figures(self, history):
        ...
        targets = [
            ...
            isinstance(t, PhasePickTarget) and t.weight_polarity > 0.]
        ...
        for icluster, percentage, imodels in by_cluster:
            ...
            best_model = stats.get_best_x(problem, models, misfits)
            best_source = problem.get_source(best_model)
            best_mt = best_source.pyrocko_moment_tensor()

            mean_model = stats.get_mean_x(models)
            mean_source = problem.get_source(mean_model)

            sources = [problem.get_source(x) for x in models]
            mts = [source.pyrocko_moment_tensor() for source in sources]

            if self.plot_polarities:

                if self.plot_polarities_model == 'all':
                    sources_polarities = sources
                    models_polarities = models
                elif self.plot_polarities_model == 'best':
                    sources_polarities = [best_source]
                    models_polarities = [best_model]
                elif self.plot_polarities_model == 'mean':
                    sources_polarities = [mean_source]
                    models_polarities = [mean_model]
                else:
                    raise meta.GrondError(
                        'Invalid argument for plot_polarities_model')

                polarities = num.zeros((len(sources_polarities), ntargets, 4))

                for isource, (source, model) in enumerate(
                        zip(sources_polarities, models_polarities)):

                    results = problem.evaluate(model, targets=targets)
                    for itarget, result in enumerate(results):
                        result = results[itarget]
                        polarities[isource, itarget, :] = (
                            result.polarity_obs,
                            result.polarity_syn,
                            result.azimuth,
                            result.takeoff_angle)
            ...
            if self.plot_polarities:
                azi = polarities[:, :, 2].flatten()
                takeoff = polarities[:, :, 3].flatten()

                upper = takeoff > 90.
                azi[upper] += 180.
                takeoff[upper] = 180. - takeoff[upper]

                rtp = num.ones((azi.size, 3))
                rtp[:, 1] = num.deg2rad(takeoff)
                rtp[:, 2] = num.deg2rad(90.-azi)

                # to 3D coordinates (x, y, z)
                points = beachball.numpy_rtp2xyz(rtp)

                # project to 2D with same projection as used in beachball
                x, y = 0.5 * scale * beachball.project(points).T

                positive = (polarities[:, :, 0] > 0.).flatten()
                negative = num.logical_not(positive)
            ...
            axes.set_xlim(-1.1*0.5*scale, 1.1*0.5*scale)
            axes.set_ylim(-1.1*0.5*scale, 1.1*0.5*scale)
            axes.set_axis_off()
