...
class Dataset(object):
    ...
    def __init__(self, event_name=None):
        ...
        self.clippings = {}
        self.exclude = set()
        self.include_nslc = None
        self.include_nsl_xx = None
        self.include = None
    ...
    def add_exclude(self, exclude=[], filenames=None):
        logger.debug('Loading exclude definitions...')
        if filenames:
            ...
            for filename in filenames:
                if op.exists(filename):
                    ...
                    logger.warning('No such exclude file: %s' % filename)
    ...
    def add_include(self, include=[], filenames=None):
        logger.debug('Loading included stations...')
        if filenames:
            include = list(include)
            for filename in filenames:
                with open(filename, 'r') as f:
                    include.extend(s.strip() for s in f.read().splitlines())

        if self.include_nslc is None:
            self.include_nslc = set()
            self.include = set()
            self.include_nsl_xx = set()

        for x in include:
            if isinstance(x, str):
                x = tuple(x.split('.'))
            if len(x) == 4:
                self.include_nslc.add(x)
                self.include_nsl_xx.add(x[:3])
            else:
                self.include.add(x)
    ...
    def is_excluded(self, obj):
        try:
            nslc = self.get_nslc(obj)
            if nslc in self.exclude:
                return True
        ...
        return (
            nsl in self.exclude or
            nsl[1:2] in self.exclude or
            nsl[:2] in self.exclude)
    ...
    def is_included(self, obj):
        ...
        if (
                nsl in self.include or
                nsl[1:2] in self.include or
                nsl[:2] in self.include):

            return True

        try:
            nslc = self.get_nslc(obj)
            if nslc in self.include_nslc:
                return True

        except InvalidObject:
            return nsl in self.include_nsl_xx
    ...
    def get_station(self, obj):
        if self.is_excluded(obj):
            raise NotFound('Station is in exclude list:', self.get_nsl(obj))

        if not self.is_included(obj):
            raise NotFound(
                'Station is not on include list:', self.get_nsl(obj))
    ...
    def get_stations(self):
        return [self.stations[k] for k in sorted(self.stations)
                if not self.is_excluded(self.stations[k])
                and self.is_included(self.stations[k])]
    ...
    def get_response(self, obj, quantity='displacement'):
        ...
        if self.is_excluded(obj):
            raise NotFound(
                'Response is in exclude list:', self.get_nslc(obj))

        if not self.is_included(obj):
            raise NotFound(
                'Response is not on include list:', self.get_nslc(obj))
    ...
    def get_waveform_raw(
        ...
        if self.is_excluded((net, sta, loc, cha)):
            raise NotFound(
                'Waveform is in exclude list:', (net, sta, loc, cha))

        if not self.is_included((net, sta, loc, cha)):
            raise NotFound(
                'Waveform is not on include list:', (net, sta, loc, cha))
    ...
    def _get_waveform(
        ...
        if self.is_excluded(nslc):
            raise NotFound(
                'Waveform is excluded:', nslc)

        if not self.is_included(nslc):
            raise NotFound(
                'Waveform is not on include list:', nslc)
...
class DatasetConfig(HasPaths):
    ...
    picks_paths = List.T(
        Path.T())
    exclude_paths = List.T(
        Path.T(),
        help='List of text files with excluded stations.')
    exclude = List.T(
        String.T(),
        help='Stations/components to be excluded according to their STA, '
             'NET.STA, NET.STA.LOC, or NET.STA.LOC.CHA codes.')
    include_paths = List.T(
        Path.T(),
        help='List of text files with included stations.')
    include = List.T(
        String.T(),
        optional=True,
        help='If not None, list of stations/components to include according '
             ...
             'Note: ''when including on channel level, both, the raw and '
    ...
    blacklist = List.T(String.T(), optional=True)
    whitelist = List.T(String.T(), optional=True)
    blacklist_paths = List.T(Path.T(), optional=True)
    whitelist_paths = List.T(Path.T(), optional=True)
    ...
    def __init__(self, *args, **kwargs):
        ...
        # for backward compatibility
        if self.blacklist:
            self.exclude.extend(self.blacklist)
            self.blacklist = None
        if self.whitelist:
            self.include.extend(self.whitelist)
            self.whitelist = None
        if self.blacklist_paths:
            self.exclude_paths.extend(self.blacklist_paths)
            self.blacklist_paths = None
        if self.whitelist_paths:
            self.include_paths.extend(self.whitelist_paths)
            self.whitelist_paths = None
    ...
    def get_dataset(self, event_name):
        if event_name not in self._ds:
            ...
            try:
                ...
                ds.add_exclude(self.exclude)
                ds.add_exclude(filenames=fp(self.exclude_paths))
                if self.include:
                    ds.add_include(self.include)
                if self.include_paths:
                    ds.add_include(filenames=fp(self.include_paths))
