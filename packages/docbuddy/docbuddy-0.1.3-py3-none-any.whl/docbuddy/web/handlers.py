"""FastHTML request handlers for DocBuddy web interface."""
from fasthtml.common import *
from docbuddy.main import ask_question, preview_matches
from docbuddy.config import get_config
from docbuddy.core import kb_info

def get_index(request):
    """Render the index page."""
    config = get_config()
    web_config = config.get("web", {})
    title = web_config.get("title", "DocBuddy")
...
def post_question(request):
    ...
    if not question:
        return render_template(
            "index.html",
            title=get_config().get("web", {}).get("title", "DocBuddy"),
...
def get_model_info(request):
    ...
    return render_template(
        "model_info.html",
        title=config.get("web", {}).get("title", "DocBuddy") + " - Models",
        default_model=config["llm"]["default_model"],
        models=models
    )
...
def get_templates(request):
    ...
    return render_template(
        "templates.html",
        title=config.get("web", {}).get("title", "DocBuddy") + " - Templates",
...
def get_kb_status(request):
    ...
    return render_template(
        "kb_status.html",
        title=get_config().get("web", {}).get("title", "DocBuddy") + " - Knowledge Base",
