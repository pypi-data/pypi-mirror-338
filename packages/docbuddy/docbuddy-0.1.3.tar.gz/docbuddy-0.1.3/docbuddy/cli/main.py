"""Command-line interface for DocBuddy."""
import os
import sys
import typer
import json
...
from docbuddy.core import (
    ask_question, 
    preview_matches, 
    build_or_rebuild_kb, 
    get_kb_info
)
from docbuddy.config import (
    get_config, 
    get_default_model, 
    get_source_dir, 
    get_rag_config
...
app = typer.Typer(
    help="DocBuddy: A document assistant powered by LLMs and RAG",
    add_completion=False,
)

# Create subcommands
tui_app = typer.Typer(help="Terminal User Interface for DocBuddy")
web_app = typer.Typer(help="Web Interface for DocBuddy")
...
def web_main(
    ...
    """Launch the Web Interface for DocBuddy."""
    # Only continue if this is the command being invoked
    if ctx.invoked_subcommand is None:
        try:
            # Import the serve_app function from the web module
            from docbuddy.web.app import serve_app
            # Run the web app
