...
if TYPE_CHECKING:
    from fused_batch.models.internal import JobResponse, Jobs, RunResponse
...
def render_command_toolkit(job_id: str):
    ...
    for command_description, command_code in [
        (
            "Get job config",
            f'fused_batch.experimental.job("{job_id}", content_type="fused_job_id")',
        ),
        # ("Get status", f'fused_batch.api.job_get_status("{job_id}")'),
        ("Tail logs", f'fused_batch.api.job_tail_logs("{job_id}")'),
        ("Print logs", f'fused_batch.api.job_print_logs("{job_id}")'),
        ("Get execution time", f'fused_batch.api.job_get_exec_time("{job_id}")'),
        ("Cancel", f'fused_batch.api.job_cancel("{job_id}")'),
    ]:
...
def fused_jobs_repr(jobs: Jobs) -> str:
    obj_type = f"fused_batch.{jobs.__class__.__name__}"
    header_components = [
