...
from fused_batch.models.udf._specialized_udfs import GeoPandasUdfV2Callable
from fused_batch.models.udf.udf import GeoPandasUdfV2
...
# is right when using "@fused_batch.udf()" -- but this does mean it's wrong when
# using "@fused_batch.udf".
def udf(
    fn: Optional[Callable] = None,
    *,
    ...
    def remove_params_from_decorator(node):
        if isinstance(node, ast.Call):
            if (
                ...
                and node.func.value.id in ["fused", "fused_batch"]
