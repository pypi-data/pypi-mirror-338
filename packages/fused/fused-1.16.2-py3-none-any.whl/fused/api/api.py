...
def _run_async_safely(coro: Coroutine[Any, Any, T]) -> T:
    """
    Bridge to run an async coroutine from a synchronous context.

    Works whether or not there's already a running event loop:
...
class FusedAPI:
    ...
    async def _fetch_efs_all_pages(
        self, total_pages: int, params, list_url: str, payload: dict
    ):
        ...
        async with aiohttp.ClientSession() as session:
            tasks = [
                asyncio.create_task(fetch_page(session, page))
    ...
    def _list_efs(self, path: str, details: bool, client_id: str):
        ...
        payload = {"item": path}
        params = {"limit": OPTIONS.efs_items_per_request}
        ...
        total_pages = result["total_items"] // OPTIONS.efs_items_per_request + 1

        items = result["items"]
        if total_pages >= 1:
            items.extend(
                _run_async_safely(
                    self._fetch_efs_all_pages(
                        total_pages, params, list_request_url, payload
                    )
                )
            )
