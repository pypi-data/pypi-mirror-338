...
class ErrorId:
    ...
    def ModuleFix(path, model):
        ...
        template = f"""
            Provide only the Windows simple command to fix this issue: {output}.  
            Do not include any explanation or commands for Linux or Mac.  
            do not use "``````" while generating the code.
            Except this there were several other anomalies are there, try to avoid them.
            no need to run everything, if it is a command then run, else don't run 
            only run code to install dependencies, not other codes.
        """
