...
async def init(
    ...
    if max_connections < 2:
        raise ValueError('Pool size should be greater than 1')
    pool = BlockingConnectionPool.from_url(
        url=url, max_connections=max_connections, timeout=timeout,
        protocol='3')
    await pool.make_connection()
...
def with_redis(*, name: str):
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            async with StrictRedis.from_pool(pool) as redis:
                kwargs['redis'] = redis
