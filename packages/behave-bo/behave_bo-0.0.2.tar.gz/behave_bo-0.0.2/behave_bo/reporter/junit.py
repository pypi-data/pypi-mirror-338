...
class JunitReportsCombiner:
    ...
    def __init__(
        ...
        self._report_filename_pattern = fr'(.*?){self._scenario_group_name_pattern}\.xml'  # noqa
    ...
    def prepare_report_files_paths(self):
        ...
        for _, _, files in os.walk(self._reports_dir_path, topdown=False):
            for filename in files:
                match = re.match(self._report_filename_pattern, filename)
                if match:
    ...
    def collect_reports_data_from_files(self):
        ...
        for feature_name, path in self._reports_parts_files:
    ...
    def export_top_scenarios_by_time(self):
        ...
        for time, time_process, tc_name, tc_status, tc_pid, tc_url in sorted(self._test_cases_with_time, key=lambda k: k[0], reverse=True):
            message_time = f'\t{time} sec. <sup>(pt {time_process} sec.)</sup>'
            message_status = f'<sup>pid {tc_pid}</sup>'

            if tc_url:
                lines.append(f'<li class="{tc_status}">{message_time} <a href="{tc_url}" target="blank">{tc_name}</a> {message_status}</li>')
            else:
                lines.append(f'<li class="{tc_status}">{message_time} {tc_name} {message_status}</li>')

        with open(os.path.join(os.path.dirname(__file__), 'top_scenarios_template.html')) as ft:
            html_template = Template(ft.read())

        with open(os.path.join(self._reports_dir_path, f'top_scenarios.html'), 'w') as f:
            f.write(html_template.substitute(body=f"<ul>{''.join(lines)}</ul>"))
