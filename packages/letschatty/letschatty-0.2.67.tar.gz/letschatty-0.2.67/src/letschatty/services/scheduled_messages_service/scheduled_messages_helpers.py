...
from letschatty.services.chat.chat_helper import ChatHelper
...
class ScheduledMessagesHelper:
    ...
    def instantiate_sm_free_message(messages_from_agent: SendMessagesFromAgentToChat) -> ScheduledMessages:
        ...
        return ScheduledMessages(messages=messages_from_agent.messages, scheduled_at=messages_from_agent.scheduled_at, forced_send=messages_from_agent.forced_send, agent_email=messages_from_agent.agent_email, subtype=ScheduledMessageSubtype.FREE_MESSAGE)
    ...
    def create_sm(chat: Chat, messages_from_agent: SendMessagesFromAgentToChat) -> ScheduledMessages:
        ...
        """
        sm = ScheduledMessages(messages=messages_from_agent.messages, scheduled_at=messages_from_agent.scheduled_at, forced_send=messages_from_agent.forced_send, agent_email=messages_from_agent.agent_email, subtype=ScheduledMessageSubtype.FREE_MESSAGE)
    ...
    def instantiate_sm_template(filled_template_data: FilledTemplateData) -> ScheduledMessages:
        ...
        return ScheduledMessages(filled_template_data=filled_template_data, scheduled_at=filled_template_data.scheduled_at, forced_send=filled_template_data.forced_send, agent_email=filled_template_data.agent_email, subtype=ScheduledMessageSubtype.TEMPLATE)
    ...
    def create_sm_template(chat: Chat, filled_template_data: FilledTemplateData) -> ScheduledMessages:
        ...
        """
        sm = ScheduledMessages(filled_template_data=filled_template_data, scheduled_at=filled_template_data.scheduled_at, forced_send=filled_template_data.forced_send, agent_email=filled_template_data.agent_email, subtype=ScheduledMessageSubtype.TEMPLATE)
        chat.scheduled_messages.append(sm)
        body = f"Scheduled message at {sm.scheduled_at} #{sm.id} created by the agent {chat.agent_id}"
        logger.debug(f"{body} | SM status: {sm.status} | SM id: {sm.id} | chat id: {chat.identifier}")
        central_notif_content = ChattyContentCentral(body=body, status=CentralNotificationStatus.INFO)
    ...
    @staticmethod
    def cancel_sm(chat: Chat, sm_id: str) -> None:
        """
        ...
        logger.debug(f"{body} | SM status: {sm.status} | SM id: {sm.id} | chat id: {chat.identifier}")
        central_notif_content = ChattyContentCentral(body=body, status=CentralNotificationStatus.WARNING, CTA="get_scheduled_messages")
        central_notif = CentralNotificationFactory.scheduled_message_status(sm=sm, content=central_notif_content)
        chat.add_central_notification(central_notif)
    ...
    def to_be_sent_sm(chat: Chat, sm_id: str) -> Optional[ScheduledMessages]:
        ...
        if sm.status != ScheduledMessageStatus.SCHEDULED:
            return None
        if sm.subtype == ScheduledMessageSubtype.FREE_MESSAGE and not ChatHelper.is_free_conversation_open(chat):
