...
class StressTest:
    def __init__(
        ...
        Args:
            ...
            exclude_tests: List of test function names to exclude
        """
        self.num_users = num_users
        self.num_rounds = num_rounds
        self.base_url = base_url
        self.exclude_tests = exclude_tests or [
            "test_langgraph_global_disaster_news_can_cause_25_limites"
        ]
    ...
    def _get_test_methods(self) -> List[str]:
        """Get all test methods from TestAPIFunctionality class, excluding the ones in exclude_tests."""
        all_methods = [
            method_name
            for method_name, method in inspect.getmembers(
                TestAPIFunctionality, predicate=inspect.isfunction
            )
            if method_name.startswith("test_") and method_name not in self.exclude_tests
        ]
        return all_methods
    ...
    def run_stress_test(self) -> Dict[str, Any]:
        ...
        summary = {
            "config": {
                ...
                "excluded_tests": self.exclude_tests,
            },
            "statistics": {
    ...
    def _print_summary(self, summary: Dict[str, Any]) -> None:
        ...
        print(f"  Excluded tests: {', '.join(config['excluded_tests'])}")
...
def main():
    ...
    exclude_tests = ["test_langgraph_global_disaster_news_can_cause_25_limites"]
