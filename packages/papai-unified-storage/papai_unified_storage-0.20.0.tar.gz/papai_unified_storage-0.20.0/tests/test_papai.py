...
def test_set_verbose(log_fn_attribute_name="log_debug_fn"):
    fs = filesystem("file")

    assert hasattr(fs, log_fn_attribute_name)

    set_verbose(fs, True)
    assert getattr(fs, log_fn_attribute_name) == print

    set_verbose(fs, False)
    assert getattr(fs, log_fn_attribute_name) == dummy_fn
