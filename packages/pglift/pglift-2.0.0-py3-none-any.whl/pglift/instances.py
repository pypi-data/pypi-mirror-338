...
@contextlib.asynccontextmanager
async def stopped(instance: Instance, *, timeout: int = 10) -> AsyncIterator[None]:
...
async def _(arg: tuple[str, str], /, *, settings: Settings) -> interface.Instance:
    ...
    name, version = arg
    pg_instance = PostgreSQLInstance.system_lookup(name, version, settings)
...
async def is_in_recovery(cnx: db.Connection) -> bool:
    ...
    r = await db.fetchone(
        cnx, "SELECT pg_is_in_recovery()", row_factory=psycopg.rows.args_row(bool)
    )
