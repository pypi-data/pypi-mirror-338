...
class OsvePtrLogger(OsvePtrAbstract):
    ...
    def onPtrBlockStart(self, blockData):

        # --------------------------------------------------------------------------------------------------------------
        # SWI Pointing, Stability, and Duration Compatibility Check
        # --------------------------------------------------------------------------------------------------------------
        # Activate the SWI check if the block has SWI either as PRIME or DESIGNER.
        if not self.remove_obs_comp:
            if "observations" in blockData:
                observations = blockData["observations"]["observations"]
                for observation in observations:
                    if observation["unit"] == 'SWI' and (observation["type"] == 'PRIME' or observation["type"] == 'DESIGNER'):
                        self.swi_check = True
                        try:
                            ...
                            print(f'[WARNING] {"<PTWR>"} {blockData["block_start"]} SWI observation {observation["definition"]} has no target, setting to JUPITER as default.')
                            self.onMsgReceived('WARNING', 'PTWR', blockData["block_start"],
                                               f'SWI observation {observation["definition"]} has no target, setting to JUPITER as default.')
                        # First check is simple: duration of the PTR block
                        violation, message = self._swi_block_duration_check(blockData, observation["definition"])
                        if violation:
                            print(f'[WARNING] {"<PTWR>"} {blockData["block_start"]} {message}')
                            self.onMsgReceived('WARNING', 'PTWR', f'{blockData["block_start"]}', message)

        # --------------------------------------------------------------------------------------------------------------
        # PEPLo NIM Pointing and Duration Compatibility Check
        # --------------------------------------------------------------------------------------------------------------
        # Activate the check if the block has the PEPLO observation.
        if "observations" in blockData:
            observations = blockData["observations"]["observations"]
            for observation in observations:
                if observation["definition"] == 'PEL_FLYBY_CLOSEST_APPROACH':
        ...
        # --------------------------------------------------------------------------------------------------------------
        self.px_illumination_block_violation = False
    ...
    def onPtrBlockEnd(self, blockData):
        ...
        if self.px_illumination_block_violation and blockData['block_type'] != 'SLEW':
            ...
            self.onMsgReceived('ERROR', 'PTWR', blockData["block_end"],
                           f'Block has +X Panel Illuminated.')
        return 0
                ...
                if violation:
                    if not self.swi_block_violation:
                        for key, value in flags.items():
                            if value != 'OK':
                                print(f'[WARNING] {"<PTWR>"} {data["time"]}Z {value} for {self.swi_target.capitalize()}')
                                self.onMsgReceived('WARNING', 'PTWR', f'{data["time"]}Z', value + f' for {self.swi_target.capitalize()}')
                                self.swi_block_violation = True
                else:
                    if self.swi_block_violation:
                        print(f'[INFO]    {"<PTWR>"} {data["time"]}Z SWI Pointing and/or drift range recovered for {self.swi_target.capitalize()}.')
                        self.onMsgReceived('INFO', 'PTWR', f'{data["time"]}Z', f'SWI Pointing and/or drift range recovered for {self.swi_target.capitalize()}.')
                    self.swi_block_violation = False
    ...
    def _extract_error_messages(self, block_data, verbose, slew_prev=None, slew_next=None):
        ...
        for log_data in block_data.get("block_logs", []):
            if log_data["severity"] != "DEBUG" and (log_data["module"] in ["AGM","AGE","PTWR"]):
                ...
                # We do a pre-processing of the error messages to tweak the severity of different errors.
                # - The -X Panel Illumination correction should be solved with:
                #   https://s2e2.cosmos.esa.int/jira/browse/SC6-435
                # - The Attitude angular velocity depends on updates of the AGM config file.
                if ("Attitude angular acceleration" in log_data["text"] or
                        "Attitude angular velocity" in log_data["text"]):
                        log_data["severity"] = "WARNING"
                if "-X Panel Illumination Start" in log_data["text"]:
    ...
    def _swi_pointing_drift(self, data):
        """
        Check SWI pointing/drift compatibilitys.
        ...
        # The compatibility is only checked every 30 seconds.
        if self.swi_et_prev:
    ...
    def _swi_block_duration_check(self, blockData, observation_definition):
        ...
        if block_duration < DURMIN:
            violation = True
            message = f'SWI observation {observation_definition} duration is less than the minimum required duration: {block_duration / 60:.2f} < {DURMIN / 60:.2f}  min'
        else:
            violation = False
            message = ''
