...
class DGuardLite:
    ...
    def _load_data(self, wav_data, sample_rate):
        ...
        if sample_rate != self.sample_rate:
            if isinstance(wav_data, np.ndarray):
                ...
                wav_data = self.resampler_8000(wav_data).view(1,-1)
        if isinstance(wav_data, np.ndarray):
    ...
    def encode_data(self, wav_data, sample_rate):
        if self.apply_vad:
            segments, data = self.vad_data(wav_data, sample_rate) # List[Dict], shape=(num_samples)
        else:
