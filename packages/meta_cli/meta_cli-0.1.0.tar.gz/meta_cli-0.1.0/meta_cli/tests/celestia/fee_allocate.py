...
def test_update_and_query_basic_params(client):
    """
    测试修改手续费的基础配置
    - 只有dao地址才有权限更改
    ...
    :return:
    """
    dao = client.query_create_address("dao")
    no_dao = client.query_create_address("no_dao")

    # params
    feeTreasuryAddress = client.query_create_address("feeTreasuryAddress")
    devOperatorAddress = client.query_create_address("devOperatorAddress")
    allocatePeriod = "50"

    # - 1. 使用非dao地址修改相关地址
    client.transfer_coin(dao, no_dao, "50000")

    tx_resp = client.execute(client.build("tx", "feeAllocate", ["update-basic-params",
    ...
    tx_resp = client.execute(client.build("tx", "feeAllocate",
                                          ["update-basic-params", feeTreasuryAddress, devOperatorAddress,
                                           allocatePeriod], **{"from": no_dao, "fees": "10000udmec"}))
    logger.info(tx_resp)
    assert tx_resp["code"] == 0
    tx_receipt = client.wait_for_tx_confirmation(tx_resp["txhash"])
    assert tx_receipt["code"] == 11201 and "permission denied" in tx_receipt["raw_log"]

    # - 2. 使用dao修改相关地址
    cmd = client.build("tx", "feeAllocate",
                       ["update-basic-params", feeTreasuryAddress, devOperatorAddress,
                        allocatePeriod], **{"from": dao})
    tx_resp = client.execute(cmd)
    assert tx_resp["code"] == 0
    assert client.wait_for_tx_confirmation(tx_resp["txhash"])['code'] == 0
    ...
    # - 4. 查询相关配置信息
    query_resp = client.execute(client.build(
...
def test_update_and_query_fee_allocate_rate(client):
    """
    测试修改手续费的分配比例
    ...
    no_dao = client.query_create_address("no_dao")
    dao = client.query_create_address("dao")
    client.transfer_coin(dao, no_dao, "10000")
    ...
    # ---------- 阶段1：非DAO地址操作 ----------
    resp = client.execute(
        client.build("tx", "feeAllocate",
                     ["update-fee-allocate-rate", *case["params"]], **{"from": no_dao, "fees": f"10000{client.denom}"}))
    ca.common_assert(client, resp, case)
    ...
    for case in valid_cases:
        ...
        query_resp = client.execute(
            client.build("q", "feeAllocate", ["allocate-rate"]))
        ca.assert_query_match(query_resp, "allocate-rate",
                              case["expected"]["query_check"])


def test_withdraw_treasury_fee(client, me_client):
    """
    测试金库提取手续费
    - 只有dao地址才有权限提取
    - 提取至celestia网络地址
    ...
    :return:
    """
    receiver_fee = client.query_create_address("receiver_fee")
    dao = client.query_create_address("dao")
    no_dao = client.query_create_address("no_dao")
    amount = "100"
    client.transfer_coin(dao, no_dao, "20000")
    client.transfer_coin(no_dao, dao, "10000", fees="4000")
    ...
    for case in cases:
        resp = client.execute(
            client.build("tx", "feeAllocate",
                         ["withdraw-treasury-fee", *case["params"]], **{"from": no_dao, "fees": "1000" + client.denom}))
        ca.common_assert(client, resp, case)
    ...
    for case in valid_cases:
        before_balance = client.query_balance(receiver_fee)

        resp = client.execute(
            client.build("tx", "feeAllocate",
                         ["withdraw-treasury-fee", *case["params"]], **{"from": dao, "fees": "1000" + client.denom}))
        ca.common_assert(client, resp, case)

        after_balance = client.query_balance(receiver_fee)
        assert int(after_balance["amount"]) == int(before_balance["amount"]) + int(amount)

    ibc_transfer_cases = [
        {
            "case": "dao地址提取金额手续费-至me网络地址",
            "params": ("transfer", client.channels.get("to_me"), receiver_fee, amount, "true"),
            "expected": {
                "error": False,
                "check_fail": False,
                "deliver_fail": False,
    ...
    for case in ibc_transfer_cases:
        ...
        resp = client.execute(
            client.build("tx", "feeAllocate",
                         ["withdraw-treasury-fee", *case["params"]], **{"from": dao, "fees": "1000" + client.denom}))
        tx_receipt = ca.common_assert(client, resp, case)
        print(tx_receipt)
        # TODO
...
def test_fees_gather_allocate(client):
    """
    测试业务流程，手续费收取与分配
    ...
    """
    dao = client.query_create_address("dao")
    user01 = client.query_create_address("user01")
    fee_collector = client.query_create_address("fee_collector", "remote")
    feeAllocate = client.query_create_address("feeAllocate", "remote")

    client.transfer_coin(dao, user01, "1000000")
    user01_balance = client.query_balance(user01)
    logger.info(f"user01_balance: {user01_balance}")

    feeAllocate_balance = client.query_balance(feeAllocate)
    logger.info(
        f"feeAllocate:{feeAllocate}, feeAllocate_balance: {feeAllocate_balance}")

    fees = "1000"
    client.transfer_coin(user01, dao, "1", fees)

    query_basic_params = client.execute(
        client.build("q", "feeAllocate", "basic-params"))
    allocate_rate = client.execute(
        client.build("q", "feeAllocate", "allocate-rate"))
    ...
    feeTreasuryAddress_balance = client.query_balance(feeTreasuryAddress)
    devOperatorAddress_balance = client.query_balance(devOperatorAddress)

    status = client.execute(client.build("status", ""))
    ...
    while True:
        ...
        feeAllocate_balance_after = client.query_balance(feeAllocate)
        status = client.execute(client.build("status", ""))
        height = status.get("SyncInfo").get("latest_block_height")
    ...
    feeTreasuryAddress_balance_after = client.query_balance(feeTreasuryAddress)
    devOperatorAddress_balance_after = client.query_balance(devOperatorAddress)
...
def test_allocate_period(client):
    """
    测试手续费分配周期
    ...
    """
    dao = client.query_create_address("dao")
    user02 = client.query_create_address("user02")
    feeAllocate = client.query_create_address("feeAllocate", "remote")
    client.transfer_coin(dao, user02, "1000000")
    fees = "1000"
    client.transfer_coin(user02, dao, "1", fees)
    ...
    while True:
        ...
        if feeAllocate_balance_after.get("amount") == "0":
            break
    height = client.execute(client.build("status", "")).get(
    ...
    tx_resp = client.execute(client.build("tx", "feeAllocate",
                                          ["update-basic-params", feeTreasuryAddress, devOperatorAddress,
                                           3], **{"from": dao}))
    assert tx_resp["code"] == 0
    resp = client.wait_for_tx_confirmation(tx_resp["txhash"])
    resp['code'] == 0
