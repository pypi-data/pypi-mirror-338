    ...
    if args.metrics:
        if args.format == "sentences":
            metrics = {
                ...
                "avg_sentence_length": sum(len(s) for s in segmenter.segment_to_sentences(text)) / len(segmenter.segment_to_sentences(text)) if segmenter.segment_to_sentences(text) else 0,
            }
        elif args.format == "paragraphs":
            metrics = {
                "num_paragraphs": len(segmenter.segment_to_paragraphs(text)),
                "avg_paragraph_length": sum(len(p) for p in segmenter.segment_to_paragraphs(text)) / len(segmenter.segment_to_paragraphs(text)) if segmenter.segment_to_paragraphs(text) else 0,
            }
        else:
            metrics = {
                "num_sentences": len(segmenter.segment_to_sentences(text)),
