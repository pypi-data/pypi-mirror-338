...
class FeatureExtractorProtocol(Protocol):
    ...
    def get_char_features(self, text: str, left_window: int = 5, right_window: int = 5) -> FeatureMatrix:
    ...
    def process_annotated_text(self, text: str, left_window: int = 5, right_window: int = 5) -> Tuple[
        str, FeatureMatrix, PositionLabels]:
...
class FeatureExtractor:
    ...
    def _setup_caches(self, cache_size: int) -> None:
        ...
        self.is_in_abbreviation = lru_cache(maxsize=cache_size)(self._is_in_abbreviation)
    ...
    def _is_quote_balanced(self, text: str, position: int) -> bool:
        ...
        if text[position] not in OPENING_QUOTES and text[position] not in CLOSING_QUOTES:
            return True
            
        # Count quotes up to this position
        straight_double_count = text[:position+1].count('"')
        curly_double_open_count = text[:position+1].count('\u201c')  # left double quote
        curly_double_close_count = text[:position+1].count('\u201d')  # right double quote
        straight_single_count = text[:position+1].count("'")
        curly_single_open_count = text[:position+1].count('\u2018')  # left single quote
        curly_single_close_count = text[:position+1].count('\u2019')  # right single quote
        ...
        return (is_straight_double_balanced and is_curly_double_balanced and 
                is_straight_single_balanced and is_curly_single_balanced)
    ...
    def _is_word_likely_complete(self, text: str, position: int) -> bool:
        ...
        if (next_char.isspace() or 
            next_char in TERMINAL_SENTENCE_CHAR_LIST or 
            next_char in PUNCTUATION_CHAR_LIST):
            return True
    ...
    def _is_in_list_item(self, text: str, position: int, window_size: int = 20) -> bool:
        ...
        if (char in '.,;:()[]' or 
            (position > 0 and text[position-1] in '.,;:()') or
            (position < text_len-1 and text[position+1] in '.,;:()[]')):
            ...
            has_colon_before = ':' in context_before[-min_intro_len:] if len(context_before) >= min_intro_len else False
            ...
            if char in ';,' or (position > 0 and text[position-1] in ';,'):
                ...
                for conj in LIST_CONJUNCTIONS:
                    ...
                    if conj_pos > 0 and conj_pos < 5 and (context_after[conj_pos-1] == ';' or context_after[conj_pos-1] == ','):
                        return True
        
        return False
        
    def _is_semicolon_in_list(self, text: str, position: int, window_size: int = 50) -> bool:
        """
    ...
    def get_char_features(self, text: str, left_window: int = 5, right_window: int = 5, 
                         positions: Optional[List[int]] = None) -> FeatureMatrix:
        """
        ...
        if self.use_numpy:
            return self._get_char_features_numpy(text, left_window, right_window, positions)
        ...
        primary_term_flags = [1 if (i < n and text[i] in PRIMARY_TERMINATORS) else 0 for i in range(n)]
        secondary_term_flags = [1 if (i < n and text[i] in SECONDARY_TERMINATORS) else 0 for i in range(n)]
        ...
        semicolon_in_list_flags = [self._is_semicolon_in_list(text, i) for i in range(n)]
        ...
        for feature_idx, i in enumerate(position_indices):
            ...
            for j in range(-left_window, right_window + 1):
                ...
                if pos < 0 or pos >= n:
                    features[feature_idx][idx] = -3  # Out-of-bounds placeholder (whitespace value)
                else:
            ...
            features[feature_idx][window_end + 1] = 1 if primary_term_flags[i] else (
                -1 if secondary_term_flags[i] else 0
            )
            ...
            features[feature_idx][window_end + 4] = 1 if followed_by_lowercase_flags[i] else 0
            ...
            features[feature_idx][window_end + 6] = 1 if semicolon_in_list_flags[i] else 0
    ...
    def _get_char_features_numpy(self, text: str, left_window: int = 5, right_window: int = 5,
                              positions: Optional[List[int]] = None) -> FeatureMatrix:
        """
        ...
        if positions is None:
            ...
            position_indices = np.array([p for p in positions if 0 <= p < n], dtype=np.int32)
        ...
        if positions is not None and len(position_indices) < n / 4:  # Only compute for positions if less than 25% of text
            ...
            primary_term_flags = np.array([1 if text[i] in PRIMARY_TERMINATORS else 0 
                                         for i in position_indices], dtype=np.int8)
            secondary_term_flags = np.array([1 if text[i] in SECONDARY_TERMINATORS else 0 
                                           for i in position_indices], dtype=np.int8)
            quote_balanced_flags = np.array([self._is_quote_balanced(text, i) for i in position_indices], dtype=np.int8)
            word_complete_flags = np.array([self._is_word_likely_complete(text, i) for i in position_indices], dtype=np.int8)
            followed_by_lowercase_flags = np.array([1 if (i < n-1 and text[i+1].islower()) else 0 
                                                  for i in position_indices], dtype=np.int8)
            ...
            for idx, pos in enumerate(position_indices):
                ...
                if (char in '.,;:()[]0123456789abcdefghijklmnopqrstuvwxyz•·○●■□▪▫' or
                    (pos > 0 and text[pos-1] in '.,;:()') or
                    (pos < n-1 and text[pos+1] in '.,;:()')):
                    in_list_item_flags[idx] = self._is_in_list_item(text, pos)
                    if char == ';':
                        semicolon_in_list_flags[idx] = self._is_semicolon_in_list(text, pos)
                    if char == ':' or (pos > 0 and text[pos-1] == ':') or (pos < n-1 and text[pos+1] == ':'):
            ...
            abbr_flags = np.array([self.is_in_abbreviation(text, i) for i in range(n)], dtype=np.int8)
            primary_term_flags = np.array([1 if (i < n and text[i] in PRIMARY_TERMINATORS) else 0 
                                         for i in range(n)], dtype=np.int8)
            secondary_term_flags = np.array([1 if (i < n and text[i] in SECONDARY_TERMINATORS) else 0 
                                           for i in range(n)], dtype=np.int8)
            quote_balanced_flags = np.array([self._is_quote_balanced(text, i) for i in range(n)], dtype=np.int8)
            word_complete_flags = np.array([self._is_word_likely_complete(text, i) for i in range(n)], dtype=np.int8)
            followed_by_lowercase_flags = np.array([1 if (i < n-1 and text[i+1].islower()) else 0 
                                                  for i in range(n)], dtype=np.int8)
        ...
        num_positions = len(position_indices)
        features = np.full((num_positions, feature_size), -3, dtype=np.int16)  # Fill with whitespace placeholder
        ...
        for i, pos in enumerate(position_indices):
            ...
            if positions is not None and len(position_indices) < n / 4:
                ...
                features[i, window_size + 1] = terminator_feature[idx]    # Terminator type
                features[i, window_size + 2] = quote_balanced_flags[idx]  # Quote balance
                features[i, window_size + 3] = word_complete_flags[idx]   # Word completion
                features[i, window_size + 4] = followed_by_lowercase_flags[idx]  # Followed by lowercase
                features[i, window_size + 5] = in_list_item_flags[idx]    # Is in list item
                features[i, window_size + 6] = semicolon_in_list_flags[idx]  # Is semicolon in list
                ...
                features[i, window_size + 1] = terminator_feature[pos]    # Terminator type
                features[i, window_size + 2] = quote_balanced_flags[pos]  # Quote balance
                features[i, window_size + 3] = word_complete_flags[pos]   # Word completion
                features[i, window_size + 4] = followed_by_lowercase_flags[pos]  # Followed by lowercase
                features[i, window_size + 5] = in_list_item_flags[pos]    # Is in list item
                features[i, window_size + 6] = semicolon_in_list_flags[pos]  # Is semicolon in list
                features[i, window_size + 7] = near_colon_flags[pos]      # Is near colon
        
        # Convert back to list for API compatibility
        return features.tolist()
        
    def _process_chunk(self, chunk_range: Tuple[int, int], text: str, 
                     left_window: int, right_window: int) -> FeatureMatrix:
    ...
    def mark_annotation_positions(self, text: str) -> PositionLabels:
        ...
        while i < text_len:
            ...
            if (i + sentence_len <= text_len and text[i:i + sentence_len] == SENTENCE_TAG):
                if result:  # Mark the previous character
                    result[-1] = 1  # Binary classification: 1 for any boundary
                i += sentence_len
            # Check for paragraph tag
            elif (i + paragraph_len <= text_len and text[i:i + paragraph_len] == PARAGRAPH_TAG):
                if result:  # Mark the previous character
                    result[-1] = 1  # Binary classification: 1 for any boundary
                i += paragraph_len
            else:
    ...
    def process_annotated_text(self, text: str, left_window: int = 5, right_window: int = 5,
                              num_workers: int = 0) -> Tuple[str, FeatureMatrix, PositionLabels]:
        """
        ...
        if num_workers > 1 and text_len > 10000:
            ...
            chunks = [(i, min(i + chunk_size, text_len)) 
    ...
    def print_text_analysis(self, text: str, left_window: int = 5, right_window: int = 5) -> None:
        """
        ...
        Args:
            ...
            right_window (int, optional): Size of right context window. Defaults to 5.
        """
        clean_text, features, labels = self.process_annotated_text(text, left_window, right_window)

        print("Character | Features | Label")
        print("-" * 50)

        for idx, (char, feature_vector, label) in enumerate(zip(clean_text, features, labels)):
