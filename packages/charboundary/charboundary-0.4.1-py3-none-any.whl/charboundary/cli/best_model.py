...
def add_best_model_args(subparsers) -> None:
    ...
    """
    parser = subparsers.add_parser("best-model", help="Train the best model based on evaluation data")
    
    # Required arguments
    parser.add_argument("--data", required=True, help="Path to the training data file")
    parser.add_argument("--output", required=True, help="Path to save the best trained model")
    
    # Optional arguments
    parser.add_argument("--validation", help="Path to validation data for model selection")
    parser.add_argument("--metrics-file", help="Path to save metrics as JSON")
    
    # Model parameter ranges for grid search
    parser.add_argument("--left-window-values", type=int, nargs="+", default=[3, 5, 7],
                        help="Left window sizes to try (default: 3, 5, 7)")
    parser.add_argument("--right-window-values", type=int, nargs="+", default=[3, 5, 7],
                        help="Right window sizes to try (default: 3, 5, 7)")
    parser.add_argument("--n-estimators-values", type=int, nargs="+", default=[50, 100, 200],
