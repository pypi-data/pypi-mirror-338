...
from timecapsule.zone_manager import TZ
...
class TimeCapsule:
    ...
    def parse(time_str: str, ref_time: datetime | None = None) -> datetime | None:
        ...
        if normalized == "now":
            return now
        if parsed := TimeCapsule._parse_simple(normalized, now):
            return parsed
        ...
        try:  # Fall back to dateutil parser if available
            ...
            return TimeCapsule.ensure_future(
                TimeCapsule.ensure_tz(parsed), now, force_future="today" not in normalized
            )
            ...
            if 1 <= hour <= 12 and 0 <= minute < 60:  # If AM/PM is explicitly specified
                if len(parts) == 2:
                    ...
                    if current_hour < 12:  # It's currently AM
                        ...
                        hour += 12
                return TimeCapsule.adjust_for_tomorrow_if_needed(now, hour, minute)
    ...
    def adjust_for_tomorrow_if_needed(
        ...
        time = time or now
        time = TimeCapsule.ensure_tz(time)
    ...
    def get_pretty_time(time: datetime | timedelta, **kwargs: Any) -> str:
        ...
        if isinstance(time, datetime):
            return TimeCapsule._format_datetime(time, **kwargs)
        return TimeCapsule._format_timedelta(time)

    @staticmethod
    def _parse_simple(time_str: str, ref_time: datetime) -> datetime | None:
        """Parse common time formats using simple string manipulation."""
        if time := TimeCapsule._parse_12_hour(ref_time, time_str):
            return time
        if time := TimeCapsule._parse_24_hour(ref_time, time_str):
            return time
        return None
    ...
    def get_day_number(day: str) -> int:
        ...
        return TimeCapsule.DAYS.index(day)


# Partial functions for common use cases
get_pretty_time = partial(TimeCapsule.get_pretty_time)
get_capitalized_time = partial(TimeCapsule.get_pretty_time, capitalize=True)
get_time_only = partial(TimeCapsule.get_pretty_time, time_only=True)
get_weekday_time = partial(TimeCapsule.get_pretty_time, weekday=True)
