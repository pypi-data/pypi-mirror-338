...
def _get_constraint_weights(
    ...
    constraint_weights_list: Dict[int, coo_array] = {}

    for i in range(num_constraints):
        constraint_weights_list[i] = coo_array((variables_length,), dtype=np.float32)
    ...
    for column in columns:
        ...
        constraint_weights_list[row_index].col = np.append(constraint_weights_list[row_index].col, column_index)
        constraint_weights_list[row_index].data = np.append(constraint_weights_list[row_index].data, column.coeff())

    return constraint_weights_list
...
def _create_constraints(model: Model, variables_map: Dict[str, int], mps: MPSObject) -> None:
    ...
    for i, bounds in enumerate(constraint_bounds):
        if bounds[0] == bounds[1]:
            model.add_equality_constraint(
                constraint_mask=constraint_weights[i],
                limit=np.array(bounds[0], dtype=np.float32) # does not matter here upper or lower
            )
        else:
            model.add_inequality_constraint(
                constraint_mask=constraint_weights[i],
                constraint_bounds=np.array(bounds, dtype=np.float32)
            )
