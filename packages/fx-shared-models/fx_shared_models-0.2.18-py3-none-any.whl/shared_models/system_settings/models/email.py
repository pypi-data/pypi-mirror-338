...
class EmailContent(SystemSetting):
    ...
    content_sections = models.JSONField(
        help_text="Dict mapping template placeholders to content. Content can include any variables in {{ variable }} format.",
        default=dict
    )
    ...
    def clean(self):
        ...
        # Only validate that content sections match template placeholders
        template_placeholders = set(self.system_email.template.content_placeholders)
        content_sections = set(self.content_sections.keys())
        
        # Check for missing required placeholders
        missing_placeholders = template_placeholders - content_sections
        if missing_placeholders:
            raise ValidationError({
                'content_sections': f'Missing content for required placeholders: {", ".join(missing_placeholders)}'
            })
        
        # Check for extra placeholders that aren't in the template
        extra_placeholders = content_sections - template_placeholders
        if extra_placeholders:
            raise ValidationError({
                'content_sections': f'Content sections contain placeholders not in template: {", ".join(extra_placeholders)}'
            })
        
        # Check that all variables used in content are available
