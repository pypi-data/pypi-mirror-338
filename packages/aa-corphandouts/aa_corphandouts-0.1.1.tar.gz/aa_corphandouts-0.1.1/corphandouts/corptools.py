...
def update_corp_item_names_bulk(item_name_ids: list[dict]):
    """
    Bulk update item names from a list of:
    [
        {
            "item_id": int,
            "name": str,
    ...
    """
    logger.debug(item_name_ids)
    corp_assets = []
    for item in item_name_ids:
        corp_asset = CorpAsset.objects.get(item_id=item["item_id"])
        corp_asset.name = item["name"]
        corp_assets.append(corp_asset)
    logger.debug(corp_assets)
    CorpAsset.objects.bulk_update(corp_assets, ["name"])
